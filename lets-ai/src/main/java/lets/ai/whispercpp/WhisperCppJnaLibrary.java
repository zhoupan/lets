...
import lets.ai.whispercpp.model.WhisperModelLoader;
...
public interface WhisperCppJnaLibrary extends Library {
  ...
  /**
   * Allocate (almost) all memory needed for the model by loading from a file without allocating the
   * state.
   *
   * @param path_model Path to the model file
   * @return Whisper context on success, null on failure
  ...
  /**
   * Allocate (almost) all memory needed for the model by loading from a buffer without allocating
   * the state.
   *
   * @param buffer Model buffer
   * @param buffer_size Size of the model buffer
  ...
  /**
   * Allocate (almost) all memory needed for the model using a model loader without allocating the
   * state.
   *
   * @param loader Model loader
   * @return Whisper context on success, null on failure
  ...
  /**
   * Convert RAW PCM audio to log mel spectrogram. The resulting spectrogram is stored inside the
   * default state of the provided whisper context.
   *
  ...
  int whisper_pcm_to_mel_with_state(
      Pointer ctx, Pointer state, final float[] samples, int n_samples, int n_threads);

  /**
   * This can be used to set a custom log mel spectrogram inside the default state of the provided
   * whisper context. Use this instead of whisper_pcm_to_mel() if you want to provide your own log
   * mel spectrogram. n_mel must be 80
   *
   * @return 0 on success
   */
  ...
  int whisper_set_mel_with_state(
      Pointer ctx, Pointer state, final float[] data, int n_len, int n_mel);

  /**
   * Run the Whisper encoder on the log mel spectrogram stored inside the default state in the
   * provided whisper context. Make sure to call whisper_pcm_to_mel() or whisper_set_mel() first.
  ...
  /**
   * Run the Whisper decoder to obtain the logits and probabilities for the next token. Make sure to
   * call whisper_encode() first. tokens + n_tokens is the provided context for the decoder. n_past
   * is the number of tokens to use from previous decoder calls. Returns 0 on success TODO: add
   * support for multiple decoders
   */
  ...
  int whisper_decode_with_state(
      Pointer ctx, Pointer state, Pointer tokens, int n_tokens, int n_past, int n_threads);

  /**
   * Convert the provided text into tokens. The tokens pointer must be large enough to hold the
   * resulting tokens. Returns the number of tokens on success, no more than n_max_tokens Returns -1
   * on failure TODO: not sure if correct
   */
  ...
  /**
   * @return the id of the specified language, returns -1 if not found. Examples: "de" -> 2 "german"
   *     -> 2
   */
  int whisper_lang_id(String lang);

  /**
   * @return the short string of the specified language id (e.g. 2 -> "de"), returns nullptr if not
   *     found
   */
  String whisper_lang_str(int id);

  /**
   * Use mel data at offset_ms to try and auto-detect the spoken language. Make sure to call
   * whisper_pcm_to_mel() or whisper_set_mel() first Returns the top language id or negative on
   * failure If not null, fills the lang_probs array with the probabilities of all languages The
   * array must be whisper_lang_max_id() + 1 in size
   *
   * <p>ref: https://github.com/openai/whisper/blob/main/whisper/decoding.py#L18-L69
   */
  int whisper_lang_auto_detect(Pointer ctx, int offset_ms, int n_threads, float[] lang_probs);

  int whisper_lang_auto_detect_with_state(
      Pointer ctx, Pointer state, int offset_ms, int n_threads, float[] lang_probs);
  ...
  /**
   * Token logits obtained from the last call to whisper_decode(). The logits for the last token are
   * stored in the last row Rows: n_tokens Cols: n_vocab
   */
  ...
  // Note: Even if `whisper_full_params is stripped back to just 4 ints, JNA throws "Invalid memory
  // access"
  ...
  int whisper_full_with_state(
      Pointer ctx, Pointer state, WhisperFullParams params, final float[] samples, int n_samples);
  ...
  String whisper_full_get_token_text_from_state(
      Pointer ctx, Pointer state, int i_segment, int i_token);
  ...
  WhisperTokenData whisper_full_get_token_data_from_state(
