...
public interface CharStream {
  ...
  /**
   * @return the column number of the first character for current token (being matched after the
   *     last call to beginToken).
   */
  int getBeginColumn();

  /**
   * @return the line number of the first character for current token (being matched after the last
   *     call to BeginToken).
   */
  int getBeginLine();

  /**
   * @return the column number of the last character for current token (being matched after the last
   *     call to BeginToken).
   */
  int getEndColumn();

  /**
   * @return the line number of the last character for current token (being matched after the last
   *     call to BeginToken).
   */
  int getEndLine();
  ...
  /**
   * @return the next character that marks the beginning of the next token. All characters must
  ...
  /**
   * @return a string made up of characters from the marked token beginning to the current buffer
   *     position. Implementations have the choice of returning anything that they want to. For
   *     example, for efficiency, one might decide to just return null, which is a valid
   *     implementation.
   */
  String getImage();

  /**
   * @return an array of characters that make up the suffix of length 'len' for the currently
   *     matched token. This is used to build up the matched string for use in actions in the case
   *     of MORE. A simple and inefficient implementation of this is as follows:
   *     <pre>
   *   {
   *      String t = getImage();
...
/* ParserGeneratorCC - OriginalChecksum=a0cd961909fda7de9f3c3e529068897e (do not edit this line) */
