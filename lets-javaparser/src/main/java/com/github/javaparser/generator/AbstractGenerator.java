...
public abstract class AbstractGenerator {
  ...
  /**
   * Utility method that looks for a method or constructor with an identical signature as "callable"
   * and replaces it with callable. If not found, adds callable. When the new callable has no
   * javadoc, any old javadoc will be kept.
   */
   ...
   */
  private <T extends NodeWithAnnotations<?>> void annotate(
      T node, Class<?> annotation, Expression content) {
  ...
  protected <T extends Node & NodeWithAnnotations<?>> void removeAnnotation(
      T node, Class<?> annotation) {
    node.getAnnotations()
        .removeIf(
            annotationExpr ->
   ...
   * @param <T> Only accept nodes which accept annotations.
   */
  protected <T extends NodeWithAnnotations<?>> void annotateSuppressWarnings(
      T node, String warningType) {
   ...
   * This is not currently used directly by any current generators, but may be useful when changing
   * a generator and you need to get rid of a set of outdated methods.
   */
  ...
  /**
   * Utility method that looks for a method or constructor with an identical signature as "callable"
   * and replaces it with callable. If not found, fails. When the new callable has no javadoc, any
   * old javadoc will be kept. The method or constructor is annotated with the generator class.
   */
