...
public final class JavaParser {
  ...
  /**
   * Instantiate the parser with default configuration. Note that parsing can also be done with the
   * static methods {@link StaticJavaParser}. Creating an instance will reduce setup time between
   * parsing files.
   */
  public JavaParser() {
  ...
  /**
   * Instantiate the parser. Note that parsing can also be done with the static methods {@link
   * StaticJavaParser}. Creating an instance will reduce setup time between parsing files.
   */
  ...
  /**
   * Parses source code. It takes the source code from a Provider. The start indicates what can be
   * found in the source code (compilation unit, block, import...)
   *
   * @param start refer to the constants in ParseStart to see what can be parsed.
   * @param provider refer to Providers to see how you can read source. The provider will be closed
   *     after parsing.
  ...
  public <N extends Node> ParseResult<N> parse(ParseStart<N> start, Provider provider) {
    ...
    try {
      ...
      ParseResult<N> result =
          new ParseResult<>(resultNode, parser.problems, parser.getCommentsCollection());
      for (Processor processor : processors) {
  ...
  /**
   * Parses the Java code contained in the {@link InputStream} and returns a {@link CompilationUnit}
   * that represents it.
   *
  ...
  /**
   * Parses the Java code contained in a {@link File} and returns a {@link CompilationUnit} that
   * represents it.
   *
   ...
   */
  @Deprecated
  public ParseResult<CompilationUnit> parse(final File file, final Charset encoding)
  ...
  public ParseResult<CompilationUnit> parse(final File file) throws FileNotFoundException {
    ...
    result
        .getResult()
        .ifPresent(cu -> cu.setStorage(file.toPath(), configuration.getCharacterEncoding()));
    return result;
  }

  /**
   * Parses the Java code contained in a file and returns a {@link CompilationUnit} that represents
   * it.
   *
   ...
   */
  @Deprecated
  public ParseResult<CompilationUnit> parse(final Path path, final Charset encoding)
      throws IOException {
  ...
  /**
   * Parses the Java code contained in a resource and returns a {@link CompilationUnit} that
   * represents it.<br>
   *
   * @param path path to a resource containing Java source code. As resource is accessed through a
   *     class loader, a leading "/" is not allowed in pathToResource
   * @return CompilationUnit representing the Java source code
   * @throws ParseProblemException if the source code has parser errors
   * @throws IOException the path could not be accessed
   */
   ...
   */
  @Deprecated
  public ParseResult<CompilationUnit> parseResource(final String path, Charset encoding)
      throws IOException {
  ...
  /**
   * Parses the Java code contained in code and returns a {@link CompilationUnit} that represents
   * it.
   *
  ...
  /**
   * Parses the Java block contained in a {@link String} and returns a {@link BlockStmt} that
   * represents it.
   *
  ...
  /**
   * Parses the Java import contained in a {@link String} and returns a {@link ImportDeclaration}
   * that represents it.
   *
  ...
  /**
   * Parses the Java expression contained in a {@link String} and returns a {@link Expression} that
   * represents it.
   *
  ...
  /**
   * Parses the Java annotation contained in a {@link String} and returns a {@link AnnotationExpr}
   * that represents it.
   *
  ...
  /**
   * Parses the Java annotation body declaration(e.g fields or methods) contained in a {@link
   * String} and returns a {@link BodyDeclaration} that represents it.
   *
  ...
  /**
   * Parses a Java class or interface body declaration(e.g fields or methods) and returns a {@link
  ...
  /**
   * Parses a Java class or interface type name and returns a {@link ClassOrInterfaceType} that
   * represents it.
   *
  ...
  /**
   * Parses a variable declaration expression and returns a {@link
   * com.github.javaparser.ast.expr.VariableDeclarationExpr} that represents it.
   *
   ...
   * @throws ParseProblemException if the source code has parser errors
   */
  public ParseResult<ExplicitConstructorInvocationStmt> parseExplicitConstructorInvocationStmt(
      String statement) {
