...
/**
 * The configuration that is used by the parser. Note that this can be changed even when reusing the
 * same JavaParser instance. It will pick up the changes.
 */
public class ParserConfiguration {
  ...
  public enum LanguageLevel {
    ...
    /**
     * Java 10 -- including incubator/preview/second preview features. Note that preview features,
     * unless otherwise specified, follow the grammar and behaviour of the latest released JEP for
     * that feature.
     */
    ...
    /**
     * Java 11 -- including incubator/preview/second preview features. Note that preview features,
     * unless otherwise specified, follow the grammar and behaviour of the latest released JEP for
     * that feature.
     */
    ...
    /**
     * Java 12 -- including incubator/preview/second preview features. Note that preview features,
     * unless otherwise specified, follow the grammar and behaviour of the latest released JEP for
     * that feature.
     *
     * <ul>
     *   <li>Switch expressions are permitted, with a single label only and no yield.
     * </ul>
     */
    ...
    /**
     * Java 13 -- including incubator/preview/second preview features. Note that preview features,
     * unless otherwise specified, follow the grammar and behaviour of the latest released JEP for
     * that feature.
     *
     * <ul>
     *   <li>Switch expressions are permitted, with a single label only.
     * </ul>
     */
    ...
    /**
     * Java 14 -- including incubator/preview/second preview features. Note that preview features,
     * unless otherwise specified, follow the grammar and behaviour of the latest released JEP for
     * that feature.
     */
    ...
    /**
     * Java 15 -- including incubator/preview/second preview features. Note that preview features,
     * unless otherwise specified, follow the grammar and behaviour of the latest released JEP for
     * that feature.
     */
    ...
    /**
     * Java 16 -- including incubator/preview/second preview features. Note that preview features,
     * unless otherwise specified, follow the grammar and behaviour of the latest released JEP for
     * that feature.
     */
    ...
    /**
     * Java 17 -- including incubator/preview/second preview features. Note that preview features,
     * unless otherwise specified, follow the grammar and behaviour of the latest released JEP for
     * that feature.
     */
  ...
  // TODO: Add a configurable option e.g. setDesiredLineEnding(...) to replace/swap out existing
  // line endings
                ...
                root -> {
                  ...
                  if (!mapping.isEmpty()) {
                    root.walk(
                        node ->
                            node.getRange()
                                .ifPresent(range -> node.setRange(mapping.transform(range))));
                  }
                });
      }
    }
    ...
    public void postProcess(ParseResult<? extends Node> result, ParserConfiguration configuration) {
      if (isDetectOriginalLineSeparator()) {
        result
            ...
            .ifPresent(
                rootNode -> {
                  LineSeparator detectedLineSeparator =
                      _lineEndingProcessingProvider.getDetectedLineEnding();
                  // Set the line ending on the root node
                  rootNode.setData(Node.LINE_SEPARATOR_KEY, detectedLineSeparator);
                  // // Set the line ending on all children of the root node -- FIXME: Should ignore
                  // """textblocks"""
                              ...
                              .ifPresent(
                                  comments ->
                                      new CommentsInserter(configuration)
                                          .insertComments(
                                              resultNode, comments.copy().getComments())));
                }
              }
            });
    processors.add(
                ...
                if (languageLevel != null) {
                  ...
                  if (languageLevel.validator != null) {
                    languageLevel.validator.accept(
                        ...
                        new ProblemReporter(newProblem -> result.getProblems().add(newProblem)));
                  }
                }
              }
            });
  ...
  /**
   * Whether to run CommentsInserter, which will put the comments that were found in the source code
   * into the comment and javadoc fields of the nodes it thinks they refer to.
   */
  ...
  /**
   * When a TAB character is encountered during parsing, the column position will be increased by
   * this value. By default it is 1.
   */
  ...
  /**
   * Disabled by default. When this is enabled, LexicalPreservingPrinter.print can be used to
   * reproduce the original formatting of the file.
   */
  ...
  /**
   * When set to true, unicode escape handling is done by preprocessing the whole input, meaning
   * that all unicode escapes are turned into unicode characters before parsing. That means the AST
   * will never contain literal unicode escapes. However, positions in the AST will point to the
   * original input, which is exactly the same as without this option. Without this option enabled,
