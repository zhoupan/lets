...
/**
 * A method call on an object or a class. <br>
 * {@code circle.circumference()} <br>
 * In {@code a.<String>bb(15);}, a is the scope, String is a type argument, bb is the name and 15 is
 * an argument.
 *
...
public class MethodCallExpr extends Expression
  ...
  public MethodCallExpr(
      final Expression scope, final String name, final NodeList<Expression> arguments) {
  ...
  public MethodCallExpr(
      final Expression scope, final SimpleName name, final NodeList<Expression> arguments) {
   ...
   * {@link ResolvedMethodDeclaration} representing the declaration of the constructor invoked by
   * this {@code MethodCallExpr} is returned. Otherwise, an {@link UnsolvedSymbolException} is
   * thrown.
   *
   * @return a {@link ResolvedMethodDeclaration} representing the declaration of the invoked method.
   * @throws UnsolvedSymbolException if the declaration corresponding to the method call expression
  ...
  public boolean isPolyExpression() {
    ...
    // 2. If the invocation is qualified (that is, any form of MethodInvocation except for the form
    // [MethodName (
    // [ArgumentList] )]), then the invocation elides TypeArguments to the left of the Identifier.
    if (isQualified() && !elidesTypeArguments()) {
      return false;
    }
    // 3. The method to be invoked, as determined by the following subsections, is generic (§8.4.4)
    // and has a
    // return type that mentions at least one of the method's type parameters.
