...
/**
 * This class represents the entire compilation unit. Each java file denotes a compilation unit. A
 * compilation unit start with an optional package declaration, followed by zero or more import
 * declarations, followed by zero or more type declarations.
 *
...
public class CompilationUnit extends Node {
  ...
  /**
   * @deprecated getComments was a too generic name and it could be confused with getComment or
   *     getAllContainedComments Use {@link #getAllComments()} instead
   */
  @Deprecated
  ...
  /**
   * Retrieves the list of imports declared in this compilation unit or {@code null} if there is no
   * import.
   *
   ...
   * If this compilation unit has no package declaration (default package), {@code Optional.none()}
   * is returned.
   *
  ...
  /**
   * Sets the list of imports of this compilation unit. The list is initially {@code null}.
   *
  ...
  /**
   * adds an import if not implicitly imported by java (i.e. java.lang) or added before. Asterisk
   * imports overrule the other imports within the same package.
   *
   ...
   *
   * @param packageDeclaration the packageDeclaration declaration to set or {@code null} to default
   *     package
   */
  @Generated("com.github.javaparser.generator.core.node.PropertyGenerator")
  public CompilationUnit setPackageDeclaration(final PackageDeclaration packageDeclaration) {
    ...
    notifyPropertyChange(
        ObservableProperty.PACKAGE_DECLARATION, this.packageDeclaration, packageDeclaration);
  ...
  public CompilationUnit addImport(Class<?> clazz) {
    ...
    if (clazz.isAnonymousClass() || clazz.isLocalClass())
      throw new IllegalArgumentException(
          clazz.getName()
              + " is an anonymous or local class therefore it can't be added with addImport");
   ...
   * @return the newly created class
   */
  public AnnotationDeclaration addAnnotationDeclaration(
      String name, Modifier.Keyword... modifiers) {
    AnnotationDeclaration annotationDeclaration =
        new AnnotationDeclaration(createModifierList(modifiers), name);
  ...
  public Optional<ClassOrInterfaceDeclaration> getClassByName(String className) {
    return getTypes().stream()
        .filter(
            type ->
                type.getNameAsString().equals(className)
  ...
  public Optional<ClassOrInterfaceDeclaration> getInterfaceByName(String interfaceName) {
    return getTypes().stream()
        .filter(
            type ->
                type.getNameAsString().equals(interfaceName)
                    && type instanceof ClassOrInterfaceDeclaration
                    && ((ClassOrInterfaceDeclaration) type).isInterface())
        .findFirst()
        .map(t -> (ClassOrInterfaceDeclaration) t);
  ...
  /**
   * @return the name that the primary type in this file should have, according to the filename in
   *     {@link Storage#getFileName()}. Empty if no file information is present (when this
   *     compilation unit wasn't parsed from a file.)
   */
  ...
  /**
   * @return the type whose name corresponds to the file name. Empty if no file information is
   *     present (when this compilation unit wasn't parsed from a file.) If for some strange reason
   *     there are multiple types of this name, the first one is returned.
   */
  public Optional<TypeDeclaration<?>> getPrimaryType() {
    return getPrimaryTypeName()
        .flatMap(
            name -> getTypes().stream().filter(t -> t.getNameAsString().equals(name)).findFirst());
  }
  ...
  public Optional<AnnotationDeclaration> getAnnotationDeclarationByName(String annotationName) {
    return getTypes().stream()
        .filter(
            type ->
                type.getNameAsString().equals(annotationName)
  ...
  public Optional<RecordDeclaration> getRecordByName(String recordName) {
    return getTypes().stream()
        .filter(
            type -> type.getNameAsString().equals(recordName) && type instanceof RecordDeclaration)
        .findFirst()
  ...
  /**
   * @return information about where this compilation unit was loaded from, or empty if it wasn't
   *     loaded from a file.
   */
  ...
  /**
   * Recalculates the ranges of all nodes by looking at the sizes of the tokens. This is useful when
   * you have manually inserted or deleted tokens and still want to use the ranges.
   */
  ...
  /**
   * Information about where this compilation unit was loaded from. This class only stores the
   * absolute location. For more flexibility use SourceRoot.
   */
  public static class Storage {
    ...
    /**
     * @return the source root directory, calculated from the path of this compiation unit, and the
     *     package declaration of this compilation unit. If the package declaration is invalid (when
     *     it does not match the end of the path) a RuntimeException is thrown.
     */
    ...
    /**
     * Saves a compilation unit to its original location with formatting and encoding according to
     * the function and encoding passed as a parameter.
     *
     * @param makeOutput a function that formats the compilation unit
