...
public abstract class TypeDeclaration<T extends TypeDeclaration<?>> extends BodyDeclaration<T>
   ...
   */
  public List<CallableDeclaration<?>> getCallablesWithSignature(
  ...
  /**
   * Returns the fully qualified name of this type, derived only from information available in this
   * compilation unit. (So no symbol solving happens.) If the declared type is a local class
   * declaration, it will return Optional.empty(). If the declared type is a local record
   * declaration, it will return Optional.empty(). If the declared type is not contained in a
   * compilation unit, it will return Optional.empty().
   *
  ...
  /**
   * @return is this type's parent a TypeDeclaration? NOTE: many people are confused over
   *     terminology. Refer to https://docs.oracle.com/javase/tutorial/java/javaOO/nested.html .
   */
