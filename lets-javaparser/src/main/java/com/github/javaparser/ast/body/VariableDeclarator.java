 ...
 * In {@code int x = 14, y = 3;} "int x = 14" and "int y = 3" are VariableDeclarators.
 *
 * <p>The type is on all of the variable declarators because, thanks to array brackets, each
 * variable can have a different type.
 *
...
public class VariableDeclarator extends Node
  ...
  @OptionalProperty @NonEmptyProperty private Expression initializer;
   ...
   * @param initializer What this variable should be initialized to. An {@link
   *     com.github.javaparser.ast.expr.AssignExpr} is unnecessary as the {@code =} operator is
   *     already added.
   */
  ...
  protected void customInitialization() {
    // We register an observer on the type property. When it is changed the MaximumCommonType is
    // changes as well,
    // because it is derived from the type of the variables it contains, for this reason we notify
    // about the change
    register(
          ...
          public void propertyChange(
            ...
            if (property == ObservableProperty.TYPE) {
              ...
              if (vd.getParentNode().isPresent()
                  && vd.getParentNode().get() instanceof NodeWithVariables) {
                NodeWithVariables<?> nodeWithVariables =
                    (NodeWithVariables<?>) vd.getParentNode().get();
                // We calculate the value the property will assume after the change will be
                // completed
                ...
                Optional<Type> newMaxCommonType =
                    NodeWithVariables.calculateMaximumCommonType(types);
