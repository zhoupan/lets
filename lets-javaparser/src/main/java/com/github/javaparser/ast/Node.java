 ...
 * <h2>Construction</h2>
 *
 * <p>The tree is built by instantiating the required nodes, then adding them to other nodes. If it
 * is the parser who is building the tree, it will use the largest constructor, the one with "range"
 * as the first parameter. If you want to manually instantiate nodes, we suggest to...
 *
 * <ul>
 *   <li>use a convenience method, like "addStatement(...)", or if none are available...
 *   <li>use a convenient constructor, like ClassOrInterfaceType(String name), or if none are
 *       available...
 *   <li>use the default constructor.
 *   <li>Alternatively, use one of the JavaParser.parse(snippet) methods.
 * </ul>
 *
 ...
 * <h2>Parent/child</h2>
 *
 * <p>The parent node field is managed automatically and can be seen as read only. Note that there
 * is only one parent, and trying to use the same node in two places will lead to unexpected
 * behaviour. It is advised to clone() a node before moving it around.
 *
 * <h2>Comments</h2>
 *
 * <p>Each Node can have one associated comment which describes it and a number of "orphan comments"
 * which it contains but are not specifically associated to any child.
 *
 * <h2>Positions</h2>
 *
 * <p>When the parser creates nodes, it sets their source code position in the "range" field. When
 * you manually instantiate nodes, their range is not set. The top left character is position 1, 1.
 * Note that since this is an <i>abstract</i> syntax tree, it leaves out a lot of text from the
 * original source file, like where braces or comma's are exactly. Therefore there is no position
 * information on everything in the original source file.
 *
 * <h2>Observers</h2>
 *
 * <p>It is possible to add observers to the tree. Any change in the tree is sent as an event to any
 * observers watching.
 *
 * <h2>Visitors</h2>
 *
 * <p>The most comfortable way of working with an abstract syntax tree is using visitors. You can
 * use one of the visitors in the visitor package, or extend one of them. A visitor can be "run" by
 * calling accept on a node:
 *
...
public abstract class Node
  ...
  public enum ObserverRegistrationMode {
    ...
    /**
     * Notify for changes happening on this node and all its descendants. The descendants existing
     * at the moment in which the observer was registered will be observed immediately. As new nodes
     * are attached later they are automatically registered to be observed.
     */
    SELF_PROPAGATING
  }
  ...
  /**
   * Called in every constructor for node specific code. It can't be written in the constructor
   * itself because it will be overwritten during code generation.
   */
  ...
  /**
   * @param range the range of characters in the source code that this node covers. null can be used
   *     to indicate that no range information is known, or that it is not of interest.
   */
  @Override
  ...
  /**
   * @return pretty printed source code for this node and its children. Formatting can be configured
   *     with parameter PrinterConfiguration.
   */
  ...
  /**
   * Contains all nodes that have this node set as their parent. You can add and remove nodes from
   * this list by adding or removing nodes from the fields of this node.
   *
  ...
  /**
   * This is a list of Comment which are inside the node and are not associated with any meaningful
   * AST Node.
   *
   * <p>For example, comments at the end of methods (immediately before the parenthesis) or at the
   * end of CompilationUnit are orphan comments.
   *
   * <p>When more than one comment preceeds a statement, the one immediately preceding it it is
   * associated with the statements, while the others are orphans.
   *
   * <p>Changes to this list are not persisted.
   *
  ...
  /**
   * This is the list of Comment which are contained in the Node either because they are properly
   * associated to one of its children or because they are floating around inside the Node
   *
  ...
  /**
   * Assign a new parent to this node, removing it from the list of children of the previous parent,
   * if any.
   *
   ...
   * @deprecated use {@link Node#findAll(Class)} but be aware that findAll also considers the
   *     initial node.
   */
  @Deprecated
  ...
  /**
   * Sets data for this node using the given key. For information on creating DataKey, see {@link
   * DataKey}.
   *
   ...
   *
   * @return true if removed, false if it is a required property of the parent, or if the parent
   *     isn't set.
  ...
  /**
   * Forcibly removes this node from the AST. If it cannot be removed from the parent with remove(),
   * it will try to remove its parent instead, until it finds a node that can be removed, or no
   * parent can be found.
   *
   * <p>Since everything at CompilationUnit level is removable, this method will only (silently)
   * fail when the node is in a detached AST fragment.
   */
  ...
  /**
   * Register a new observer for the given node. Depending on the mode specified also descendants,
   * existing and new, could be observed. For more details see <i>ObserverRegistrationMode</i>.
   */
  ...
  /**
   * Register the observer for the current node and all the contained node and nodelists,
   * recursively.
   */
  ...
  /**
   * @return whether this node was successfully parsed or not. If it was not, only the range and
   *     tokenRange fields will be valid.
   */
  ...
  public SymbolResolver getSymbolResolver() {
    return findCompilationUnit()
        ...
        .orElseThrow(
            () -> new IllegalStateException("The node is not inserted in a CompilationUnit"));
  }
  ...
  /**
   * Walks the AST, calling the consumer for every node, with traversal algorithm "traversal". <br>
  ...
  /**
   * Walks the AST with pre-order traversal, returning all nodes of type "nodeType" that match the
   * predicate.
   */
  ...
  /**
   * Walks the AST, applying the function for every node, with traversal algorithm "traversal". If
   * the function returns something else than null, the traversal is stopped and the function result
   * is returned. <br>
  ...
  /**
   * Walks the AST with pre-order traversal, returning the first node of type "nodeType" or empty()
   * if none is found.
   */
  ...
  /**
   * Walks the AST with pre-order traversal, returning the first node of type "nodeType" that
   * matches "predicate" or empty() if none is found.
   */
  ...
  /**
   * Determines whether this node is an ancestor of the given node. A node is <i>not</i> an ancestor
   * of itself.
   *
   * @param descendant the node for which to determine whether it has this node as an ancestor.
   * @return {@code true} if this node is an ancestor of the given node, and {@code false}
   *     otherwise.
  ...
  /**
   * Iterates over the parent of the node, then the parent's parent, then the parent's parent's
   * parent, until running out of parents.
   */
  ...
  public boolean hasScope() {
    return (NodeWithOptionalScope.class.isAssignableFrom(this.getClass())
        ...
        || (NodeWithScope.class.isAssignableFrom(this.getClass())
            && ((NodeWithScope) this).getScope() != null);
  }
