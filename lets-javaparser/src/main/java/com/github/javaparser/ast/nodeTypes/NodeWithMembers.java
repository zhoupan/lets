...
public interface NodeWithMembers<N extends Node> extends NodeWithSimpleName<N> {
   ...
   */
  default FieldDeclaration addField(
      Class<?> typeClass, String name, Modifier.Keyword... modifiers) {
  ...
  default List<MethodDeclaration> getMethodsByName(String name) {
    return unmodifiableList(
        getMethods().stream().filter(m -> m.getNameAsString().equals(name)).collect(toList()));
  }
  ...
  /**
   * Try to find a {@link MethodDeclaration} by its parameter types. The given parameter types must
   * <i>literally</i> match the declared types of this node's parameters, so passing the string
   * {@code "List"} to this method will find all methods that have exactly one parameter whose type
   * is declared as {@code List}, but not methods with exactly one parameter whose type is declared
   * as {@code java.util.List} or {@code java.awt.List}. Conversely, passing the string {@code
   * "java.util.List"} to this method will find all methods that have exactly one parameter whose
   * type is declared as {@code java.util.List}, but not if the parameter type is declared as {@code
   * List}. Similarly, note that generics are matched as well: If there is a method that has a
   * parameter declared as {@code List&lt;String&gt;}, then it will be considered as a match only if
   * the given string is {@code "List&lt;String&gt;"}, but not if the given string is only {@code
   * "List"}.
   *
   * @param paramTypes the types of parameters like {@code "Map&lt;Integer, String&gt;", "int"} to
   *     match {@code void foo(Map&lt;Integer,String&gt; myMap, int number)}
   * @return the methods found
   */
  default List<MethodDeclaration> getMethodsByParameterTypes(String... paramTypes) {
    return unmodifiableList(
        getMethods().stream().filter(m -> m.hasParametersOfType(paramTypes)).collect(toList()));
  }

  /**
   * Try to find {@link MethodDeclaration}s by their name and parameter types. Parameter types are
   * matched exactly as in the case of {@link #getMethodsByParameterTypes(String...)}.
   *
   * @param paramTypes the types of parameters like {@code "Map&lt;Integer, String&gt;", "int"} to
   *     match {@code void foo(Map&lt;Integer,String&gt; myMap, int number)}
   * @return the methods found
  ...
  /**
   * Try to find a {@link MethodDeclaration} by its parameter types. Note that this is a match in
   * SimpleName, so {@code java.awt.List} and {@code java.util.List} are identical to this
   * algorithm. In addition, note that it is the erasure of each type which is considered, so
   * passing {@code List.class} to this method will return all methods that have exactly one
   * parameter whose type is named {@code List}, regardless of whether the parameter type is
   * declared without generics as {@code List}, or with generics as {@code List&lt;String&gt;}, or
   * {@code List&lt;Integer&gt;}, etc.
   *
   * @param paramTypes the types of parameters like {@code Map.class, int.class} to match {@code
  ...
  /**
   * Find all constructors in the members of this node. Note that only "normal" constructors, not
   * the "compact" constructors", within {@link RecordDeclaration} are included in the output of
   * this method.
   *
  ...
  /**
   * Try to find a {@link ConstructorDeclaration} by its parameter types. The given parameter types
   * must <i>literally</i> match the declared types of the desired constructor, so passing the
   * string {@code "List"} to this method will search for a constructor that has exactly one
   * parameter whose type is declared as {@code List}, but not for a constructor with exactly one
   * parameter whose type is declared as {@code java.util.List} or {@code java.awt.List}.
   * Conversely, passing the string {@code "java.util.List"} to this method will search for a
   * constructor that has exactly one parameter whose type is declared as {@code java.util.List},
   * but not for a constructor whose type is declared as {@code List}. Similarly, note that generics
   * are matched as well: If there is a constructor that has a parameter declared as {@code
   * List&lt;String&gt;}, then it will be considered as a match only if the given string is {@code
   * "List&lt;String&gt;"}, but not if the given string is only {@code "List"}.
   *
   * @param paramTypes the types of parameters like {@code "Map&lt;Integer, String&gt;", "int"} to
   *     match {@code Foo(Map&lt;Integer,String&gt; myMap, int number)}
   * @return the constructor found, if any.
   */
   ...
   * in SimpleName, so {@code java.awt.List} and {@code java.util.List} are identical to this
   * algorithm. In addition, note that it is the erasure of each type which is considered, so
   * passing {@code List.class} to this method will search for a constructor that has exactly one
   * parameter whose type is named {@code List}, regardless of whether the parameter type is
   * declared without generics as {@code List}, or with generics as {@code List&lt;String&gt;}, or
   * {@code List&lt;Integer&gt;}, etc.
   *
  ...
  default Optional<FieldDeclaration> getFieldByName(String name) {
    return getMembers().stream()
        ...
        .filter(f -> f.getVariables().stream().anyMatch(var -> var.getNameAsString().equals(name)))
        .findFirst();
  }
