...
/**
 * Access of a field of an object or a class. <br>
 * In {@code person.name} "name" is the name and "person" is the scope.
 *
...
public class FieldAccessExpr extends Expression
  ...
  public FieldAccessExpr(
      TokenRange tokenRange, Expression scope, NodeList<Type> typeArguments, SimpleName name) {
    super(tokenRange);
    setScope(scope);
   ...
   * @throws UnsolvedSymbolException if the declaration corresponding to the field access expression
  ...
  public boolean isInternal() {
    return this.getParentNode().isPresent()
        && this.getParentNode().get() instanceof FieldAccessExpr;
  }

  /**
   * Indicate if this FieldAccessExpr is top level, i.e., it is not directly contained in a larger
   * FieldAccessExpr.
   */
