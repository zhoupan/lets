 ...
 *
 * <ul>
 *   <li>The first one has label 1 and no statements.
 *   <li>The second has label 2 and two statements (the println and the break).
 *   <li>The third, the default, has no label and one statement.
 * </ul>
 *
 * <h2>Java 12-</h2>
 *
 ...
 *
 * <ul>
 *   <li>The first one is of type EXPRESSION and stores its {@link Expression} in an {@link
 *       ExpressionStmt} which is stored as the first and only statement in statements.
 *   <li>The second one is of type BLOCK and stores its {@link BlockStmt} as the first and only
 *       statement in statements.
 *   <li>The third one is of type THROWS_STATEMENT and stores its {@link ThrowStmt} as the first and
 *       only statement in statements.
 * </ul>
 *
 * <pre>{@code
...
public class SwitchEntry extends Node implements NodeWithStatements<SwitchEntry> {
  ...
  @OptionalProperty private Expression guard;
  ...
  /**
   * This constructor exists for backwards compatibility for code that instantiated `SwitchEntries`
   * before the `isDefault` and guard fields were added.
   */
  public SwitchEntry(
   ...
   */
  public SwitchEntry(
      final NodeList<Expression> labels, final Type type, final NodeList<Statement> statements) {
  ...
  /**
   * This is required for the ConcreteSyntaxModel, specifically to determine whether this entry uses
   * the classic switch statement syntax (e.g. `case X: ...`) or the newer switch expression syntax
   * (`case X -> ...`).
   *
   * <p>The entry type is STATEMENT_GROUP in the switch statement case and all other values are for
   * the various switch expressions.
   */
  @DerivedProperty
