 ...
 * <h2>Java 13-</h2>
 *
 * A text block is a multi-line string. It was introduced in JEP 355. The content of "value" is
 * byte-for-byte exactly what is in the source code.
 */
public class TextBlockLiteralExpr extends LiteralStringValueExpr {
  ...
  /**
   * Most of the algorithm for stripIndent, stopping just before concatenating all the lines into a
   * single string. Useful for tools.
   */
  public Stream<String> stripIndentOfLines() {
    ...
    int commonWhiteSpacePrefixSize =
        range(0, rawLines.length)
    ...
    return Arrays.stream(rawLines)
        .map(
            l ->
                l.length() < commonWhiteSpacePrefixSize
                    ? l
                    : l.substring(commonWhiteSpacePrefixSize))
