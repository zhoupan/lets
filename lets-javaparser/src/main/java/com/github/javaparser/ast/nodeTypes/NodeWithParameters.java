...
public interface NodeWithParameters<N extends Node> {
   ...
   *
   * @param className the name of the class, ex : org.test.Foo or Foo if you added manually the
   *     import
   ...
   * match the declared types of this node's parameters, so passing the string {@code "List"} to
   * this method will be considered a match if this node has exactly one parameter whose type is
   * declared as {@code List}, but not if the parameter type is declared as {@code java.util.List}
   * or {@code java.awt.List}. Conversely, passing the string {@code "java.util.List"} to this
   * method will be considered a match if this node has exactly one parameter whose type is declared
   * as {@code java.util.List}, but not if the parameter type is declared as {@code List}.
   * Similarly, note that generics are matched as well: If this node has one parameter declared as
  ...
  /**
   * Check if the parameters have certain types. Note that this is a match in SimpleName, so {@code
   * java.awt.List} and {@code java.util.List} are identical to this algorithm. In addition, note
   * that it is the erasure of each type which is considered, so passing {@code List.class} to this
   * method will be considered a match if this node has exactly one parameter whose type is named
   * {@code List}, regardless of whether the parameter type is declared without generics as {@code
   * List}, or with generics as {@code List&lt;String&gt;}, or {@code List&lt;Integer&gt;}, etc.
   *
  ...
  default boolean hasParametersOfType(Class<?>... paramTypes) {
    return // if p.getType() is a class or interface type, we want to consider its erasure, i.e., if
    // the parameter
