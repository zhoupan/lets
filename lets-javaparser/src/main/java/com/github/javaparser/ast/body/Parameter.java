...
/**
 * The parameters to a method or lambda. Lambda parameters may have inferred types, in that case
 * "type" is UnknownType. <br>
 * Note that <a
 * href="https://en.wikipedia.org/wiki/Parameter_(computer_programming)#Parameters_and_arguments">parameters
 * are different from arguments.</a> <br>
 * "String x" and "float y" are the parameters in {@code int abc(String x, float y) {...}} <br>
 * All annotations preceding the type will be set on this object, not on the type. JavaParser
 * doesn't know if it they are applicable to the parameter or the type.
 *
...
public class Parameter extends Node
  ...
  public Parameter setVarArgsAnnotations(final NodeList<AnnotationExpr> varArgsAnnotations) {
    ...
    notifyPropertyChange(
        ObservableProperty.VAR_ARGS_ANNOTATIONS, this.varArgsAnnotations, varArgsAnnotations);
  ...
  /**
   * Record components (parameters here) are implicitly final, even without the explicitly-added
   * modifier. https://openjdk.java.net/jeps/359#Restrictions-on-records
   *
   * <p>If wanting to find out if the keyword {@code final} has been explicitly added to this
   * parameter, you should use {@code node.hasModifier(Modifier.Keyword.FINAL)}
   *
   * @return true if the node parameter is explicitly final (keyword attached) or implicitly final
   *     (e.g. parameters to a record)
   */
  @Override
