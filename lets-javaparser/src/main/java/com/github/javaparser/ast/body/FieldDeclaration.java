...
/**
 * The declaration of a field in a class. "private static int a=15*15;" in this example: {@code
 * class X { private static int a=15*15; }} <br>
...
public class FieldDeclaration extends BodyDeclaration<FieldDeclaration>
  ...
  /**
   * Create a getter for this field, <b>will only work if this field declares only 1 identifier and
   * if this field is already added to a ClassOrInterfaceDeclaration</b>
   *
   * @return the {@link MethodDeclaration} created
   * @throws IllegalStateException if there is more than 1 variable identifier or if this field
   *     isn't attached to a class or enum
   */
  public MethodDeclaration createGetter() {
    if (getVariables().size() != 1)
      throw new IllegalStateException(
          "You can use this only when the field declares only 1 variable name");
    ...
    String fieldNameUpper =
        fieldName.toUpperCase().substring(0, 1) + fieldName.substring(1, fieldName.length());
    final MethodDeclaration getter;
    getter =
        parentClass
            .map(clazz -> clazz.addMethod("get" + fieldNameUpper, Modifier.Keyword.PUBLIC))
            .orElseGet(
                () -> parentEnum.get().addMethod("get" + fieldNameUpper, Modifier.Keyword.PUBLIC));
  ...
  /**
   * Create a setter for this field, <b>will only work if this field declares only 1 identifier and
   * if this field is already added to a ClassOrInterfaceDeclaration</b>
   *
   * @return the {@link MethodDeclaration} created
   * @throws IllegalStateException if there is more than 1 variable identifier or if this field
  ...
  public MethodDeclaration createSetter() {
    ...
    setter =
        parentClass
            ...
            .orElseGet(
                () -> parentEnum.get().addMethod("set" + fieldNameUpper, Modifier.Keyword.PUBLIC));
    ...
    blockStmt2.addStatement(
        new AssignExpr(
            new NameExpr("this." + fieldName), new NameExpr(fieldName), Operator.ASSIGN));
    return setter;
  }
