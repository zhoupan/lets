...
/**
 * A list of nodes. It usually has a parent node. Unlike normal Nodes, this does not mean that it is
 * a child of that parent. Instead, this list will make every node it contains a child of its
 * parent. This way, a NodeList does not create an extra level inside the AST.
 *
...
public class NodeList<N extends Node>
  ...
  @InternalProperty private final List<N> innerList = new ArrayList<>(0);
  ...
  public N set(int index, N element) {
    if (index < 0 || index >= innerList.size()) {
      throw new IllegalArgumentException(
          "Illegal index. The index should be between 0 and "
              + innerList.size()
  ...
  private void notifyElementAdded(int index, Node nodeAddedOrRemoved) {
    this.observers.forEach(
        o -> o.listChange(this, AstObserver.ListChangeType.ADDITION, index, nodeAddedOrRemoved));
  }

  private void notifyElementRemoved(int index, Node nodeAddedOrRemoved) {
    this.observers.forEach(
        o -> o.listChange(this, AstObserver.ListChangeType.REMOVAL, index, nodeAddedOrRemoved));
  }

  private void notifyElementReplaced(int index, Node nodeAddedOrRemoved) {
    this.observers.forEach(
        o -> o.listReplacement(this, index, this.get(index), nodeAddedOrRemoved));
  }
