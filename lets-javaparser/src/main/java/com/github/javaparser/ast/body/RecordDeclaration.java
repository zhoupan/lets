 ...
 *
 * <p>A definition of a record.<br>
 * {@code record X(...) { ... }}
 *
 * <p>Note that the syntax of records is substantively different to standard
 * classes/interfaces/enums. Specifically, note that record header contains the component
 * declarations - where a "component" is defined as a non-static field.
 *
 * <p>This is in contrast to "normal" classes, where fields declarations are within the class body
 * (optionally then initialised within a constructor.
 *
 * <p>Also note that the constructor for records does not accept any parameters.
 *
 * <p>Consider this example from https://openjdk.java.net/jeps/359
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>To access these non-static field declarations, use {@link RecordDeclaration#getParameters()}
 *
 * @author Roger Howell
 * @see <a href="https://openjdk.java.net/jeps/359">https://openjdk.java.net/jeps/395</a>
 * @see <a href="https://docs.oracle.com/javase/specs/jls/se16/html/jls-8.html#jls-8.10">JLS 8.10 -
 *     Record Classes</a>
 * @since 3.22.0
 */
public class RecordDeclaration extends TypeDeclaration<RecordDeclaration>
  ...
  @Generated("com.github.javaparser.generator.core.node.PropertyGenerator")
  public RecordDeclaration setImplementedTypes(
      final NodeList<ClassOrInterfaceType> implementedTypes) {
  ...
  // TODO document and remove duplication between here and
   ...
   * {}}. Records are different, where the record declaration can include parameters e.g. {@code
   * record X(int a) {}}. Additionally, note that the constructor for a record does not allow the
   * declaration of parameters. See the JEP for details.
   *
   ...
   *
   * <p>If wanting to find out if the keyword {@code final} is explicitly added to this parameter,
  ...
  /**
   * Record components are implicitly static when nested (i.e. when the parent isn't a compilation
   * unit). https://openjdk.java.net/jeps/359#Restrictions-on-records
   *
   * @return True if the record declaration is nested, otherwise use the default method
   *     implementation.
   */
  @Override
  ...
  /**
   * @return Only the "compact" constructors within this record, not "normal" constructors (which
   *     are obtainable via {@link #getConstructors()}).
   */
