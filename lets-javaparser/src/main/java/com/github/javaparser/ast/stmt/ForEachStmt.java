 ...
 * {@code for(Object o: objects) { ... }} It was introduced in Java 5.
 *
...
public class ForEachStmt extends Statement implements NodeWithBody<ForEachStmt> {
   ...
   * Note that any foreach statement's variable declaration expression (as returned by {@link
   * #getVariable()}) always has exactly one variable declarator.
   *
   * <p>Calling this method on a foreach statement {@code forEachStmt} is equivalent to calling
   ...
   * Note that any foreach statement's variable declaration expression (as returned by {@link
   * #getVariable()}) always has either no modifiers, or a single {@code final} modifier.
   *
   * <p>Calling this method on a foreach statement {@code forEachStmt} is equivalent to calling
   ...
   *
   * @return {@code true} if this foreach statement's variable is {@code final}, and {@code false}
   *     otherwise.
   */
