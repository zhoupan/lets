...
/**
 * A class or an interface type. <br>
 * {@code Object} <br>
 ...
 * <p>
 *
 * <p>Note that the syntax is ambiguous here, and JavaParser does not know what is to the left of
 * the class. It assumes cases like {@code Map.Entry} where Map is the scope of Entry. In {@code
 * java.util.Punchcard}, it will not recognize that java and util are parts of the package name.
 * Instead, it will set util as the scope of Punchcard, as a ClassOrInterfaceType (which it is not).
 * In turn, util will have java as its scope, also as a ClassOrInterfaceType
 *
...
public class ClassOrInterfaceType extends ReferenceType
  ...
  /**
   * @deprecated use JavaParser.parseClassOrInterfaceType instead. This constructor does not
   *     understand generics.
   */
  ...
  public String asString() {
    ...
    getTypeArguments()
        .ifPresent(
            ta -> str.append(ta.stream().map(Type::asString).collect(joining(",", "<", ">"))));
    return str.toString();
  }
  ...
  public ResolvedType convertToUsage(Context context) {
    ...
    return new ReferenceTypeImpl(
        (ResolvedReferenceTypeDeclaration) typeDeclaration, typeParameters);
  }
}
