...
/**
 * A type parameter. Examples: <br>
 * {@code <}<b>{@code U}</b>{@code > U getU() { ... }} <br>
 * {@code class D <}<b>{@code @Brain T extends B & A & @Tripe C}</b>{@code > { ... }}
 *
 * <p>U and T are type parameter names. <br>
...
public class TypeParameter extends ReferenceType
  ...
  /**
   * Return the list of {@link ClassOrInterfaceType} that this parameter extends. Return {@code
   * null} null if there are no type.
   *
