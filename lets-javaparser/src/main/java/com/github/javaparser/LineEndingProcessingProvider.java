  ...
  public int read(char[] buffer, final int offset, int len) throws IOException {
    ...
    while (pos < stop) {
      ...
      if (lookup.isPresent()) {
        ...
        // TODO: Make this more generic than just CRLF (e.g. track the previous char rather than the
        ...
        // If "this" (current) char <strong>is</strong> a line separator, set the next loop's
        // "previous" to this
        previousLineSeparator = lineSeparator;
      } else {
        // If "this" (current) char <strong>is not</strong> a line separator, set the next loop's
        // "previous" to null
