...
abstract class GeneratedJavaParserBase {
  ...
  /**
   * Return the list of JavaParser specific tokens that have been encountered while parsing code
   * using this parser.
   *
  ...
  TokenRange range(Node begin, Node end) {
    if (storeTokens) {
      return new TokenRange(
          begin.getTokenRange().get().getBegin(), end.getTokenRange().get().getEnd());
    }
    return null;
  }
  ...
  /* Called from within a catch block to skip forward to a known token,
  and report the occurred exception as a problem. */
  TokenRange recoverStatement(
      int recoveryTokenType, int lBraceType, int rBraceType, ParseException p) {
    JavaToken begin = null;
    if (p.currentToken != null) {
      begin = token();
    }
  ...
  /**
   * Propagate expansion of the range on the right to the parent. This is necessary when the right
   * border of the child is determining the right border of the parent (i.e., the child is the last
   * element of the parent). In this case when we "enlarge" the child we should enlarge also the
   * parent.
   */
  private void propagateRangeGrowthOnRight(Node node, Node endNode) {
    if (storeTokens) {
      node.getParentNode()
          .ifPresent(
              nodeParent -> {
                boolean isChildOnTheRightBorderOfParent =
                    node.getTokenRange()
                        .get()
                        .getEnd()
  ...
  ArrayCreationExpr juggleArrayCreation(
    ...
    for (int i = 0; i < arrayAnnotations.size(); i++) {
      levels.add(
          new ArrayCreationLevel(levelRanges.get(i), dimensions.get(i), arrayAnnotations.get(i)));
    }
