...
/**
 * A representation of line endings, that can be used throughout the codebase. <br>
 * This is to replace {@code Utils.EOL} which is not explicit in representing the system's EOL
 * character. <br>
 * It also exposes helper methods for, e.g., detection of the line ending of a given string.
 *
 * @author Roger Howell
 * @see <a
 *     href="https://github.com/javaparser/javaparser/issues/2647">https://github.com/javaparser/javaparser/issues/2647</a>
 */
public enum LineSeparator {
  ...
  SYSTEM(
      ...
      "SYSTEM : ("
          ...
          + ")"),
  /**
   * The ARBITRARY line ending can be used where we do not care about the line separator, only that
   * we use the same one consistently
   */
  ARBITRARY("\n", "ARBITRARY (\\n)"),
  /**
   * The MIXED line ending is used where strings appear to have multiple different line separators
   * e.g. {@code "line 1\nline 2\rline 3\r\n"} or {@code "line 1\nline 2\rline 3\nline 4\n"}
   */
  MIXED("", "MIXED"),
  /**
   * The UNKNOWN line ending can be used in the case where the given string has not yet been
   * analysed to determine its line separator
   */
  UNKNOWN("", "UNKNOWN"),
  /**
   * The NONE line ending is used where there are precisely zero line endings e.g. a simple one-line
   * string
   */
  NONE("", "NONE");
  ...
  public boolean isStandardEol() {
    ...
    return equalsString(LineSeparator.CR)
        || equalsString(LineSeparator.LF)
        || equalsString(LineSeparator.CRLF);
  }
  ...
  //    // TODO: Determine if this should be used within TokenTypes.java -- thus leaving this as
  // private for now.
