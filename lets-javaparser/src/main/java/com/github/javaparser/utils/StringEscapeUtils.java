 ...
 *
 * <p>Unescapes escaped chars in strings.
 */
public final class StringEscapeUtils {
   ...
   * another {@code '\'}.
   *
   * <p>This can be replaced by String::translateEscapes in JDK 13
   *
   ...
   * Adapted from apache commons-lang3 project.
   *
   * <p>An API for translating text. Its core use is to escape and unescape text. Because escaping
   * and unescaping is completely contextual, the API does not present two separate signatures.
   *
   * @since 3.0
   */
   ...
   * Adapted from apache commons-lang3 project.
   *
   * <p>Translates a value using a lookup table.
   *
   * @since 3.0
   */
  private static class LookupTranslator extends CharSequenceTranslator {
     ...
     *
     * <p>Note that, as of Lang 3.1, the key to the lookup table is converted to a java.lang.String.
     * This is because we need the key to support hashCode and equals(Object), allowing it to be the
     * key for a HashMap. See LANG-882.
     *
    ...
    /** {@inheritDoc} */
    @Override
    protected int translate(final CharSequence input, final int index, final Writer out)
        throws IOException {
   ...
   * Adapted from apache commons-lang3 project.
   *
   * <p>Executes a sequence of translators one after the other. Execution ends whenever the first
   * translator consumes codepoints from the input.
   *
   * @since 3.0
   */
   ...
   * Adapted from apache commons-lang3 project.
   *
   * <p>Translate escaped octal Strings back to their octal values.
   *
   * <p>For example, "\45" should go back to being the specific value (a %).
   *
   * <p>Note that this currently only supports the viable range of octal for Java; namely 1 to 377.
   * This is because parsing Java is the main use case.
   *
   * @since 3.0
   */
  private static class OctalUnescaper extends CharSequenceTranslator {
    ...
    /**
     * Checks if the given char is an octal digit. Octal digits are the character representations of
     * the digits 0 to 7.
     *
     * @param ch the char to check
     * @return true if the given char is the character representation of one of the digits from 0 to
     *     7
     */
   ...
   * Adapted from apache commons-lang3 project.
   *
   * <p>Translates escaped Unicode values of the form \\u+\d\d\d\d back to Unicode. It supports
