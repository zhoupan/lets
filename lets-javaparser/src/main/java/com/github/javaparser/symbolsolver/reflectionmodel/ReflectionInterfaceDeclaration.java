...
public class ReflectionInterfaceDeclaration extends AbstractTypeDeclaration
  ...
  public Optional<MethodUsage> solveMethodAsUsage(
    ...
    Optional<MethodUsage> res =
        ReflectionMethodResolutionLogic.solveMethodAsUsage(
    ...
    if (res.isPresent()) {
      ...
      try {
        ...
        for (int j = 0; j < parameters.size(); j++) {
          methodUsage =
              methodUsage.replaceParamType(j, inferenceContext.resolve(parameters.get(j)));
        }
  ...
  public boolean canBeAssignedTo(ResolvedReferenceTypeDeclaration other) {
    ...
    if (this.clazz.getSuperclass() != null
        && new ReflectionInterfaceDeclaration(clazz.getSuperclass(), typeSolver)
  ...
  public List<ResolvedReferenceType> getAncestors(boolean acceptIncompleteList) {
    // we do not attempt to perform any symbol solving when analyzing ancestors in the reflection
    // model, so we can
