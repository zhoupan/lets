...
public class JavaParserClassDeclaration extends AbstractClassDeclaration
  ...
  /**
   * This method is deprecated because the context is an implementation detail that should not be
   * exposed. Ideally this method should become private. For this reason all further usages of this
   * method are discouraged.
   */
  @Deprecated
  ...
  public Optional<ResolvedReferenceType> getSuperClass() {
    ...
    if (wrappedNode.getExtendedTypes().isEmpty()) {
      // All objects implicitly extend java.lang.Object -- inject it here (only when this isn't
      // java.lang.Object)
  ...
  /**
   * Resolution should move out of declarations, so that they are pure declarations and the
  ...
  public List<ResolvedReferenceType> getAncestors(boolean acceptIncompleteList) {
    ...
    for (ClassOrInterfaceType implemented : wrappedNode.getImplementedTypes()) {
      try {
        ...
        // in case we could not resolve some implemented interface, we may still be able to resolve
        // the
  ...
  private ResolvedReferenceType toReferenceType(ClassOrInterfaceType classOrInterfaceType) {
    ...
    if (!ref.isSolved()) {
      ...
      if (localScope.isPresent()) {
        String localName =
            localScope.get().getName().getId() + "." + classOrInterfaceType.getName().getId();
