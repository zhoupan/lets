...
public class LeastUpperBoundLogic {
  ...
  public ResolvedType lub(Set<ResolvedType> types) {
    ...
    //
    // The reason for computing the set of erased supertypes is to deal with situations where the
    // set of types
    // includes several distinct parameterizations of a generic type.
    //
    // For example, given List<String> and List<Object>, simply intersecting the sets
    // ST(List<String>) = {
    // List<String>, Collection<String>, Object } and ST(List<Object>) = { List<Object>,
    // Collection<Object>, Object
    // } would yield a set { Object }, and we would have lost track of the fact that the upper bound
    // can safely be
    // assumed to be a List.
    //
    // In contrast, intersecting EST(List<String>) = { List, Collection, Object } and
    // EST(List<Object>) = { List,
    // Collection, Object } yields { List, Collection, Object }, which will eventually enable us to
    // produce
    // List<?>.
    //
    ...
    //
    // Because we are seeking to infer more precise types, we wish to filter out any candidates that
    // are supertypes
    // of other candidates.
    ...
    // The first line defines lcp() on a set, such as Relevant(List), as an operation on a list of
    // the elements of
    // the set.
    // The next line defines the operation on such lists, as a pairwise reduction on the elements of
    // the list.
    ...
    //
    // Let the "candidate" parameterization of G, Candidate(G), be the most specific
    ...
    //
    // and where, if any of these elements are generic, we use the candidate parameterization (so as
    // to recover
    // type arguments):
    //
    ...
    // Formally, there may exist some other type T such that all of U1 ... Uk are subtypes of T and
    // T is a subtype
    // of lub(U1, ..., Uk).
    ...
    // Readers familiar with recursive types should note that an infinite type is not the same as a
    // recursive type.
    Collection<ResolvedType> erasedTypeParameterizations =
        relevantParameterizations.get(erasedBest);
  ...
  /**
   * Let MEC, the minimal erased candidate set for U1 ... Uk, be: MEC = { V | V in EC, and for all W
   * != V in EC, it is not the case that W <: V }
   *
  ...
  /**
   * For any element G of MEC that is a generic type, let the "relevant" parameterizations of G,
   * Relevant(G), be: Relevant(G) = { V | 1 ≤ i ≤ k: V in ST(Ui) and V = G<...> }
   *
  ...
  private Multimap<ResolvedType, ResolvedType> relevantParameterizations(
    ...
    Multimap<ResolvedType, ResolvedType> result =
  ...
  private ResolvedType best(List<ResolvedType> minimalCandidates) {
    Collections.sort(
        minimalCandidates,
        (t1, t2) -> {
          // Sort minimal candidates by name with classes before interfaces, to guarantee always the
          // same type is
  ...
  private TypeSubstitution substitution(
      List<Pair<ResolvedTypeParameterDeclaration, ResolvedType>> pairs) {
