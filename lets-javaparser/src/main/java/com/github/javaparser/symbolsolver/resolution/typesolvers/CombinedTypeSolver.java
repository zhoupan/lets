...
/**
 * A container for type solvers. All solving is done by the contained type solvers. This helps you
 * when an API asks for a single type solver, but you need several.
 *
...
public class CombinedTypeSolver implements TypeSolver {
   ...
   * If it returns {@code true} the exception will be ignored, and solving will continue using the
   * next solver in line. If it returns {@code false} the exception will be thrown, stopping the
   * solving process.
   *
   * <p>Main use case for this is to circumvent bugs or missing functionality in some type solvers.
   * If for example solver A has a bug resulting in a {@link NullPointerException}, you could use a
   * {@link ExceptionHandlers#getTypeBasedWhitelist(Class...) whitelist} to ignore that type of
   * exception. A secondary solver would then be able to step in when such an error occurs.
   *
  ...
  /**
   * Append a type solver to the current solver. <br>
    ...
    for (TypeSolver ts : elements) {
      try {
        ...
        if (!exceptionHandler.test(e)) {
          // we shouldn't ignore this exception
          throw e;
        }
      }
    }
  ...
  public static class ExceptionHandlers {
    ...
    /**
     * Ignores any exception that is {@link Class#isAssignableFrom(Class) assignable from} {@link
     ...
     * @see CombinedTypeSolver#setExceptionHandler(Predicate)
     * @see #getTypeBasedWhitelist(Class...)
     * @return A filter that ignores an exception if <b>none</b> of the listed classes are {@link
     *     Class#isAssignableFrom(Class) assignable from} the thrown exception class.
     */
    public static Predicate<Exception> getTypeBasedBlacklist(
        Class<? extends Exception>... blacklist) {
      return e -> {
     ...
     * @return A filter that ignores an exception if <b>any</b> of the listed classes are {@link
     *     Class#isAssignableFrom(Class) assignable from} the thrown exception class.
     */
    public static Predicate<Exception> getTypeBasedWhitelist(
        Class<? extends Exception>... whitelist) {
      return e -> {
