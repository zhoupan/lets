...
public class CompilationUnitContext extends AbstractJavaParserContext<CompilationUnit> {
  ...
  public SymbolReference<ResolvedTypeDeclaration> solveType(
    ...
    if (wrappedNode.getTypes() != null) {
      ...
      // another member
      // class or interface called "B". Since the type that we're looking for can be nested
      // arbitrarily deeply
      // ("A.B.C.D"), we look for the outermost type ("A" in the previous example) first, then
      // recursively invoke
      ...
      if (name.indexOf('.') > -1) {
        ...
        if (outerMostRef != null
          ...
          ref =
              ((JavaParserClassDeclaration) outerMostRef.getCorrespondingDeclaration())
          ...
          ref =
              ((JavaParserInterfaceDeclaration) outerMostRef.getCorrespondingDeclaration())
                  .solveType(name.substring(name.indexOf(".") + 1));
        }
        ...
        if (importDecl.isAsterisk()) {
          ...
          if (this.wrappedNode.getPackageDeclaration().isPresent()
              ...
              && this.wrappedNode.getTypes().stream()
                  .anyMatch(
                      it -> it.getName().getIdentifier().equals(toSimpleName(importString)))) {
            // We are using a static import on a type defined in this file. It means the value was
            // not found at
