 ...
 *
 * <p>Except for the special treatment of while, do, and for statements whose condition expression
 * has the constant value true, the values of expressions are not taken into account in the flow
 * analysis.
 *
 * <p>See JLS 14.21
 *
...
public class ControlFlowLogic {
   ...
   * while, do, or for statement of the immediately enclosing method or initializer; this statement,
   * which is called the break target, then immediately completes normally.
   *
   * <p>A break statement with label Identifier attempts to transfer control to the enclosing
   * labeled statement (§14.7) that has the same Identifier as its label; this statement, which is
   * called the break target, then immediately completes normally. In this case, the break target
   ...
   * try blocks contain the break statement and all finally clauses of those try statements can
   * complete normally.
   */
  ...
  public boolean isReachable(Statement statement) {
    ...
    // – There is at least one switch label after the last switch block statement group. – The
    ...
    // – There is a statement preceding it in the switch block and that preceding statement can
    // complete normally.
    //
    ...
    // – The do statement contains a reachable continue statement with no label, and the do
    // statement is the innermost while, do, or for statement that contains that continue statement,
    // and the continue statement continues that do statement, and the condition expression is not a
    // constant expression with value true.
    // – The do statement contains a reachable continue statement with a label L, and the do
    // statement has label L, and the continue statement continues that do statement, and the
    ...
    //
    // • A synchronized statement can complete normally iff the contained statement can complete
    // normally.
    ...
    // – Either the type of C's parameter is an unchecked exception type or Exception or a
    // superclass of Exception, or some expression or throw statement in the try block is reachable
    // and can throw a checked exception whose type is assignable to the type of C's parameter. (An
    // expression is reachable iff the innermost statement containing it is reachable.)
    ...
    //        BLOCKS AND STATEMENTS Unreachable Statements 14.21
    // The then-statement is reachable iff the if-then-else statement is reachable and the condition
    // expression is not a constant expression whose value is false.
    //        The else-statement is reachable iff the if-then-else statement is reachable and the
    // condition expression is not a constant expression whose value is true.
