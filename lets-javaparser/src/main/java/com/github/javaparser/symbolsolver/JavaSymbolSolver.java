...
/**
 * This implementation of the SymbolResolver wraps the functionality of the library to make them
 * easily usable from JavaParser nodes.
 *
 * <p>An instance of this class should be created once and then injected in all the CompilationUnit
 * for which we want to enable symbol resolution. To do so the method inject can be used, or you can
 * use {@link com.github.javaparser.ParserConfiguration#setSymbolResolver(SymbolResolver)} and the
 * parser will do the injection for you.
 *
...
public class JavaSymbolSolver implements SymbolResolver {
  ...
  /**
   * Register this SymbolResolver into a CompilationUnit, so that symbol resolution becomes
   * available to all nodes part of the CompilationUnit.
   */
  ...
  public <T> T resolveDeclaration(Node node, Class<T> resultClass) {
    if (node instanceof MethodDeclaration) {
      return resultClass.cast(
          new JavaParserMethodDeclaration((MethodDeclaration) node, typeSolver));
    }
    ...
    if (node instanceof EnumConstantDeclaration) {
      ...
      ResolvedEnumConstantDeclaration resolved =
          enumDeclaration.getEnumConstants().stream()
    ...
    if (node instanceof ConstructorDeclaration) {
      ...
      ResolvedConstructorDeclaration resolved =
          resolvedTypeDeclaration.getConstructors().stream()
              .filter(c -> c instanceof JavaParserConstructorDeclaration)
              .filter(
                  c ->
                      ((JavaParserConstructorDeclaration<?>) c).getWrappedNode()
                          == constructorDeclaration)
              .findFirst()
              .orElseThrow(
                  () ->
                      new RuntimeException(
                          "This constructor cannot be found in its parent. This seems wrong"));
      if (resultClass.isInstance(resolved)) {
        return resultClass.cast(resolved);
      }
    }
    ...
    if (node instanceof AnnotationMemberDeclaration) {
      ...
      ResolvedAnnotationMemberDeclaration resolved =
          annotationDeclaration.getAnnotationMembers().stream()
    ...
    if (node instanceof VariableDeclarator) {
      ...
      if (node.getParentNode().isPresent()
        ...
        throw new UnsupportedOperationException(
            "Parent of VariableDeclarator is: " + node.getParentNode());
      }
      if (resultClass.isInstance(resolved)) {
        return resultClass.cast(resolved);
      }
    ...
    throw new UnsupportedOperationException(
        "Unable to find the declaration of type "
  ...
  private Optional<ResolvedParameterDeclaration> resolveParameterDeclaration(
    ...
    ResolvedFieldDeclaration rfd =
        resolvedReferenceTypeDeclaration.getField(parameter.getNameAsString());
    if (rfd == null) return Optional.empty();
    ResolvedParameterDeclaration resolvedParameterDeclaration =
        new ResolvedParameterDeclaration() {
  ...
  public <T> T toResolvedType(Type javaparserType, Class<T> resultClass) {
    ...
    throw new UnsupportedOperationException(
        "Unable to get the resolved type of class "
            + resultClass.getSimpleName()
            + " from "
            + javaparserType);
  }
  ...
  public ResolvedReferenceTypeDeclaration toTypeDeclaration(Node node) {
    ...
    throw new IllegalArgumentException(
        "Cannot get a reference type declaration from " + node.getClass().getCanonicalName());
  }
}
