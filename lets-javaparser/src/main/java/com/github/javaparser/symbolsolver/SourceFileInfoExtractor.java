...
/**
 * Resolves resolvable nodes from one or more source files, and reports the results. It is mainly
 * intended as an example usage of JavaSymbolSolver.
 *
...
public class SourceFileInfoExtractor {
  ...
  private void solveMethodCalls(Node node) {
    if (node instanceof MethodCallExpr) {
      out.println(
          "  Line " + lineNr(node) + ") " + node + " ==> " + toString((MethodCallExpr) node));
    }
