...
public class MethodCallExprContext extends AbstractJavaParserContext<MethodCallExpr> {
  ...
  public Optional<ResolvedType> solveGenericType(String name) {
    ...
    // Method calls can have generic types defined, for example: {@code expr.<T1, T2>method(x, y,
    // z);} or {@code super.<T, E>check2(val1, val2).}
    ResolvedType typeOfScope = JavaParserFacade.get(typeSolver).getType(nodeScope.get());
    Optional<ResolvedType> resolvedType =
        typeOfScope.asReferenceType().getGenericParameterByName(name);

    // TODO/FIXME: Consider if we should check if the result is present, else delegate "up" the
    // context chain (e.g. {@code solveGenericTypeInParent()})
    return resolvedType;
  }
  ...
  public Optional<MethodUsage> solveMethodAsUsage(String name, List<ResolvedType> argumentsTypes) {
    ...
    for (int i = 0; i < argumentsTypes.size(); i++) {
      ...
      // for example if I replace T=U with String because I know that T=String I can derive that
      // also U equal String
  ...
  private MethodUsage resolveMethodTypeParametersFromExplicitList(
  ...
  public SymbolReference<ResolvedMethodDeclaration> solveMethod(
    ...
    Collection<ResolvedReferenceTypeDeclaration> rrtds =
        findTypeDeclarations(wrappedNode.getScope());
  ...
  private Optional<MethodUsage> solveMethodAsUsage(
    ...
    if (ref.isPresent()) {
      ...
      // At this stage I should derive from the context and the value some information on the type
      // parameters
      // for example, when calling:
      // myStream.collect(Collectors.toList())
      // I should be able to figure out that considering the type of the stream (e.g.,
      // Stream<String>)
      ...
      // In our example Stream.T equal to String, so the R (and the result of the call to collect)
      // is
      // List<? super String>
      ...
      ResolvedType returnType =
          refType.useThisTypeParametersOnTheGivenType(methodUsage.returnType());
      ...
      for (int i = 0; i < methodUsage.getParamTypes().size(); i++) {
        ResolvedType replaced =
            refType.useThisTypeParametersOnTheGivenType(methodUsage.getParamTypes().get(i));
  ...
  private void inferTypes(
    ...
    if (source.isWildcard() && target.isWildcard()) {
      if (source.asWildcard().isBounded() && target.asWildcard().isBounded()) {
        inferTypes(
            source.asWildcard().getBoundedType(), target.asWildcard().getBoundedType(), mappings);
      }
      return;
    }
  ...
  private MethodUsage resolveMethodTypeParameters(
      MethodUsage methodUsage, List<ResolvedType> actualParamTypes) {
    ...
    if (methodUsage.getDeclaration().hasVariadicParameter()) {
      if (actualParamTypes.size() == methodUsage.getDeclaration().getNumberOfParams()) {
        ...
        if (lastActualParamType.isArray()) {
          ...
          // in cases where, the expected type is assignable by the actual reference type of the
          // array
          // (Files.newInputStream(path, options) and options is a variadic argument of type
          // OpenOption)
          // or the expected type is a generic type (Arrays.asList(T... a)) and the component type
          // of the array type is a reference type
          // or the expected type is not a generic (IntStream.of(int... values)) and the component
    ...
    for (ResolvedTypeParameterDeclaration tp : typeParameters.keySet()) {
      ...
      if (!alreadyCollected) {
        Set<ResolvedType> resolvedTypes =
            findResolvedTypesByTypeVariable(typeParameterName, typeParameters);
   ...
   */
  private Map<String, ResolvedType> reduceResolvedTypesByTypeVariable(
  ...
  private void matchTypeParameters(
    ...
    if (expectedType.isTypeVariable()) {
      ...
      // in case of primitive type, the expected type must be compared with the boxed type of the
      // actual type
      if (type.isPrimitive()) {
      ...
      // avoid cases where the actual type has no type parameters but the expected one has. Such as:
      // "classX extends classY<Integer>"
  ...
  private Optional<MethodUsage> solveMethodAsUsage(
    ...
    if (type instanceof ResolvedReferenceType) {
      return solveMethodAsUsage(
          (ResolvedReferenceType) type, name, argumentsTypes, invokationContext);
    }
    ...
    if (type instanceof ResolvedTypeVariable) {
      return solveMethodAsUsage(
          (ResolvedTypeVariable) type, name, argumentsTypes, invokationContext);
    }
    if (type instanceof ResolvedWildcard) {
      ResolvedWildcard wildcardUsage = (ResolvedWildcard) type;
      if (wildcardUsage.isSuper()) {
        return solveMethodAsUsage(
            wildcardUsage.getBoundedType(), name, argumentsTypes, invokationContext);
      }
    ...
    if (type.isReferenceType()) {
      for (Pair<ResolvedTypeParameterDeclaration, ResolvedType> entry :
        ...
        if (entry.a.declaredOnType()
            ...
            && scope.asReferenceType().getGenericParameterByName(entry.a.getName()).isPresent()) {
          type =
              type.replaceTypeVariables(
                  entry.a,
                  scope.asReferenceType().getGenericParameterByName(entry.a.getName()).get(),
                  inferredTypes);
        }
      }
    }
