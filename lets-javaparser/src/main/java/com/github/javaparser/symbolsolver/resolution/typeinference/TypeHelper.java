...
/**
 * The term "type" is used loosely in this chapter to include type-like syntax that contains
 * inference variables.
 *
 * <p>Assertions that involve inference variables are assertions about every proper type that can be
...
public class TypeHelper {
   ...
   * @return
   */
  public static boolean isCompatibleInAStrictInvocationContext(
      Expression expression, ResolvedType t) {
  ...
  public static boolean isCompatibleInALooseInvocationContext(
    ...
    // throw new UnsupportedOperationException("Unable to determine if " + expression + " is
    // compatible in a loose invocation context with type " + t);
  ...
  public static boolean isCompatibleInALooseInvocationContext(ResolvedType s, ResolvedType t) {
    // Loose invocation contexts allow a more permissive set of conversions, because they are only
    // used for a
    // particular invocation if no applicable declaration can be found using strict invocation
    // contexts. Loose
  ...
  public static ResolvedType toBoxedType(
      ResolvedPrimitiveType primitiveType, TypeSolver typeSolver) {
  ...
  public static boolean areCompatibleThroughWideningReferenceConversion(
      ResolvedType s, ResolvedType t) {
    Optional<ResolvedPrimitiveType> correspondingPrimitiveTypeForS =
        Arrays.stream(ResolvedPrimitiveType.values())
    ...
    throw new UnsupportedOperationException(
        "areCompatibleThroughWideningReferenceConversion s=" + s + ", t=" + t);
  }

  public static boolean areCompatibleThroughWideningPrimitiveConversion(
      ResolvedType s, ResolvedType t) {
  ...
  /**
   * See JLS 4.10.4. Least Upper Bound. The least upper bound, or "lub", of a set of reference types
   * is a shared supertype that is more specific than any other shared supertype (that is, no other
   * shared supertype is a subtype of the least upper bound).
   */
  ...
  private static ResolvedReferenceType nonWildcardParameterizationOf(
    ...
    ResolvedReferenceTypeDeclaration originalTypeDeclaration =
        originalType
            .getTypeDeclaration()
