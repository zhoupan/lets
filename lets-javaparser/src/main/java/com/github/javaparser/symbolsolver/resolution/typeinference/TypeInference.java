...
public class TypeInference {
  ...
  public static MethodUsage toMethodUsage(
    ...
    Optional<InstantiationSet> instantiationSetOpt =
        typeInference.instantiationInference(call, methodDeclaration);
  ...
  /**
   * Determine whether a potentially applicable generic method m is applicable for a method
   * invocation that provides no explicit type arguments.
   */
  public boolean invocationApplicabilityInference(
    ...
    //            Type actualType =
    // JavaParserFacade.get(typeSolver).getType(methodCallExpr.getArgument(i));
  ...
  public BoundSet invocationTypeInferenceBoundsSetB3() {
    // Given a method invocation that provides no explicit type arguments, and a corresponding most
    // specific
    // applicable generic method m, the process to infer the invocation type (§15.12.2.6) of the
    // chosen method is
    // as follows:
    //
    // - Let θ be the substitution [P1:=α1, ..., Pp:=αp] defined in §18.5.1 to replace the type
    // parameters of m with inference variables.
    //
    // - Let B2 be the bound set produced by reduction in order to demonstrate that m is applicable
    // in §18.5.1. (While it was necessary in §18.5.1 to demonstrate that the inference variables in
    // B2 could be resolved, in order to establish applicability, the instantiations produced by
    // this resolution step are not considered part of B2.)
    //
    // - If the invocation is not a poly expression, let the bound set B3 be the same as B2.
    //
    //   If the invocation is a poly expression, let the bound set B3 be derived from B2 as follows.
    // Let R be the
    ...
    //
    //   - Otherwise, if R θ is a parameterized type, G<A1, ..., An>, and one of A1, ..., An is a
    // wildcard, then,
    //     for fresh inference variables β1, ..., βn, the constraint formula ‹G<β1, ..., βn> → T› is
    // reduced and
    ...
    //
    //     - T is a reference type, but is not a wildcard-parameterized type, and either i) B2
    // contains a bound of
    //       one of the forms α = S or S <: α, where S is a wildcard-parameterized type, or ii) B2
    // contains two
    //       bounds of the forms S1 <: α and S2 <: α, where S1 and S2 have supertypes that are two
    // different
    //       parameterizations of the same generic class or interface.
    //
    //     - T is a parameterization of a generic class or interface, G, and B2 contains a bound of
    // one of the
    //       forms α = S or S <: α, where there exists no type of the form G<...> that is a
    // supertype of S, but the
    ...
    //
    //     then α is resolved in B2, and where the capture of the resulting instantiation of α is U,
    // the constraint
  ...
  public void invocationTypeInference() {
    ...
    // applicable by strict or loose invocation, let F1, ..., Fk be the formal parameter types of m;
    // if m is applicable by variable arity invocation, let F1, ..., Fk the first k variable arity
    // parameter types of m (§15.12.2.4). Then:
    //
    ...
    //
    // For a poly class instance creation expression (§15.9) or a poly method invocation expression
    // (§15.12), C contains all the constraint formulas that would appear in the set C generated by
    // §18.5.2 when inferring the poly expression's invocation type.
    //
    // For a parenthesized expression, the search is applied recursively to the contained
    // expression.
    //
    // For a conditional expression, the search is applied recursively to the second and third
    // operands.
    //
    ...
    //
    // If ei is a poly class instance creation expression (§15.9) or a poly method invocation
    // expression (§15.12), C contains all the constraint formulas that would appear in the set C
    // generated by §18.5.2 when inferring the poly expression's invocation type.
    //
    // If ei is a parenthesized expression, these rules are applied recursively to the contained
    // expression.
    //
    // If ei is a conditional expression, these rules are applied recursively to the second and
    // third operands.
    //
    // While C is not empty, the following process is repeated, starting with the bound set B3 and
    // accumulating new bounds into a "current" bound set, ultimately producing a new bound set, B4:
    //
    // A subset of constraints is selected in C, satisfying the property that, for each constraint,
    // no input variable can influence an output variable of another constraint in C. The terms
    // input variable and output variable are defined below. An inference variable α can influence
    // an inference variable β if α depends on the resolution of β (§18.4), or vice versa; or if
    // there exists a third inference variable γ such that α can influence γ and γ can influence β.
    //
    // If this subset is empty, then there is a cycle (or cycles) in the graph of dependencies
    // between constraints. In this case, all constraints are considered that participate in a
    // dependency cycle (or cycles) and do not depend on any constraints outside of the cycle (or
    // cycles). A single constraint is selected from the considered constraints, as follows:
    //
    // If any of the considered constraints have the form ‹Expression → T›, then the selected
    // constraint is the considered constraint of this form that contains the expression to the left
    // (§3.5) of the expression of every other considered constraint of this form.
    //
    //        If no considered constraint has the form ‹Expression → T›, then the selected
    // constraint is the considered constraint that contains the expression to the left of the
    // expression of every other considered constraint.
    //
    //        The selected constraint(s) are removed from C.
    //
    //        The input variables α1, ..., αm of all the selected constraint(s) are resolved.
    //
    //        Where T1, ..., Tm are the instantiations of α1, ..., αm, the substitution [α1:=T1,
    // ..., αm:=Tm] is applied to every constraint.
    //
    //        The constraint(s) resulting from substitution are reduced and incorporated with the
    // current bound set.
    //
    // Finally, if B4 does not contain the bound false, the inference variables in B4 are resolved.
    //
    // If resolution succeeds with instantiations T1, ..., Tp for inference variables α1, ..., αp,
    // let θ' be the substitution [P1:=T1, ..., Pp:=Tp]. Then:
    //
    // If unchecked conversion was necessary for the method to be applicable during constraint set
    // reduction in §18.5.1, then the parameter types of the invocation type of m are obtained by
    // applying θ' to the parameter types of m's type, and the return type and thrown types of the
    // invocation type of m are given by the erasure of the return type and thrown types of m's
    // type.
    //
    // If unchecked conversion was not necessary for the method to be applicable, then the
    // invocation type of m is obtained by applying θ' to the type of m.
    //
    // If B4 contains the bound false, or if resolution fails, then a compile-time error occurs.
    //
    // Invocation type inference may require carefully sequencing the reduction of constraint
    // formulas of the forms ‹Expression → T›, ‹LambdaExpression →throws T›, and ‹MethodReference
    // →throws T›. To facilitate this sequencing, the input variables of these constraints are
    // defined as follows:
    //
    ...
    //
    //        If T is a functional interface type, and a function type can be derived from T
    // (§15.27.3), then the input variables include i) if the lambda expression is implicitly typed,
    // the inference variables mentioned by the function type's parameter types; and ii) if the
    // function type's return type, R, is not void, then for each result expression e in the lambda
    // body (or for the body itself if it is an expression), the input variables of ‹e → R›.
    //
    ...
    // If T is an inference variable, it is the (only) input variable.
    //
    //        If T is a functional interface type, and a function type can be derived, as described
    // in §15.27.3, the input variables include i) if the lambda expression is implicitly typed, the
    // inference variables mentioned by the function type's parameter types; and ii) the inference
    // variables mentioned by the function type's return type.
    //
    //        Otherwise, there are no input variables.
    //
    ...
    // If T is an inference variable, it is the (only) input variable.
    //
    //        If T is a functional interface type with a function type, and if the method reference
    // is inexact (§15.13.1), the input variables are the inference variables mentioned by the
    ...
    //        If T is a functional interface type with a function type, and if the method reference
    // is inexact (§15.13.1), the input variables are the inference variables mentioned by the
    // function type's parameter types and the function type's return type.
    //
    //        Otherwise, there are no input variables.
    //
    //        For ‹Expression → T›, if Expression is a parenthesized expression:
    //
    // Where the contained expression of Expression is Expression', the input variables are the
    // input variables of ‹Expression' → T›.
    //
    // For ‹ConditionalExpression → T›:
    //
    // Where the conditional expression has the form e1 ? e2 : e3, the input variables are the input
    // variables of ‹e2 → T› and ‹e3 → T›.
    //
  ...
  public void functionalInterfaceParameterizationInference(
    ...
    for (int i = 0; i < n; i++) {
      throw new UnsupportedOperationException();
      // Type pi =
      // JavaParserFacade.get(typeSolver).convertToUsage(lambdaExpr.getParameters().get(i).getType(), lambdaExpr);
      // Type qi =
      // JavaParserFacade.get(typeSolver).convertToUsage(interfaceDeclaration.getm.get(i).getType(),
      // lambdaExpr);
    ...
    // their bounds), or if F<A'1, ..., A'm> is not a subtype of F<A1, ..., Am>, no valid
  ...
  public boolean moreSpecificMethodInference(
    ...
    // When testing that one applicable method is more specific than another (§15.12.2.5), where the
    // second method
    ...
    //
    // - If m1 and m2 are applicable by strict or loose invocation (§15.12.2.2, §15.12.2.3), then
    // let S1, ..., Sk be the formal parameter types of m1, and let T1, ..., Tk be the result of θ
    // applied to the formal parameter types of m2.
    //
    // - If m1 and m2 are applicable by variable arity invocation (§15.12.2.4), then let S1, ..., Sk
    // be the first k variable arity parameter types of m1, and let T1, ..., Tk be the result of θ
    // applied to the first k variable arity parameter types of m2.
    //
    // Note that no substitution is applied to S1, ..., Sk; even if m1 is generic, the type
    // parameters of m1 are treated as type variables, not inference variables.
    //
    // The process to determine if m1 is more specific than m2 is as follows:
    //
    // - First, an initial bound set, B, is constructed from the declared bounds of P1, ..., Pp, as
    // specified in §18.1.3.
    //
    ...
    //
    //   Otherwise, if Ti is not a functional interface type, the constraint formula ‹Si <: Ti› is
    // generated.
    //
    //   Otherwise, Ti is a parameterization of a functional interface, I. It must be determined
    // whether Si satisfies the following five conditions:
    //
    ...
    //
    //   4. If Si is an intersection type, at least one element of the intersection is not a
    // superinterface of I, nor a parameterization of a superinterface of I.
    //
    //   5. If Si is an intersection type, no element of the intersection is a subinterface of I,
    // nor a parameterization of a subinterface of I.
    //
    //   If all five conditions are true, then the following constraint formulas or bounds are
    // generated (where U1 ... Uk and R1 are the parameter types and return type of the function
    // type of the capture of Si, and V1 ... Vk and R2 are the parameter types and return type of
    // the function type of Ti):
    //
    ...
    //     - If R2 is void, true.
    //
    //     - Otherwise, if R1 and R2 are functional interface types, and neither interface is a
    // subinterface of the other, and ei has at least one result expression, then these rules are
    // applied recursively to R1 and R2, for each result expression in ei.
    //
    //     - Otherwise, if R1 is a primitive type and R2 is not, and ei has at least one result
    // expression, and each result expression of ei is a standalone expression (§15.2) of a
    // primitive type, true.
    //
    //     - Otherwise, if R2 is a primitive type and R1 is not, and ei has at least one result
    // expression, and each result expression of ei is either a standalone expression of a reference
    // type or a poly expression, true.
    //
    //     - Otherwise, ‹R1 <: R2›.
    //
    ...
    //     - If R2 is void, true.
    //
    //     - Otherwise, if R1 is a primitive type and R2 is not, and the compile-time declaration
    // for ei has a primitive return type, true.
    //
    //     - Otherwise if R2 is a primitive type and R1 is not, and the compile-time declaration for
    // ei has a reference return type, true.
    //
    //     - Otherwise, ‹R1 <: R2›.
    //
    //   - If ei is a parenthesized expression, these rules are applied recursively to the contained
    // expression.
    //
    //   - If ei is a conditional expression, these rules are applied recursively to each of the
    // second and third operands.
    //
    //   - Otherwise, false.
    //
    //   If the five constraints on Si are not satisfied, the constraint formula ‹Si <: Ti› is
    // generated instead.
    //
    // - Third, if m2 is applicable by variable arity invocation and has k+1 parameters, then where
    // Sk+1 is the k+1'th variable arity parameter type of m1 and Tk+1 is the result of θ applied to
    // the k+1'th variable arity parameter type of m2, the constraint ‹Sk+1 <: Tk+1› is generated.
    //
    // - Fourth, the generated bounds and constraint formulas are reduced and incorporated with B to
    // produce a bound set B'.
    //
    //   If B' does not contain the bound false, and resolution of all the inference variables in B'
  ...
  private boolean isPertinentToApplicability(Expression argument) {
    // An argument expression is considered pertinent to applicability for a potentially applicable
    // method m
  ...
  private ConstraintFormulaSet constraintSetFromArgumentsSubstitution(
      List<ResolvedType> Fs, List<Expression> es, Substitution theta, int k) {
    ConstraintFormulaSet constraintFormulaSet = ConstraintFormulaSet.empty();
    for (int i = 0; i < k; i++) {
      Expression ei = es.get(i);
      ResolvedType fi = Fs.get(i);
