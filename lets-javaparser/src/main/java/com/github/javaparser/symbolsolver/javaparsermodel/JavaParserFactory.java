...
public class JavaParserFactory {
  ...
  public static Context getContext(Node node, TypeSolver typeSolver) {
    ...
    if (node instanceof NameExpr) {
      // to resolve a name when in a fieldAccess context, we can go up until we get a node other
      // than FieldAccessExpr,
      // in order to prevent a infinite loop if the name is the same as the field (ie x.x, x.y.x, or
      // x.y.z.x)
