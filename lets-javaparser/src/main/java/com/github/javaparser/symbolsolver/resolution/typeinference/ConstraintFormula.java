...
/**
 * Constraint formulas are assertions of compatibility or subtyping that may involve inference
 * variables.
 *
...
public abstract class ConstraintFormula {
      ...
      if (constraintFormulas.size() <= index) {
        throw new IllegalArgumentException(
            "Constraint with index "
                + index
                + " is not available as there are "
                + constraintFormulas.size()
                + " constraints");
      }
  ...
  /**
   * A formula is reduced to one or both of: i) A bound or bound set, which is to be incorporated
   * with the "current" bound set. Initially, the current bound set is empty. ii) Further constraint
   * formulas, which are to be reduced recursively.
   */
