...
public class StatementContext<N extends Statement> extends AbstractJavaParserContext<N> {
  ...
  public static SymbolReference<? extends ResolvedValueDeclaration> solveInBlock(
    ...
    for (int i = position - 1; i >= 0; i--) {
      SymbolDeclarator symbolDeclarator =
          JavaParserFactory.getSymbolDeclarator(blockStmt.getStatements().get(i), typeSolver);
  ...
  public static Optional<Value> solveInBlockAsValue(
      String name, TypeSolver typeSolver, Statement stmt) {
    Optional<Node> optionalParentNode = stmt.getParentNode();
    if (!optionalParentNode.isPresent()) {
  ...
  protected Optional<Value> solveWithAsValue(SymbolDeclarator symbolDeclarator, String name) {
    ...
    // TODO: Try to get the context of the declarator / initialisations -- then check if the
    // declarations themselves match (or vice versa)
   ...
   * @param iterateAdjacentStmts flag to iterate adjacent statements, should be set to {@code true}
   *     except when calling itself in order to prevent revisiting already visited symbols.
  ...
  private SymbolReference<? extends ResolvedValueDeclaration> solveSymbol(
    ...
    if (parentOfWrappedNode instanceof NodeWithStatements) {
      ...
      while (statementListIterator.hasPrevious()) {
        Context prevContext =
            JavaParserFactory.getContext(statementListIterator.previous(), typeSolver);
        if (prevContext instanceof BlockStmtContext) {
          // Issue #3631
          // We have an explicit check for "BlockStmtContext" to avoid resolving the variable x with
          // the
        ...
        if (prevContext instanceof StatementContext) {
          // We have an explicit check for "StatementContext" to prevent a factorial increase of
          // visited statements.
          //
          // For example consider the following:
          //   String a = "a";
          ...
          //   String c = get();
          //
          // If we simply call "prevContext.solveSymbol(name)" we will call the current method with
          // the adjacent statement "prevContext".
          // Then "prevContext" will look at its previous adjacent statement. And so on and so
          // forth.
          // When there are no more previous statements in this chain of method calls, we come back
          // to here...
          ...
          //
          // So what we do instead with this check is we pass in a flag to say "Do not look at
          // previous adjacent statements".
          // Since each visited "prevContext" does not look at its adjacent statements we only visit
          // each statement once in this while loop.
