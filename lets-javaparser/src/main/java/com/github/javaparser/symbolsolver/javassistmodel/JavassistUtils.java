...
class JavassistUtils {
    ...
    for (CtMethod method : ctClass.getDeclaredMethods()) {
      if (method.getName().equals(name)
        ...
        MethodUsage methodUsage =
            new MethodUsage(new JavassistMethodDeclaration(method, typeSolver));
  ...
  static SymbolReference<ResolvedMethodDeclaration> solveMethod(
    ...
    Predicate<CtMethod> staticOnlyCheck =
        m -> !staticOnly || java.lang.reflect.Modifier.isStatic(m.getModifiers());
    for (CtMethod method : ctClass.getDeclaredMethods()) {
    ...
    for (ResolvedReferenceType ancestorRefType : scopeType.getAncestors()) {
      Optional<ResolvedReferenceTypeDeclaration> ancestorTypeDeclOpt =
          ancestorRefType.getTypeDeclaration();
  ...
  static ResolvedType signatureTypeToType(
    ...
    if (signatureType instanceof SignatureAttribute.TypeVariable) {
      SignatureAttribute.TypeVariable typeVariableSignature =
          (SignatureAttribute.TypeVariable) signatureType;
    ...
    if (signatureType instanceof SignatureAttribute.ArrayType) {
      ...
      ResolvedType baseType =
          signatureTypeToType(arrayType.getComponentType(), typeSolver, typeParametrizable);
  ...
  private static String getTypeName(SignatureAttribute.ClassType classType) {
    ...
    return declaringClass == null
        ? classType.getName()
        : getTypeName(declaringClass) + "." + classType.getName();
  }
  ...
  private static ResolvedType objectTypeArgumentToType(
    ...
    if (typeArgument instanceof SignatureAttribute.ArrayType) {
      return new ResolvedArrayType(
          signatureTypeToType(
              ((SignatureAttribute.ArrayType) typeArgument).getComponentType(),
              typeSolver,
              typeParametrizable));
    }
   ...
   *
   * <p>The name is not available, if
   *
   * <ul>
   *   <li>the method is abstract, i.e. explicitly declared as abstract or it is a non-default
   *       interface method
   *   <li>methods and constructors from jar files, which have been compiled without debug symbols
   * </ul>
   *
   * <p>The parameters are counted from 0, skipping the implicit {@code this} parameter of
   * non-static methods.
   *
