...
public class JavaParserFacade {
  ...
  /**
   * Note that the addition of the modifier {@code synchronized} is specific and directly in
   * response to issue #2668. <br>
   * This <strong>MUST NOT</strong> be misinterpreted as a signal that JavaParser is safe to use
   * within a multi-threaded environment. <br>
   * <br>
   * Additional discussion and context from a user attempting multithreading can be found within
   * issue #2671 . <br>
   *
   * @see <a
   *     href="https://github.com/javaparser/javaparser/issues/2668">https://github.com/javaparser/javaparser/issues/2668</a>
   * @see <a
   *     href="https://github.com/javaparser/javaparser/issues/2671">https://github.com/javaparser/javaparser/issues/2671</a>
   */
  ...
  public SymbolReference<ResolvedConstructorDeclaration> solve(
    ...
    if (objectCreationExpr.getAnonymousClassBody().isPresent()) {
      ...
      ResolvedType classDecl =
          JavaParserFacade.get(typeSolver)
              .convert(objectCreationExpr.getType(), objectCreationExpr);
    ...
    SymbolReference<ResolvedConstructorDeclaration> res =
        ConstructorResolutionLogic.findMostApplicable(
            typeDecl.getConstructors(), argumentTypes, typeSolver);
    for (LambdaArgumentTypePlaceholder placeholder : placeholders) {
      placeholder.setMethod(res);
    }
    return res;
    ...
    if (typeDeclarationSymbolReference.isSolved()) {
      ResolvedAnnotationDeclaration annotationDeclaration =
          (ResolvedAnnotationDeclaration)
              typeDeclarationSymbolReference.getCorrespondingDeclaration();
   ...
   *
   * <p>This method was originally intended to get the type of a value: any value has a type.
   *
   * <p>For example:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>Now, users started using also of names of types itself, which do not have a type.
   *
   * <p>For example:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>To accommodate this usage and avoid confusion this method return the type itself when used
   * on the name of type.
   */
    ...
    try {
      ...
      if (node instanceof NameExpr) {
        ...
        if (typeDeclaration.isSolved()
            && typeDeclaration.getCorrespondingDeclaration()
                instanceof ResolvedReferenceTypeDeclaration) {
  ...
  protected MethodUsage toMethodUsage(
  ...
  /**
   * Should return more like a TypeApplication: a TypeDeclaration and possible typeParametersValues
   * or array modifiers.
   */
   ...
   *
   * <p>NOTE: See {@link #findContainingTypeDeclOrObjectCreationExpr} if wanting to include
   * anonymous inner classes.
   *
   * <p>For example, these all return X: {@code public interface X { ... node here ... }} {@code
   * public class X { ... node here ... }} {@code public enum X { ... node here ... }}
   *
  ...
  /**
   * Where a node has an interface/class/enum declaration -- or an object creation expression
   * (anonymous inner class) -- as its ancestor, return the nearest one.
   *
   * <p>NOTE: See {@link #findContainingTypeDecl} if wanting to not include anonymous inner classes.
   *
   * <p>For example, these all return X:
   *
   * <ul>
   *   <li>{@code public interface X { ... node here ... }}
   *   <li>{@code public class X { ... node here ... }}
   *   <li>{@code public enum X { ... node here ... }}
   *   <li>
   *       <pre>{@code
   * new ActionListener() {
   *      ... node here ...
   ...
   *
   * @param node The Node whose ancestors will be traversed,
   * @return The first class/interface/enum declaration -- or object creation expression (anonymous
   *     inner class) -- in the Node's ancestry.
   */
  ...
  /**
   * Where a node has an interface/class/enum declaration -- or an object creation expression in an
   * inner class references an outer class -- as its ancestor, return the declaration corresponding
   * to the class name specified.
   */
  protected Node findContainingTypeDeclOrObjectCreationExpr(Node node, String className) {
    ...
    while (true) {
      ...
      if (parent instanceof BodyDeclaration) {
        if (parent instanceof TypeDeclaration
            && ((TypeDeclaration<?>) parent).getFullyQualifiedName().get().endsWith(className)) {
          return parent;
        }
        detachFlag = true;
      }
      if (parent instanceof ObjectCreationExpr
          && ((ObjectCreationExpr) parent).getType().getName().asString().equals(className)) {
        if (detachFlag) {
          return parent;
        }
      }
  ...
  public MethodUsage solveMethodAsUsage(MethodCallExpr call) {
      ...
      for (Expression param : call.getArguments()) {
        ...
        try {
          ...
          throw failureHandler.handle(
              e,
              String.format(
                  "Error calculating the type of parameter %s of method call %s", param, call));
        }
    ...
    if (!methodUsage.isPresent()) {
      throw new UnsolvedSymbolException(
          "Method '"
              + call.getName()
              + "' cannot be resolved in context "
              + call
              + " (line: "
              + call.getRange().map(r -> "" + r.begin.line).orElse("??")
              + ") "
              + context
              + ". Parameter types: "
              + params);
