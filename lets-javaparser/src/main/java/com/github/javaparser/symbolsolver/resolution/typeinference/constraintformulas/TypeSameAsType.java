...
public class TypeSameAsType extends ConstraintFormula {
  ...
  public ReductionResult reduce(BoundSet currentBoundSet) {
    ...
    //
    // - If S has the form ? and T has the form ? extends T', the constraint reduces to ‹Object =
    // T'›.
    //
    // - If S has the form ? extends S' and T has the form ?, the constraint reduces to ‹S' =
    // Object›.
    //
    // - If S has the form ? extends S' and T has the form ? extends T', the constraint reduces to
    // ‹S' = T'›.
    //
    // - If S has the form ? super S' and T has the form ? super T', the constraint reduces to ‹S' =
    // T'›.
    //
    // - Otherwise, the constraint reduces to false.
