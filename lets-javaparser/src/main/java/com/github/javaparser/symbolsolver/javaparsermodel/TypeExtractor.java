...
public class TypeExtractor extends DefaultVisitorAdapter {
  ...
  public TypeExtractor(TypeSolver typeSolver, JavaParserFacade facade) {
    ...
    stringReferenceType =
        new LazyType(v -> new ReferenceTypeImpl(typeSolver.solveType(JAVA_LANG_STRING)));
  }
  ...
  public ResolvedType visit(ArrayCreationExpr node, Boolean solveLambdas) {
    ResolvedType res =
        facade.convertToUsage(
            node.getElementType(), JavaParserFactory.getContext(node, typeSolver));
  ...
  public ResolvedType visit(BinaryExpr node, Boolean solveLambdas) {
    switch (node.getOperator()) {
      ...
      case BINARY_OR:
      case XOR:
        return facade.getBinaryTypeConcrete(
            node.getLeft(), node.getRight(), solveLambdas, node.getOperator());
      case LESS_EQUALS:
      case LESS:
      case GREATER:
      case GREATER_EQUALS:
  ...
  public ResolvedType visit(ClassExpr node, Boolean solveLambdas) {
    ...
    return new ReferenceTypeImpl(
        typeSolver.solveType(Class.class.getCanonicalName()), ImmutableList.of(jssType));
  }
  ...
  public ResolvedType visit(ConditionalExpr node, Boolean solveLambdas) {
    ...
    ConditionalExprHandler rce =
        ConditionalExprResolver.getConditionExprHandler(thenExpr, elseExpr);
    try {
  ...
  private boolean isCompatible(ResolvedType resolvedType, ResolvedPrimitiveType primitiveType) {
    return (resolvedType.isPrimitive() && resolvedType.asPrimitive().equals(primitiveType))
        || (resolvedType.isReferenceType()
            && resolvedType.asReferenceType().isUnboxableTo(primitiveType));
  }
  ...
  /**
   * Java Parser can't differentiate between packages, internal types, and fields. All three are
   * lumped together into FieldAccessExpr. We need to differentiate them.
   */
  private ResolvedType solveDotExpressionType(
      ResolvedReferenceTypeDeclaration parentType, FieldAccessExpr node) {
  ...
  public ResolvedType visit(FieldAccessExpr node, Boolean solveLambdas) {
    ...
    if (node.getScope() instanceof NameExpr || node.getScope() instanceof FieldAccessExpr) {
      ...
      SymbolReference<ResolvedTypeDeclaration> typeAccessedStatically =
          JavaParserFactory.getContext(node, typeSolver).solveType(staticValue.toString());
      ...
      SymbolReference<ResolvedReferenceTypeDeclaration> sr =
          typeSolver.tryToSolveType(node.getScope().toString());
      if (sr.isSolved()) {
    ...
    try {
      ...
      SymbolReference<ResolvedReferenceTypeDeclaration> sref =
          typeSolver.tryToSolveType(node.toString());
      if (sref.isSolved()) {
  ...
  public ResolvedType visit(TypeExpr node, Boolean solveLambdas) {
    ...
    SymbolReference<ResolvedTypeDeclaration> typeDeclarationSymbolReference =
        JavaParserFactory.getContext(classOrInterfaceType, typeSolver).solveType(nameWithScope);
    if (typeDeclarationSymbolReference.isSolved()) {
      return new ReferenceTypeImpl(
          typeDeclarationSymbolReference.getCorrespondingDeclaration().asReferenceType());
    }
  ...
  public ResolvedType visit(ThisExpr node, Boolean solveLambdas) {
    ...
    if (node.getTypeName().isPresent()) {
      ...
      try {
        return new ReferenceTypeImpl(
            facade.getTypeDeclaration(
                facade.findContainingTypeDeclOrObjectCreationExpr(node, className)));
      } catch (IllegalStateException e) {
        // trying another approach from type solver
        Optional<CompilationUnit> cu = node.findAncestor(CompilationUnit.class);
        SymbolReference<ResolvedReferenceTypeDeclaration> clazz =
            typeSolver.tryToSolveType(className);
        if (clazz.isSolved()) {
  ...
  public ResolvedType visit(SuperExpr node, Boolean solveLambdas) {
    ...
    if (node.getTypeName().isPresent()) {
      String className = node.getTypeName().get().asString();
      SymbolReference<ResolvedTypeDeclaration> resolvedTypeNameRef =
          JavaParserFactory.getContext(node, typeSolver).solveType(className);
      if (resolvedTypeNameRef.isSolved()) {
        // Cfr JLS $15.12.1
        ResolvedTypeDeclaration resolvedTypeName =
            resolvedTypeNameRef.getCorrespondingDeclaration();
        if (resolvedTypeName.isInterface()) {
          return new ReferenceTypeImpl(resolvedTypeName.asInterface());
        }
        if (resolvedTypeName.isClass()) {
          // TODO: Maybe include a presence check? e.g. in the case of `java.lang.Object` there will
          // be no superclass.
    ...
    if (typeOfNode instanceof ResolvedClassDeclaration) {
      // TODO: Maybe include a presence check? e.g. in the case of `java.lang.Object` there will be
      // no superclass.
  ...
  public ResolvedType visit(LambdaExpr node, Boolean solveLambdas) {
    ...
    if (parentNode instanceof MethodCallExpr) {
      ...
      Log.trace(
          "getType on lambda expr %s", () -> refMethod.getCorrespondingDeclaration().getName());
          ...
          if (scope instanceof NameExpr) {
            ...
            try {
              SymbolReference<ResolvedTypeDeclaration> type =
                  JavaParserFactory.getContext(nameExpr, typeSolver)
                      .solveType(nameExpr.getName().getId());
              if (type.isSolved()) {
                staticCall = true;
              }
            } catch (Exception e) {
  ...
  private ResolvedType resolveLambda(LambdaExpr node, ResolvedType result) {
    ...
    // solveLambdas with it, to derive so the values. We should also consider the value returned by
    // the
    // lambdas
    ...
    if (functionalMethod.isPresent()) {
      ...
      ResolvedType functionalInterfaceType =
          ReferenceTypeImpl.undeterminedParameters(
              functionalMethod.get().getDeclaration().declaringType());
      ...
      if (lambdaExpr.getBody() instanceof ExpressionStmt) {
        ...
        if (returnStmts.size() > 0) {
          Set<ResolvedType> resolvedTypes =
              returnStmts.stream()
                  .map(
                      returnStmt ->
                          returnStmt
        ...
        throw new UnsupportedOperationException(
            "Cannot resolve the type of lambda expression body " + lambdaExpr.getBody());
      }
      ...
      ResolvedType functionalTypeWithReturn =
          funcInterfaceCtx.resolve(funcInterfaceCtx.addSingle(functionalInterfaceType));
  ...
  public ResolvedType visit(MethodReferenceExpr node, Boolean solveLambdas) {
    ...
    if (parentNode instanceof MethodCallExpr) {
      ...
      Log.trace(
          "getType on method reference expr %s",
      ...
      if (solveLambdas) {
        ...
        Context ctx = JavaParserFactory.getContext(node, typeSolver);
        result = result.solveGenericTypes(ctx);
        // We should find out which is the functional method (e.g., apply) and replace the params of
        // the
        // solveLambdas with it, to derive so the values. We should also consider the value returned
        // by the
        // lambdas
        Optional<MethodUsage> functionalMethodOpt =
            FunctionalInterfaceLogic.getFunctionalMethod(result);
        if (functionalMethodOpt.isPresent()) {
          ...
          for (Pair<ResolvedTypeParameterDeclaration, ResolvedType> typeParamDecl :
            ...
            functionalMethod =
                functionalMethod.replaceTypeParameter(typeParamDecl.a, typeParamDecl.b);
          }
          // replace wildcards
          ...
          ResolvedType actualType =
              facade.toMethodUsage(node, functionalMethod.getParamTypes()).returnType();
  ...
  public ResolvedType visit(FieldDeclaration node, Boolean solveLambdas) {
    ...
    throw new IllegalArgumentException(
        "Cannot resolve the type of a field with multiple variable declarations. Pick one");
  }
