...
public class JarTypeSolver implements TypeSolver {
  ...
  /**
   * @deprecated Use of this static method (previously following singleton pattern) is strongly
   *     discouraged and will be removed in a future version. For now, it has been modified to
   *     return a new instance to prevent the IllegalStateException being thrown (as reported in
   *     #2547), allowing it to be called multiple times.
   */
  @Deprecated
   ...
   *
   * <p>The entries in Jar files follows the format {@code
   * com/github/javaparser/ASTParser$JJCalls.class} while in the type solver we need to work with
   * {@code com.github.javaparser.ASTParser.JJCalls}.
   *
  ...
  private static String convertEntryPathToClassName(String entryPath) {
    if (!entryPath.endsWith(CLASS_EXTENSION)) {
      throw new IllegalArgumentException(
          String.format("The entry path should end with %s", CLASS_EXTENSION));
    }
   ...
   *
   * <p>The entries in Jar files follows the format {@code
   * com/github/javaparser/ASTParser$JJCalls.class} while in the class pool we need to work with
   * {@code com.github.javaparser.ASTParser$JJCalls}.
   *
   * @param entryPath The entryPath to be converted.
   ...
   *
   * <p>The content will be dumped into a temporary file to be used in the type solver.
   *
   ...
   *
   * <p>This file will be deleted when the virtual machine terminates.
   *
   ...
   *
   * <p>When we create a new {@link JarTypeSolver} we should store the list of solvable types.
   *
