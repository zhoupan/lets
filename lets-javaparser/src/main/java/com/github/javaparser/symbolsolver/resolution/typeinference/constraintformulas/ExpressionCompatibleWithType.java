...
public class ExpressionCompatibleWithType extends ConstraintFormula {
  ...
  public ReductionResult reduce(BoundSet currentBoundSet) {
    ...
    if (expression.isPolyExpression()) {
      ...
      // invocation
      //   type when targeting T, as defined in §18.5.2. (For a class instance creation expression,
      // the
      ...
      if (expression.isLambdaExpr()) {
        ...
        // that
        //   F<A'1, ..., A'm> is a subtype of F<A1, ..., Am> is not performed (instead, it is
        // asserted with a
        ...
        //
        //   - Otherwise,) the congruence of LambdaExpression with the target function type is
        // asserted as
        //     follows:
        //
          ...
          if (TypeHelper.isProperType(R)) {
            ...
            if (lambdaExpr.getBody().isBlockStmt()) {
              getAllReturnExpressions(lambdaExpr.getBody().asBlockStmt())
                  .forEach(
                      e -> constraints.add(new ExpressionCompatibleWithType(typeSolver, e, R)));
            } else {
      ...
      if (expression.isMethodReferenceExpr()) {
        ...
        //
        // - Otherwise, if the method reference is exact (§15.13.1), then let P1, ..., Pn be the
        // parameter types of the function type of T, and let F1, ..., Fk be the parameter types of
        // the potentially applicable method. The constraint reduces to a new set of constraints, as
        // follows:
        //
        //   - In the special case where n = k+1, the parameter of type P1 is to act as the target
        // reference of the invocation. The method reference expression necessarily has the form
        // ReferenceType :: [TypeArguments] Identifier. The constraint reduces to ‹P1 <:
        // ReferenceType› and, for all i (2 ≤ i ≤ n), ‹Pi → Fi-1›.
        //
        //     In all other cases, n = k, and the constraint reduces to, for all i (1 ≤ i ≤ n), ‹Pi
        // → Fi›.
        //
        //   - If the function type's result is not void, let R be its return type. Then, if the
        // result of the potentially applicable compile-time declaration is void, the constraint
        // reduces to false. Otherwise, the constraint reduces to ‹R' → R›, where R' is the result
        // of applying capture conversion (§5.1.10) to the return type of the potentially applicable
        ...
        //
        //   - Otherwise, a search for a compile-time declaration is performed, as specified in
        // §15.13.1. If there is no compile-time declaration for the method reference, the
        // constraint reduces to false. Otherwise, there is a compile-time declaration, and:
        //
        ...
        // compile-time declaration is a generic method, and the return type of the compile-time
        // declaration mentions at least one of the method's type parameters, then the constraint
        // reduces to the bound set B3 which would be used to determine the method reference's
        // invocation type when targeting the return type of the function type, as defined in
        // §18.5.2. B3 may contain new inference variables, as well as dependencies between these
  ...
  private boolean isCompatibleInAssignmentContext(
      Expression expression, ResolvedType type, TypeSolver typeSolver) {
  ...
  private MethodType replaceTypeVariablesWithInferenceVariables(MethodType methodType) {
    ...
    for (ResolvedType formalArg : methodType.getFormalArgumentTypes()) {
      newFormalArgumentTypes.add(
          replaceTypeVariablesWithInferenceVariables(formalArg, correspondences));
    }
