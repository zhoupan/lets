...
public class BoundSet {
  ...
  /**
   * It is sometimes convenient to refer to an empty bound set with the symbol true; this is merely
   * out of convenience, and the two are interchangeable.
   */
  ...
  private Optional<Pair<SameAsBound, SameAsBound>> findPairSameAs(
  ...
  private <T> T forEachPairSameAs(
      Processor<SameAsBound, SameAsBound, T> processor, T initialValue) {
  ...
  private <T> T forEachPairSameAndSubtype(
      Processor<SameAsBound, SubtypeOfBound, T> processor, T initialValue) {
    T currentValue = initialValue;
    for (int i = 0; i < bounds.size(); i++) {
      Bound bi = bounds.get(i);
      if (bi instanceof SameAsBound) {
  ...
  private <T> T forEachPairSubtypeAndSubtype(
      Processor<SubtypeOfBound, SubtypeOfBound, T> processor, T initialValue) {
    T currentValue = initialValue;
    for (int i = 0; i < bounds.size(); i++) {
      Bound bi = bounds.get(i);
  ...
  private List<Pair<ResolvedReferenceType, ResolvedReferenceType>> findPairsOfCommonAncestors(
      ResolvedReferenceType r1, ResolvedReferenceType r2) {
    ...
    for (ResolvedReferenceType rtFrom1 : set1) {
      for (ResolvedReferenceType rtFrom2 : set2) {
        Optional<ResolvedReferenceTypeDeclaration> rtFrom1TypeDeclaration =
            rtFrom1.getTypeDeclaration();
        Optional<ResolvedReferenceTypeDeclaration> rtFrom2TypeDeclaration =
            rtFrom2.getTypeDeclaration();
  ...
  /**
   * Maintains a set of inference variable bounds, ensuring that these are consistent as new bounds
   * are added. Because the bounds on one variable can sometimes impact the possible choices for
   * another variable, this process propagates bounds between such interdependent variables.
   */
  ...
  public BoundSet deriveImpliedBounds(TypeSolver typeSolver) {
    // As bound sets are constructed and grown during inference, it is possible that new bounds can
    // be inferred
    // based on the assertions of the original bounds. The process of incorporation identifies these
    // new bounds
    // and adds them to the bound set.
    //
    // Incorporation can happen in two scenarios. One scenario is that the bound set contains
    // complementary pairs
    // of bounds; this implies new constraint formulas, as specified in §18.3.1. The other scenario
    // is that the
    // bound set contains a bound involving capture conversion; this implies new bounds and may
    // imply new
    // constraint formulas, as specified in §18.3.2. In both scenarios, any new constraint formulas
    // are reduced,
    // and any new bounds are added to the bound set. This may trigger further incorporation;
    // ultimately, the set
    // will reach a fixed point and no further bounds can be inferred.
    //
    // If incorporation of a bound set has reached a fixed point, and the set does not contain the
    // bound false,
    // then the bound set has the following properties:
    //
    // - For each combination of a proper lower bound L and a proper upper bound U of an inference
    // variable, L <: U.
    //
    // - If every inference variable mentioned by a bound has an instantiation, the bound is
    // satisfied by the
    ...
    //
    // When a bound set contains a pair of bounds that match one of the following rules, a new
    // constraint formula
    // is implied:
    //
            ...
            (a, b, currentConstraintSet) -> {
              if (areSameTypeInference(a.getS(), b.getS())) {
                currentConstraintSet =
                    currentConstraintSet.withConstraint(new TypeSameAsType(a.getT(), b.getT()));
              }
              if (areSameTypeInference(a.getS(), b.getT())) {
                currentConstraintSet =
                    currentConstraintSet.withConstraint(new TypeSameAsType(a.getS(), b.getT()));
              }
              if (areSameTypeInference(a.getT(), b.getS())) {
                currentConstraintSet =
                    currentConstraintSet.withConstraint(new TypeSameAsType(a.getT(), b.getS()));
              }
              if (areSameTypeInference(a.getT(), b.getT())) {
                currentConstraintSet =
                    currentConstraintSet.withConstraint(new TypeSameAsType(a.getS(), b.getS()));
              }
              ...
              if (areSameTypeInference(a.getS(), b.getS())) {
                currentConstraintSet =
                    currentConstraintSet.withConstraint(
                        new TypeSubtypeOfType(typeSolver, a.getT(), b.getT()));
              }
              if (areSameTypeInference(a.getT(), b.getS())) {
                currentConstraintSet =
                    currentConstraintSet.withConstraint(
                        new TypeSubtypeOfType(typeSolver, a.getS(), b.getT()));
              }
              return currentConstraintSet;
            },
            newConstraintsSet);
        ...
        forEachPairSameAndSubtype(
            (a, b, currentConstraintSet) -> {
              ...
              if (areSameTypeInference(a.getT(), b.getT())) {
                currentConstraintSet =
                    currentConstraintSet.withConstraint(
                        new TypeSubtypeOfType(typeSolver, b.getS(), a.getS()));
              }
              return currentConstraintSet;
            },
            newConstraintsSet);
        ...
        forEachPairSameAs(
            (a, b, currentConstraintSet) -> {
              if (a.getS().isInferenceVariable() && isProperType(a.getT())) {
                ...
                Substitution sub =
                    Substitution.empty().withPair(alpha.getTypeParameterDeclaration(), U);
                currentConstraintSet =
                    currentConstraintSet.withConstraint(
                        new TypeSameAsType(sub.apply(S), sub.apply(T)));
              }
            ...
            (a, b, currentConstraintSet) -> {
              if (a.getS().isInferenceVariable() && b.getS().isInferenceVariable()) {
                if (a.getT().isReferenceType() && b.getT().isReferenceType()) {
                  ...
                  List<Pair<ResolvedReferenceType, ResolvedReferenceType>> pairs =
                      findPairsOfCommonAncestors(S, T);
                  for (Pair<ResolvedReferenceType, ResolvedReferenceType> pair : pairs) {
                    for (int i = 0;
                        i
                            < Math.min(
                                pair.a.typeParametersValues().size(),
                                pair.b.typeParametersValues().size());
                        i++) {
                      ...
                      if (!si.isWildcard() && !ti.isWildcard()) {
                        currentConstraintSet =
                            currentConstraintSet.withConstraint(new TypeSameAsType(si, ti));
                      }
                    }
                  }
                }
    ...
    for (Bound b :
      ...
      //
      // A set of bounds on α1, ..., αn is implied, constructed from the declared bounds of P1, ...,
      // Pn as specified
      // in §18.1.3.
      //
  ...
  private boolean thereIsSomeJSuchThatβequalAlphaJ(
      Set<InferenceVariable> alphas, InferenceVariable beta) {
  ...
  private Optional<Set<InferenceVariable>> smallestSetWithProperty(
      Set<InferenceVariable> uninstantiatedVariables, List<VariableDependency> dependencies) {
  ...
  /**
   * if αi depends on the resolution of a variable β, then either β has an instantiation or there is
   * some j such that β = αj
   *
   * @return
   */
  private boolean hasProperty(
      Set<InferenceVariable> alphas, List<VariableDependency> dependencies) {
    for (InferenceVariable alphaI : alphas) {
      for (InferenceVariable beta :
          dependencies.stream()
  ...
  /**
   * Examines the bounds on an inference variable and determines an instantiation that is compatible
   * with those bounds. It also decides the order in which interdependent inference variables are to
   * be resolved.
   */
  public Optional<InstantiationSet> performResolution(
    ...
    //
    // Dependencies in the bound set may require that the variables be resolved in a particular
    // order, or that
    // additional variables be resolved. Dependencies are specified as follows:
    //
    // - Given a bound of one of the following forms, where T is either an inference variable β or a
    // type that
    // mentions β:
    //
    //   - α = T
    ...
    for (Set<InferenceVariable> alphas :
      ...
      if (!hasSomeCaptureForAlphas) {
        ...
        for (InferenceVariable alphaI : alphas) {
          Set<ResolvedType> properLowerBounds =
              bounds.stream()
      ...
      // i (1 ≤ i ≤ n), or;
        ...
        //
        //   - For all i (1 ≤ i ≤ n), where αi has upper bounds U1, ..., Uk, let the upper bound of
        // Yi be
  ...
  private Set<Set<InferenceVariable>> allPossibleSetsWithProperty(
      Set<InferenceVariable> allElements, List<VariableDependency> dependencies) {
  ...
  private boolean thereAreProperSubsets(
      Set<InferenceVariable> aSet, Set<Set<InferenceVariable>> allPossibleSets) {
  ...
  private boolean isTheFirstAProperSubsetOfTheSecond(
      Set<InferenceVariable> subset, Set<InferenceVariable> originalSet) {
    return originalSet.containsAll(subset) && originalSet.size() > subset.size();
  }

  private Set<Set<InferenceVariable>> allSetsWithProperty(
      Set<InferenceVariable> allElements, List<VariableDependency> dependencies) {
    Set<Set<InferenceVariable>> allPossibleSets =
        allPossibleSetsWithProperty(allElements, dependencies);
  ...
  private boolean properUpperBoundsAreAtMostExceptionThrowableAndObject(
  ...
  public List<Bound> getProperUpperBoundsFor(InferenceVariable inferenceVariable) {
    return bounds.stream()
        .filter(b -> b.isProperUpperBoundFor(inferenceVariable).isPresent())
        .collect(Collectors.toList());
  }
}
