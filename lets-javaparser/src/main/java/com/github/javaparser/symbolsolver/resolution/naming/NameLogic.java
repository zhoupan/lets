...
/**
 * NameLogic contains a set of static methods to implement the abstraction of a "Name" as defined in
 * Chapter 6 of the JLS. This code could be moved to an interface or base class in a successive
 * version of JavaParser.
 */
public class NameLogic {
   ...
   *
   * <p>Note that while most specific AST classes either always represent names or never represent
   * names there are exceptions as the FieldAccessExpr
   */
   ...
   *
   * <p>This classification is purely syntactical, i.e., it does not require symbol resolution. For
   * this reason in the future this could be moved to the core module of JavaParser.
   */
  public static NameRole classifyRole(Node name) {
    ...
    if (!name.getParentNode().isPresent()) {
      throw new IllegalArgumentException(
          "We cannot understand the role of a name if it has no parent");
    }
    if (whenParentIs(
        Name.class, name, (p, c) -> p.getQualifier().isPresent() && p.getQualifier().get() == c)) {
    ...
    if (whenParentIs(
        ...
        name,
        (p, c) -> p.getExtendedTypes().contains(c) || p.getImplementedTypes().contains(c))) {
      return NameRole.REFERENCE;
    }
    ...
    if (whenParentIs(
        FieldAccessExpr.class, name, (p, c) -> p.getName() == c || p.getScope() == c)) {
      return NameRole.REFERENCE;
    }
    ...
    throw new UnsupportedOperationException(
        "Unable to classify role of name contained in "
  ...
  public static NameCategory classifyReference(Node name, TypeSolver typeSolver) {
    if (!name.getParentNode().isPresent()) {
      throw new IllegalArgumentException(
          "We cannot understand the category of a name if it has no parent");
    }
    if (classifyRole(name) != NameRole.REFERENCE) {
      throw new IllegalArgumentException(
          "This method can be used only to classify names used as references");
    }

    // JLS 6.5
    // First, context causes a name syntactically to fall into one of seven categories: ModuleName,
    // PackageName,
  ...
  private static NameCategory reclassificationOfContextuallyAmbiguousNames(
    ...
    if (!ambiguousCategory.isNeedingDisambiguation()) {
      throw new IllegalArgumentException(
          "The Name Category is not ambiguous: " + ambiguousCategory);
    }
  ...
  private static NameCategory reclassificationOfContextuallyAmbiguousPackageOrTypeName(
    ...
    //
    // If the PackageOrTypeName, Q, is a valid TypeIdentifier and occurs in the scope of a type
    // named Q, then the
    // PackageOrTypeName is reclassified as a TypeName.
    //
    // Otherwise, the PackageOrTypeName is reclassified as a PackageName. The meaning of the
    // PackageOrTypeName is
    ...
    //
    // Given a qualified PackageOrTypeName of the form Q.Id, if Id is a valid TypeIdentifier and the
    // type or package
    // denoted by Q has a member type named Id, then the qualified PackageOrTypeName name is
    // reclassified as a
    // TypeName.
    //
    // Otherwise, it is reclassified as a PackageName. The meaning of the qualified
    // PackageOrTypeName is the meaning
    ...
    throw new UnsupportedOperationException(
        "This is unexpected: the name is neither simple or qualified");
  }
  ...
  private static NameCategory reclassificationOfContextuallyAmbiguousQualifiedAmbiguousName(
    ...
    // then the name to
    // the left of the "." is first reclassified, for it is itself an AmbiguousName. There is then a
    // choice:
    ...
    //
    //      * If the Identifier is a valid TypeIdentifier, and there is a package whose name is the
    // name to the left
    //        of the ".", and that package contains a declaration of a type whose name is the same
    // as the Identifier,
    ...
    //
    //      * Otherwise, if the Identifier is a valid TypeIdentifier and is the name of a member
    // type of the type
    ...
    if (leftNameCategory == NameCategory.TYPE_NAME) {
      SymbolReference<ResolvedTypeDeclaration> scopeTypeRef =
          JavaParserFactory.getContext(leftName, typeSolver)
      ...
      throw new UnsolvedSymbolException(
          "Unable to solve context type: " + NameLogic.nameAsString(leftName));
    }

    // * If the name to the left of the "." is reclassified as an ExpressionName, then this
    // AmbiguousName is
    //   reclassified as an ExpressionName. A later step determines whether or not a member with the
    // name Identifier
    //   actually exists.
  ...
  private static NameCategory reclassificationOfContextuallyAmbiguousSimpleAmbiguousName(
    ...
    //
    // * If the Identifier appears within the scope (§6.3) of a local variable declaration (§14.4)
    // or parameter
    //   declaration (§8.4.1, §8.8.1, §14.20) or field declaration (§8.3) with that name, then the
    // AmbiguousName is
    ...
    // * Otherwise, if a field of that name is declared in the compilation unit (§7.3) containing
    // the Identifier by
    //   a single-static-import declaration (§7.5.3), or by a static-import-on-demand declaration
    // (§7.5.4) then the
    //   AmbiguousName is reclassified as an ExpressionName.
    //
    // * Otherwise, if the Identifier is a valid TypeIdentifier and appears within the scope (§6.3)
    // of a top level
    //   class (§8 (Classes)) or interface type declaration (§9 (Interfaces)), a local class
    // declaration (§14.3) or
    //   member type declaration (§8.5, §9.5) with that name, then the AmbiguousName is reclassified
    // as a TypeName.
    //
    // * Otherwise, if the Identifier is a valid TypeIdentifier and a type of that name is declared
    // in the
    //   compilation unit (§7.3) containing the Identifier, either by a single-type-import
    // declaration (§7.5.1), or
    //   by a type-import-on-demand declaration (§7.5.2), or by a single-static-import declaration
    // (§7.5.3), or by
    //   a static-import-on-demand declaration (§7.5.4), then the AmbiguousName is reclassified as a
    // TypeName.
    //
    // Otherwise, the AmbiguousName is reclassified as a PackageName. A later step determines
    // whether or not a
   ...
   *
   * <p>Most users do not want to call directly this method but call classifyReference instead.
   */
  public static NameCategory syntacticClassificationAccordingToContext(Node name) {
    ...
    if (name.getParentNode().isPresent()
        && name.getParentNode().get() instanceof ClassOrInterfaceType) {
      return NameCategory.TYPE_NAME;
    }
  ...
  private static boolean isSyntacticallyAAmbiguousName(Node name) {
    ...
    // 2. To the left of the rightmost . that occurs before the "(" in a method invocation
    // expression

    if (whenParentIs(
        MethodCallExpr.class, name, (p, c) -> p.hasScope() && p.getScope().get() == c)) {
      return true;
    }
  ...
  private static boolean isSyntacticallyAPackageOrTypeName(Node name) {
    ...
    return whenParentIs(
        ...
        name,
        (p, c) -> !p.isStatic() && p.isAsterisk() && p.getName() == name);
  }
  ...
  private static boolean isSyntacticallyATypeName(Node name) {
    ...
    // 10. To the left of .Identifier or .super.Identifier in a qualified method invocation
    // expression (§15.12)
    //
    // 11. To the left of .super:: in a method reference expression (§15.13)
    //
    // As the Identifier or dotted Identifier sequence that constitutes any ReferenceType (including
    // a
    // ReferenceType to the left of the brackets in an array type, or to the left of the < in a
    // parameterized type,
    // or in a non-wildcard type argument of a parameterized type, or in an extends or super clause
    // of a wildcard
    ...
    // 3. The return type of a method (§8.4, §9.4) (including the type of an element of an
    // annotation type (§9.6.1))
    ...
    if (whenParentIs(
        ConstructorDeclaration.class, name, (p, c) -> p.getThrownExceptions().contains(c))) {
      return true;
    }

    // 5. In an extends clause of a type parameter declaration of a generic class, interface,
    // method, or
    ...
    if (whenParentIs(
        ...
        (p1, c1) ->
            p1.getType() == c1
                && whenParentIs(
                    FieldDeclaration.class, p1, (p2, c2) -> p2.getVariables().contains(c2)))) {
      return true;
    }
    ...
    //
    // 11. In an explicit type argument list to an explicit constructor invocation statement or
    // class instance
    ...
    if (whenParentIs(
        ClassOrInterfaceType.class,
        name,
        (p, c) -> p.getTypeArguments().isPresent() && p.getTypeArguments().get().contains(c))) {
      return true;
    }
    if (whenParentIs(
        MethodCallExpr.class,
    ...
    // 12. In an unqualified class instance creation expression, either as the class type to be
    // instantiated (§15.9)
    //     or as the direct superclass or direct superinterface of an anonymous class to be
    // instantiated (§15.9.5)
    ...
    // 16. In a method reference expression (§15.13), as the reference type to search for a member
    ...
    // The extraction of a TypeName from the identifiers of a ReferenceType in the 16 contexts above
    // is intended to
    // apply recursively to all sub-terms of the ReferenceType, such as its element type and any
    // type arguments.
    //
    // For example, suppose a field declaration uses the type p.q.Foo[]. The brackets of the array
    // type are ignored,
    // and the term p.q.Foo is extracted as a dotted sequence of Identifiers to the left of the
    // brackets in an array
    // type, and classified as a TypeName. A later step determines which of p, q, and Foo is a type
    // name or a
    // package name.
    //
    // As another example, suppose a cast operator uses the type p.q.Foo<? extends String>. The term
    // p.q.Foo is
    // again extracted as a dotted sequence of Identifier terms, this time to the left of the < in a
    // parameterized
    // type, and classified as a TypeName. The term String is extracted as an Identifier in an
    // extends clause of a
  ...
  private static boolean isSyntacticallyAnExpressionName(Node name) {
    ...
    if (whenParentIs(
        ObjectCreationExpr.class, name, (ne, c2) -> ne.hasScope() && ne.getScope().get() == c2)) {
      return true;
    }
        ...
        (nameExpr, c) ->
            nameExpr.getName() == c
                && whenParentIs(
                    UnaryExpr.class,
                    nameExpr,
                    (ne, c2) -> ne.getExpression() == c2 && ne.isPostfix()))) {
      return true;
    }
    if (whenParentIs(
        UnaryExpr.class, name, (ne, c2) -> ne.getExpression() == c2 && ne.isPostfix())) {
      return true;
    }
    ...
    if (whenParentIs(
        ...
        (nameExpr, c) ->
            nameExpr.getName() == c
                && whenParentIs(
                    TryStmt.class, nameExpr, (ne, c2) -> ne.getResources().contains(c2)))) {
      return true;
    }
    if (whenParentIs(
        NameExpr.class,
        name,
        (p1 /*NameExpr*/, c1 /*SimpleName*/) ->
            p1.getName() == c1
                && whenParentIs(
                    VariableDeclarator.class,
                    p1,
                    (p2, c2) ->
                        p2.getInitializer().isPresent()
                            && p2.getInitializer().get() == c2
                            && whenParentIs(
                                VariableDeclarationExpr.class,
                                p2,
                                (p3, c3) ->
                                    p3.getVariables().contains(c3)
                                        && whenParentIs(
                                            TryStmt.class,
                                            p3,
            ...
            p2.getInitializer().isPresent()
                ...
                && whenParentIs(
                    ...
                    (p3, c3) ->
                        p3.getVariables().contains(c3)
                            && whenParentIs(
                                TryStmt.class, p3, (p4, c4) -> p4.getResources().contains(c4))))) {
      return true;
    }
  ...
  public static String nameAsString(Node name) {
    ...
    if (name instanceof FieldAccessExpr) {
      ...
      if (isAName(fieldAccessExpr.getScope())) {
        return nameAsString(fieldAccessExpr.getScope())
            + "."
            + nameAsString(fieldAccessExpr.getName());
      }
      throw new IllegalArgumentException();
    }
    if (name instanceof NameExpr) {
  ...
  private static <P extends Node, C extends Node> boolean whenParentIs(
    ...
    if (child.getParentNode().isPresent()) {
      ...
      return parentClass.isInstance(parent)
          && predicate.isSatisfied(parentClass.cast(parent), child);
    }
    return false;
  }
}
