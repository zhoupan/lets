...
public class NumericConditionalExprHandler implements ConditionalExprHandler {
  ...
  public ResolvedType resolveType() {
    ...
    if (thenExpr.isPrimitive()
        ...
        && elseExpr
            .asReferenceType()
            .isUnboxableTo(
                (ResolvedPrimitiveType) ResolvedPrimitiveType.byName(qnameTypeThenExpr))) {
      return thenExpr;
    }
    if (elseExpr.isPrimitive()
        ...
        && thenExpr
            .asReferenceType()
            .isUnboxableTo(
                (ResolvedPrimitiveType) ResolvedPrimitiveType.byName(qnameTypeElseExpr))) {
      return elseExpr;
    }
    /*
    ...
    // and third operands.
    ResolvedPrimitiveType PrimitiveThenExpr =
        thenExpr.isPrimitive()
            ? thenExpr.asPrimitive()
            : thenExpr.asReferenceType().toUnboxedType().get();
    ResolvedPrimitiveType PrimitiveElseExpr =
        elseExpr.isPrimitive()
  ...
  protected boolean relaxMatch(ResolvedType type, ResolvedPrimitiveType... types) {
    return exactMatch(type, types)
        || (type.isReferenceType()
            && Arrays.stream(types)
                .anyMatch(
                    t -> type.asReferenceType().getQualifiedName().equals(t.getBoxTypeQName())));
  }
  ...
  protected boolean isResolvableTo(
      ResolvedPrimitiveType toType, ResolvedType resolvedType, boolean verifyBoxedType) {
