...
public class TypeSubtypeOfType extends ConstraintFormula {
  ...
  public ReductionResult reduce(BoundSet currentBoundSet) {
    ...
    // parameterized class or interface type (directly or indirectly), let A1, ..., An be the type
    // arguments of T. Among the supertypes of S, a corresponding class or interface type is
    // identified, with type arguments B1, ..., Bn. If no such type exists, the constraint reduces
    // to false. Otherwise, the constraint reduces to the following new constraints: for all i (1 ≤
    // i ≤ n), ‹Bi <= Ai›.
    //
    //   - If T is any other class or interface type, then the constraint reduces to true if T is
    // among the supertypes of S, and false otherwise.
    //
    //   - If T is an array type, T'[], then among the supertypes of S that are array types, a most
    // specific type is identified, S'[] (this may be S itself). If no such array type exists, the
    // constraint reduces to false. Otherwise:
    //
    //     - If neither S' nor T' is a primitive type, the constraint reduces to ‹S' <: T'›.
    //
    //     - Otherwise, the constraint reduces to true if S' and T' are the same primitive type, and
    // false otherwise.
    //
