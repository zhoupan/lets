...
public class JavaParserTypeDeclarationAdapter {
  ...
  public SymbolReference<ResolvedTypeDeclaration> solveType(
    ...
    for (BodyDeclaration<?> member : this.wrappedNode.getMembers()) {
      if (member.isTypeDeclaration()) {
        ...
        if (internalType.getName().getId().equals(name)
            && compareTypeParameters(internalType, typeArguments)) {
          return SymbolReference.solved(
        ...
        if (name.startsWith(wrappedNode.getName().getId() + "." + internalType.getName().getId())) {
          return JavaParserFactory.getContext(internalType, typeSolver)
              .solveType(name.substring(wrappedNode.getName().getId().length() + 1), typeArguments);
        }
        if (name.startsWith(internalType.getName().getId() + ".")) {
          return JavaParserFactory.getContext(internalType, typeSolver)
              .solveType(
                  name.substring(internalType.getName().getId().length() + 1), typeArguments);
        }
      }
    }
    ...
    SymbolReference<ResolvedTypeDeclaration> symbolRef =
        context
            .getParent()
    ...
    if (wrappedNode instanceof NodeWithExtends) {
      ...
      for (ClassOrInterfaceType extendedType : nodeWithExtends.getExtendedTypes()) {
        if (extendedType.getName().getId().equals(name)
            && compareTypeArguments(extendedType, typeArguments)) {
          return context
              .getParent()
              .orElseThrow(() -> new RuntimeException("Parent context unexpectedly empty."))
  ...
  private boolean compareTypeParameters(
    ...
    if (typeDeclaration instanceof NodeWithTypeParameters) {
      return compareTypeParameters(
          (NodeWithTypeParameters<?>) typeDeclaration, resolvedTypeArguments);
    }
    return true;
  }

  /**
   * Recursively checks the ancestors of the {@param declaration} if an internal type is declared
   * with a name equal to {@param name}. TODO: Edit to remove return of null (favouring a return of
   * optional)
   *
   * @return A ResolvedTypeDeclaration matching the {@param name}, null otherwise
   */
  private ResolvedTypeDeclaration checkAncestorsForType(
    ...
    for (ResolvedReferenceType ancestor : declaration.getAncestors(true)) {
      try {
        ...
        ResolvedReferenceTypeDeclaration ancestorReferenceTypeDeclaration =
            ancestor
                .getTypeDeclaration()
        ...
        for (ResolvedTypeDeclaration internalTypeDeclaration :
          ...
          if (internalTypeDeclaration instanceof ResolvedReferenceTypeDeclaration) {
            ...
            if (resolvedReferenceTypeDeclaration instanceof HasAccessSpecifier) {
              visible =
                  ((HasAccessSpecifier) resolvedReferenceTypeDeclaration).accessSpecifier()
  ...
  public SymbolReference<ResolvedMethodDeclaration> solveMethod(
    ...
    // Note that we want to avoid infinite recursion when a class is using its own method - see
    // issue #75
