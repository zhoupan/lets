...
public class ConstraintFormulaSet {
  ...
  /**
   * Takes a compatibility assertion about an expression or type, called a constraint formula, and
   * reduces it to a set of bounds on inference variables. Often, a constraint formula reduces to
   * other constraint formulas, which must be recursively reduced. A procedure is followed to
   * identify these additional constraint formulas and, ultimately, to express via a bound set the
   * conditions under which the choices for inferred types would render each constraint formula
   * true.
   */
