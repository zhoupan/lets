...
public class MethodResolutionLogic {
  ...
  private static List<ResolvedType> groupVariadicParamValues(
    ...
    List<ResolvedType> variadicValues =
        argumentsTypes.subList(startVariadic, argumentsTypes.size());
  ...
  /**
   * Note the specific naming here -- parameters are part of the method declaration, while arguments
   * are the values passed when calling a method. Note that "needle" refers to that value being used
   * as a search/query term to match against.
   *
   * @return true, if the given ResolvedMethodDeclaration matches the given name/types (normally
   *     obtained from a MethodUsage)
  ...
  private static boolean isApplicable(
    ...
    if (!methodIsDeclaredWithVariadicParameter
      ...
      // If it is not variadic, and the number of parameters/arguments are unequal -- this is not a
      // match.
      return false;
    }
    ...
    if (methodIsDeclaredWithVariadicParameter) {
      ...
      if (countOfNeedleArgumentsPassed <= (countOfMethodParametersDeclared - 2)) {
        // If it is variadic, and the number of arguments are short by **two or more** -- this is
        // not a match.
      ...
      // e.g. foo(String s, String... s2) {} --- consider the first argument, then group the
      // remainder as an array
      ...
      for (ResolvedTypeParameterDeclaration tp : methodDeclaration.getTypeParameters()) {
        expectedVariadicParameterType =
            replaceTypeParam(expectedVariadicParameterType, tp, typeSolver);
      }

      if (countOfNeedleArgumentsPassed > countOfMethodParametersDeclared) {
        // If it is variadic, and we have an "excess" of arguments, group the "trailing" arguments
        // into an array.
        // Confirm all of these grouped "trailing" arguments have the required type -- if not, this
        // is not a valid type. (Maybe this is also done later..?)
        for (int variadicArgumentIndex = countOfMethodParametersDeclared;
          ...
          if (!argumentIsAssignableToVariadicComponentType) {
            // If any of the arguments are not assignable to the expected variadic type, this is not
            // a match.
            return false;
          }
        }
    ...
    // If variadic parameters are possible then they will have been "grouped" into a single
    // argument.
    // At this point, therefore, the number of arguments must be equal -- if they're not, then there
    // is no match.
    ...
    for (int i = 0; i < countOfMethodParametersDeclared; i++) {
      ...
      boolean isAssignableWithoutSubstitution =
          expectedDeclaredType.isAssignableBy(actualArgumentType)
              || (methodDeclaration.getParam(i).isVariadic()
                  && convertToVariadicParameter(expectedDeclaredType)
      ...
      if (!isAssignableWithoutSubstitution) {
        ...
        if (!expectedDeclaredType.isAssignableBy(actualArgumentType)) {
          ...
          // if the expected is java.lang.Math.max(double,double) and the type parameters are
          // defined with constrain
          // for example LambdaConstraintType{bound=TypeVariable
          // {ReflectionTypeParameter{typeVariable=T}}}, LambdaConstraintType{bound=TypeVariable
          ...
          if (methodIsDeclaredWithVariadicParameter && i == countOfMethodParametersDeclared - 1) {
            if (convertToVariadicParameter(expectedDeclaredType)
                .isAssignableBy(actualArgumentType)) {
              continue;
            }
          }
  ...
  private static List<ResolvedType> groupTrailingArgumentsIntoArray(
    ...
    if (countOfNeedleArgumentsPassed == (countOfMethodParametersDeclared - 1)) {
      ...
      if (finalArgumentIsArray) {
        // Treat as an array of values -- in which case the expected parameter type is the common
        // type of this array.
        // no need to do anything
        // expectedVariadicParameterType = actualArgumentType.asArrayType().getComponentType();
      } else {
        // Treat as a single value -- in which case, the expected parameter type is the same as the
        // single value.
        needleArgumentTypes =
            groupVariadicParamValues(
                needleArgumentTypes,
  ...
  public static boolean isAssignableMatchTypeParameters(
    ...
    if (expected.isReferenceType()
        && ResolvedPrimitiveType.isBoxType(expected)
        && actual.isPrimitive()) {
      ResolvedPrimitiveType expectedType =
          ResolvedPrimitiveType.byBoxTypeQName(expected.asReferenceType().getQualifiedName())
              .get()
              .asPrimitive();
  ...
  public static ResolvedType replaceTypeParam(
    ...
    if (type.isArray()) {
      return new ResolvedArrayType(
          replaceTypeParam(type.asArrayType().getComponentType(), tp, typeSolver));
    }
    if (type.isReferenceType()) {
      ResolvedReferenceType result = type.asReferenceType();
      result =
          result
              .transformTypeParameters(typeParam -> replaceTypeParam(typeParam, tp, typeSolver))
              .asReferenceType();
      return result;
    }
    throw new UnsupportedOperationException(
   ...
   * as a search/query term to match against.
   *
   * @return true, if the given MethodUsage matches the given name/types (normally obtained from a
   *     ResolvedMethodDeclaration)
   * @see {@link MethodResolutionLogic#isApplicable(ResolvedMethodDeclaration, String, List,
   *     TypeSolver)} }
   * @see {@link MethodResolutionLogic#isApplicable(ResolvedMethodDeclaration, String, List,
   *     TypeSolver, boolean)}
   */
  public static boolean isApplicable(
    ...
    // If the counts do not match and we have provided too few arguments, this is not a match. Note
    // that variadic parameters
    // allow you to omit the vararg, which would allow a difference of one, but a difference in
    // count of 2 or more is not a match.
    if (!(needleParameterCount == countOfMethodUsageArgumentsPassed)
        && needleParameterCount < lastMethodUsageArgumentIndex) {
      return false;
    }

    // Iterate over the arguments given to the method, and compare their types against the given
    ...
    for (int i = 0; i < needleParameterCount; i++) {
      ...
      boolean reachedVariadicParam =
          methodIsDeclaredWithVariadicParameter && i >= lastMethodUsageArgumentIndex;
      if (!reachedVariadicParam) {
        // Not yet reached the variadic parameters -- the expected type is just whatever is at that
        // position.
        expectedArgumentType = methodUsage.getParamType(i);
      } else {
        // We have reached the variadic parameters -- the expected type is the type of the last
        // declared parameter.
        expectedArgumentType = methodUsage.getParamType(lastMethodUsageArgumentIndex);
        // Note that the given variadic value might be an array - if so, use the array's component
        // type rather.
        // This is only valid if ONE argument has been given to the vararg parameter.
        // Example: {@code void test(String... s) {}} and {@code test(stringArray)} -- {@code
        // String... is assignable by stringArray}
        // Example: {@code void test(String[]... s) {}} and {@code test(stringArrayArray)} -- {@code
        // String[]... is assignable by stringArrayArray}
      ...
      // Consider type parameters directly on the method declaration, and ALSO on the enclosing type
      // (e.g. a class)
      List<ResolvedTypeParameterDeclaration> typeParameters =
      ...
      for (Map.Entry<ResolvedTypeParameterDeclaration, ResolvedType> entry :
          derivedValues.entrySet()) {
        ResolvedTypeParameterDeclaration tp = entry.getKey();
        expectedTypeWithInference =
            expectedTypeWithInference.replaceTypeVariables(tp, entry.getValue());
      }

      // Consider cases where type variables can be replaced (e.g. add(E element) vs add(String
      // element))
      for (ResolvedTypeParameterDeclaration tp : typeParameters) {
      ...
      for (ResolvedTypeParameterDeclaration tp : typeParameters) {
        if (tp.getBounds().isEmpty()) {
          expectedTypeWithSubstitutions =
              expectedTypeWithSubstitutions.replaceTypeVariables(
   ...
   * </pre>
   *
   * <p>The example above would return a distinct list of persons containing only one person per
   * name.
   */
  ...
  /**
   * @param methods we expect the methods to be ordered such that inherited methods are later in the
   *     list
   */
  ...
  public static SymbolReference<ResolvedMethodDeclaration> findMostApplicable(
    ...
    List<ResolvedMethodDeclaration> applicableMethods =
        methods.stream()
    ...
    // If there are multiple possible methods found, null arguments can help to eliminate some
    // matches.
    ...
    for (int i = 1; i < applicableMethods.size(); i++) {
      ...
      if (isMoreSpecific(winningCandidate, other, argumentsTypes)) {
        ...
        // One applicable method m1 is more specific than another applicable method m2, for an
        // invocation with argument
        // expressions e1, ..., ek, if any of the following are true:
        // m2 is generic, and m1 is inferred to be more specific than m2 for argument expressions
        // e1, ..., ek by §18.5.4.
  ...
  protected static boolean isExactMatch(
      ResolvedMethodLikeDeclaration method, List<ResolvedType> argumentsTypes) {
  ...
  private static boolean isMoreSpecific(
      ResolvedMethodDeclaration methodA,
    ...
    // If one method declaration has exactly the correct amount of parameters and is not variadic
    // then it is always
    // preferred to a declaration that is variadic (and hence possibly also has a different amount
    // of parameters).
    if (!aVariadic
    ...
    // If both methods are variadic but the calling method omits any varArgs, bump the omitted args
    // to
    // ensure the varargs type is considered when determining which method is more specific
    if (aVariadic
        && bVariadic
        && aNumberOfParams == bNumberOfParams
        && numberOfArgs == aNumberOfParams - 1) {
      omittedArgs++;
    }
    ...
    for (int i = 0; i < numberOfArgs + omittedArgs; i++) {
      ...
      // Safety: if a type is null it means a signature with too few parameters managed to get to
      // this point.
      ...
      // Widening primitive conversions have priority over boxing/unboxing conversions when finding
      // the most
      // applicable method. E.g. assume we have method call foo(1) and declarations foo(long) and
      // foo(Integer).
      // The method call will call foo(long), as it requires a widening primitive conversion from
      // int to long
      ...
      if (argType != null
        ...
        // and the type of paramA or paramB (which are not more specific at this stage) is
        // java.lang.Object
      ...
      // If we get to this point then we check whether one of the methods contains a parameter type
      // that is more
      // specific. If it does, we can assume the entire declaration is more specific as we would
      // otherwise have
  ...
  private static boolean isMoreSpecific(
      MethodUsage methodA, MethodUsage methodB, List<ResolvedType> argumentTypes) {
  ...
  public static Optional<MethodUsage> findMostApplicableUsage(
      ...
      List<ResolvedType> argumentsTypes,
      TypeSolver typeSolver) {
    List<MethodUsage> applicableMethods =
        methods.stream()
