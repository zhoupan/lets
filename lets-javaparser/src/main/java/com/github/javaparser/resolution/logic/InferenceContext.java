  ...
  private void registerCorrespondance(ResolvedType formalType, ResolvedType actualType) {
    if (formalType.isReferenceType() && actualType.isReferenceType()) {
      ...
      if (!formalTypeAsReference
        ...
        List<ResolvedType> correspondingFormalType =
            "java.lang.Object".equals(formalParamTypeQName)
                ? Stream.concat(
        ...
        if (correspondingFormalType.isEmpty()) {
          ...
          List<ResolvedType> correspondingActualType =
              ancestors.stream()
          ...
          if (actualTypeAsReference.isRawType()) {
            ...
            for (ResolvedType formalTypeParameter : formalTypeAsReference.typeParametersValues()) {
              registerCorrespondance(
                  formalTypeParameter, actualTypeAsReference.typeParametersValues().get(i));
              i++;
            }
          }
        }
      ...
      registerCorrespondance(
          formalType.asArrayType().getComponentType(), actualType.asArrayType().getComponentType());
      ...
      if (actualType.isWildcard()) {
        ...
        if (formalWildcard.isBounded()
            && formalWildcard.getBoundedType() instanceof InferenceVariableType) {
      ...
      if (formalTypeAsReference.isJavaLangObject()) {
        ...
        throw new UnsupportedOperationException(
            formalType.describe() + " " + actualType.describe());
      }
    } else {
