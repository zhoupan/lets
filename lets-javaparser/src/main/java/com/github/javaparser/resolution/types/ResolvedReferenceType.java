...
/**
 * A ReferenceType like a class, an interface or an enum. Note that this type can contain also the
 * values specified for the type parameters.
 *
...
public abstract class ResolvedReferenceType
  ...
  public ResolvedReferenceType(
    ...
    ResolvedTypeParametersMap.Builder typeParametersMapBuilder =
        new ResolvedTypeParametersMap.Builder();
    for (int i = 0; i < typeArguments.size(); i++) {
      typeParametersMapBuilder.setValue(
          typeDeclaration.getTypeParameters().get(i), typeArguments.get(i));
    }
  ...
  // /
  /**
   * Return all ancestors, that means all superclasses and interfaces. This list should always
   * include Object (unless this is a reference to Object). The type typeParametersValues should be
   * expressed in terms of this type typeParametersValues. The default order of presenting ancestors
   * corresponds to a search in depth.
   *
   * <p>For example, given:
   *
   * <p>class Foo&lt;A, B&gt; {} class Bar&lt;C&gt; extends Foo&lt;C, String&gt; {}
   *
   * <p>a call to getAllAncestors on a reference to Bar having type parameter Boolean should include
   * Foo&lt;Boolean, String&gt;.
   */
   ...
   * Return all ancestors, that means all superclasses and interfaces. This list should always
   * include Object (unless this is a reference to Object). The type typeParametersValues should be
   * expressed in terms of this type typeParametersValues.
   */
  public abstract List<ResolvedReferenceType> getAllAncestors(
      Function<ResolvedReferenceTypeDeclaration, List<ResolvedReferenceType>> traverser);

  /**
   * Return direct ancestors, that means the superclasses and interfaces implemented directly. This
   * list should include Object if the class has no other superclass or the interface is not
   * extending another interface. There is an exception for Object itself.
   */
  ...
  // /
  /**
   * Get the type associated with the type parameter with the given name. It returns Optional.empty
   * unless the type declaration declares a type parameter with the given name.
   */
  @Override
  ...
  /**
   * Get the values for all type parameters declared on this type. The list can be empty for raw
   * types.
   */
  public List<ResolvedType> typeParametersValues() {
    return this.typeParametersMap.isEmpty()
  ...
  /**
   * Get the values for all type parameters declared on this type. In case of raw types the values
   * correspond to TypeVariables.
   */
    ...
    if (!isRawType()) {
      for (int i = 0; i < typeDeclaration.getTypeParameters().size(); i++) {
        typeParametersMap.add(
            new Pair<>(typeDeclaration.getTypeParameters().get(i), typeParametersValues().get(i)));
      }
    }
  ...
  /**
   * The type of the field could be different from the one in the corresponding FieldDeclaration
   * because type variables would be solved.
   */
  ...
  @Override
  public Optional<ResolvedType> typeParamValue(
  ...
  /**
   * Get a list of all the methods available on this type. This list includes methods declared in
   * this type and methods inherited. This list includes methods of all sort of visibility. However
   * it does not include methods that have been overwritten.
   */
  ...
  /**
   * Fields which are visible to inheritors. They include all inherited fields which are visible to
   * this type plus all declared fields which are not private.
   */
  public List<ResolvedFieldDeclaration> getAllFieldsVisibleToInheritors() {
    List<ResolvedFieldDeclaration> res =
        new LinkedList<>(
            this.getDeclaredFields().stream()
  ...
  // Protected methods
  //
  protected abstract ResolvedReferenceType create(
      ResolvedReferenceTypeDeclaration typeDeclaration, List<ResolvedType> typeParameters);
  ...
  protected boolean isCorrespondingBoxingType(String typeName) {
    ResolvedPrimitiveType resolvedPrimitiveType =
        (ResolvedPrimitiveType) ResolvedPrimitiveType.byName(typeName);
        ...
        if (!thisParam.equals(otherParam)) {
          if (thisParam instanceof ResolvedWildcard) {
            ...
            if (thisParamAsWildcard.isSuper()
              ...
              // ok
            } else if (thisParamAsWildcard.isExtends()
  ...
  // Private methods
  //
  private boolean compareConsideringVariableTypeParameters(
    ...
    if (bounds.size() == 1) {
      ...
      boolean hasTypeParameter =
          boundType.isReferenceType() && !boundType.asReferenceType().typeParametersMap.isEmpty();
  ...
  public abstract ResolvedReferenceType deriveTypeParameters(
      ResolvedTypeParametersMap typeParametersMap);
  ...
  public boolean isUnboxable() {
    return Arrays.stream(ResolvedPrimitiveType.values())
        .anyMatch(pt -> getQualifiedName().equals(pt.getBoxTypeQName()));
  }
  ...
  public Optional<ResolvedPrimitiveType> toUnboxedType() {
    return Arrays.stream(ResolvedPrimitiveType.values())
        .filter(pt -> this.asReferenceType().getQualifiedName().equals(pt.getBoxTypeQName()))
        .findFirst();
  }
