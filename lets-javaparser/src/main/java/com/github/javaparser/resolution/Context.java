...
public interface Context {
  ...
  /**
   * @return The parent context, if there is one. For example, a method exists within a compilation
   *     unit.
   */
   ...
   * Contexts which have generics available to it will override this method. For example class and
  ...
  /**
   * Default to being unable to solve any reference in this context, delegating solving to the
   * parent context. Contexts which exist as the "parent" of a resolvable type will override this
   * method. For example, a compilation unit can contain classes. A class declaration can also
   * contain types (e.g. a subclass).
   *
   ...
   * @deprecated Consider using method {@link #solveType(String, List)} that also consider the type
   *     arguments. If you want to keep to use the new function, but keep the same behavior consider
   *     passing type arguments as {@code null}.
   */
  @Deprecated
  ...
  /**
   * Method used to solve a name with an expected list of type arguments. <br>
   * This method differs from {@link Context#solveType(String)} by taking the type arguments in
   * consideration. For example, lets imagine that we have a project containing the following
   * classes:
   *
   * <ul>
   *   <li>com/example/Alpha.java
   *   <li>com/example/Beta.java
   * </ul>
   *
   * Where Alpha creates a inner interface called CustomInterface and Beta implements
   * Alpha.CustomInterface and also declares a inner interface called CustomInterface with type
   * arguments. Using this method we can specify which type arguments we are expecting and will be
   * resolved with the type matching that declaration.
   *
   ...
   * @param name The name to be solved.
   * @return The declaration associated with the given type name.
   * @deprecated Consider using method {@link #solveTypeInParentContext(String, List)} that also
   *     consider the type arguments. If you want to keep to use the new function, but keep the same
   *     behavior consider passing type arguments as {@code null}.
   */
  @Deprecated
  ...
  /* Symbol resolution */
  /**
   * Used where a symbol is being used (e.g. solving {@code x} when used as an argument {@code
   * doubleThis(x)}, or calculation {@code return x * 2;}).
   *
  ...
  default SymbolReference<? extends ResolvedValueDeclaration> solveSymbolInParentContext(
      String name) {
    Optional<Context> optionalParentContext = getParent();
    if (!optionalParentContext.isPresent()) {
      return SymbolReference.unsolved();
  ...
  /**
   * The fields that are declared and in this immediate context made visible to a given child. This
   * list could include values which are shadowed.
   */
  ...
  /**
   * The local variables that are declared in this immediate context and made visible to a given
   * child. This list could include values which are shadowed.
   */
  ...
  /**
   * The pattern expressions that are declared in this immediate context and made visible to a given
   * child. This list could include values which are shadowed.
   */
   ...
   *
   * <p>To do it consider local variables that are visible in a certain scope as defined in JLS 6.3.
   * Scope of a Declaration.
   *
   * <p>1. The scope of a local variable declaration in a block (§14.4) is the rest of the block in
   * which the declaration appears, starting with its own initializer and including any further
   * declarators to the right in the local variable declaration statement.
   *
   * <p>2. The scope of a local variable declared in the ForInit part of a basic for statement
   * (§14.14.1) includes all of the following: 2.1 Its own initializer 2.2 Any further declarators
   * to the right in the ForInit part of the for statement 2.3 The Expression and ForUpdate parts of
   * the for statement 2.4 The contained Statement
   *
   * <p>3. The scope of a local variable declared in the FormalParameter part of an enhanced for
   * statement (§14.14.2) is the contained Statement. 4. The scope of a parameter of an exception
   * handler that is declared in a catch clause of a try statement (§14.20) is the entire block
   * associated with the catch.
   *
   * <p>5. The scope of a variable declared in the ResourceSpecification of a try-with-resources
   * statement (§14.20.3) is from the declaration rightward over the remainder of the
   * ResourceSpecification and the entire try block associated with the try-with-resources
   * statement.
   */
  default Optional<VariableDeclarator> localVariableDeclarationInScope(String name) {
    ...
    // If we don't find the variable locally, escalate up the scope hierarchy to see if it is
    // declared there.
  ...
  default Optional<Parameter> parameterDeclarationInScope(String name) {
    ...
    Node wrappedNode = getWrappedNode();
    Context parentContext = getParent().get();
    Optional<Parameter> localResolutionResults =
        parentContext.parametersExposedToChild(wrappedNode).stream()
            .filter(vd -> vd.getNameAsString().equals(name))
            .findFirst();
    if (localResolutionResults.isPresent()) {
      return localResolutionResults;
  ...
  /**
   * With respect to solving, the AST "parent" of a block statement is not necessarily the same as
   * the scope parent. <br>
   * Example: <br>
   *
   * <pre>{@code
  ...
  default Optional<PatternExpr> patternExprInScope(String name) {
    ...
    Optional<PatternExpr> localResolutionResults =
        parentContext.patternExprsExposedToChild(wrappedNode).stream()
            .filter(
                vd ->
  ...
  default Optional<ResolvedFieldDeclaration> fieldDeclarationInScope(String name) {
    ...
    // If we don't find the field locally, escalate up the scope hierarchy to see if it is declared
    // there.
  ...
  /**
   * We find the method declaration which is the best match for the given name and list of
   * typeParametersValues.
   */
  default SymbolReference<ResolvedConstructorDeclaration> solveConstructor(
