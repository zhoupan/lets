...
public class ConstructorResolutionLogic {
  ...
  private static boolean isApplicable(
    ...
    for (int i = 0; i < constructor.getNumberOfParams(); i++) {
      ...
      boolean isAssignableWithoutSubstitution =
          expectedType.isAssignableBy(actualType)
  ...
  /**
   * @param constructors we expect the methods to be ordered such that inherited methods are later
   *     in the list
  ...
  public static SymbolReference<ResolvedConstructorDeclaration> findMostApplicable(
    ...
    List<ResolvedConstructorDeclaration> applicableConstructors =
        constructors.stream()
