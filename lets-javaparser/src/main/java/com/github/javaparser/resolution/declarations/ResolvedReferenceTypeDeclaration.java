...
public interface ResolvedReferenceTypeDeclaration
  ...
  // /
  /**
   * Resolves the types of all direct ancestors (i.e., the directly extended class and the directly
   * implemented interfaces) and returns the list of ancestors as a list of resolved reference
   * types.
   *
   * <p>In case any ancestor cannot be resolved, an {@code UnsolvedSymbolException} is thrown. In
   * order to obtain a list of only the resolvable direct ancestors, use {@link
   * #getAncestors(boolean)} and pass the value {@code true}.
   *
   * <p>Note that an ancestor can be parametrized types with values specified. For example:
   *
   * <p>class A implements Comparable&lt;String&gt; {}
   *
   * <p>In this case the ancestor is Comparable&lt;String&gt;
   *
   ...
   * types.
   *
   * <p>If {@code acceptIncompleteList} is {@code false}, then an {@code UnsolvedSymbolException} is
   * thrown if any ancestor cannot be resolved. Otherwise, a list of only the resolvable direct
   * ancestors is returned.
   *
   * @param acceptIncompleteList When set to {@code false}, this method throws an {@link
   *     UnsolvedSymbolException} if one or more ancestor could not be resolved. When set to {@code
   *     true}, this method does not throw an {@link UnsolvedSymbolException}, but the list of
   *     returned ancestors may be incomplete in case one or more ancestor could not be resolved.
   * @return The list of resolved ancestors.
   * @throws UnsolvedSymbolException if some ancestor could not be resolved and {@code
   *     acceptIncompleteList} is set to {@code false}.
   */
  List<ResolvedReferenceType> getAncestors(boolean acceptIncompleteList);

  /**
   * The list of all the ancestors of the current declaration, direct and indirect. This list does
   * not contains duplicates with the exact same type parameters. For example if A inherits from B,
   * and B inherits from C and implements D, and C inherits from E By default the traversal is depth
   * first
   */
   ...
   * The list of all the ancestors of the current declaration, direct and indirect. This list does
   * not contains duplicates with the exact same type parameters. For example if A inherits from B,
   * and B inherits from C and implements D, and C inherits from E Apply the specified traversal
   */
  default List<ResolvedReferenceType> getAllAncestors(
   ...
   */
  Function<ResolvedReferenceTypeDeclaration, List<ResolvedReferenceType>> depthFirstFunc =
      (rrtd) -> {
   ...
   */
  Function<ResolvedReferenceTypeDeclaration, List<ResolvedReferenceType>> breadthFirstFunc =
      (rrtd) -> {
        List<ResolvedReferenceType> ancestors = new ArrayList<>();
        // We want to avoid infinite recursion in case of Object having Object as ancestor
        if (!rrtd.isJavaLangObject()) {
          ...
          while (!queuedAncestors.isEmpty()) {
            ...
            queuedAncestor
                ...
                .ifPresent(
                    rtd ->
                        new LinkedHashSet<>(queuedAncestor.getDirectAncestors())
                            .stream()
                                .forEach(
                                    ancestor -> {
  ...
  // /
  /**
   * Note that the type of the field should be expressed using the type variables of this particular
   * type. Consider for example:
   *
   * <p>class Foo<E> { E field; }
   *
   * <p>class Bar extends Foo<String> { }
   *
   * <p>When calling getField("field") on Foo I should get a FieldDeclaration with type E, while
   * calling it on Bar I should get a FieldDeclaration with type String.
   */
  default ResolvedFieldDeclaration getField(String name) {
    Optional<ResolvedFieldDeclaration> field =
        this.getAllFields().stream().filter(f -> f.getName().equals(name)).findFirst();
    if (field.isPresent()) {
      return field.get();
    }
  ...
  default ResolvedFieldDeclaration getVisibleField(String name) {
    Optional<ResolvedFieldDeclaration> field =
        getVisibleFields().stream().filter(f -> f.getName().equals(name)).findFirst();
    if (field.isPresent()) {
      return field.get();
    }
  ...
  default List<ResolvedFieldDeclaration> getVisibleFields() {
    return getAllFields().stream()
        .filter(
            f -> f.declaringType().equals(this) || f.accessSpecifier() != AccessSpecifier.PRIVATE)
  ...
  /**
   * Return a list of all the methods declared of this type declaration, either declared or
   * inherited. Note that it should not include overridden methods.
   */
  ...
  // /
  /**
   * Can we assign instances of the given type to variables having the type defined by this
   * declaration?
   */
  boolean isAssignableBy(ResolvedType type);

  /**
   * Can we assign instances of the type defined by this declaration to variables having the type
   ...
   * By default, the subclasses do not inherit the annotation declared on the parent class. However,
   * there is a way to propagate particular annotations throughout the class hierarchy using
  ...
  default boolean hasAnnotation(String qualifiedName) {
    ...
    return isClass()
        && getAllAncestors().stream()
            ...
            .anyMatch(
                rrtd ->
                    rrtd.hasDirectlyAnnotation(qualifiedName)
  ...
  /**
   * Returns the resolved annotation corresponding to the specified name and declared in this type
   * declaration.
   */
  ...
  /**
   * This means that the type has a functional method. Conceptually, a functional interface has
   * exactly one abstract method. Typically these classes has the FunctionInterface annotation but
  ...
  /**
   * We don't make this _ex_plicit in the data representation because that would affect codegen and
   * make everything generate like {@code <T extends Object>} instead of {@code <T>}
   *
