 ...
 *
 * <p>Note that methods with the prefix `demand` indicate that if the search value is not found, an
...
public final class Navigator {
  ...
  public static ClassOrInterfaceDeclaration demandClassOrInterface(
    ...
    return findType(compilationUnit, qualifiedName)
        .map(
            res ->
                res.toClassOrInterfaceDeclaration()
                    .orElseThrow(
                        () ->
                            new IllegalStateException(
                                "Type is not a class or an interface, it is "
                                    + res.getClass().getCanonicalName())))
        .orElseThrow(() -> new IllegalStateException("No type named '" + qualifiedName + "'found"));
  }

  /**
   * Returns the {@code (i+1)}'th constructor of the given type declaration, in textual order. The
   * constructor that appears first has the index 0, the second one the index 1, and so on.
   *
   * @param td The type declaration to search in. Note that only classes and enums have
   *     constructors.
  ...
  public static ConstructorDeclaration demandConstructor(TypeDeclaration<?> td, int index) {
    // TODO: Refactor to use `td.findAll(ConstructorDeclaration.class);` - potential difference re:
    // searching only immediate children?
  ...
  public static ClassOrInterfaceDeclaration demandInterface(
      CompilationUnit cu, String qualifiedName) {
    ClassOrInterfaceDeclaration cd = demandClassOrInterface(cu, qualifiedName);
    if (!cd.isInterface()) {
  ...
  /**
   * Traverses the parent chain starting at {@code node} and returns the first Node that returns
   * make {@code isAcceptedParentNode} evaluate to {@code true}.
   */
  ...
  /**
   * Looks among the type declared in the Compilation Unit for one having the specified name. The
   * name can be qualified with respect to the compilation unit. For example, if the compilation
   * unit is in package a.b; and it contains two top level classes named C and D, with class E being
   * defined inside D then the qualifiedName that can be resolved are "C", "D", and "D.E".
   */
  ...
  /**
   * Looks among the type declared in the TypeDeclaration for one having the specified name. The
   * name can be qualified with respect to the TypeDeclaration. For example, if the class
   * declaration defines class D and class D contains an internal class named E then the
   * qualifiedName that can be resolved are "D", and "D.E".
   */
