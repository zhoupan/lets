...
class CommentsInserter {
  ...
  private void insertComments(CompilationUnit cu, TreeSet<Comment> comments) {
    ...
    if (cu.getPackageDeclaration().isPresent()
        && (children.isEmpty()
            || PositionUtils.areInOrder(firstComment, cu.getPackageDeclaration().get()))) {
  ...
  /**
   * This method try to attributes the nodes received to child of the node. It returns the node that
   * were not attributed.
   */
  ...
  private void attributeLineCommentsOnSameLine(
      TreeSet<Comment> commentsToAttribute, List<Node> children) {
