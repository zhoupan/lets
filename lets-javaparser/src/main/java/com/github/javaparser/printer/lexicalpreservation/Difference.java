...
/**
 * A Difference should give me a sequence of elements I should find (to indicate the context)
 * followed by a list of elements to remove or to add and follow by another sequence of elements.
 *
 * <p>I should later be able to apply such difference to a nodeText.
 */
public class Difference {
   ...
   */
  List<TextElement> processIndentation(
      List<TextElement> indentation, List<TextElement> prevElements) {
    ...
    indentation =
        takeWhile(
            prevElements.subList(eolIndex + 1, prevElements.size()),
            element -> element.isWhiteSpace());
    return indentation;
  }
  ...
  /**
   * If we are at the beginning of a line, with just spaces or tabs before/after the position of the
   * deleted element we should force the space to be the same as the current indentation. This
   * method handles the following case if we remove the modifier {@code public} ([ ] is an indent
   * character) {@code [ ][ ]public[ ][ ][ ]void[ ]m{} <-1--> <---2---> 1/ current indentation 2/
   * these whitespaces must be removed } should produce {@code [ ][ ]void[ ]m{} }
   */
  ...
  private int considerIndentation(
      NodeText nodeText, int nodeTextIndex, int numberOfCharactersToPreserve) {
  ...
  private int removeExtraCharacters(NodeText nodeText, int nodeTextIndex, int extraCharacters) {
    ...
    while (nodeTextIndex >= 0
        && nodeTextIndex < nodeText.numberOfElements()
  ...
  /**
   * Starting at {@code nodeTextIndex} this method tries to determine how many contiguous spaces
   * there are between the previous end of line and the next non whitespace (or tab) character
   *
   * @param nodeText List of elements to analyze
   * @param nodeTextIndex Starting position in the input list
   * @return EnforcingIndentationContext Data structure that hold the starting position of the first
   *     whitespace char and The number of consecutive whitespace (or tab) characters
   */
  private EnforcingIndentationContext defineEnforcingIndentationContext(
      NodeText nodeText, int startIndex) {
  ...
  /**
   * Node that we have calculate the Difference we can apply to a concrete NodeText, modifying it
   * according to the difference (adding and removing the elements provided).
   */
  void apply() {
        ...
        if (diffElement.isAdded()) {
          ...
          if (diffElement.isKept()) {
            ...
            throw new UnsupportedOperationException(
                "Unable to apply operations from "
                    + diffElement.getClass().getSimpleName()
                    + " to "
  ...
  /**
   * Maps all Removed elements as keys to their corresponding RemovedGroup. A RemovedGroup contains
   * all consecutive Removed elements. <br>
   * Example:
   *
   * <pre>
  ...
  private void applyRemovedDiffElement(
    ...
    if (removed.isChild() && originalElementIsChild) {
      ...
      if (originalElementChild.isComment()) {
        ...
        // If the comment is associated to the node we want to remove we remove it as well,
        ...
        // If we delete the first element, it is possible that there is an indentation to be deleted
        // which is stored in the parent node.
        ...
        if (originalIndex == 0 && removed.getChild().getParentNode().isPresent()) {
          ...
          // If we are trying to delete the first element of a node and that node is also the first
          // element of the parent node, we need to look for the grandfather node which logically
          // contains the indentation characters.
          // This is the case, for example, when trying to delete an annotation positioned on a
          // method declaration.
          ...
          if (!parentNodeText.getElements().isEmpty()
            ...
            parentNodeText =
                LexicalPreservingPrinter.getOrCreateNodeText(parentNode.getParentNode().get());
          }
          indentationTokens =
              LexicalPreservingPrinter.findIndentation(startingNodeForFindingIndentation);
        }
        nodeText.removeElement(originalIndex);
        // When we don't try to remove a complete line
        // and removing the element is not the first action of a replacement (removal followed by
        // addition)
        ...
        if (isEnforcingIndentationActivable(removedGroup)) {
          // Since the element has been deleted we try to start the analysis from the element
        ...
        if (originalElements.size() > originalIndex && originalIndex > 0) {
          if (originalElements.get(originalIndex).isWhiteSpace()
            ...
            // Are we to understand that we can only do this if there is a single modification to
            // process
        ...
        // We need to know if, in the original list of elements, the deleted child node is
        // immediately followed by the same comment.
        ...
        if (isRemovingIndentationActivable(removedGroup)) {
          // Since the element has been deleted we try to start the analysis from the previous
          // element
          ...
          if (originalIndex == 0
              && !indentationTokens.isEmpty()
              && !isInlined(nodeText, originalIndex)) {
            for (TextElement indentationToken : indentationTokens) {
              parentNodeText.removeElement(
                  parentNodeText.findElement(
                      indentationToken.and(indentationToken.matchByRange())));
            }
          }
        }
        diffIndex++;
      ...
      if (originalElement.isPrimitive()) {
        ...
        throw new UnsupportedOperationException(
            "removed " + removed.getElement() + " vs " + originalElement);
      }
  ...
  /**
   * Cleans the line of left over space if there is unnecessary indentation and the element will not
   * be replaced
   */
      ...
      if (indentation.isPresent() && !isReplaced(lastElementIndex)) {
        for (int i = 0; i < indentation.get(); i++) {
          if (originalElements.get(originalIndex).isSpaceOrTab()) {
            ...
            // If the current element is not a space itself we remove the space in front of (before)
            // it
  ...
  private void applyKeptDiffElement(
    ...
    if (originalElement.isComment()) {
      ...
      if (originalElement.isWhiteSpaceOrComment()) {
        ...
        // for example:
        // List i : in this case originalElement is "List" and the next token is space. There is
        // nothing to skip. in the originalElements list.
        ...
        // token is "<" so we have to skip all the tokens which are used in the typed arguments
        // declaration [<][List][<][String][>][>](6 tokens) in the originalElements list.
        ...
        // for example:
        // java.lang.Object is represented in originalElement as a list of tokens "java", ".",
        // "lang", ".", "Object".
        ...
        originalIndex++;
      } else if ((originalElement.isIdentifier() || originalElement.isKeyword())
          && isArrayType(kept)) {
        int tokenToSkip =
            getIndexToNextTokenElementInArrayType(
                (TokenTextElement) originalElement, getArrayLevel(kept));
        diffIndex++;
        ...
        if (kept.isPrimitiveType()) {
          ...
          throw new UnsupportedOperationException(
              "kept " + kept.getElement() + " vs " + originalElement);
        }
      }
  ...
  private boolean isNodeWithTypeArguments(DifferenceElement element) {
    ...
    Optional<NodeList<Type>> typeArgs =
        ((NodeWithTypeArguments) child.getChild()).getTypeArguments();
  ...
  private int getIndexToNextTokenElement(TokenTextElement element, int nestedDiamondOperator) {
    ...
    return step +=
        getIndexToNextTokenElement(new TokenTextElement(nextToken), nestedDiamondOperator);
  }
  ...
  private int getIndexToNextTokenElementInArrayType(TokenTextElement element, int arrayLevel) {
    ...
    // recursively analyze token to skip
    return step +=
        getIndexToNextTokenElementInArrayType(new TokenTextElement(nextToken), arrayLevel);
  }
  ...
  private void applyAddedDiffElement(Added added) {
    ...
    if (!used) {
      ...
      boolean previousIsAComment =
          originalIndex > 0 && nodeText.getTextElement(originalIndex - 1).isComment();
      ...
      boolean previousIsWhiteSpace =
          originalIndex > 0 && nodeText.getTextElement(originalIndex - 1).isWhiteSpace();
      boolean commentIsBeforeAddedElement =
          currentIsAComment
              && addedTextElement.getRange().isPresent()
              && nodeText
                  .getTextElement(originalIndex)
                  .getRange()
                  .map(range -> range.isBefore(addedTextElement.getRange().get()))
                  .orElse(false);
      if (sufficientTokensRemainToSkip && currentIsAComment && commentIsBeforeAddedElement) {
        ...
        // or if the previous character is a space (it could be the case if we want to replace a
        // statement)
        // Example 1 : if we insert a statement (a duplicated method call expression ) after this
        // one <code>  value();\n\n</code>
  ...
  private int adjustIndentation(
    ...
    if (nodeTextIndex < nodeText.numberOfElements()
      ...
      indentationAdj =
          indentationAdj.subList(
              0,
              indentationAdj.size() - Math.min(STANDARD_INDENTATION_SIZE, indentationAdj.size()));
    } else if (followedByUnindent) {
      indentationAdj =
          indentationAdj.subList(0, Math.max(0, indentationAdj.size() - STANDARD_INDENTATION_SIZE));
    }
