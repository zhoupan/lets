...
public class XmlPrinter {
  ...
  /**
   * Generate a xml string for given AST Node. Tag name of root element in the result document will
   * be "root".
   *
  ...
  /**
   * Output XML data from an AST node to a String Builder. This method is kept for backward
   * compatilibity only and should be removed in future releases.
   *
   * @param node AST node to be converted to XML
   ...
   *
   * <p>Returned stringWriter is not closed upon return because doing so {@link StringWriter#close()
   * has no effect}. So users of this method are not required to close it.
   *
   * @param node AST node to be converted to XML
   * @param name Tag name of root element in the resulting document
   * @return Stringwriter filled with XML document
   * @throws RuntimeXMLStreamException Unchecked exception wrapping checked {@link
   *     XMLStreamException}, when any error on producing XML output occours
   */
   ...
   *
   * <p>This method creates a {@link XMLStreamWriter} that writes to given writer and delegates
   * execution to {@link #outputDocument(Node, String, XMLStreamWriter)}
   *
   * <p>Provided writer is NOT closed at the end of execution of this method.
   *
   * @param node AST node to be converted to XML
   ...
   */
  public void outputDocument(Node node, String name, XMLStreamWriter xmlWriter)
   ...
   *
   * <p>This method outputs an XML Element with given tag name to writer. It is used recursively for
   * generating nested elements corresponding to AST.
   *
   * <p>For generating a complete XML document from an AST node, {@link outputDocument(String, Node,
   * XMLStreamWriter)} should be used instead.
   *
   * <p>Provided xmlWriter is NOT closed at the end of execution of this method.
   *
   * @param node AST node to be converted to XML
  ...
  public void outputNode(Node node, String name, XMLStreamWriter xmlWriter)
    ...
    Predicate<PropertyMetaModel> nonNullNode =
        propertyMetaModel -> propertyMetaModel.getValue(node) != null;
 ...
 *
 * <p>Used for generating methods without checked exceptions, but allowing to selectively capture of
 * XMLStreamException at higher levels.
 */
