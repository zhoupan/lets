...
class DifferenceElementCalculator {
  ...
  private static boolean replacement(CsmElement a, CsmElement b) {
    if (a instanceof CsmIndent
        || b instanceof CsmIndent
        || a instanceof CsmUnindent
        || b instanceof CsmUnindent) {
      return false;
    }
    if (a instanceof CsmChild) {
      if (b instanceof CsmChild) {
  ...
  /**
   * Calculate the Difference between two CalculatedSyntaxModel elements, determining which elements
   * were kept, which were added and which were removed.
   */
  static List<DifferenceElement> calculate(
    ...
    //
    // We would calculate the Difference between "qwerty" and "qwer" then we know the A is kept, and
    // then we
    ...
    while (commonChildrenIndex < commonChildren.size()) {
      ...
      posOfNextChildInOriginal =
          childrenInOriginal.stream()
      ...
      posOfNextChildInAfter =
          childrenInAfter.stream()
              .filter(i -> i.equals(child))
              .map(i -> i.position)
              .filter(position -> position > currentPosOfNextChildInAfter)
              .findFirst()
              .orElse(posOfNextChildInAfter);
      // Imagine that the common elements has been moved, for example in the case where the
      // parameters of a method are reversed
      ...
      if (originalIndex < posOfNextChildInOriginal || afterIndex < posOfNextChildInAfter) {
        ...
        CalculatedSyntaxModel originalSub =
            originalIndex < posOfNextChildInOriginal
                ? original.sub(originalIndex, posOfNextChildInOriginal)
                : new CalculatedSyntaxModel(Collections.EMPTY_LIST);
        CalculatedSyntaxModel afterSub =
            afterIndex < posOfNextChildInAfter
      ...
      originalIndex =
          originalIndex <= posOfNextChildInOriginal ? posOfNextChildInOriginal + 1 : originalIndex;
    ...
    if (originalIndex < original.elements.size() || afterIndex < after.elements.size()) {
      CalculatedSyntaxModel originalSub =
          originalIndex < original.elements.size()
              ? original.sub(originalIndex, original.elements.size())
              : new CalculatedSyntaxModel(Collections.EMPTY_LIST);
      CalculatedSyntaxModel afterSub =
          afterIndex < after.elements.size()
  ...
  private static int considerRemoval(
      CsmElement removedElement, int originalIndex, List<DifferenceElement> elements) {
    ...
    if (removedElement instanceof CsmChild) {
      ...
      if (removedChild.getChild() instanceof Type
        ...
        NodeText nodeTextForChild =
            LexicalPreservingPrinter.getOrCreateNodeText(removedChild.getChild());
    ...
    do {
      if (originalIndex < original.elements.size() && afterIndex >= after.elements.size()) {
        ...
        if ((nextOriginal instanceof CsmMix) && (nextAfter instanceof CsmMix)) {
          ...
          // We can try to remove the element or add it and look which one leads to the lower
          // difference
  ...
  /**
   * Remove from the difference all the elements related to indentation. This is mainly intended for
   * test purposes.
   */
  static void removeIndentationElements(List<DifferenceElement> elements) {
    elements.removeIf(
        el -> el.getElement() instanceof CsmIndent || el.getElement() instanceof CsmUnindent);
  }
}
