...
class LexicalDifferenceCalculator {
  ...
  /**
   * The ConcreteSyntaxModel represents the general format. This model is a calculated version of
   * the ConcreteSyntaxModel, with no condition, no lists, just tokens and node children.
   */
  ...
  public static class CsmChild implements CsmElement {
    ...
    public void prettyPrint(Node node, SourcePrinter printer) {
      throw new UnsupportedOperationException(
          "The prettyPrint method is not supported or implemented");
    }
  ...
  List<DifferenceElement> calculateListAdditionDifference(
    ...
    CalculatedSyntaxModel after =
        calculatedSyntaxModelAfterListAddition(
            element, observableProperty, nodeList, index, nodeAdded);
    List<DifferenceElement> differenceElements =
        DifferenceElementCalculator.calculate(original, after);
   ...
   */
  private void replaceEolTokens(
      List<DifferenceElement> differenceElements, LineSeparator lineSeparator) {
  ...
  List<DifferenceElement> calculateListReplacementDifference(
    ...
    CalculatedSyntaxModel after =
        calculatedSyntaxModelAfterListReplacement(
            element, observableProperty, nodeList, index, newValue);
    return DifferenceElementCalculator.calculate(original, after);
  }
  ...
  void calculatePropertyChange(
    ...
    CalculatedSyntaxModel after =
        calculatedSyntaxModelAfterPropertyChange(
            element, observedNode, property, oldValue, newValue);
    List<DifferenceElement> differenceElements =
        DifferenceElementCalculator.calculate(original, after);
  ...
  private void calculatedSyntaxModelForNode(
      CsmElement csm, Node node, List<CsmElement> elements, Change change) {
    if (csm instanceof CsmSequence) {
      CsmSequence csmSequence = (CsmSequence) csm;
      csmSequence
          .getElements()
          .forEach(e -> calculatedSyntaxModelForNode(e, node, elements, change));
          ...
          if (optional.isPresent()) {
            if (!(optional.get() instanceof NodeList)) {
              throw new IllegalStateException(
                  "Expected NodeList, found " + optional.get().getClass().getCanonicalName());
            }
        ...
        if (!collection.isEmpty()) {
          ...
          for (Iterator<?> it = collection.iterator(); it.hasNext(); ) {
            ...
            if (value instanceof Modifier) {
              ...
              throw new UnsupportedOperationException(
                  "Not supported value found: " + it.next().getClass().getSimpleName());
            }
            if (it.hasNext()) {
              calculatedSyntaxModelForNode(csmList.getSeparatorPost(), node, elements, change);
            }
      ...
      if (change instanceof PropertyChange) {
        elements.add(
            new CsmToken(
                GeneratedJavaParserConstants.CHAR,
                "'" + ((PropertyChange) change).getNewValue() + "'"));
      } else {
        elements.add(
            new CsmToken(
  ...
  CalculatedSyntaxModel calculatedSyntaxModelAfterPropertyChange(
    ...
    calculatedSyntaxModelForNode(
        csm, node, elements, new PropertyChange(property, oldValue, newValue));
    return new CalculatedSyntaxModel(elements);
  }
  ...
  CalculatedSyntaxModel calculatedSyntaxModelAfterListRemoval(
    ...
    Node container = nodeList.getParentNodeForChildren();
    calculatedSyntaxModelForNode(
        csm, container, elements, new ListRemovalChange(observableProperty, index));
    return new CalculatedSyntaxModel(elements);
  }
  ...
  CalculatedSyntaxModel calculatedSyntaxModelAfterListAddition(
    ...
    return calculatedSyntaxModelAfterListAddition(
        csm, observableProperty, nodeList, index, nodeAdded);
  }
