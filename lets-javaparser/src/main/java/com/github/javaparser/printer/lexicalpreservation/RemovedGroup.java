...
/**
 * This class represents a group of {@link Removed} elements. The {@link Removed} elements are
 * ideally consecutive for the methods in this class to work correctly.
 *
 * <p>This class consists of methods that calculate information to better handle the difference
 * application for the containing {@link Removed} elements.
 *
 * @see Iterable
 * @author ThLeu
 */
final class RemovedGroup implements Iterable<Removed> {
   ...
   * @throws IllegalArgumentException if the firstElementIndex is null or the removedList is empty
   *     or null
   */
  ...
  /**
   * Returns true if the RemovedGroup equates to a complete line This is the case if there are only
   * spaces and tabs left on the line besides the Removed elements. <br>
   * Example:
   *
   * <pre>
  ...
  private boolean hasOnlyWhitespace(
      Removed startElement, Function<TokenRange, Boolean> hasOnlyWhitespaceFunction) {
  ...
  /**
   * Returns the indentation in front of this RemovedGroup if possible. Sometimes the first deleted
   * element may be a line break because the <code>ConcreteSyntaxModel</code> generates a line break
   * before the members (for example FieldDeclaration). In this case a remove operation on the
   * member will generate the deletion of the first line break. It is therefore necessary to avoid
   * taking this element into account so we're looking for the first element that isn't a line
   * break.. For example class Foo { int x; } If there is something else than whitespace in front
   * this element, Optional.empty() is returned.
   *
      ...
      if (tokenRange.isPresent()) {
        ...
        if (hasOnlyWhitespaceJavaTokenInFrontFunction.apply(begin)) {
          ...
          while (previousToken.isPresent()
              && (TokenTypes.isWhitespaceButNotEndOfLine(previousToken.get().getKind()))) {
            indentation++;
