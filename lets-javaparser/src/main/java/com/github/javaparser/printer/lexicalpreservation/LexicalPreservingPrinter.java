...
/**
 * A Lexical Preserving Printer is used to capture all the lexical information while parsing, update
 * them when operating on the AST and then used them to reproduce the source code in its original
 * formatting including the AST changes.
 */
public class LexicalPreservingPrinter {
  ...
  /**
   * The nodetext for a node is stored in the node's data field. This is the key to set and retrieve
   * it.
   */
   ...
   * <ol>
   *   <li>Parse some code
   *   <li>Call this setup method on the result
   *   <li>Make changes to the AST as desired
   *   <li>Use one of the print methods on this class to print out the original source code with
   *       your changes added
   * </ol>
   *
  ...
  private static class Observer extends PropagatingAstObserver {
    ...
    public void concretePropertyChange(
      ...
      if (property == ObservableProperty.COMMENT) {
        ...
        if (oldValue == null) { // this case corresponds to the addition of a comment
          ...
          LineSeparator lineSeparator =
              observedNode.getLineEndingStyleOrDefault(LineSeparator.SYSTEM);
          ...
          if (oldValue instanceof Comment) {
            ...
            throw new UnsupportedOperationException(
                "Trying to remove something that is not a comment!");
          }
        } else {
          ...
          if (matchingTokens.size() != 1) {
            throw new IllegalStateException(
                "The matching comment to be replaced could not be found");
          }
          Comment newComment = (Comment) newValue;
          TokenTextElement matchingElement = matchingTokens.get(0);
          nodeText.replace(
              matchingElement.and(matchingElement.matchByRange()), makeCommentToken(newComment));
        }
      }
      ...
      LEXICAL_DIFFERENCE_CALCULATOR.calculatePropertyChange(
          nodeText, observedNode, property, oldValue, newValue);
    }
    ...
    private TokenTextElement makeCommentToken(Comment newComment) {
      if (newComment.isJavadocComment()) {
        return new TokenTextElement(
            JAVADOC_COMMENT,
            newComment.getHeader() + newComment.getContent() + newComment.getFooter());
      }
      if (newComment.isLineComment()) {
        return new TokenTextElement(
            SINGLE_LINE_COMMENT, newComment.getHeader() + newComment.getContent());
      }
    ...
    private int getIndexOfComment(Comment oldValue, NodeText nodeText) {
      ...
      // If no matching TokenTextElements were found, we try searching through ChildTextElements as
      // well
     ...
     * Comment
     */
    private List<ChildTextElement> findChildTextElementForComment(
      ...
      if (matchingChildElements.size() > 1) {
        ...
        matchingChildElements =
            matchingChildElements.stream()
                .filter(t -> t.getChild().hasRange() && oldValue.hasRange())
                .filter(
                    t ->
                        t.getChild().getRange().get().equals(oldValue.getRange().get())
    ...
    private List<ChildTextElement> selectMatchingChildElements(
        Comment oldValue, NodeText nodeText) {
      List<ChildTextElement> result = new ArrayList<>();
      List<ChildTextElement> childTextElements =
      ...
      while (iterator.hasNext()) {
        ...
        if (textElement.isComment()
            && isSameComment(((Comment) textElement.getChild()), oldValue)) {
    ...
    private List<TokenTextElement> findTokenTextElementForComment(
        Comment oldValue, NodeText nodeText) {
      List<TokenTextElement> matchingTokens;
      ...
      // To check that a comment matches in the list of tokens, if exists the range must be always
      // checked,
    ...
    /**
     * This method inserts new space tokens at the given {@code index}. If a new comment is added to
     * the token list at the position following {@code index}, the new comment and the node will
    ...
    public void concreteListChange(
      ...
      if (type == AstObserver.ListChangeType.REMOVAL) {
        differenceElements =
            LEXICAL_DIFFERENCE_CALCULATOR.calculateListRemovalDifference(
                findNodeListName(changedList), changedList, index);
      } else if (type == AstObserver.ListChangeType.ADDITION) {
        differenceElements =
            LEXICAL_DIFFERENCE_CALCULATOR.calculateListAdditionDifference(
    ...
    public void concreteListReplacement(
        NodeList<?> changedList, int index, Node oldValue, Node newValue) {
      NodeText nodeText = getOrCreateNodeText(changedList.getParentNodeForChildren());
  ...
  private static void storeInitialText(Node root) {
    ...
    // We go over tokens and find to which nodes they belong. Note that we do not traverse the
    // tokens as they were
    // on a list but as they were organized in a tree. At each time we select only the branch
            ...
            rootTokenRange -> {
              for (JavaToken token : rootTokenRange) {
                Range tokenRange =
                    token
                        .getRange()
                        .orElseThrow(() -> new RuntimeException("Token without range: " + token));
                Node owner =
                    root.findByRange(tokenRange)
                        .orElseThrow(
                            () -> new RuntimeException("Token without node owning it: " + token));
                if (!tokensByNode.containsKey(owner)) {
                  tokensByNode.put(owner, new LinkedList<>());
                }
                tokensByNode.get(owner).add(token);
              }
              // Now that we know the tokens we use them to create the initial NodeText for each
              // node
              new TreeVisitor() {
  ...
  private static void prettyPrintingTextNode(Node node, NodeText nodeText) {
    ...
    if (node instanceof JavadocComment) {
      ...
      nodeText.addToken(
          JAVADOC_COMMENT,
          comment.getHeader() + ((JavadocComment) node).getContent() + comment.getFooter());
      return;
    }
    if (node instanceof BlockComment) {
      ...
      nodeText.addToken(
          MULTI_LINE_COMMENT,
          comment.getHeader() + ((BlockComment) node).getContent() + comment.getFooter());
      return;
    }
    ...
    if (node instanceof Modifier) {
      ...
      nodeText.addToken(
          LexicalDifferenceCalculator.toToken(modifier), modifier.getKeyword().asString());
      return;
    }
  ...
  private static NodeText interpret(Node node, CsmElement csm, NodeText nodeText) {
    ...
    node.getComment()
        .ifPresent(
            comment -> {
              // new comment has no range so in this case we want to force the comment before the
              // node
              if (!comment.hasRange()) {
                LineSeparator lineSeparator =
                    node.getLineEndingStyleOrDefault(LineSeparator.SYSTEM);
    ...
    for (CsmElement element : calculatedSyntaxModel.elements) {
      if (element instanceof CsmIndent) {
        ...
        if (calculatedSyntaxModel.elements.size() > indexCurrentElement
            && !(calculatedSyntaxModel.elements.get(indexCurrentElement + 1)
                instanceof CsmUnindent)) {
      ...
      if (pendingIndentation
          && !(element instanceof CsmToken && ((CsmToken) element).isNewLine())) {
      ...
      if (element instanceof LexicalDifferenceCalculator.CsmChild) {
        ...
        // Indentation should probably be dealt with before because an indentation has effects also
        // on the
        // following lines
  ...
  private static ObservableProperty findNodeListName(NodeList<?> nodeList) {
    ...
    throw new IllegalArgumentException(
        "Cannot find list name of NodeList of size " + nodeList.size());
  }
}
