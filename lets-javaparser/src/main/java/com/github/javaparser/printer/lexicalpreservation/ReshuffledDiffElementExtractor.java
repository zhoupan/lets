...
public class ReshuffledDiffElementExtractor {
  ...
  public void extract(List<DifferenceElement> diffElements) {
    ...
    while (iterator.hasNext()) {
      ...
      if (diffElement instanceof Reshuffled) {
        ...
        // First, let's see how many tokens we need to attribute to the previous version of the of
        // the CsmMix
        CsmMix elementsFromPreviousOrder = reshuffled.getPreviousOrder();
        CsmMix elementsFromNextOrder = reshuffled.getNextOrder();
        // This contains indexes from elementsFromNextOrder to indexes from
        ...
        List<Integer> nodeTextIndexOfPreviousElements =
            findIndexOfCorrespondingNodeTextElement(
                elementsFromPreviousOrder.getElements(), nodeText);
        ...
        int lastNodeTextIndex =
            nodeTextIndexOfPreviousElements.stream().max(Integer::compareTo).orElse(-1);
        ...
        if (lastNodeTextIndex != -1) {
          for (int ntIndex = 0; ntIndex <= lastNodeTextIndex; ntIndex++) {
            if (nodeTextIndexToPreviousCSMIndex.containsKey(ntIndex)) {
              ...
              if (elementsToAddBeforeGivenOriginalCSMElement.containsKey(
              ...
              boolean toBeKept =
                  correspondanceBetweenNextOrderAndPreviousOrder.containsValue(
                      indexOfOriginalCSMElement);
              if (toBeKept) {
            ...
            // else we have a simple node text element, without associated csm element, just keep
            // ignore it
          }
        }
  ...
  private List<Integer> findIndexOfCorrespondingNodeTextElement(
      List<CsmElement> elements, NodeText nodeText) {
    ...
    while (csmElementListIterator.hasNext()) {
      ...
      PeekingIterator<TextElement> nodeTextListIterator =
          new PeekingIterator<>(nodeText.getElements());
          ...
          if (isCorresponding) {
            ...
            if (!isFirstIterationOnNodeTextElements && !isFirstIterationOnCsmElements) {
              TextElement previousTextElement =
                  nodeText.getTextElement(currentTextElementIndex - 1);
            ...
            if (csmElementListIterator.hasNext()) {
              ...
              hasSameNextElement =
                  elements
                      .get(csmElementListIterator.nextIndex())
            ...
            if (hasSamePreviousElement && hasSameNextElement) {
              ...
              potentialMatches.putIfAbsent(
                  MatchClassification.NEXT_AND_SAME, currentTextElementIndex);
            } else {
      ...
      Optional<MatchClassification> bestMatchKey =
          potentialMatches.keySet().stream()
  ...
  private boolean isAlmostCorrespondingElement(TextElement textElement, CsmElement csmElement) {
    ...
    return textElement.isWhiteSpace()
        && csmElement instanceof CsmToken
        && ((CsmToken) csmElement).isWhiteSpace();
  }
}
