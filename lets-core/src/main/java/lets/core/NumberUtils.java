...
public class NumberUtils {
  ...
  /**
   * {@code NumberUtils} instances should NOT be constructed in standard programming. Instead, the
   * class should be used as {@code NumberUtils.toInt("6");}.
   *
   ...
   *
   * <p>If the string {@code str} is {@code null}, {@code 0.0f} is returned.
   *
   * <pre>
   ...
   *
   * <p>If the string {@code str} is {@code null}, the default value is returned.
   *
   * <pre>
   ...
   *
   * <p>If the string {@code str} is {@code null}, {@code 0.0d} is returned.
   *
   * <pre>
  ...
  /**
   * Convert a {@code String} to a {@code double}, returning a default value if the conversion
   * fails.
   *
   * <p>If the string {@code str} is {@code null}, the default value is returned.
   *
   ...
   *
   * <p>If the {@code BigDecimal} {@code value} is {@code null}, then the specified default value is
   * returned.
   *
   * <pre>
   *   NumberUtils.toDouble(null)                     = 0.0d
  ...
  /**
   * Convert a {@code String} to an {@code short}, returning a default value if the conversion
   * fails.
   *
   * <p>If the string is {@code null}, the default value is returned.
   *
   ...
   *
   * <p>Note, the scale of a {@code BigDecimal} is the number of digits to the right of the decimal
   * point.
   *
   ...
   * @since 3.8
   */
  public static BigDecimal toScaledBigDecimal(
      final BigDecimal value, final int scale, final RoundingMode roundingMode) {
    if (value == null) {
   ...
   */
  public static BigDecimal toScaledBigDecimal(
      final Float value, final int scale, final RoundingMode roundingMode) {
    if (value == null) {
      return BigDecimal.ZERO;
    }
   ...
   */
  public static BigDecimal toScaledBigDecimal(
      final Double value, final int scale, final RoundingMode roundingMode) {
    if (value == null) {
      return BigDecimal.ZERO;
    }
    return toScaledBigDecimal(BigDecimal.valueOf(value), scale, roundingMode);
   ...
   */
  public static BigDecimal toScaledBigDecimal(
      final String value, final int scale, final RoundingMode roundingMode) {
    if (value == null) {
      return BigDecimal.ZERO;
    }
   ...
   *
   * <p>Then, the value is examined for a type qualifier on the end, i.e. one of {@code 'f', 'F',
   * 'd', 'D', 'l', 'L'}. If it is found, it starts trying to create successively larger types from
   * the type specified until one is found that can represent the value.
   *
   * <p>If a type specifier is not found, it will check for a decimal point and then try
   * successively larger types from {@code Integer} to {@code BigInteger} and from {@code Float} to
   * {@code BigDecimal}.
   *
   * <p>Integral values with a leading {@code 0} will be interpreted as octal; the returned number
   ...
   * Utility method for {@link #createNumber(java.lang.String)}.
   *
   * <p>Returns {@code true} if s is {@code null}.
   *
  ...
  /**
   * Convert a {@code String} to a {@code Integer}, handling hex (0xhhhh) and octal (0dddd)
   * notations. N.B. a leading zero means octal; spaces are not trimmed.
   *
   * <p>Returns {@code null} if the string is {@code null}.
   *
   * @param str a {@code String} to convert, may be null
  ...
  /**
   * Convert a {@code String} to a {@code Long}; since 3.1 it handles hex (0Xhhhh) and octal (0ddd)
   * notations. N.B. a leading zero means octal; spaces are not trimmed.
   *
   * <p>Returns {@code null} if the string is {@code null}.
   *
  ...
  /**
   * Convert a {@code String} to a {@code BigInteger}; since 3.2 it handles hex (0x or #) and octal
   * (0) notations.
   *
   * <p>Returns {@code null} if the string is {@code null}.
   *
   ...
   * @throws IllegalArgumentException if {@code array} is {@code null}
   * @throws IllegalArgumentException if {@code array} is empty
   * @see IEEE754rUtils#min(double[]) IEEE754rUtils for a version of this method that handles NaN
   *     differently
   ...
   *
   * <p>If any value is {@code NaN}, {@code NaN} is returned. Infinity is handled.
   *
   * @param a value 1
   * @param b value 2
   * @param c value 3
   ...
   *
   * <p>{@code Null} and empty String will return {@code false}.
   *
   ...
   *
   * <p>Valid numbers include hexadecimal marked with the {@code 0x} or {@code 0X} qualifier, octal
   * numbers, scientific notation and numbers marked with a type qualifier (e.g. 123L).
   *
   * <p>Non-hexadecimal strings beginning with a leading zero are treated as octal values. Thus the
   * string {@code 09} will return {@code false}, since {@code 9} is not a valid octal value.
   * However, numbers beginning with {@code 0.} are treated as decimal.
   *
   * <p>{@code null} and empty/blank {@code String} will return {@code false}.
   *
   * <p>Note, {@link #createNumber(String)} should return a number for every input resulting in
   * {@code true}.
   *
   * @param str the {@code String} to check
   * @return {@code true} if the string is a correctly formatted number
   * @since 3.3 the code supports hex {@code 0Xhhh} an octal {@code 0ddd} validation
   * @deprecated This feature will be removed in Lang 4.0, use {@link
  ...
  public static boolean isCreatable(final String str) {
    ...
    if (sz > start + 1
        && chars[start] == '0'
        && !StringUtils.contains(str, '.')) { // leading 0, skip if is a decimal
      // number
  ...
  /**
   * Compares to {@code long} values numerically. This is the same functionality as provided in Java
   * 7.
   *
  ...
  /**
   * Compares to {@code short} values numerically. This is the same functionality as provided in
   * Java 7.
   *
  ...
  /**
   * Compares two {@code byte} values numerically. This is the same functionality as provided in
   * Java 7.
   *
