 ...
 *
 * <p>Caveat: Instances of {@link Random} are not cryptographically secure.
 *
 * <p>Please note that the Apache Commons project provides a component dedicated to pseudo-random
 * number generation, namely <a href="https://commons.apache.org/rng">Commons RNG</a>, that may be a
 * better choice for applications with more stringent requirements (performance and/or correctness).
 *
 * @since 3.3
 */
public class RandomUtils {
  ...
  public static int nextInt(
      final int startInclusive, final int endExclusive, boolean allowNegative) {
    Validate.isTrue(
        endExclusive >= startInclusive, "Start value must be smaller or equal to end value.");
    if (!allowNegative) {
      Validate.isTrue(startInclusive >= 0, "Both range values must be non-negative.");
  ...
  public static double nextDouble(
      final double startInclusive, final double endExclusive, boolean allowNegative) {
    Validate.isTrue(
        endExclusive >= startInclusive, "Start value must be smaller or equal to end value.");
    if (!allowNegative) {
      Validate.isTrue(startInclusive >= 0, "Both range values must be non-negative.");
