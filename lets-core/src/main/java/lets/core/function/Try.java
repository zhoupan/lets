...
/**
 * A container object which may either contain a nullable value in case of <em>success</em> or an
 * exception in case of <em>failure</em>.
 *
 * <p>Instances of this class should be returned by methods instead of {@link Optional} when callers
 * might want to report the exception via logging or by wrapping it in another exception at a later
 * point in time, e.g. via {@link #getOrThrow(Function)}.
 *
 * <p>Moreover, it makes it particularly convenient to attach follow-up actions should the {@code
 * Try} have been successful (cf. {@link #andThen} and {@link #andThenTry}) or fallback actions
 * should it not have been (cf. {@link #orElse} and {@link #orElseTry}).
 */
public abstract class Try<V> {

  /**
   * Call the supplied {@link Callable} and return a successful {@code Try} that contains the
   * returned value or, in case an exception was thrown, a failed {@code Try} that contains the
   * exception.
   *
   * @param action the action to try; must not be {@code null}
   * @return a succeeded or failed {@code Try} depending on the outcome of the supplied action;
   *     never {@code null}
   * @see #success(Object)
   ...
   * @return a succeeded {@code Try} that contains the supplied value; never {@code null}
   */
  ...
  /**
   * If this {@code Try} is a success, apply the supplied transformer to its value and return a new
   * successful or failed {@code Try} depending on the transformer's outcome; if this {@code Try} is
  ...
  /**
   * If this {@code Try} is a success, apply the supplied function to its value and return the
   * resulting {@code Try}; if this {@code Try} is a failure, do nothing.
   *
  ...
  /**
   * If this {@code Try} is a failure, call the supplied action and return a new successful or
   * failed {@code Try} depending on the action's outcome; if this {@code Try} is a success, do
   * nothing.
   *
   * @param action the action to try; must not be {@code null}
   * @return a succeeded or failed {@code Try}; never {@code null}
  ...
  /**
   * If this {@code Try} is a failure, call the supplied supplier and return the resulting {@code
   * Try}; if this {@code Try} is a success, do nothing.
   *
  ...
  /**
   * If this {@code Try} is a success, get the contained value; if this {@code Try} is a failure,
  ...
  /**
   * If this {@code Try} is a success, get the contained value; if this {@code Try} is a failure,
   * call the supplied {@link Function} with the contained exception and throw the resulting {@link
   * Exception}.
   *
   * @param exceptionTransformer the transformer to be called with the contained exception, if
   *     available; must not be {@code null}
   * @return the contained value, if available
   * @throws E if this {@code Try} is a failure
   */
  public abstract <E extends Exception> V getOrThrow(
      Function<? super Exception, E> exceptionTransformer) throws E;

  /**
   * If this {@code Try} is a success, call the supplied {@link Consumer} with the contained value;
   * otherwise, do nothing.
   *
   * @param valueConsumer the consumer to be called with the contained value, if available; must not
   *     be {@code null}
  ...
  /**
   * If this {@code Try} is a failure, call the supplied {@link Consumer} with the contained
   * exception; otherwise, do nothing.
   *
   * @param causeConsumer the consumer to be called with the contained exception, if available; must
   *     not be {@code null}
   * @return the same {@code Try} for method chaining
   */
  ...
  /**
   * If this {@code Try} is a failure, return an empty {@link Optional}; if this {@code Try} is a
   * success, wrap the contained value using {@link Optional#ofNullable(Object)}.
   *
   ...
   *
   * <p>The {@code Transformer} interface is similar to {@link Function}, except that a {@code
   * Transformer} may throw an exception.
   */
  @FunctionalInterface
