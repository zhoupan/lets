...
import lets.core.codec.BinaryDecoder;
import lets.core.codec.BinaryEncoder;
import lets.core.codec.DecoderException;
import lets.core.codec.EncoderException;
import lets.core.codec.StringDecoder;
 ...
 * 1521</a>.
 *
 * <p>The Quoted-Printable encoding is intended to represent data that largely consists of octets
 * that correspond to printable characters in the ASCII character set. It encodes the data in such a
 * way that the resulting octets are unlikely to be modified by mail transport. If the data being
 * encoded are mostly ASCII text, the encoded form of the data remains largely recognizable by
 * humans. A body which is entirely ASCII may also be encoded in Quoted-Printable to ensure the
 * integrity of the data should the message pass through a character- translating, and/or
 * line-wrapping gateway.
 *
 * <p>Note:
 *
 * <p>Depending on the selected {@code strict} parameter, this class will implement a different set
 * of rules of the quoted-printable spec:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Originally, this class only supported the non-strict mode, but the codec in this partial form
 * could already be used for certain applications that do not require quoted-printable line
 * formatting (rules #3, #4, #5), for instance Q codec. The strict mode has been added in 1.10.
 *
 * <p>This class is immutable and thread-safe.
 *
 * @see <a href="http://www.ietf.org/rfc/rfc1521.txt">RFC 1521 MIME (Multipurpose Internet Mail
 *     Extensions) Part One: Mechanisms for Specifying and Describing the Format of Internet Message
 *     Bodies </a>
 * @since 1.3
 */
public class QuotedPrintableCodec
  ...
  /**
   * Return the byte at position {@code index} of the byte array and make sure it is unsigned.
   *
   ...
   *
   * <p>This function fully implements the quoted-printable encoding specification (rule #1 through
  ...
  /**
   * Encodes a string into its quoted-printable form using the default string Charset. Unsafe
   ...
   * @param sourceStr quoted-printable string to convert into its original form
   * @return original string
   * @throws DecoderException Thrown if quoted-printable decoding is unsuccessful. Thrown if Charset
   *     is not supported.
   * @see #getCharset()
   */
  @Override
   ...
   * @throws EncoderException Thrown if quoted-printable encoding is not applicable to objects of
  ...
  public Object encode(final Object obj) throws EncoderException {
    if (obj == null) {
      ...
      throw new EncoderException(
          "Objects of type " + obj.getClass().getName() + " cannot be quoted-printable encoded");
    }
  }

  /**
   * Decodes a quoted-printable object into its original form. Escaped characters are converted back
   * to their original representation.
   *
   * @param obj quoted-printable object to convert into its original form
   * @return original object
   * @throws DecoderException Thrown if the argument is not a {@code String} or {@code byte[]}.
  ...
  public Object decode(final Object obj) throws DecoderException {
    if (obj == null) {
      ...
      throw new DecoderException(
          "Objects of type " + obj.getClass().getName() + " cannot be quoted-printable decoded");
    }
  }
  ...
  /**
   * Encodes a string into its quoted-printable form using the specified Charset. Unsafe characters
   * are escaped.
   *
   * <p>Depending on the selection of the {@code strict} parameter, this function either implements
   * the full ruleset or only a subset of quoted-printable encoding specification (rule #1 and rule
