...
public class Md5Crypt {
   ...
   *
   * @param keyBytes plaintext string to hash.
   * @param salt An APR1 salt. The salt may be null, in which case a salt is generated for you using
   *     {@link ThreadLocalRandom}; for more secure salts consider using {@link SecureRandom} to
   ...
   * using {@link SecureRandom} to generate your own salts and calling {@link #apr1Crypt(byte[],
   * String)}.
   *
   * @param keyBytes plaintext string to hash.
   * @return the hash value
   ...
   *
   * @param keyBytes plaintext string to hash.
   * @param salt salt string including the prefix and optionally garbage at the end. The salt may be
   *     null, in which case a salt is generated for you using {@link ThreadLocalRandom}; for more
   *     secure salts consider using {@link SecureRandom} to generate your own salts.
   * @return the hash value
   * @throws IllegalArgumentException if the salt does not match the allowed pattern
   * @throws IllegalArgumentException when a {@link java.security.NoSuchAlgorithmException} is
   *     caught.
   ...
   *
   * <p>A salt is generated for you using {@link ThreadLocalRandom}; for more secure salts consider
   * using {@link SecureRandom} to generate your own salts and calling {@link #md5Crypt(byte[],
   * String)}.
   *
   * @param keyBytes plaintext string to hash.
   * @return the hash value
   ...
   * @since 1.12
   */
  public static String md5Crypt(
      final byte[] keyBytes, final String salt, final String prefix, final Random random) {
    ...
    if (salt == null) {
      ...
      final Pattern p =
          Pattern.compile("^" + prefix.replace("$", "\\$") + "([\\.\\/a-zA-Z0-9]{1,8}).*");
