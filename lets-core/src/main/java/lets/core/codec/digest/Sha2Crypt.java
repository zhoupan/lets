...
public class Sha2Crypt {
   ...
   * using {@link SecureRandom} to generate your own salts and calling {@link #sha256Crypt(byte[],
   * String)}.
   *
   ...
   *     salt is generated for you using {@link SecureRandom}. If one does not want to use {@link
   *     SecureRandom}, you can pass your own {@link Random} in {@link #sha256Crypt(byte[], String,
   *     Random)}.
  ...
  public static String sha256Crypt(final byte[] keyBytes, String salt) {
    ...
    return sha2Crypt(
        keyBytes, salt, SHA256_PREFIX, SHA256_BLOCKSIZE, MessageDigestAlgorithms.SHA_256);
  }
   ...
   */
  private static String sha2Crypt(
      final byte[] keyBytes,
      final String salt,
    ...
    // int buflen = salt_prefix.length() - 1 + ROUNDS_PREFIX.length() + 9 + 1 + salt_string.length()
    // + 1 + 86 + 1;
   ...
   *
   * <p>A salt is generated for you using {@link ThreadLocalRandom}; for more secure salts consider
   * using {@link SecureRandom} to generate your own salts and calling {@link #sha512Crypt(byte[],
   * String)}.
   *
   * @param keyBytes plaintext to hash
   * @return complete hash value
   ...
   * @param keyBytes plaintext to hash
   * @param salt real salt value without prefix or "rounds=". The salt may be null, in which case a
   *     salt is generated for you using {@link SecureRandom}; if you want to use a {@link Random}
   *     object other than {@link SecureRandom} then we suggest you provide it using {@link
  ...
  public static String sha512Crypt(final byte[] keyBytes, String salt) {
    ...
    return sha2Crypt(
        keyBytes, salt, SHA512_PREFIX, SHA512_BLOCKSIZE, MessageDigestAlgorithms.SHA_512);
  }
