...
/**
 * Operations to simplify common {@link java.security.MessageDigest} tasks. This class is immutable
 * and thread-safe. However the MessageDigest instances it creates generally won't be.
 *
 * <p>The {@link MessageDigestAlgorithms} class provides constants for standard digest algorithms
 * that can be used with the {@link #getDigest(String)} method and other methods that require the
 * Digest algorithm name.
 *
 * <p>Note: the class has short-hand methods for all the algorithms present as standard in Java 6.
 * This approach requires lots of methods for each algorithm, and quickly becomes unwieldy. The
 * following code works with all algorithms:
 *
 * <pre>
 * import static lets.core.codec.digest.MessageDigestAlgorithms.SHA_224;
 * ...
...
public class DigestUtils {
  ...
  /**
   * Reads through a byte array and returns the digest for the data. Provided for symmetry with
   * other methods.
   *
   ...
   * @since 1.11
   */
  public static byte[] digest(final MessageDigest messageDigest, final File data)
      throws IOException {
   ...
   */
  public static byte[] digest(final MessageDigest messageDigest, final InputStream data)
      throws IOException {
    return updateDigest(messageDigest, data).digest();
  }
  ...
  public static byte[] digest(
      final MessageDigest messageDigest, final Path data, final OpenOption... options)
      throws IOException {
    return updateDigest(messageDigest, data, options).digest();
  }

  /**
   * Reads through a RandomAccessFile using non-blocking-io (NIO) and returns the digest for the
   * data
   *
   * @param messageDigest The MessageDigest to use (e.g. MD5)
   * @param data Data to digest
   ...
   * @since 1.14
   */
  public static byte[] digest(final MessageDigest messageDigest, final RandomAccessFile data)
      throws IOException {
    return updateDigest(messageDigest, data).digest();
  }
   ...
   *     >Appendix A in the Java Cryptography Architecture Reference Guide</a> for information about
  ...
  /**
   * Returns a {@code MessageDigest} for the given {@code algorithm} or a default if there is a
   * problem getting the algorithm.
   *
   * @param algorithm the name of the algorithm requested. See <a
   *     href="http://docs.oracle.com/javase/6/docs/technotes/guides/security/crypto/CryptoSpec.html#AppA"
  ...
  public static MessageDigest getDigest(
      final String algorithm, final MessageDigest defaultMessageDigest) {
    try {
      return MessageDigest.getInstance(algorithm);
   ...
   * @throws IllegalArgumentException when a {@link NoSuchAlgorithmException} is caught, which
   *     should not happen on Oracle Java 9 andgreater.
   ...
   * @throws IllegalArgumentException when a {@link NoSuchAlgorithmException} is caught, which
   *     should not happen on Oracle Java 9 and greater.
  ...
  public static MessageDigest updateDigest(
      final MessageDigest messageDigest, final byte[] valueToDigest) {
   ...
   */
  public static MessageDigest updateDigest(
      final MessageDigest messageDigest, final ByteBuffer valueToDigest) {
    messageDigest.update(valueToDigest);
    return messageDigest;
  }
   ...
   * @since 1.11
   */
  public static MessageDigest updateDigest(final MessageDigest digest, final File data)
      throws IOException {
  ...
  /**
   * Reads through a RandomAccessFile and updates the digest for the data using non-blocking-io
   * (NIO).
   *
   * <p>TODO Decide if this should be public.
   *
   * @param digest The MessageDigest to use (e.g. MD5)
   * @param data Data to digest
   * @return the digest
   ...
   * @since 1.14
   */
  private static MessageDigest updateDigest(final MessageDigest digest, final FileChannel data)
      throws IOException {
   ...
   */
  public static MessageDigest updateDigest(
      final MessageDigest digest, final InputStream inputStream) throws IOException {
   ...
   */
  public static MessageDigest updateDigest(
      final MessageDigest digest, final Path path, final OpenOption... options) throws IOException {
    try (final BufferedInputStream inputStream =
        new BufferedInputStream(Files.newInputStream(path, options))) {
      return updateDigest(digest, inputStream);
    }
  }
   ...
   *
   * <p>To update the digest using a different charset for the conversion, convert the String to a
   * byte array using {@link String#getBytes(java.nio.charset.Charset)} and pass that to the {@link
   * DigestUtils#updateDigest(MessageDigest, byte[])} method
   *
   * @param messageDigest the {@link MessageDigest} to update
   * @param valueToDigest the value to update the {@link MessageDigest} with; converted to bytes
   *     using {@link StringUtils#getBytesUtf8(String)}
   * @return the updated {@link MessageDigest}
   * @since 1.7
   */
  public static MessageDigest updateDigest(
      final MessageDigest messageDigest, final String valueToDigest) {
   ...
   *
   * <p>This can then be used to create digests using methods such as {@link #digest(byte[])} and
   * {@link #digestAsHex(File)}.
   *
