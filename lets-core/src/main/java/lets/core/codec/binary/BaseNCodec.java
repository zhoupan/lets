...
public abstract class BaseNCodec implements BinaryEncoder, BinaryDecoder {
   ...
   * </ul>
   *
   * <p>When strict decoding is enabled it is expected that the decoded bytes will be re-encoded to
   * a byte array that matches the original, i.e. no changes occur on the final character. This
   ...
   *
   * <p>The default is lenient. If the decoding policy is strict, then decoding will raise an {@link
   * IllegalArgumentException} if trailing bits are not part of a valid encoding. Decoding will
   * compose trailing bits into 8-bit bytes and discard the remainder.
   *
  ...
  /**
   * Returns whether or not the {@code octet} is in the current alphabet. Does not allow whitespace
   * or pad.
   *
