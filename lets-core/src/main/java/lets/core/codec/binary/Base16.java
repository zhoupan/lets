...
import lets.core.codec.CodecPolicy;
 ...
 *
 * <p>This implementation strictly follows RFC 4648, and as such unlike the {@link Base32} and
 * {@link Base64} implementations, it does not ignore invalid alphabet characters or whitespace,
 * neither does it offer chunking or padding characters.
 *
 * <p>The only additional feature above those specified in RFC 4648 is support for working with a
 * lower-case alphabet in addition to the default upper-case alphabet.
 *
...
public class Base16 extends BaseNCodec {
  ...
  /**
   * BASE16 characters are 4 bits in length. They are formed by taking an 8-bit group, which is
   * converted into two BASE16 characters.
   */
  ...
  /**
   * This array is a lookup table that translates Unicode characters drawn from the "Base16
   * Alphabet" (as specified in Table 5 of RFC 4648) into their 4-bit positive integer equivalents.
   * Characters that are not in the Base16 alphabet but fall within the bounds of the array are
   * translated to -1.
   */
  ...
  /**
   * This array is a lookup table that translates 4-bit positive integer index values into their
   * "Base16 Alphabet" equivalents as specified in Table 5 of RFC 4648.
   */
  ...
  /**
   * This array is a lookup table that translates Unicode characters drawn from the a lower-case
   * "Base16 Alphabet" into their 4-bit positive integer equivalents. Characters that are not in the
   * Base16 alphabet but fall within the bounds of the array are translated to -1.
   */
  ...
  /**
   * This array is a lookup table that translates 4-bit positive integer index values into their
   * "Base16 Alphabet" lower-case equivalents.
   */
  ...
  void decode(final byte[] data, int offset, final int length, final Context context) {
    ...
    if (availableChars == 1 && availableChars == dataLen) {
      context.ibitWorkArea =
          decodeOctet(data[offset])
              + 1; // store 1/2 byte for next invocation of decode, we offset by +1 as empty-value
      // is 0
      return;
    }
   ...
   * @return {@code true} if the value is defined in the the Base16 alphabet {@code false}
   *     otherwise.
   */
  @Override
  ...
  /**
   * Validates whether decoding allows an entire final trailing character that cannot be used for a
   * complete byte.
   *
