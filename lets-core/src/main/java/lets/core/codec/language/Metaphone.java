 ...
 *
 * <p>Initial Java implementation by <CITE>William B. Brogden. December, 1997</CITE>. Permission
 * given by <CITE>wbrogden</CITE> for code to be used anywhere.
 *
 * <p><CITE>Hanging on the Metaphone</CITE> by <CITE>Lawrence Philips</CITE> in <CITE>Computer
 * Language of Dec. 1990, p 39.</CITE>
 *
 * <p>Note, that this does not match the algorithm that ships with PHP, or the algorithm found in
 ...
 * </ul>
 *
 * <p>They have had undocumented changes from the originally published algorithm. For more
 * information, see <a href="https://issues.apache.org/jira/browse/CODEC-57">CODEC-57</a>.
 *
 * <p>This class is conditionally thread-safe. The instance field for maximum code length is mutable
 * {@link #setMaxCodeLen(int)} but is not volatile, and accesses are not synchronized. If an
...
public class Metaphone implements StringEncoder {
  ...
  /**
   * Find the metaphone value of a String. This is similar to the soundex algorithm, but better at
   * finding similar sounding words. All input is converted to upper case. Limitations: Input format
   * is expected to be a single ASCII word with only characters in the A - Z range, no punctuation
   * or numbers.
   *
        ...
        switch (symb) {
          ...
          case 'G': // GH silent at end or before consonant
            ...
            if (isPreviousChar(local, n, 'G')) {
              // NOTE: Given that duplicated chars are removed, I don't see how this can ever be
              // true
              hard = true;
            } else {
              hard = false;
  ...
  /**
   * Encodes an Object using the metaphone algorithm. This method is provided in order to satisfy
   * the requirements of the Encoder interface, and will throw an EncoderException if the supplied
   * object is not of type java.lang.String.
   *
   * @param obj Object to encode
   * @return An object (or type java.lang.String) containing the metaphone code which corresponds to
   *     the String supplied.
