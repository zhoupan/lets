 ...
 * A phoneme rule.
 *
 * <p>Rules have a pattern, left context, right context, output phoneme, set of languages for which
 * they apply and a logical flag indicating if all languages must be in play. A rule matches if:
 *
 * <ul>
 *   <li>the pattern matches at the current position
 *   <li>the string up until the beginning of the pattern matches the left context
 *   <li>the string from the end of the pattern matches the right context
 *   <li>logical is ALL and all languages are in scope; or
 *   <li>logical is any other value and at least one language is in scope
 * </ul>
 *
 * <p>Rules are typically generated by parsing rules resources. In normal use, there will be no need
 * for the user to explicitly construct their own.
 *
 * <p>Rules are immutable and thread-safe.
 *
 * <p><b>Rules resources</b>
 *
 * <p>Rules are typically loaded from resource files. These are UTF-8 encoded text files. They are
 ...
 *
 * <ul>
 *   <li><b>Rules:</b> whitespace separated, double-quoted strings. There should be 4 columns to
 *       each row, and these will be interpreted as:
 *       <ol>
 *         <li>pattern
 *         <li>left context
 *         <li>right context
...
public class Rule {
  ...
  public static final class Phoneme implements PhonemeExpr {
    ...
    /**
     * Returns a new Phoneme with the same text but a union of its current language set and the
     * given one.
     *
   ...
   */
  public static List<Rule> getInstance(
   ...
   * @param rt the RuleType to consider
   * @param langs the set of languages to consider
   * @return a map containing all Rules that apply, grouped by the first character of the rule
   *     pattern
   * @since 1.9
   */
  public static Map<String, List<Rule>> getInstanceMap(
      final NameType nameType, final RuleType rt, final Languages.LanguageSet langs) {
    return langs.isSingleton()
        ? getInstanceMap(nameType, rt, langs.getAny())
  ...
  public static Map<String, List<Rule>> getInstanceMap(
    ...
    if (rules == null) {
      throw new IllegalArgumentException(
          String.format("No rules found for %s, %s, %s.", nameType.getName(), rt.getName(), lang));
    }
  ...
  /**
   * Attempts to compile the regex into direct string ops, falling back to Pattern and Matcher in
   * the worst case.
   *
  ...
  /**
   * Gets the right context. This is a regular expression that must match to the right of the
   * pattern.
   *
   ...
   * {@code lContext} matches {@code input} up to {@code i}, {@code pattern} matches at i and {@code
   * rContext} matches from the end of the match of {@code pattern} to the end of {@code input}.
   *
