...
import lets.core.codec.language.bm.Languages.LanguageSet;
import lets.core.codec.language.bm.Rule.Phoneme;
...
public class PhoneticEngine {
  ...
  static final class PhonemeBuilder {
    ...
    /**
     * An empty builder where all phonemes must come from some set of languages. This will contain a
     * single phoneme of zero characters. This can then be appended to. This should be the only way
     * to create a new phoneme from scratch.
     *
     ...
     *
     * <p>This will lengthen phonemes that have compatible language sets to the expression, and drop
   ...
   * particular offset. After invocation, the values {@code i} and {@code found} are updated. {@code
   * i} points to the index of the next char in {@code input} that must be processed next (the input
   * up to that index having been processed already), and {@code found} indicates if a matching rule
   * was found or not. In the case where a matching rule was found, {@code phonemeBuilder} is
   * replaced with a new builder containing the phonemes updated by the matching rule.
   *
   * <p>Although this class is not thread-safe (it has mutable unprotected fields), it is not shared
   * between threads as it is constructed as needed by the calling methods.
   *
   * @since 1.6
   */
  private static final class RulesApplication {
     ...
     * context and pattern. Then applies this rule to the phoneme builder to produce updated
     * phonemes. If there was no match, {@code i} is advanced one and the character is silently
  ...
  static {
    ...
    NAME_PREFIXES.put(
        ...
        Collections.unmodifiableSet(
            new HashSet<>(
                Arrays.asList(
                    "al", "el", "da", "dal", "de", "del", "dela", "de la", "della", "des", "di",
                    "do", "dos", "du", "van", "von"))));
    NAME_PREFIXES.put(
        NameType.GENERIC,
        Collections.unmodifiableSet(
            new HashSet<>(
                Arrays.asList(
                    "da", "dal", "de", "del", "dela", "de la", "della", "des", "di", "do", "dos",
                    "du", "van", "von"))));
  }
  ...
  /**
   * Encodes an input string into an output phonetic representation, given a set of possible origin
   * languages.
   *
  ...
  public String encode(String input, final Languages.LanguageSet languageSet) {
    ...
    // rules that apply to a specific language that may be ambiguous or wrong if applied to other
    // languages
