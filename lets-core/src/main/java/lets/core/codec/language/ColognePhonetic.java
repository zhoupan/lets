 ...
 *
 * <p>Implements the <a href="http://de.wikipedia.org/wiki/K%C3%B6lner_Phonetik">K&ouml;lner
 * Phonetik</a> (Cologne Phonetic) algorithm issued by Hans Joachim Postel in 1969.
 *
 * <p>The <i>K&ouml;lner Phonetik</i> is a phonetic algorithm which is optimized for the German
 * language. It is related to the well-known soundex algorithm.
 *
 * <h2>Algorithm</h2>
 *
 * <ul>
 ...
 *       href="http://en.wikipedia.org/wiki/Germanic_umlaut">germanic umlauts</a>, removal of non
 *       alphabetical characters) the letters of the supplied text are replaced by their phonetic
 ...
 *   <li>
 *       <h3>Step 3:</h3>
 *       Removal of all codes "0" except at the beginning. This means that two or more identical
 *       consecutive digits can occur if they occur after removing the "0" digits.
 *       <h4>Example:</h4>
 ...
 *
 * @see <a href="http://de.wikipedia.org/wiki/K%C3%B6lner_Phonetik">Wikipedia (de): K&ouml;lner
 *     Phonetik (in German)</a>
 * @since 1.5
 */
public class ColognePhonetic implements StringEncoder {
  ...
  /**
   * This class is not thread-safe; the field {@link #length} is mutable. However, it is not shared
   * between threads, as it is constructed on demand by the method {@link
  ...
  private class CologneOutputBuffer extends CologneBuffer {
    ...
    /**
     * Stores the next code in the output buffer, keeping track of the previous code. '0' is only
     * stored if it is the first entry. Ignored chars are never stored. If the code is the same as
     * the last code (whether stored or not) it is not stored.
     *
   ...
   *
   * <p>In contrast to the initial description of the algorithm, this implementation does the
   ...
   * @return {@code true} if the encoding the first string equals the encoding of the second string,
  ...
  /**
   * Converts the string to upper case and replaces Germanic umlaut characters The following
   * characters are mapped:
   *
   * <ul>
   *   <li>capital A, umlaut mark
   *   <li>capital U, umlaut mark
   *   <li>capital O, umlaut mark
   *   <li>small sharp s, German
   * </ul>
   */
