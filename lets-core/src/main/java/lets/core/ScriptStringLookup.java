 ...
 *
 * <p>Execute the script with the engine name in the format "EngineName:Script".
 *
 * <p>For example: {@code "javascript:3 + 4"}.
 *
...
final class ScriptStringLookup extends AbstractStringLookup {
  ...
  /**
   * Execute the script with the engine name in the format "EngineName:Script". Extra colons will be
   * ignored.
   *
   * <p>For example: {@code "javascript:3 + 4"}.
   *
  ...
  public String lookup(final String key) {
    ...
    if (keyLen != 2) {
      throw IllegalArgumentExceptions.format(
          "Bad script key format [%s]; expected format is EngineName:Script.", key);
    }
    ...
    try {
      ...
      throw IllegalArgumentExceptions.format(
          e, "Error in script engine [%s] evaluating script [%s].", engineName, script);
    }
  }
}
