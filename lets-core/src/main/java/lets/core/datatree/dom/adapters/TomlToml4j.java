 ...
 * adjustable with the following System Properties. If there is only one (eg. only the "toml4j")
...
public class TomlToml4j extends AbstractTextAdapter {
  ...
  public TomlToml4j() {
    try {
      mapper =
          new TomlWriter.Builder().timeZone(TimeZone.getTimeZone(Config.DEFAULT_TIME_ZONE)).build();
