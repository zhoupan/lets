...
import static lets.core.datatree.dom.TreeWriterRegistry.suggestDependency;
...
public class TreeReaderRegistry {
   ...
   * @param reader TreeReader instance for parsing the specified format
   */
  ...
  private static final TreeReader getReader(String format, boolean throwException) {
    ...
    try {
      ...
      if (className == null || className.isEmpty()) {
        throw new IllegalArgumentException(
            "System Property \""
                + propertyName
                + "\" not found! "
                + "This property defines a custom reader class for the \""
                + format
                + "\" format.");
      }
      ...
      if (throwException) {
        ...
        throw new IllegalArgumentException(
            "Unable to create reader for format \""
                + format
                + "\"! Set the -D"
                + propertyName
                + "=package.ReaderClass initial parameter to specify the proper reader class.",
            cause);
      }
    }
