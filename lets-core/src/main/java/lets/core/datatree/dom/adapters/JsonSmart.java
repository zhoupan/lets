 ...
 * (eg. only the "json-smart") implementation on the classpath, this step is NOT necessary, the
...
public class JsonSmart extends AbstractTextAdapter {
  ...
  static {
    ...
    tryToAddSerializers(
        "io.datatree.dom.adapters.JsonSmartBsonSerializers", JSONValue.defaultWriter);

    // BASE64
    addSerializer(
        JSONValue.defaultWriter,
        byte[].class,
        (value, out) -> {
          out.append('"');
  ...
  public static final <T> void addSerializer(
      JsonWriter writer, Class<T> type, CheckedBiConsumer<T, Appendable> consumer) {
