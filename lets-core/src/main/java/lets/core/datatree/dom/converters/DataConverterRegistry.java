 ...
 * <br>
 * DataConverterRegistry.register(String.class, InetAddress.class, (from) -&gt; { <br>
...
public class DataConverterRegistry extends AbstractConverterSet {
  ...
  private static final HashMap<DataConverterKey<?, ?>, DataConverter<?, ?>> converters =
      new HashMap<>(512);

  // --- ANY -> TO CONVERTER MAP ---

  private static final HashMap<Class<?>, DataConverter<?, ?>> defaultConverters =
      new HashMap<>(128);
  ...
  static {
    ...
    try {
      Class.forName("lets.core.datatree.dom.converters.BasicConverterSet");
    } catch (Throwable ignored) {
    }

    // Load converters for MongoDB / BSON object types
    try {
      Class.forName("lets.core.datatree.dom.converters.BsonConverterSet");
    } catch (Throwable ignored) {
    }
  }
  ...
  public static final <TO, FROM> void register(
      Class<TO> to, Class<FROM> from, DataConverter<TO, FROM> converter) {
   ...
   *
   * @param <TO> target class to convert the source object
  ...
  public static final <TO, FROM> TO convert(Class<TO> to, FROM from) {
    ...
    // Convert FROM -> TO
    DataConverterKey<TO, FROM> key =
        new DataConverterKey<TO, FROM>(to, (Class<FROM>) from.getClass());
