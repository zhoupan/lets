 ...
 * <br>
 * If there is more than one XML-RPC implementation on classpath, the preferred implementation is
 * adjustable with the following System Properties. If there is only one (eg. only the "sojo")
 * implementation on the classpath, this step is NOT necessary, the DataTree API will use this JSON
 * API automatically.<br>
 * <br>
        ...
        (input) -> {
          ...
          if (xml.contains("ex:")) {
            if (xml.contains("<methodResponse>")) {
              xml =
                  xml.replace(
                      "<methodResponse>", "<methodResponse xmlns:ex=\"http://xml-rpc.org\">");
            } else {
