 ...
 *
 */
package lets.core.datatree.dom;
...
public class TreeWriterRegistry {
  ...
  private static final ConcurrentHashMap<String, TreeWriter> writers =
      new ConcurrentHashMap<String, TreeWriter>();
   ...
   *
   * @param format name of the format (eg. "json", "xml", "csv", etc.)
   * @param writer TreeWriter instance for generating the specified format
   */
   ...
   *
   * @param format name of the format (eg. "custom", "csv", etc.)
   */
   ...
   *
   * @param format name of the format (eg. "json", "xml", etc.)
  ...
  private static final TreeWriter getWriter(String format, boolean throwException) {
    ...
    try {
      ...
      if (className == null || className.isEmpty()) {
        throw new IllegalArgumentException(
            "System Property \""
                + propertyName
                + "\" not found! "
                + "This property defines a custom writer class for the \""
                + format
                + "\" format.");
      }
      ...
      if (throwException) {
        ...
        throw new IllegalArgumentException(
            "Unable to create writer for format \""
                + format
                + "\"! Set the -D"
                + propertyName
                + "=package.WriterClass initial parameter to specify the proper writer class.",
            cause);
      }
    }
  ...
  static {
    addSample("yaml", "org.yaml", "snakeyaml", "1.28");
    addSample("csv", "com.opencsv", "opencsv", "5.4");
    addSample("xstream", "xstream", "xstream", "1.2.2");
    addSample("toml", "com.moandjiezana.toml", "toml4j", "0.7.2");
    addSample("smile", "com.fasterxml.jackson.dataformat", "jackson-dataformat-smile", "2.12.3");
    addSample("cbor", "com.fasterxml.jackson.dataformat", "jackson-dataformat-cbor", "2.12.3");
    addSample("bson", "de.undercouch", "bson4jackson", "2.12.0");
    addSample("msgpack", "org.msgpack", "msgpack", "0.6.12");
    addSample("ion", "software.amazon.ion", "ion-java", "1.5.1");
    addSample("kryo", "com.esotericsoftware", "kryo", "5.1.1");
  }
  ...
  protected static final void suggestDependency(String format) {
    ...
    System.err.println();
    System.err.println(
        "...or download the JAR from https://mvnrepository.com/artifact/"
            + samples[0]
            + "/"
            + samples[1]);
    System.err.println();
