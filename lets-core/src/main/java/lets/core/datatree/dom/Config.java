 ...
 *
 * <ul>
 *   <li>-Ddatatree.time.zone=os
 *   <li>-Ddatatree.timestamp.format="yyyy-MM-dd'T'HH:mm:ss.SSSX"
 *   <li>-Ddatatree.cache.size=1024
 *   <li>-Ddatatree.pool.size=16
 *   <li>-Ddatatree.use.timestamps=true
 *   <li>-Ddatatree.adapter.packages=your.adapter.package1,your.adapter.package2
 *   <li>-Ddatatree.base64.codec=your.base64.Codec
 * </ul>
 *
...
public class Config {
  ...
  /**
   * Name of the metadata node. Metadata node contains processing instructions, similar to "head"
   * block in HTML pages.<br>
   * <br>
  ...
  static {
    ...
    DEFAULT_TIME_ZONE =
        "os".equalsIgnoreCase(timeZoneID) ? TimeZone.getDefault().getID() : timeZoneID;
    TimeZone timeZone = TimeZone.getTimeZone(DEFAULT_TIME_ZONE);

    // Timestamp format
    TIMESTAMP_FORMAT =
        System.getProperty("datatree.timestamp.format", "yyyy-MM-dd'T'HH:mm:ss.SSSX");
    ...
    try {
      ...
      size =
          Integer.parseInt(
              System.getProperty("datatree.pool.size", Integer.toString(defaultPoolSize)));
    } catch (Exception cause) {
      cause.printStackTrace();
      size = 16;
    }
