/*
 * Copyright (c) 2018 zhoupan (https://github.com/zhoupan).
 *
...
/**
 * ES6-like Promise, based on the Java8's CompletableFuture API. A Promise is an object that may
 * produce a single value some time in the future: either a resolved value, or a reason that it's
 * not resolved (e.g., a network error occurred). Promise users can attach callbacks to handle the
 * fulfilled value or the reason for rejection. Sample waterfall processing:<br>
 *
 * <pre>
...
public class Promise {
  ...
  /** An internal CompletableFuture, what does the working logic of this Promise. */
  ...
  /**
   * Returns a Promise object that is resolved with the given value. Allowed Object types of the
   * "value" parameter are: Tree, String, int, double, byte, float, short, long, boolean, byte[],
   * UUID, Date, InetAddress, BigInteger, BigDecimal, and Java Collections with these types.
   *
   * @param value value of the new Promise
   * @return new RESOLVED/COMPLETED Promise
   */
   ...
   *
   * @param error error state of the new Promise
   ...
   * </pre>
   *
   * @param initializer internal initalizer logic
   */
   ...
   *
   * @param value Promise, CompletableFuture, Tree, String, int, double, byte, float, short, long,
   *     boolean, byte[], UUID, Date, InetAddress, BigInteger, BigDecimal, and Java Collections with
   *     these types.
   */
  ...
  /**
   * Promises can be used to unnest asynchronous functions and allows one to chain multiple
   * functions together - increasing readability and making individual functions, within the chain,
   * more reusable. Sample code:
   *
   * <pre>
   ...
   * </pre>
   *
   * @param action next action in the invocation chain (allowed return types: Promise,
   *     CompletableFuture, Tree, String, int, double, byte, float, short, long, boolean, byte[],
   *     UUID, Date, InetAddress, BigInteger, BigDecimal, and Java Collections with these types)
   ...
   * </pre>
   *
   * @param action next action in the invocation chain
   * @return output Promise
   */
  ...
  /**
   * The catchError() method returns a Promise and deals with rejected cases only. Sample:
   *
   * <pre>
   ...
   * </pre>
   *
   * @param action error handler of the previous "next" handlers
   * @return output Promise (allowed return types: Tree, String, int, double, byte, float, short,
   *     long, boolean, byte[], UUID, Date, InetAddress, BigInteger, BigDecimal, and Java
   ...
   * </pre>
   *
   * @return {@code true} if this invocation caused this Promise to transition to a completed state,
   *     else {@code false}
   */
   ...
   * </pre>
   *
   * @param value the result value (allowed types: Tree, String, int, double, byte, float, short,
   *     long, boolean, byte[], UUID, Date, InetAddress, BigInteger, BigDecimal, and Java
   *     Collections with these types)
   * @return {@code true} if this invocation caused this Promise to transition to a completed state,
   *     else {@code false}
  ...
  /**
   * If not already completed, sets the exception state to the given exception. Sample code:<br>
   *
   * <pre>
  ...
  /**
   * Returns {@code true} if this Promise completed in any fashion: normally, exceptionally, or via
   * cancellation.
   *
   ...
   * operation, do not use this method unless it is absolutely necessary for something. Rather use
   * the "then" and "catchError" methods.
   *
   * @return result Tree structure
   * @throws Exception any (interruption, execution, user-level, etc.) exception
   */
  ...
  /**
   * Returns a new Promise that is completed when all of the given Promise complete. If any of the
   * given Promise complete exceptionally, then the returned Promise also does so, with a Promise
   * holding this exception as its cause.
   *
   * @param promises array of Promises
   * @return a new Promise that is completed when all of the given Promises complete
   */
  ...
  /**
   * Returns a new Promise that is completed when any of the given Promises complete, with the same
   * result. Otherwise, if it completed exceptionally, the returned Promise also does so, with a
   * CompletionException holding this exception as its cause.
   *
   * @param promises Collection of Promises
   * @return a new Promise that is completed with the result or exception of any of the given
   *     Promises when one completes
   */
  ...
  protected static final CompletableFuture<Tree> toCompletableFuture(Object object) {
    ...
    if (object instanceof CompletionStage) {
      return (((CompletionStage<?>) object).toCompletableFuture())
          .thenCompose(Promise::toCompletableFuture);
    }
