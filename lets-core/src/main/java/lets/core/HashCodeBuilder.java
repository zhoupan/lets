 ...
 *
 * <p>This class enables a good {@code hashCode} method to be built for any class. It follows the
 * rules laid out in the book <a
 * href="http://www.oracle.com/technetwork/java/effectivejava-136174.html">Effective Java</a> by
 * Joshua Bloch. Writing a good {@code hashCode} method is actually quite difficult. This class aims
 * to simplify the process.
 *
 * <p>The following is the approach taken. When appending a data field, the current total is
 ...
 * hash code of 674, namely 17 * 37 + 45.
 *
 * <p>All relevant fields from the object should be included in the {@code hashCode} method. Derived
 * fields may be excluded. In general, any field used in the {@code equals} method must be used in
 ...
 * </pre>
 *
 * <p>If required, the superclass {@code hashCode()} can be added using {@link #appendSuper}.
 *
 * <p>Alternatively, there is a method that uses reflection to determine the fields to test. Because
 * these fields are usually private, the method, {@code reflectionHashCode}, uses {@code
 ...
 * </pre>
 *
 * <p>The {@link HashCodeExclude} annotation can be used to exclude fields from being used by the
 * {@code reflectionHashCode} methods.
 *
 * @since 1.0
 */
public class HashCodeBuilder implements Builder<Integer> {
  ...
  /*
   * NOTE: we cannot store the actual objects in a HashSet, as that would use the very hashCode() we are in the process
   * of calculating.
   *
   * So we generate a one-to-one mapping from the original object to a new object.
   *
   * Now HashSet uses equals() to determine if two elements with the same hash code really are equal, so we also need to
   * ensure that the replacement objects are only equal if the original objects are identical.
   *
   ...
   *
   * <p>Transient members will be not be used, as they are likely derived fields, and not part of
   * the value of the {@code Object}.
   *
   * <p>Static fields will not be tested. Superclass fields will be included.
   *
   * <p>Two randomly chosen, non-zero, odd numbers must be passed in. Ideally these should be
   ...
   * {@code Object}.
   *
   * <p>Static fields will not be included. Superclass fields will be included up to and including
   ...
   * Uses reflection to build a valid hash code from the fields of {@code object}.
   *
   * <p>This constructor uses two hard coded choices for the constants needed to build a hash code.
   *
   * <p>It uses {@code AccessibleObject.setAccessible} to gain access to private fields. This means
   * that it will throw a security exception if run under a security manager, if the permissions are
   * not set up correctly. It is also not as efficient as testing explicitly.
   ...
   * {@code Object}.
   *
   * <p>Static fields will not be tested. Superclass fields will be included. If no fields are found
   ...
   *
   * <p>This adds {@code 1} when true, and {@code 0} when false to the {@code hashCode}.
   *
   * <p>This is in contrast to the standard {@code java.lang.Boolean.hashCode} handling, which
   * computes a {@code hashCode} value of {@code 1231} for {@code java.lang.Boolean} instances that
   * represent {@code true} or {@code 1237} for {@code java.lang.Boolean} instances that represent
   * {@code false}.
   *
   * <p>This is in accordance with the <i>Effective Java</i> design.
   *
  ...
  /**
   * The computed {@code hashCode} from toHashCode() is returned due to the likelihood of bugs in
   * mis-calling toHashCode() and the unlikeliness of it mattering what the hashCode for
