...
/**
 * Duration formatting utilities and constants. The following table describes the tokens used in the
 * pattern language for formatting.
 *
 * <table border="1">
 ...
 * </table>
 *
 * <b>Note: It's not currently possible to include a single-quote in a format.</b> <br>
 * Token values are printed using decimal digits. A token character can be repeated to ensure that
 * the field occupies a certain minimum size. Values will be left-padded with 0 unless padding is
 * disabled in the method invocation.
 *
 * @since 2.1
 */
public class DurationFormatUtils {
  ...
  /**
   * Pattern used with {@code FastDateFormat} and {@code SimpleDateFormat} for the ISO 8601 period
   * format used in durations.
   *
   * @see lets.core.apache.commons.lang3.time.FastDateFormat
   ...
   *
   * <p>This method formats durations using the days and lower fields of the ISO format pattern,
   * such as P7D6TH5M4.321S.
   *
   * @param durationMillis the duration to format
   * @return the formatted duration, not null
   */
   ...
   * @return the long
   */
  public static long secondsFromMilliseconds(long milliseconds) {
   ...
   * @return the long
   */
  public static long minutesFromMilliseconds(long milliseconds) {
    return milliseconds / DateUtils.MILLIS_PER_MINUTE;
  }
   ...
   * @return the long
   */
  public static long hoursFromMilliseconds(long milliseconds) {
    return milliseconds / DateUtils.MILLIS_PER_HOUR;
  }
   ...
   * @return the long
   */
  public static long daysFromMilliseconds(long milliseconds) {
    return milliseconds / DateUtils.MILLIS_PER_DAY;
  }
  ...
  public static long days(Date from, Date to) {
    return Math.abs(daysFromMilliseconds(from.getTime() - to.getTime()));
  }
  ...
  public static long hours(Date from, Date to) {
    return Math.abs(hoursFromMilliseconds(from.getTime() - to.getTime()));
  }
  ...
  public static long minutes(Date from, Date to) {
    return Math.abs(minutesFromMilliseconds(from.getTime() - to.getTime()));
  }
  ...
  public static String formatPeriodISO(final long startMillis, final long endMillis) {
    return formatPeriod(
        startMillis, endMillis, ISO_EXTENDED_FORMAT_PATTERN, false, TimeZone.getDefault());
  }
  ...
  public static String formatPeriod(
      final long startMillis, final long endMillis, final String format) {
  ...
  static class Token {
    ...
    /**
     * Returns a hash code for the token equal to the hash code for the token's value. Thus 'TT' and
     * 'TTTT' will have the same hash code.
     *
