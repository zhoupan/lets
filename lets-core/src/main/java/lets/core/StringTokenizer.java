...
/**
 * Tokenizes a string based on delimiters (separators) and supporting quoting and ignored character
 * concepts.
 *
 * <p>This class can split a String into many smaller strings. It aims to do a similar job to {@link
 * java.util.StringTokenizer StringTokenizer}, however it offers much more control and flexibility
 * including implementing the {@code ListIterator} interface. By default, it is set up like {@code
 * StringTokenizer}.
 *
 * <p>The input String is split into a number of <i>tokens</i>. Each token is separated from the
 * next String by a <i>delimiter</i>. One or more delimiter characters must be specified.
 *
 * <p>Each token may be surrounded by quotes. The <i>quote</i> matcher specifies the quote
 * character(s). A quote may be escaped within a quoted section by duplicating itself.
 *
 * <p>Between each token and the delimiter are potentially characters that need trimming. The
 * <i>trimmer</i> matcher specifies these characters. One usage might be to trim whitespace
 * characters.
 *
 * <p>At any point outside the quotes there might potentially be invalid characters. The
 * <i>ignored</i> matcher specifies these characters to be removed. One usage might be to remove new
 * line characters.
 *
 * <p>Empty tokens may be removed or returned as null.
 *
 * <pre>
...
public class StringTokenizer implements ListIterator<String>, Cloneable {
  ...
  /**
   * Gets a new tokenizer instance which parses Comma Separated Value strings initializing it with
   * the given input. The default for CSV processing will be trim whitespace from both ends (which
  ...
  /**
   * Gets a new tokenizer instance which parses Tab Separated Value strings. The default for CSV
   * processing will be trim whitespace from both ends (which can be overridden with the setTrimmer
   * method).
   *
   * <p>You must call a "reset" method to set the string which you want to parse.
   *
   ...
   * but with no text to tokenize.
   *
   * <p>This constructor is normally used with {@link #reset(String)}.
   */
  ...
  /**
   * Constructs a tokenizer splitting on the specified delimiter character and handling quotes using
  ...
  /**
   * Constructs a tokenizer splitting using the specified delimiter matcher and handling quotes
   * using the specified quote matcher.
   *
   * @param input the string which is to be parsed, not cloned
   * @param delim the field delimiter character
   * @param quote the field quoted string character
  ...
  /**
   * Creates a new instance of this Tokenizer. The new instance is reset so that it will be at the
   * start of the token list. If a {@link CloneNotSupportedException} is caught, return {@code
   * null}.
   *
   ...
   *
   * <p>These characters are ignored when parsing the String, unless they are within a quoted
   * region. The default value is not to ignore anything.
   *
   ...
   *
   * <p>The quote character is used to wrap data between the tokens. This enables delimiters to be
   * entered as data. The default value is '"' (double quote).
   *
   ...
   *
   * <p>These characters are trimmed off on each side of the delimiter until the token or quote is
   * found. The default value is not to trim anything.
   *
  ...
  /**
   * Tests whether the tokenizer currently returns empty tokens as null. The default for this
   * property is false.
   *
  ...
  /**
   * Tests whether the tokenizer currently ignores empty tokens. The default for this property is
   * true.
   *
  ...
  /**
   * Tests if the characters at the index specified match the quote already matched in
   * readNextToken().
   *
  ...
  /**
   * Gets the next token from the String. Equivalent to {@link #next()} except it returns null
   * rather than throwing {@link NoSuchElementException} when no tokens remain.
   *
   ...
   * @return The starting position of the next field (the character immediately after the
   *     delimiter), or -1 if end of string found
   */
   ...
   * @param quoteStart the start position of the matched quote, 0 if no quoting
   * @param quoteLen the length of the matched quote, 0 if no quoting
   * @return The starting position of the next field (the character immediately after the delimiter,
   *     or if end of string found, then the length of string
   */
   ...
   *
   * <p>This method allows the same tokenizer to be reused for the same String.
   *
  ...
  /**
   * Resets this tokenizer, giving it a new input string to parse. In this manner you can re-use a
   * tokenizer with the same settings on multiple input lines.
   *
   ...
   *
   * <p>The delimiter is used to separate one token from another.
   *
  ...
  /**
   * Sets whether the tokenizer should return empty tokens as null. The default for this property is
   * false.
   *
   ...
   *
   * <p>This character is ignored when parsing the String, unless it is within a quoted region.
   *
  ...
  /**
   * Sets whether the tokenizer should ignore and not return empty tokens. The default for this
   * property is true.
   *
   ...
   * automatically by other (public) methods when required.
   *
   * <p>This method exists to allow subclasses to add code before or after the tokenization. For
   * example, a subclass could alter the character array, offset or count to be parsed, or call the
   * tokenizer multiple times on multiple strings. It is also be possible to filter the results.
   *
   * <p>{@code StrTokenizer} will always pass a zero offset and a count equal to the length of the
   * array to this method, however a subclass may pass other values, or even an entirely different
   * array.
   *
