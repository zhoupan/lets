 ...
 *
 * <p>Note that the semicolon is optional.
 *
 * @since 1.0
 */
public class NumericEntityUnescaper extends CharSequenceTranslator {
  ...
  /** Default options. */
  private static final EnumSet<OPTION> DEFAULT_OPTIONS =
   ...
   * (whether to allow, error or ignore the semicolon on the end of a numeric entity to being
   * missing).
   *
   ...
   *
   * <p>Note that the default behavior is to ignore them.
   *
   * @param options to apply to this unescaper
   */
  public NumericEntityUnescaper(final OPTION... options) {
    this.options =
        ArrayUtils.isEmpty(options) ? DEFAULT_OPTIONS : EnumSet.copyOf(Arrays.asList(options));
  }
