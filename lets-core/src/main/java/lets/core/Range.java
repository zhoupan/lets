 ...
 *
 * <p>The objects need to either be implementations of {@link Comparable} or you need to supply a
 * {@link Comparator}.
 *
 * <p>#ThreadSafe# if the objects and comparator are thread-safe.
 *
...
public class Range<T> implements Serializable {
   ...
   */
  @Deprecated
  public static <T extends Comparable<? super T>> Range<T> between(
   ...
   * Creates a range with the specified minimum and maximum values (both inclusive).
   *
   * <p>The range uses the specified {@link Comparator} to determine where values lie in the range.
   *
   * <p>The arguments may be passed in the order (min,max) or (max,min). The getMinimum and
   * getMaximum methods will return the correct values.
   *
   ...
   * @throws NullPointerException when fromInclusive is null.
   * @throws NullPointerException when toInclusive is null.
   * @throws ClassCastException if using natural ordering and the elements are not {@link
   *     Comparable}
   * @deprecated Use {@link #of(Object, Object, Comparator)}.
   */
  @Deprecated
  public static <T> Range<T> between(
      final T fromInclusive, final T toInclusive, final Comparator<T> comparator) {
   ...
   *
   * <p>This method may fail if the ranges have two different comparators or element types.
   *
   ...
   *
   * <p>The API is reminiscent of the Comparable interface returning {@code -1} if the element is
   * before the range, {@code 0} if contained within the range and {@code 1} if the element is after
   * the range.
   *
  ...
  /**
   * Fits the given element into this range by returning the given element or, if out of bounds, the
   * range minimum if below, or the range maximum if above.
   *
   * <pre>
   ...
   *
   * @param element the element to check for, not null
   * @return the minimum, the element, or the maximum depending on the element's location relative
   *     to the range
   * @throws NullPointerException if {@code element} is {@code null}
   * @since 3.10
   */
   ...
   *
   * <p>Natural ordering uses an internal comparator implementation, thus this method never returns
   * null. See {@link #isNaturalOrdering()}.
   *
   ...
   * @return range representing the intersection of {@code this} and {@code other} ({@code this} if
   *     equal)
   ...
   *
   * <p>Natural ordering uses an internal comparator implementation, thus this method is the only
   * way to check if a null comparator was specified.
   *
   ...
   *
   * <p>Two ranges overlap if there is at least one element in common.
   *
   * <p>This method may fail if the ranges have two different comparators or element types.
   *
   ...
   *
   * <p>The format of the String is '[<i>min</i>..<i>max</i>]'.
   *
   ...
   * used to embed the minimum, maximum and comparator. Use {@code %1$s} for the minimum element,
   * {@code %2$s} for the maximum element and {@code %3$s} for the comparator. The default format
   * used by {@code toString()} is {@code [%1$s..%2$s]}.
   *
   * @param format the format string, optionally containing {@code %1$s}, {@code %2$s} and {@code
   *     %3$s}, not null
