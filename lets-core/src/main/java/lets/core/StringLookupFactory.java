...
/**
 * Create instances of string lookups or access singleton string lookups implemented in this
 * package.
 *
 * <p>The "classic" look up is {@link #mapStringLookup(Map)}.
 *
 * <p>The methods for variable interpolation (A.K.A. variable substitution) are:
 *
 * <ul>
 *   <li>{@link #interpolatorStringLookup()}.
 *   <li>{@link #interpolatorStringLookup(Map)}.
 *   <li>{@link #interpolatorStringLookup(StringLookup)}.
 *   <li>{@link #interpolatorStringLookup(Map, StringLookup, boolean)}.
 * </ul>
 *
 * <p>Unless explicitly requested otherwise, a set of default lookups are included for convenience
 * with these variable interpolation methods. These defaults are listed in the table below. However,
 * the exact lookups included can be configured through the use of the {@value
 * #DEFAULT_STRING_LOOKUPS_PROPERTY} system property. If present, this system property will be
 * parsed as a comma-separated list of lookup names, with the names being those defined by the
 * {@link DefaultStringLookup} enum. For example, setting this system property to {@code
 * "BASE64_ENCODER,ENVIRONMENT"} will only include the {@link DefaultStringLookup#BASE64_ENCODER
 * BASE64_ENCODER} and {@link DefaultStringLookup#ENVIRONMENT ENVIRONMENT} lookups. Setting the
 * property to the empty string will cause no defaults to be configured. Note that not all lookups
 * defined here and in {@link DefaultStringLookup} are included by default. Specifically, lookups
 * that can execute code (e.g., {@link DefaultStringLookup#SCRIPT SCRIPT}) and those that can result
 * in contact with remote servers (e.g., {@link DefaultStringLookup#URL URL} and {@link
 * DefaultStringLookup#DNS DNS}) are not included by default. The current set of default lookups can
 ...
 * </table>
 *
 * <p>This class also provides functional lookups used as building blocks for other lookups.
 *
 * <table>
...
public final class StringLookupFactory {
  ...
  public static final class Builder implements Supplier<StringLookupFactory> {
     ...
     *
     * <p>Path Fences apply to the file, property, and XML string lookups.
     *
  ...
  /**
   * Internal class used to construct the default {@link StringLookup} map used by {@link
  ...
  static final class DefaultStringLookupsHolder {
    ...
    static final DefaultStringLookupsHolder INSTANCE =
        new DefaultStringLookupsHolder(System.getProperties());

    /**
     * Adds the key and string lookup from {@code lookup} to {@code map}, also adding any additional
    ...
    private static void addLookup(
        final DefaultStringLookup lookup, final Map<String, StringLookup> map) {
    ...
    /**
     * Constructs a lookup map by parsing the given string. The string is expected to contain comma
     * or space-separated names of values from the {@link DefaultStringLookup} enum. If the given
     * string is null or empty, an empty map is returned.
     *
    ...
    private static Map<String, StringLookup> parseStringLookups(final String str) {
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "Invalid default string lookups definition: " + str, exc);
      }
      return lookupMap;
    }
    ...
    DefaultStringLookupsHolder(final Properties props) {
      final Map<String, StringLookup> lookups =
          props.containsKey(StringLookupFactory.DEFAULT_STRING_LOOKUPS_PROPERTY)
              ? parseStringLookups(
                  props.getProperty(StringLookupFactory.DEFAULT_STRING_LOOKUPS_PROPERTY))
  ...
  /**
   * Name of the system property used to determine the string lookups added by the {@link
   * #addDefaultStringLookups(Map)} method. Use of this property is only required in cases where the
   * set of default lookups must be modified. (See the class documentation for details.)
   *
   * @since 1.10.0
   */
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "SGVsbG9Xb3JsZCE="} to {@code "HelloWorld!"}.
   */
  static final FunctionStringLookup<String> INSTANCE_BASE64_DECODER =
      FunctionStringLookup.on(
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "HelloWorld!"} to {@code "SGVsbG9Xb3JsZCE="}. Defines the
   ...
   * </pre>
   *
   * <p>The above examples convert (on Linux) {@code "USER"} to the current user name. On Windows
   * 10, you would use {@code "USERNAME"} to the same effect.
   */
  ...
  static final FunctionStringLookup<String> INSTANCE_SYSTEM_PROPERTIES =
      FunctionStringLookup.on(System::getProperty);
   ...
   * </pre>
   *
   * <p>The above examples convert {@code } to {@code "SGVsbG9Xb3JsZCE="}.
   *
  ...
  /**
   * Returns a new function-based lookup where the request for a lookup is answered by applying the
   * function with a lookup key.
   *
   ...
   * @since 1.9
   */
  public <R, U> BiStringLookup<U> biFunctionStringLookup(
      final BiFunction<String, U, R> biFunction) {
    return BiFunctionStringLookup.on(biFunction);
  }

  /**
   * Returns the ConstantStringLookup singleton instance to look up the value of a fully-qualified
   * static final value.
   *
   * <p>Sometimes it is necessary in a configuration file to refer to a constant defined in a class.
   * This can be done with this lookup implementation. Variable names must be in the format {@code
   * apackage.AClass.AFIELD}. The {@code lookup(String)} method will split the passed in string at
   * the last dot, separating the fully qualified class name and the name of the constant (i.e.
   * <b>static final</b>) member field. Then the class is loaded and the field's value is obtained
   * using reflection.
   *
   * <p>Once retrieved values are cached for fast access. This class is thread-safe. It can be used
   * as a standard (i.e. global) lookup object and serve multiple clients concurrently.
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The above examples convert {@code java.awt.event.KeyEvent.VK_ESCAPE} to {@code "27"}.
   *
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "yyyy-MM-dd"} to todays's date, for example, {@code
   * "2019-08-04"}.
   *
   ...
   *
   * <ul>
   *   <li><b>name</b>: for the local host name, for example {@code EXAMPLE} but also {@code
   *       EXAMPLE.apache.org}.
   ...
   * </pre>
   *
   * <p>When used through a {@link StringSubstitutor}, this lookup must either be added
   * programmatically (as below) or enabled as a default lookup using the {@value
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "address|apache.org"} to the IP address of {@code
   * apache.org}.
   *
  ...
  /**
   * Returns the EnvironmentVariableStringLookup singleton instance where the lookup key is an
   ...
   *
   * <p>If this factory was built using {@link Builder#setFences(Path...)}, then the string lookup
   * is fenced and will throw an {@link IllegalArgumentException} if a lookup causes causes a path
   * to resolve outside of these fences. Otherwise, the result is unfenced to preserved behavior
   * from previous versions. <em>Using a fenced StringLookup</em>
   *
   * <p>To use a fenced {@link StringLookup}, use {@link StringLookupFactory#builder()}:
   *
   * <pre>
   ...
   *
   * <p>To use an unfenced {@link StringLookup}, use {@link StringLookupFactory#INSTANCE}:
   *
   * <pre>
   ...
   *
   * <p>To build a fenced StringSubstitutor, use:
   *
   * <pre>
   * // Make the fence the current directory
   ...
   * </pre>
   *
   * <p>Using an unfenced {@link StringSubstitutor}:
   *
   * <pre>
   * StringSubstitutor.createInterpolator().replace("... ${file:UTF-8:com/domain/document.txt} ..."));
   * </pre>
   *
   * <p>The above examples convert {@code "UTF-8:com/domain/document.txt"} to the contents of the
   * file.
   *
   ...
   *
   * <p>To use a {@link StringLookup} fenced by the current directory, use:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The above example converts {@code "UTF-8:com/domain/document.txt"} to the contents of the
   * file.
   *
   * <p>{@link StringSubstitutor} methods like {@link StringSubstitutor#replace(String)} will throw
   * a {@link IllegalArgumentException} when a file doesn't resolves in a fence.
   *
  ...
  /**
   * Returns a {@link InterpolatorStringLookup} containing the configured {@link
   * #addDefaultStringLookups(Map) default lookups}. See the class documentation for details on how
   * these defaults are configured.
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "${sys:os.name}, ${env:USER}"} to the OS name and Linux
   * user name.
   *
  ...
  /**
   * Returns a new InterpolatorStringLookup. If {@code addDefaultLookups} is {@code true}, the
   * configured {@link #addDefaultStringLookups(Map) default lookups} are included in addition to
   * the ones provided in {@code stringLookupMap}. (See the class documentation for details on how
   * default lookups are configured.)
   *
   * @param stringLookupMap the map of string lookups.
   * @param defaultStringLookup the default string lookup; this lookup is used when a variable
   *     cannot be resolved using the lookups in {@code stringLookupMap} or the configured default
   *     lookups (if enabled)
   ...
   */
  public StringLookup interpolatorStringLookup(
      final Map<String, StringLookup> stringLookupMap,
  ...
  /**
   * Returns a new InterpolatorStringLookup using the given lookup and the configured {@link
   * #addDefaultStringLookups(Map) default lookups} to resolve variables. (See the class
   * documentation for details on how default lookups are configured.)
   *
  ...
  /**
   * Returns the JavaPlatformStringLookup singleton instance. Looks up keys related to Java: Java
   * version, JRE version, VM version, and so on.
   *
   * <p>The lookup keys with examples are:
   *
   * <ul>
   *   <li><b>version</b>: "Java version 1.8.0_181"
   *   <li><b>runtime</b>: "Java(TM) SE Runtime Environment (build 1.8.0_181-b13) from Oracle
   *       Corporation"
   *   <li><b>vm</b>: "Java HotSpot(TM) 64-Bit Server VM (build 25.181-b13, mixed mode)"
   *   <li><b>os</b>: "Windows 10 10.0, architecture: amd64-64"
   *   <li><b>hardware</b>: "processors: 4, architecture: amd64-64, instruction sets: amd64"
   *   <li><b>locale</b>: "default locale: en_US, platform encoding: iso-8859-1"
   * </ul>
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   *
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "version"} to the current VM version, for example, {@code
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "canonical-name"} to the current host name, for example,
  ...
  /**
   * Returns a new map-based lookup where the request for a lookup is answered with the value for
   * that key.
   *
   ...
   *
   * <p>We looks up a value for the key in the format "DocumentPath::MyKey".
   *
   * <p>Note the use of "::" instead of ":" to allow for "C:" drive letters in paths.
   *
   * <p>For example: "com/domain/document.properties::MyKey". <em>Using a fenced StringLookup</em>
   *
   * <p>To use a fenced {@link StringLookup}, use {@link StringLookupFactory#builder()}:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "com/domain/document.properties::MyKey"} to the key value
   * in the properties file at the path "com/domain/document.properties".
   *
   ...
   *
   * <p>Looks up the value for the key in the format "DocumentPath::MyKey":.
   *
   * <p>Note the use of "::" instead of ":" to allow for "C:" drive letters in paths.
   *
   * <p>For example: "com/domain/document.properties::MyKey".
   *
   * <p>To use a {@link StringLookup} fenced by the current directory, use:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The above example converts {@code "com/domain/document.properties::MyKey"} to the key value
   * in the properties file at the path "com/domain/document.properties".
   *
   * <p>{@link StringSubstitutor} methods like {@link StringSubstitutor#replace(String)} will throw
   * a {@link IllegalArgumentException} when a file doesn't resolves in a fence.
   ...
   *
   * <p>Looks up the value for a given key in the format "BundleName:BundleKey".
   *
   * <p>For example: "com.domain.messages:MyKey".
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "com.domain.messages:MyKey"} to the key value in the
   ...
   *
   * <p>Looks up the value for a given key in the format "MyKey".
   *
   * <p>For example: "MyKey".
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   ...
   * </pre>
   *
   * <p>The above example converts {@code "MyKey"} to the key value in the resource bundle at {@code
   * "com.domain.messages"}.
   *
  ...
  /**
   * Returns the ScriptStringLookup singleton instance. NOTE: This lookup is not included as a
   * {@link #addDefaultStringLookups(Map) default lookup} unless explicitly enabled. See the class
   * level documentation for details.
   *
   * <p>Looks up the value for the key in the format "ScriptEngineName:Script".
   *
   * <p>For example: "javascript:3 + 4".
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "javascript:3 + 4"} to {@code "7"}.
   *
  ...
  /**
   * Returns the SystemPropertyStringLookup singleton instance where the lookup key is a system
   * property name.
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   *
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "os.name"} to the operating system name.
   *
   ...
   *
   * <p>Decodes URL Strings using the UTF-8 encoding.
   *
   * <p>For example: "Hello%20World%21" becomes "Hello World!".
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "Hello%20World%21"} to {@code "Hello World!"}.
   *
   ...
   * <p>Decodes URL Strings using the UTF-8 encoding.
   *
   * <p>For example: "Hello World!" becomes "Hello+World%21".
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "Hello World!"} to {@code "Hello%20World%21"}.
   *
   * @return The UrlStringLookup singleton instance.
   * @since 1.6
   */
  ...
  /**
   * Returns the UrlStringLookup singleton instance. This lookup is not included as a {@link
   * #addDefaultStringLookups(Map) default lookup} unless explicitly enabled. See the class level
   * documentation for details.
   *
   * <p>Looks up the value for the key in the format "CharsetName:URL".
   *
   * <p>For example, using the HTTP scheme: "UTF-8:http://www.google.com"
   *
   * <p>For example, using the file scheme:
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "UTF-8:https://www.apache.org"} to the contents of that
   * page.
   *
   * @return The UrlStringLookup singleton instance.
   * @since 1.5
   ...
   *
   * <p>Decodes strings according to the XML 1.0 specification.
   *
   * <p>For example: "&amp;lt;element&amp;gt;" becomes "&lt;element&gt;".
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "&lt;element&gt;"} to {@code "<element>"}.
   *
   ...
   *
   * <p>Encodes strings according to the XML 1.0 specification.
   *
   * <p>For example: "&lt;element&gt;" becomes "&amp;lt;element&amp;gt;".
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory}:
   ...
   * from previous versions.
   *
   * <p>We look up the value for the key in the format "DocumentPath:XPath".
   *
   ...
   * </pre>
   *
   * <p>The above examples convert {@code "com/domain/document.xml:/path/to/node"} to the value of
   * the XPath in the XML document.
   *
   ...
   * <p>For example: "com/domain/document.xml:/path/to/node".
   *
   * <p>Using a {@link StringLookup} from the {@link StringLookupFactory} fenced by the current
   ...
   */
  public StringLookup xmlStringLookup(
      final Map<String, Boolean> xPathFactoryFeatures, final Path... fences) {
