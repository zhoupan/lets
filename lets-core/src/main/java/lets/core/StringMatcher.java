...
public interface StringMatcher {
   ...
   *
   * <p>This method is called to check for a match. The parameter {@code pos} represents the current
   * position to be checked in the string {@code buffer} (a character array which must not be
   * changed). The API guarantees that {@code pos} is a valid index for {@code buffer}.
   *
   * <p>The matching code may check one character or many. It may check characters preceding {@code
   * pos} as well as those after.
   *
   * <p>It must return zero for no match, or a positive number if a match was found. The number
   * indicates the number of characters that matched.
   *
   ...
   *
   * <p>This method is called to check for a match against a source {@code buffer}. The parameter
   * {@code start} represents the start position to be checked in the {@code buffer} (a character
   * array which MUST not be changed). The implementation SHOULD guarantees that {@code start} is a
   * valid index in {@code buffer}.
   *
   * <p>The character array may be larger than the active area to be matched. Only values in the
   * buffer between the specified indices may be accessed, in other words: {@code bufferStart <=
   * start < bufferEnd}.
   *
   * <p>The matching code may check one character or many. It may check characters preceding {@code
   * start} as well as those after, so long as no checks exceed the bounds specified.
   *
   * <p>It must return zero for no match, or a positive number if a match was found. The number
   * indicates the number of characters that matched.
   *
   ...
   */
  default int isMatch(
      final CharSequence buffer, final int start, final int bufferStart, final int bufferEnd) {
