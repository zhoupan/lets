 ...
 * </code>
 *
 * <p>This class can be used as a direct replacement for {@code SimpleDateFormat} in most parsing
 * situations. This class is especially useful in multi-threaded server environments. {@code
 * SimpleDateFormat} is not thread-safe in any JDK version, nor will it be as Sun has closed the <a
...
public class FastDateParser implements DateParser, Serializable {
   ...
   * Locale)} or another variation of the factory methods of {@link FastDateFormat} to get a cached
      ...
      while (currentIdx < pattern.length()) {
        ...
        if (!activeQuote && isFormatLetter(c)) {
          break;
            ...
            && (++currentIdx == pattern.length() || pattern.charAt(currentIdx) != '\'')) {
  ...
  /**
   * This implementation updates the ParsePosition if the parse succeeds. However, it sets the error
   * index to the position before the failed field unlike the method {@link
   * java.text.SimpleDateFormat#parse(String, ParsePosition)} which sets the error index to after
   * the failed field.
   *
   * <p>To determine if the parse has succeeded, the caller must check if the current parse position
   ...
   *
   * @see lets.core.apache.commons.lang3.time.DateParser#parse(java.lang.String,
   ...
   * Upon success, the ParsePosition index is updated to indicate how much of the source text was
   * consumed. Not all source text needs to be consumed. Upon parse failure, ParsePosition error
   * index is updated to the offset of the source text which does not match the supplied format.
   *
   * @param source The text to parse.
   * @param pos On input, the position in the source to start parsing, on output, updated position.
   * @param calendar The calendar into which to set parsed fields.
   * @return true, if source has been parsed (pos parsePosition is updated); otherwise false (and
   *     pos errorIndex is updated)
   * @throws IllegalArgumentException when Calendar has been set to be not lenient, and a parsed
  ...
  private static Map<String, Integer> appendDisplayNames(
      final Calendar cal, final Locale locale, final int field, final StringBuilder regex) {
    final Map<String, Integer> values = new HashMap<>();

    final Map<String, Integer> displayNames =
        cal.getDisplayNames(field, Calendar.ALL_STYLES, locale);
  ...
  private abstract static class Strategy {
    ...
    abstract boolean parse(
        FastDateParser parser, Calendar calendar, String source, ParsePosition pos, int maxWidth);
  }
  ...
  private static class ISO8601TimeZoneStrategy extends PatternStrategy {
     ...
     * @return a ISO8601TimeZoneStrategy that can format TimeZone String of length {@code tokenLen}.
  ...
  private static final Strategy DAY_OF_WEEK_STRATEGY =
      new NumberStrategy(Calendar.DAY_OF_WEEK) {
        @Override
        int modify(final FastDateParser parser, final int iValue) {
