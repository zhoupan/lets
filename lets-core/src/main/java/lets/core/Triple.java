...
public abstract class Triple<L, M, R> implements Comparable<Triple<L, M, R>>, Serializable {
   ...
   * An empty array.
   *
   * <p>Consider using {@link #emptyArray()} to avoid generics warnings.
   *
   * @since 3.10.
   */
   ...
   *
   * <p>This factory allows the triple to be created using inference to obtain the generic types.
   *
   * @param <L> the left element type
   * @param <M> the middle element type
   * @param <R> the right element type
  ...
  // -----------------------------------------------------------------------
  /**
   * Compares the triple based on the left element, followed by the middle element, finally the
   * right element. The types must be {@code Comparable}.
   *
   ...
   * used to embed the left and right elements. Use {@code %1$s} for the left element, {@code %2$s}
   * for the middle and {@code %3$s} for the right element. The default format used by {@code
   * toString()} is {@code (%1$s,%2$s,%3$s)}.
   *
