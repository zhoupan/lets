...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V>
  ...
  /**
   * Strategy class used to manage {@link Reference}s. This class can be overridden if alternative
  ...
  private static final class SoftEntryReference<K, V> extends SoftReference<Entry<K, V>>
    ...
    public SoftEntryReference(
        Entry<K, V> entry, int hash, Reference<K, V> next, ReferenceQueue<Entry<K, V>> queue) {
      super(entry, queue);
      this.hash = hash;
