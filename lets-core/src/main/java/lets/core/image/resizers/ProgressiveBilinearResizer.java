...
/**
 * A {@link Resizer} which performs resizing operations by using progressive bilinear scaling.
 *
 * <p>The resizing technique used in this class is based on the technique discussed in <em>Chapter
 * 4: Images</em> of <a href="http://filthyrichclients.org">Filthy Rich Clients</a> by Chet Haase
 * and Romain Guy.
 *
 * <p>The actual implemenation of the technique is independent of the code which is provided in the
 * book.
 *
 * @author coobird
 */
public class ProgressiveBilinearResizer extends AbstractResizer {
  /** Instantiates a {@link ProgressiveBilinearResizer} with default rendering hints. */
  public ProgressiveBilinearResizer() {
    this(Collections.<RenderingHints.Key, Object>emptyMap());
  }

  /**
   * Instantiates a {@link ProgressiveBilinearResizer} with the specified rendering hints.
   *
  ...
  public void resize(BufferedImage srcImage, BufferedImage destImage) throws NullPointerException {
    ...
    /*
     * Determine the size of the first resize step should be. 1) Beginning from the target size 2) Increase each dimension by 2 3) Until
    ...
    destg.drawImage(
        tempImage, 0, 0, targetWidth, targetHeight, 0, 0, currentWidth, currentHeight, null);
    destg.dispose();
  }
}
