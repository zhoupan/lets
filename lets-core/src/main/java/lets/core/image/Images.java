...
import lets.core.image.filters.Canvas;
...
import lets.core.image.geometry.AbsoluteSize;
import lets.core.image.geometry.Coordinate;
import lets.core.image.geometry.Position;
import lets.core.image.geometry.Positions;
import lets.core.image.geometry.Region;
import lets.core.image.geometry.Size;
import lets.core.image.io.BufferedImageSource;
import lets.core.image.io.BufferedImageTarget;
import lets.core.image.io.FileImageSource;
import lets.core.image.io.FileImageTarget;
import lets.core.image.io.ImageSource;
import lets.core.image.io.InputStreamImageSource;
import lets.core.image.io.OutputStreamImageTarget;
import lets.core.image.io.URLImageSource;
import lets.core.image.name.Rename;
import lets.core.image.resizers.BicubicResizer;
import lets.core.image.resizers.BilinearResizer;
import lets.core.image.resizers.DefaultResizerFactory;
import lets.core.image.resizers.FixedResizerFactory;
import lets.core.image.resizers.ProgressiveBilinearResizer;
import lets.core.image.resizers.Resizer;
import lets.core.image.resizers.ResizerFactory;
import lets.core.image.resizers.configurations.AlphaInterpolation;
import lets.core.image.resizers.configurations.Antialiasing;
import lets.core.image.resizers.configurations.Dithering;
import lets.core.image.resizers.configurations.Rendering;
import lets.core.image.resizers.configurations.ScalingMode;
import lets.core.image.tasks.ImageSourceTask;

/**
 * Provides a fluent interface to create thumbnails.
 *
 * <p>This is the main entry point for creating thumbnails with ImageCreator.
 *
 * <p>By using the ImageCreator's fluent interface, it is possible to write thumbnail generation
 * code which resembles written English.
 *
 * <DL>
 *   <DT><B>Usage:</B>
 *   <DD>The following example code demonstrates how to use the fluent interface to create a
 *       thumbnail from multiple files from a directory, resizing them to a maximum of 200 pixels by
 *       200 pixels while preserving the aspect ratio of the original, then saving the resulting
 *       thumbnails as JPEG images with file names having {@code thumbnail.} appended to the
 *       beginning of the file name.
 *       <p>
 *       <pre>
 * Images.of(directory.listFiles())
 * .size(200, 200)
 * .outputFormat("jpeg")
 ...
 * ImageCreator</a> project page.
 *
 * <p>
 *
 * <h2>Important Implementation Notes</h2>
 *
 * Upon calling one of the {@code Images.of(...)} methods, <em>in the current implementation</em>,
 * an instance of an inner class of this class is returned. In most cases, the returned instance
 * should not be used by storing it in a local variable, as changes in the internal implementation
 * could break code in the future.
 *
 * <p>As a rule of thumb, <em>always method chain from the {@code Images.of} all the way until the
 * output method (e.g. {@code toFile}, {@code asBufferedImage}, etc.) is called without breaking
 * them down into single statements.</em> See the "Usage" section above for the intended use of the
 * ImageCreator's fluent interface.
 *
 * <DL>
 *   <DT><B>Unintended Use:</B>
 *   <DD>
 *       <pre>
 * // Unintended use - not recommended!
 * Builder&lt;File&gt; instance = Images.of("path/to/image");
...
public final class Images {
   ...
   *
   * <p>If any of the dimensions are less than or equal to 0, an {@code IllegalArgumentException} is
   * thrown with an message specifying the reason for the exception.
   *
   * <p>This method is used to perform a check on the output dimensions of a thumbnail for the
   * {@link Images#createThumbnail} methods.
   *
  ...
  private static void validateDimensions(int width, int height) {
    if (width <= 0 && height <= 0) {
      ...
      throw new IllegalArgumentException(
          "Destination image " + dimension + " must not be " + "less than or equal to 0 pixels.");
    }
  }
   ...
   *
   * @param files File names of image files for which thumbnails are to be produced for.
   * @return Reference to a builder object which is used to specify the parameters for creating the
   *     thumbnail.
   ...
   *
   * @param files {@link File} objects of image files for which thumbnails are to be produced for.
   * @return Reference to a builder object which is used to specify the parameters for creating the
   *     thumbnail.
   * @throws NullPointerException If the argument is {@code null}.
   * @throws IllegalArgumentException If the argument is an empty array.
   ...
   *
   * @param urls {@link URL} objects of image files for which thumbnails are to be produced for.
   * @return Reference to a builder object which is used to specify the parameters for creating the
   *     thumbnail.
   * @throws NullPointerException If the argument is {@code null}.
   * @throws IllegalArgumentException If the argument is an empty array.
   ...
   *
   * @param inputStreams {@link InputStream}s which provide the images for which thumbnails are to
   *     be produced for.
   * @return Reference to a builder object which is used to specify the parameters for creating the
   *     thumbnail.
   * @throws NullPointerException If the argument is {@code null}.
   ...
   *
   * @param images {@link BufferedImage}s for which thumbnails are to be produced for.
   * @return Reference to a builder object which is used to specify the parameters for creating the
   *     thumbnail.
   * @throws NullPointerException If the argument is {@code null}.
   * @throws IllegalArgumentException If the argument is an empty array.
   ...
   *
   * @param urls URLs of the images for which thumbnails are to be produced.
   * @return Reference to a builder object which is used to specify the parameters for creating the
   *     thumbnail.
   * @throws NullPointerException If the argument is {@code null}.
   * @throws IllegalArgumentException If the argument is an empty collection.
  ...
  /**
   * Indicate to make thumbnails for images obtained from the specified {@link InputStream}s.
   *
   * @param inputStreams {@link InputStream}s which provide images for which thumbnails are to be
   *     produced.
   * @return Reference to a builder object which is used to specify the parameters for creating the
   *     thumbnail.
   * @throws NullPointerException If the argument is {@code null}.
  ...
  /**
   * The builder interface for ImageCreator to set up the thumbnail generation task.
   *
   * <p>ImageCreator is intended to be used by calling one of the {@code Images.of(...)} methods,
   * then chaining methods such as {@link #size(int, int)} and {@link #outputQuality(double)} to set
   * up the thumbnail generation parameters. (See "Intended Use" below.) The end result should be
   * code that resembles English.
   *
   * <p>In most cases, holding an instance of this class in a local variable, such as seen in the
   * "Unintended Use" example below, is more verbose and less future-proof, as changes to this class
   * (which is just an inner class of the {@link Images} class) can lead to broken code when
   * attempting to use future releases of ImageCreator.
   *
   * <p>
   *
   * <DL>
   ...
   * </DL>
   *
   * <p>An instance of this class provides the fluent interface in the form of method chaining.
   * Through the fluent interface, the parameters used for the thumbnail creation, such as {@link
   * #size(int, int)} and {@link #outputQuality(double)} can be set up. Finally, to execute the
   * thumbnail creation, one of the output methods whose names start with {@code to} (e.g. {@link
   * #toFiles(Rename)}) or {@code as} (e.g. {@link #asBufferedImages()}) is called.
   *
   * <p>An instance of this class is obtained by calling one of:
   *
   * <ul>
   *   <li>{@link Images#of(BufferedImage...)}
   *   <li>{@link Images#of(File...)}
   *   <li>{@link Images#of(String...)}
   *   <li>{@link Images#of(InputStream...)}
   *   <li>{@link Images#of(URL...)}
   *   <li>{@link Images#fromImages(Iterable)}
   *   <li>{@link Images#fromFiles(Iterable)}
   *   <li>{@link Images#fromFilenames(Iterable)}
   *   <li>{@link Images#fromInputStreams(Iterable)}
   *   <li>{@link Images#fromURLs(Iterable)}
   * </ul>
   *
   * @author coobird
   */
  public static class Builder<T> {
    ...
    private static Builder<InputStream> ofInputStreams(
        Iterable<? extends InputStream> inputStreams) {
      ...
      public Iterator<BufferedImage> iterator() {
        return new Iterator<BufferedImage>() {
          ...
          public BufferedImage next() {
            ...
            BufferedImageTarget destination = new BufferedImageTarget();

            try {
              ImageCreator.createImage(
                  new ImageSourceTask<T, BufferedImage>(makeParam(), source, destination));
            } catch (IOException e) {
              return null;
            }
    ...
    private void updateStatus(Properties property, Status newStatus) {
      ...
      /*
       * The `newStatus != Status.CANNOT_SET` condition will allow the status to be set to CANNOT_SET to be set multiple times.
       */
    ...
    /**
     * An constant used to indicate that the imageType has not been specified. When this constant is
     * encountered, one should use the {@link ImageParameter#DEFAULT_IMAGE_TYPE} as the value for
     * imageType.
     */
    ...
    /*
     * Defines the fields for the builder interface, and assigns the default values.
     */
    ...
    private String outputFormat = ImageParameter.DETERMINE_FORMAT;
    private String outputFormatType = ImageParameter.DEFAULT_FORMAT_TYPE;
    private float outputQuality = ImageParameter.DEFAULT_QUALITY;
    ...
    /**
     * This field should be set to the {@link Position} to be used for cropping if cropping is
     * enabled. If cropping is disabled, then this field should be left {@code null}.
     */
    private Position croppingPosition = null;

    /** The {@link ImageFilter}s that should be applied when creating the thumbnail. */
    private Pipeline filterPipeline = new Pipeline();

    /**
     * Sets the size of the thumbnail.
     *
     * <p>For example, to create thumbnails which should fit within a bounding rectangle of 640 x
     * 480, the following code can be used:
     *
     * <pre>
     * <code>
     * Images.of(image)
     ...
     * </pre>
     *
     * <p>In the above code, the thumbnail will preserve the aspect ratio of the original image. If
     * the thumbnail should be forced to the specified size, the {@link #forceSize(int, int)} method
     * can be used instead of this method.
     *
     * <p>Once this method is called, calling the {@link #scale(double)} method will result in an
     * {@link IllegalStateException}.
     *
     * <p>Calling this method multiple times will result in an {@link IllegalStateException} to be
     * thrown.
     *
     ...
     *
     * <p>The thumbnail will have the dimensions constrained by the specified width, and the aspect
     * ratio of the original image will be preserved by the thumbnail.
     *
     * <p>Once this method is called, calling the {@link #size(int, int)} or the {@link
     * #scale(double)} method will result in an {@link IllegalStateException}.
     *
     * <p>Calling this method multiple times will result in an {@link IllegalStateException} to be
     * thrown.
     *
     ...
     *
     * <p>The thumbnail will have the dimensions constrained by the specified height, and the aspect
     * ratio of the original image will be preserved by the thumbnail.
     *
     * <p>Once this method is called, calling the {@link #size(int, int)} or the {@link
     * #scale(double)} method will result in an {@link IllegalStateException}.
     ...
     * Sets the size of the thumbnail.
     *
     * <p>The thumbnails will be forced to the specified size, therefore, the aspect ratio of the
     * original image will not be preserved in the thumbnails. Calling this method will be
     * equivalent to calling the {@link #size(int, int)} method in conjunction with the {@link
     * #keepAspectRatio(boolean)} method with the value {@code false}.
     *
     * <p>Once this method is called, calling the {@link #scale(double)} method will result in an
     * {@link IllegalStateException}.
     *
     ...
     *
     * <p>For example, to create thumbnails which are 50% the size of the original, the following
     * code can be used:
     *
     * <pre>
     * <code>
     ...
     * </pre>
     *
     * <p>Once this method is called, calling the {@link #size(int, int)} method, or the {@link
     * #scale(double, double)} method, or the {@link #keepAspectRatio(boolean)} method will result
     * in an {@link IllegalStateException}.
     *
     * <p>Calling this method multiple times will result in an {@link IllegalStateException} to be
     * thrown.
     *
     * @param scale The scaling factor to use when creating a thumbnail.
     *     <p>The value must be a {@code double} which is greater than {@code 0.0}, and not {@link
     ...
     *
     * <p>If the scaling factor for the width and height are not equal, then the thumbnail will not
     * preserve the aspect ratio of the original image.
     *
     * <p>For example, to create thumbnails which are 50% the width of the original, while 75% the
     * height of the original, the following code can be used:
     *
     * <pre>
     * <code>
     * Images.of(image)
     ...
     *
     * <p>Once this method is called, calling the {@link #size(int, int)} method, or the {@link
     * #scale(double)} method, or the {@link #keepAspectRatio(boolean)} method will result in an
     * {@link IllegalStateException}.
     *
     * <p>Calling this method multiple times will result in an {@link IllegalStateException} to be
     * thrown.
     *
     * @param scaleWidth The scaling factor to use for the width when creating a thumbnail.
     *     <p>The value must be a {@code double} which is greater than {@code 0.0}, and not {@link
     *     Double#POSITIVE_INFINITY}.
     * @param scaleHeight The scaling factor to use for the height when creating a thumbnail.
     *     <p>The value must be a {@code double} which is greater than {@code 0.0}, and not {@link
     *     Double#POSITIVE_INFINITY}.
     * @return Reference to this object.
     * @since 0.3.10
     ...
     *     <p>
     * @return Reference to this object.
     * @throws NullPointerException If the source region object is {@code null}.
     * @since 0.3.4
     */
     ...
     * @return Reference to this object.
     * @throws NullPointerException If the position and/or size is {@code null}.
     * @since 0.3.4
     */
     ...
     * @return Reference to this object.
     * @throws IllegalArgumentException If the width and/or height is less than or equal to {@code
     *     0}.
     * @since 0.3.4
     */
     ...
     * thrown.
     *
     * @param region A rectangular region which specifies the source region to use when creating the
     *     thumbnail.
    ...
    /**
     * Crops the thumbnail to the size specified when calling the {@link #size(int, int)} method,
     * positioned by the given {@link Position} object.
     *
     * <p>Calling this method will guarantee that the size of the thumbnail will be exactly the
     * dimensions specified in the {@link #size(int, int)} method.
     *
     * <p>Internally, the resizing is performed in two steps. First, the thumbnail will be sized so
     * that one of the dimensions will be sized exactly to the dimension specified in the {@code
     * size} method, while allowing the other dimension to overhang the specified dimension. Then,
     * the thumbnail will be cropped to the dimensions specified in the {@code size} method,
     * positioned using the speficied {@link Position} object.
     *
     * <p>Once this method is called, calling the {@link #scale(double)} method will result in an
     * {@link IllegalStateException}.
     *
     ...
     * thrown.
     *
     * @param position The position to which the thumbnail should be cropped to. For example, if
     *     {@link Positions#CENTER} is specified, the resulting thumbnail will be made by cropping
    ...
    /**
     * Specifies whether or not to overwrite files which already exist if they have been specified
     * as destination files.
     *
     * <p>This method will change the output behavior of the following methods:
     *
     * <ul>
     *   <li>{@link #toFile(File)}
     *   <li>{@link #toFile(String)}
     *   <li>{@link #toFiles(Iterable)}
     *   <li>{@link #toFiles(Rename)}
     *   <li>{@link #asFiles(Iterable)}
     *   <li>{@link #asFiles(Rename)}
     * </ul>
     *
     * The behavior of methods which are not listed above will not be affected by calling this
     * method.
     *
     * <p>Calling this method multiple times will result in an {@link IllegalStateException} to be
     * thrown.
     *
     * @param allowOverwrite If {@code true} then existing files will be overwritten if specified as
     *     a destination. If {@code false}, then the existing files will not be altered. For
     *     specific behavior, please refer to the specific output methods listed above.
     * @since 0.3.7
     */
     ...
     *
     * <p>Calling this method to set this parameter is optional.
     *
     * <p>Calling this method multiple times will result in an {@link IllegalStateException} to be
     * thrown.
     *
     ...
     * thrown.
     *
     * <p>This method cannot be called in conjunction with the {@link
     * #resizerFactory(ResizerFactory)} method.
     *
    ...
    /**
     * Sets the {@link ResizerFactory} object to use to decide what kind of resizing operation is to
     * be used when creating the thumbnail.
     *
     * <p>Calling this method to set this parameter is optional.
     *
     * <p>Calling this method multiple times will result in an {@link IllegalStateException} to be
     * thrown.
     *
     * <p>This method cannot be called in conjunction with the {@link #resizer(Resizer)} method.
     *
    ...
    /**
     * Sets the alpha interpolation mode when performing the resizing operation to generate the
     * thumbnail.
     *
     * <p>Calling this method to set this parameter is optional.
     *
     ...
     * <p>Calling this method to set this parameter is optional.
     *
     * <p>Calling this method multiple times will result in an {@link IllegalStateException}.
     *
     * <p>This method cannot be called in conjunction with the {@link
     * #resizerFactory(ResizerFactory)} method.
     *
     ...
     *
     * <p>Calling this method without first calling the {@link #size(int, int)} method will result
     * in an {@link IllegalStateException} to be thrown.
     *
     * <p>If this method is not called when, by default the aspect ratio of the original image is
     * preserved for the thumbnail.
     *
     * <p>Calling this method after calling the {@link #scale(double)} method or the {@link
     * #scale(double, double)} method will result in a {@link IllegalStateException}.
     *
     * @param keep {@code true} if the thumbnail is to maintain the aspect ratio of the original
     *     image, {@code false} otherwise.
     * @return Reference to this object.
     * @throws IllegalStateException If
     *     <ol>
     *       <li>the {@link #size(int, int)} has not yet been called to specify the size of the
     *           thumbnail, or
     *       <li>the {@link #scale(double)} method has been called, or
     *       <li>the {@link #scale(double, double)} method has been called, or
     *       <li>the {@link #width(int)} and/or {@link #height(int)} has been called and not
     *           preserving the aspect ratio is desired.
     *     </ol>
     */
    public Builder<T> keepAspectRatio(boolean keep) {
      if (statusMap.get(Properties.SCALE) == Status.ALREADY_SET) {
        throw new IllegalStateException(
            "Cannot specify whether to "
    ...
    /**
     * Sets the output quality of the compression algorithm used to compress the thumbnail when it
     * is written to an external destination such as a file or output stream.
     *
     * <p>The value is a {@code float} between {@code 0.0f} and {@code 1.0f} where {@code 0.0f}
     * indicates the minimum quality and {@code 1.0f} indicates the maximum quality settings should
     * be used for by the compression codec.
     *
     * <p>Calling this method to set this parameter is optional.
     *
     * <p>Calling this method in conjunction with {@link #asBufferedImage()} or {@link
     * #asBufferedImages()} will not result in any changes to the final result.
     *
     * <p>Calling this method multiple times, or the {@link #outputQuality(double)} in conjunction
     * with this method will result in an {@link IllegalStateException} to be thrown.
     *
     * @param quality The compression quality to use when writing the thumbnail.
     * @return Reference to this object.
     * @throws IllegalArgumentException If the argument is less than {@code 0.0f} or is greater than
     *     {@code 1.0f}.
     */
    public Builder<T> outputQuality(float quality) {
      if (quality < 0.0f || quality > 1.0f) {
        throw new IllegalArgumentException(
            "The quality setting must be in the range 0.0f and " + "1.0f, inclusive.");
      }
     ...
     * is written to an external destination such as a file or output stream.
     *
     * <p>The value is a {@code double} between {@code 0.0d} and {@code 1.0d} where {@code 0.0d}
     * indicates the minimum quality and {@code 1.0d} indicates the maximum quality settings should
     * be used for by the compression codec.
     *
     * <p>This method is a convenience method for {@link #outputQuality(float)} where the {@code
     * double} argument type is accepted instead of a {@code float}.
     *
     * <p>Calling this method to set this parameter is optional.
     *
     * <p>Calling this method in conjunction with {@link #asBufferedImage()} or {@link
     * #asBufferedImages()} will not result in any changes to the final result.
     *
     * <p>Calling this method multiple times, or the {@link #outputQuality(float)} in conjunction
     * with this method will result in an {@link IllegalStateException} to be thrown.
     *
     * @param quality The compression quality to use when writing the thumbnail.
     * @return Reference to this object.
     * @throws IllegalArgumentException If the argument is less than {@code 0.0d} or is greater than
     *     {@code 1.0d}.
     */
    public Builder<T> outputQuality(double quality) {
      if (quality < 0.0d || quality > 1.0d) {
        throw new IllegalArgumentException(
            "The quality setting must be in the range 0.0d and " + "1.0d, inclusive.");
      }
     ...
     *
     * <p>For example, to set the output format to JPEG, the following code can be used:
     *
     * <pre>
     * <code>
     * Images.of(image)
     ...
     * </pre>
     *
     * <p>Currently, whether or not the compression format string is valid dependents on whether the
     * Java Image I/O API recognizes the string as a format that it supports for output. (Valid
     * format names can be obtained by calling the {@link ImageIO#getWriterFormatNames()} method.)
     *
     * <p>Calling this method to set this parameter is optional.
     *
     * <p>Calling this method in conjunction with {@link #asBufferedImage()} or {@link
     ...
     * thrown.
     *
     * @param format The compression format to use when writing the thumbnail.
     * @return Reference to this object.
     * @throws IllegalArgumentException If an unsupported format is specified.
     */
    public Builder<T> outputFormat(String format) {
      if (!ImageUtils.isSupportedOutputFormat(format)) {
    ...
    public Builder<T> useOriginalFormat() {
      updateStatus(Properties.OUTPUT_FORMAT, Status.ALREADY_SET);
      outputFormat = ImageParameter.ORIGINAL_FORMAT;
      return this;
    }
     ...
     * thrown.
     *
     * @param useExifOrientation {@code true} if the Exif metadata should be used to determine the
     *     orientation of the thumbnail, {@code false} otherwise.
     * @return Reference to this object.
     * @since 0.4.3
     */
    ...
    /**
     * Indicates that the output format should be determined from the available information when
     * writing the thumbnail image.
     *
     * <p>For example, calling this method will cause the output format to be determined from the
     * file extension if thumbnails are written to files.
     *
     * <p>Calling this method multiple times will result in an {@link IllegalStateException} to be
     * thrown.
     *
    ...
    public Builder<T> determineOutputFormat() {
      updateStatus(Properties.OUTPUT_FORMAT, Status.ALREADY_SET);
      outputFormat = ImageParameter.DETERMINE_FORMAT;
      return this;
    }

    private boolean isOutputFormatNotSet() {
      return outputFormat == null || ImageParameter.DETERMINE_FORMAT.equals(outputFormat);
    }

    /**
     * Sets the compression format type of the thumbnail to write.
     *
     * <p>If the default type for the compression codec should be used, a value of {@link
     * ImageParameter#DEFAULT_FORMAT_TYPE} should be used.
     *
     * <p>Calling this method to set this parameter is optional.
     *
     * <p>Calling this method multiple times will result in an {@link IllegalStateException} to be
     * thrown.
     *
     * <p>Furthermore, if this method is called, then calling the {@link #outputFormat} method is
     * disabled, in order to prevent cases where the output format type does not exist in the format
     * specified for the {@code outputFormat} method.
     *
     * @param formatType The compression format type
     * @return Reference to this object.
     * @throws IllegalArgumentException If an unsupported format type is specified for the current
     *     output format type. Or, if the output format has not been specified before this method
     *     was called.
     */
    public Builder<T> outputFormatType(String formatType) {
      /*
       * If the output format is the original format, and the format type is being specified, it's going to be likely that the specified type
       * will not be present in all the formats, so we'll disallow it. (e.g. setting type to "JPEG", and if the original formats were JPEG and
       * PNG, then we'd have a problem.
       */
      if (formatType != ImageParameter.DEFAULT_FORMAT_TYPE && isOutputFormatNotSet()) {
        throw new IllegalArgumentException(
            "Cannot set the format type if a specific output " + "format has not been specified.");
      }

      if (!ImageUtils.isSupportedOutputFormatType(outputFormat, formatType)) {
        throw new IllegalArgumentException(
      ...
      /*
       * If the output format type is set, then we'd better make the output format unchangeable, or else we'd risk having a type that is not
     ...
     *
     * <p>This method can be called multiple times to apply multiple watermarks.
     *
     * <p>If multiple watermarks are to be applied, the watermarks will be applied in the order that
     ...
     *
     * <p>This method is a convenience method for the {@link #watermark(Position, BufferedImage,
     * float)} method, where the opacity is 50%, and the position is set to center of the thumbnail:
     *
     * <p>
     *
     * <pre>
     ...
     *
     * <p>This method is a convenience method for the {@link #watermark(Position, BufferedImage,
     * float)} method, where the opacity is 50%:
     *
     * <p>
     *
     * <pre>
     ...
     *
     * <p>The thumbnail will be rotated clockwise by the angle specified.
     *
     * <p>This method can be called multiple times to apply multiple rotations.
     *
     * <p>If multiple rotations are to be applied, the rotations will be applied in the order that
     * this method is called.
     *
     * <p>Calling this method to set this parameter is optional.
     *
     ...
     *
     * <p>This method can be called multiple times to apply multiple filters.
     *
     * <p>If multiple filters are to be applied, the filters will be applied in the order that this
     * method is called.
     *
     * <p>Calling this method to set this parameter is optional.
     *
     ...
     *
     * @throws IllegalStateException If the builder is not ready to create thumbnails, due to some
    ...
    /**
     * Returns a {@link Resizer} which is suitable for the current builder state.
     *
     * @param mode The scaling mode to use to create thumbnails.
     * @return The {@link Resizer} which is suitable for the specified scaling mode and builder
     *     state.
     */
    ...
    private void prepareResizerFactory() {
      /*
       * If the scalingMode has been set, then use scalingMode to obtain a resizer, else, use the resizer field.
       */
    ...
    /**
     * Returns a {@link ImageParameter} from the current builder state.
     *
     * @return A {@link ImageParameter} from the current builder state.
     */
    private ImageParameter makeParam() {
      prepareResizerFactory();

      int imageTypeToUse = imageType;
      if (imageType == IMAGE_TYPE_UNSPECIFIED) {
        imageTypeToUse = ImageParameter.ORIGINAL_IMAGE_TYPE;
      }

      /*
       * croppingPosition being non-null means that a crop should take place.
       */
    ...
    /**
     * Create the thumbnails and return as a {@link Iterable} of {@link BufferedImage}s.
     *
     * <p>For situations where multiple thumbnails are being generated, this method is preferred
     * over the {@link #asBufferedImages()} method, as (1) the processing does not have to complete
     * before the method returns and (2) the thumbnails can be retrieved one at a time, potentially
     * reducing the number of thumbnails which need to be retained in the heap memory, potentially
     * reducing the chance of {@link OutOfMemoryError}s from occurring.
     *
     * <p>If an {@link IOException} occurs during the processing of the thumbnail, the {@link
     * Iterable} will return a {@code null} for that element.
     *
     * @return An {@link Iterable} which will provide an {@link Iterator} which returns thumbnails
     *     as {@link BufferedImage}s.
     */
    public Iterable<BufferedImage> iterableBufferedImages() {
      checkReadiness();
      /*
       * TODO To get the precise error information, there would have to be an event notification mechanism.
       */
      return new BufferedImageIterable();
    }

    /**
     * Create the thumbnails and return as a {@link List} of {@link BufferedImage}s.
     *
     * <p>
     *
     * <h3>Note about performance</h3>
     *
     * If there are many thumbnails generated at once, it is possible that the Java virtual
     * machine's heap space will run out and an {@link OutOfMemoryError} could result.
     *
     * <p>If many thumbnails are being processed at once, then using the {@link
     * #iterableBufferedImages()} method would be preferable.
     *
     * @return A list of thumbnails.
     * @throws IOException If an problem occurred during the reading of the original images.
     */
    public List<BufferedImage> asBufferedImages() throws IOException {
      ...
      for (ImageSource<T> source : sources) {
        ...
        thumbnails.add(destination.getTarget());
      }
     ...
     *
     * <p>To call this method, the thumbnail must have been created from a single source.
     *
     ...
     * @throws IllegalArgumentException If multiple original images are specified.
     */
    ...
    /**
     * Creates the thumbnails and stores them to the files, and returns a {@link List} of {@link
     * File}s to the thumbnails.
     *
     * <p>When the destination file exists, and overwriting files has been disabled by calling the
     * {@link #allowOverwrite(boolean)} method with {@code false}, then the thumbnail with the
     * destination file already existing will not be written and the corresponding {@code File}
     * object will not be included in the {@code List} returned by this method.
     *
     * <p>The file names for the thumbnails are obtained from the given {@link Iterable}.
     *
     * @param iterable An {@link Iterable} which returns an {@link Iterator} which returns file
     *     names which should be assigned to each thumbnail.
     * @return A list of {@link File}s of the thumbnails which were created.
     * @throws IOException If a problem occurs while reading the original images or writing the
     *     thumbnails to files.
     * @since 0.3.7
     */
    public List<File> asFiles(Iterable<File> iterable) throws IOException {
      ...
      for (ImageSource<T> source : sources) {
        ...
        ImageParameter param = makeParam();

        FileImageTarget destination = new FileImageTarget(filenameIter.next(), allowOverwrite);

        try {
          ImageCreator.createImage(new ImageSourceTask<T, File>(param, source, destination));

          destinationFiles.add(destination.getTarget());
        } catch (IllegalArgumentException e) {
          /*
           * Handle the IllegalArgumentException which is thrown when the destination file already exists by not adding the current file to the
     ...
     * <p>When the destination file exists, and overwriting files has been disabled by calling the
     * {@link #allowOverwrite(boolean)} method with {@code false}, then the thumbnail with the
     * destination file already existing will not be written.
     *
     * <p>The file names for the thumbnails are obtained from the given {@link Iterable}.
     *
     * @param iterable An {@link Iterable} which returns an {@link Iterator} which returns file
    ...
    /**
     * Creates thumbnails and stores them to files using the {@link Rename} function to determine
     * the filenames. The thubnail files are returned as a {@link List}.
     *
     * <p>When the destination file exists, and overwriting files has been disabled by calling the
     * {@link #allowOverwrite(boolean)} method with {@code false}, then the thumbnail with the
     * destination file already existing will not be written and the corresponding {@code File}
     * object will not be included in the {@code List} returned by this method.
     *
     * <p>To call this method, the thumbnails must have been creates from files by calling the
     * {@link Images#of(File...)} method.
     *
     * @param rename The rename function which is used to determine the filenames of the thumbnail
     *     files to write.
     * @return A list of {@link File}s of the thumbnails which were created.
     * @throws IOException If a problem occurs while reading the original images or writing the
     *     thumbnails to files.
     * @throws IllegalStateException If the original images are not from files.
     * @since 0.3.7
     */
    ...
    /**
     * Creates thumbnails and stores them to files in the directory specified by the given {@link
     * File} object, and using the {@link Rename} function to determine the filenames. The thubnail
     ...
     * object will not be included in the {@code List} returned by this method.
     *
     * <p>Extra caution should be taken when using this method, as there are no protections in place
     * to prevent file name collisions resulting from creating thumbnails from files in separate
     * directories but having the same name. In such a case, the behavior will be depend on the
     * behavior of the {@link #allowOverwrite(boolean)} as described in the previous paragraph.
     *
     * <p>To call this method, the thumbnails must have been creates from files by calling the
     * {@link Images#of(File...)} method.
     *
     * @param destinationDir The destination directory to which the thumbnails should be written to.
     * @param rename The rename function which is used to determine the filenames of the thumbnail
     *     files to write.
     * @return A list of {@link File}s of the thumbnails which were created.
     * @throws IOException If a problem occurs while reading the original images or writing the
     *     thumbnails to files.
     * @throws IllegalStateException If the original images are not from files.
     * @throws IllegalArgumentException If the destination directory is not a directory.
     * @since 0.4.7
     */
    public List<File> asFiles(File destinationDir, Rename rename) throws IOException {
      ...
      for (ImageSource<T> source : sources) {
        ...
        FileImageTarget destination = new FileImageTarget(destinationFile, allowOverwrite);
     ...
     *     files to write.
     * @throws IOException If a problem occurs while reading the original images or writing the
     *     thumbnails to files. thumbnails to files.
     * @throws IllegalStateException If the original images are not from files.
     * @since 0.3.7
     */
    ...
    /**
     * Creates thumbnails and stores them to files in the directory specified by the given {@link
     * File} object, and using the {@link Rename} function to determine the filenames.
     *
     * <p>When the destination file exists, and overwriting files has been disabled by calling the
     * {@link #allowOverwrite(boolean)} method with {@code false}, then the thumbnail with the
     * destination file already existing will not be written.
     ...
     *
     * <p>When the destination file exists, and overwriting files has been disabled by calling the
     * {@link #allowOverwrite(boolean)} method with {@code false}, then an {@link
     * IllegalArgumentException} will be thrown.
     *
     * <p>To call this method, the thumbnail must have been created from a single source.
     *
     * @param outFile The file to which the thumbnail is to be written to.
     * @throws IOException If a problem occurs while reading the original images or writing the
     *     thumbnails to files.
     * @throws IllegalArgumentException If multiple original image files are specified, or if the
     *     destination file exists, and overwriting files is disabled.
     */
    public void toFile(File outFile) throws IOException {
      ...
      FileImageTarget destination = new FileImageTarget(outFile, allowOverwrite);

      ImageCreator.createImage(new ImageSourceTask<T, File>(makeParam(), source, destination));
    }
     ...
     * <p>To call this method, the thumbnail must have been created from a single source.
     *
     * @param outFilepath The file to which the thumbnail is to be written to.
     * @throws IOException If a problem occurs while reading the original images or writing the
     *     thumbnails to files.
     * @throws IllegalArgumentException If multiple original image files are specified, or if the
     *     destination file exists, and overwriting files is disabled.
    ...
    public void toFile(String outFilepath) throws IOException {
      ...
      FileImageTarget destination = new FileImageTarget(outFilepath, allowOverwrite);
     ...
     * <p>To call this method, the thumbnail must have been created from a single source.
     *
     * @param os The output stream to which the thumbnail is to be written to.
     * @throws IOException If a problem occurs while reading the original images or writing the
     *     thumbnails.
     * @throws IllegalArgumentException If multiple original image files are specified.
     * @throws IllegalStateException If the output format has not been specified through the {@link
    ...
    public void toOutputStream(OutputStream os) throws IOException {
      ...
      if (iter.hasNext()) {
        throw new IllegalArgumentException(
            "Cannot output multiple thumbnails to a single OutputStream.");
      }

      /*
       * if the image is from a BufferedImage, then we require that the output format be set. (or else, we can't tell what format to output
       * as!)
       */
      ...
      OutputStreamImageTarget destination = new OutputStreamImageTarget(os);

      ImageCreator.createImage(
          new ImageSourceTask<T, OutputStream>(makeParam(), source, destination));
    }

    /**
     * Creates the thumbnails and writes them to {@link OutputStream}s provided by the {@link
     * Iterable}.
     *
     * @param iterable An {@link Iterable} which returns an {@link Iterator} which returns the
     *     output stream which should be assigned to each thumbnail.
     * @throws IOException If a problem occurs while reading the original images or writing the
     *     thumbnails.
     * @throws IllegalStateException If the output format has not been specified through the {@link
     *     #outputFormat(String)} method.
    ...
    public void toOutputStreams(Iterable<? extends OutputStream> iterable) throws IOException {
      ...
      for (ImageSource<T> source : sources) {
        ...
        OutputStreamImageTarget destination = new OutputStreamImageTarget(osIter.next());
