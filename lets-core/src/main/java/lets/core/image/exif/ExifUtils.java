...
public final class ExifUtils {
   ...
   * @throws IllegalArgumentException If the {@link ImageReader} does not have the target image set,
   *     or if the reader does not have a JPEG open.
   */
  ...
  private static Orientation getOrientationFromExif(byte[] exifData) {
    ...
    /*
     * The first 2 bytes of the TIFF header contains either: "II" for Intel byte alignment (little endian), or "MM" for Motorola byte
     * alignment (big endian)
     */
    ByteOrder bo;
  ...
  private static IfdStructure readIFD(byte[] ifd, ByteOrder bo) {
    ...
    /*
     * Per section 4.6.2 of the Exif Spec, if value is smaller than 4 bytes, it will exist in the earlier byte.
     */
    ...
    if (byteSize <= 4) {
      ...
      } else if (ifdType == IfdType.BYTE
          || ifdType == IfdType.ASCII
