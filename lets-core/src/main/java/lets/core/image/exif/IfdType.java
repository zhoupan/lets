...
/**
 * This enum corresponds to the types of data present in an IFD, as defined in Section 4.6.2 of the
 * Exif Specification version 2.3.
 *
 * @author coobird
 */
public enum IfdType {
  ...
  /**
   * Two {@link #LONG} values, where the first {@code LONG} is the numerator, while the second
   * {@code LONG} is the denominator.
   */
  ...
  /**
   * Two {@link #SLONG} values, where the first {@code SLONG} is the numerator, while the second
   * {@code SLONG} is the denominator.
   */
   ...
   * @return {@link IfdType} corresponding to the IDF type value. Return {@code null} if the given
   *     value does not correspond to a valid {@link IfdType}.
   */
