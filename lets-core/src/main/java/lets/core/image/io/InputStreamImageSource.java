...
import lets.core.image.exif.ExifFilterUtils;
import lets.core.image.exif.ExifUtils;
import lets.core.image.exif.Orientation;
...
/**
 * An {@link ImageSource} which uses an {@link InputStream} to read the source image.
 *
 * @author coobird
 */
public class InputStreamImageSource extends AbstractImageSource<InputStream> {
  ...
  /**
   * Instantiates an {@link InputStreamImageSource} with the {@link InputStream} which will be used
   * to read the source image.
   *
   * @param is The {@link InputStream} which is to be used to obtain the source image.
  ...
  public BufferedImage read() throws IOException {
    ...
    if (!readers.hasNext()) {
      throw new UnsupportedFormatException(
          UnsupportedFormatException.UNKNOWN, "No suitable ImageReader found for source data.");
    }
     ...
     *
     * Issue: https://code.google.com/p/thumbnailator/issues/detail?id=69
     */
    if (param != null
      ...
      if (param.getSize() != null
          && (param.getSize().width * 2 < width && param.getSize().height * 2 < height)) {
      ...
      if (param.getSize() == null) {
        try {
          ...
          heightField.set(param, param.getHeightScalingFactor() * subsampling);
          widthField.set(param, param.getWidthScalingFactor() * subsampling);
        } catch (Exception e) {
     ...
     *
     * This seems to be one of the culprits which was causing `OutOfMemoryError`s which began appearing frequently with Java 7 Update 21.
     *
