...
public final class FlipTableConverters {
  ...
  public static String fromTableModel(TableModel tableModel) {
    if (tableModel == null) throw new NullPointerException("tableModel == null");
    List<String> headers = new ArrayList<>();
    ...
    for (int rowIndex = 0; rowIndex < tableModel.getRowCount(); rowIndex++) {
      ...
      for (int column = 0; column < columnCount; column++) {
        try {
          rowData[column] = String.valueOf(tableModel.getValueAt(rowIndex, column));
        } catch (Exception e) {
          throw new RuntimeException(e);
        }
      }
