 ...
 * versions).
 *
 * <p>In addition, the pattern {@code 'ZZ'} has been made to represent ISO 8601 extended format time
...
public class FastDatePrinter implements DatePrinter, Serializable {
   ...
   *
   * @deprecated Use {{@link #format(Date)}, {{@link #format(Calendar)}, {{@link #format(long)}, or
   *     {{@link #format(Object)}
   ...
   * @param value the value to append digits from.
   */
  private static void appendFullDigits(final Appendable buffer, int value, int minFieldWidth)
      throws IOException {
    // specialized paths for 1 to 4 digits -> avoid the memory allocation from the temporary work
    // array
    // see LANG-1248
    if (value < 10000) {
  ...
  private static class TimeZoneDisplayKey {
     ...
     */
    TimeZoneDisplayKey(
        final TimeZone timeZone, final boolean daylight, final int style, final Locale locale) {
      mTimeZone = timeZone;
      if (daylight) {
