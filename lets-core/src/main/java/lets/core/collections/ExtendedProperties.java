...
/**
 * This class extends normal Java properties by adding the possibility to use the same key many
 * times concatenating the value strings instead of overwriting them.
 *
 * <p><b>Please consider using the <code>PropertiesConfiguration</code> class in
 ...
 *   <li><i>value</i> may be separated on different lines if a backslash is placed at the end of the
 *       line that continues below.
 *   <li>If <i>value</i> is a list of strings, each token is separated by a comma ','.
 *   <li>Commas in each token are escaped placing a backslash right before the comma.
 *   <li>Backslashes are escaped by using two consecutive backslashes i.e. \\
 *   <li>If a <i>key</i> is used more than once, the values are appended as if they were on the same
 *       line separated with commas.
 *   <li>Blank lines and lines starting with character '#' are skipped.
 *   <li>If a property is named "include" (or whatever is defined by setInclude() and getInclude()
 *       and the value of that property is the full path to a file on disk, that file will be
 *       included into the ConfigurationsRepository. You can also pull in files relative to the
 *       parent configuration file. So if you have something like the following:
 *       <p>include = additional.properties
 *       <p>Then "additional.properties" is expected to be in the same directory as the parent
 *       configuration file.
 *       <p>Duplicate name values will be replaced, so be careful.
 * </ul>
 *
 ...
 * </pre>
 *
 * <p><b>NOTE</b>: this class has <b>not</b> been written for performance nor low memory usage. In
 * fact, it's way slower than it could be and generates too much memory garbage. But since
 * performance is not an issue during intialization (and there is not much time to improve it), I
 * wrote it this way. If you don't like it, go ahead and tune it up!
 *
...
public class ExtendedProperties extends Hashtable {
  ...
  /**
   * This is the name of the property that can point to other properties file for including other
   * properties files.
   */
  protected static String include = "include";

  /**
   * These are the keys in the order they listed in the configuration file. This is useful when you
   * wish to perform operations with configuration information in a particular order.
   */
   ...
   *
   * <p>When called the first time, priorVariables should be null.
   *
   * @param base string with the ${key} variables
   * @param priorVariables serves two purposes: to allow checking for loops, and creating a
   *     meaningful exception message should a loop occur. It's 0'th element will be set to the
   *     value of base from the first call. All subsequent interpolated variables are added
   *     afterward.
  ...
  /**
   * Counts the number of successive times 'ch' appears in the 'line' before the position indicated
   * by the 'index'.
   */
  ...
  /**
   * This class is used to read properties lines. These lines do not terminate with new-line chars
   * but rather when there is no backslash sign a the end of the line. This is used to concatenate
  ...
  /**
   * This class divides into tokens a property value. Token separator is "," but commas into the
   * property value are escaped using the backslash in front.
   */
   ...
   * @return value as object. Will return user value if exists, if not then default value if exists,
   *     otherwise null
   */
   ...
   * added to the configuration entry. For example, if <code>resource.loader = file</code> is
   * already present in the configuration and you <code>addProperty("resource.loader", "classpath")
   * </code> Then you will end up with a Vector like the following: <code>["file", "classpath"]
   * </code>
   *
  ...
  /**
   * Adds a key/value pair to the map. This routine does no magic morphing. It ensures the keylist
   * is maintained
   *
  ...
  /**
   * Adds a decoded property to the map w/o checking for commas - used internally when a property
   * has been broken up into strings that could contain escaped commas to prevent the inadvertent
   * vectorization.
   *
   * <p>Thanks to Leon Messerschmidt for this one.
   *
   * @param key the key to store at
   * @param value the decoded object to store
   */
   ...
   *
   * <p>The stream is not closed, but it is flushed.
   *
   ...
   *
   * <p>Warning: It will overwrite previous entries without warning.
   *
  ...
  /**
   * Get the list of the keys contained in the configuration repository that match the specified
   * prefix.
   *
  ...
  /**
   * Create an ExtendedProperties object that is a subset of this one. Take into account duplicate
   * keys by using the setProperty() in ExtendedProperties.
   *
   ...
   * Get a Vector of strings associated with the given configuration key.
   *
   * <p>The list is a copy of the internal data of this object, and as such you may alter it freely.
   *
   * @param key The configuration key.
   * @param defaultValue The default value.
   * @return The associated Vector.
   ...
   * @throws NoSuchElementException is thrown if the key doesn't map to an existing object.
  ...
  /**
   * Test whether the string represent by value maps to a boolean value or not. We will allow <code>
   * true</code>, <code>on</code>, and <code>yes</code> for a <code>true</code> boolean value, and
   * <code>false</code>, <code>off</code>, and <code>no</code> for <code>false</code> boolean
   * values. Case of value to test for boolean status is ignored.
   *
   * @param value the value to test for boolean state
   * @return <code>true</code> or <code>false</code> if the supplied text maps to a boolean value,
   ...
   * @throws NoSuchElementException is thrown if the key doesn't map to an existing object.
   * @throws ClassCastException is thrown if the key maps to an object that is not a Byte.
   * @throws NumberFormatException is thrown if the value mapped by the key has not a valid number
   *     format.
   */
   ...
   * @throws NoSuchElementException is thrown if the key doesn't map to an existing object.
   * @throws ClassCastException is thrown if the key maps to an object that is not a Short.
   * @throws NumberFormatException is thrown if the value mapped by the key has not a valid number
   *     format.
   */
  ...
  /**
   * The purpose of this method is to get the configuration resource with the given name as an
   * integer.
   *
   ...
   * @throws NoSuchElementException is thrown if the key doesn't map to an existing object.
   * @throws ClassCastException is thrown if the key maps to an object that is not a Integer.
   * @throws NumberFormatException is thrown if the value mapped by the key has not a valid number
   *     format.
   */
   ...
   * @throws NoSuchElementException is thrown if the key doesn't map to an existing object.
   * @throws ClassCastException is thrown if the key maps to an object that is not a Long.
   * @throws NumberFormatException is thrown if the value mapped by the key has not a valid number
   *     format.
   */
   ...
   * @throws NoSuchElementException is thrown if the key doesn't map to an existing object.
   * @throws ClassCastException is thrown if the key maps to an object that is not a Float.
   * @throws NumberFormatException is thrown if the value mapped by the key has not a valid number
   *     format.
   */
   ...
   * @throws NoSuchElementException is thrown if the key doesn't map to an existing object.
   * @throws ClassCastException is thrown if the key maps to an object that is not a Double.
   * @throws NumberFormatException is thrown if the value mapped by the key has not a valid number
   *     format.
   */
   ...
   *
   * <p>NOTE: From Commons Collections 3.2 this method will pick up any default parent Properties of
   * the specified input object.
   *
