...
/**
 * This <code>Map</code> wraps another <code>Map</code> implementation, using the wrapped instance
 * for its default implementation. This class is used as a framework on which to build to extensions
 * for its wrapped <code>Map</code> object which would be unavailable or inconvenient via
 * sub-classing (but usable via composition).
 *
...
public abstract class ProxyMap implements Map {
   ...
   *
   * <p>Note that the map is used for delegation, and is not copied. This is different to the normal
   * use of a <code>Map</code> parameter in collections constructors.
   *
