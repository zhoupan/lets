...
/**
 * Red-Black tree-based implementation of Map. This class guarantees that the map will be in both
 * ascending key order and ascending value order, sorted according to the natural order for the
 * key's and value's classes.
 *
 * <p>This Map is intended for applications that need to be able to look up a key-value pairing by
 * either key or value, and need to do so with equal efficiency.
 *
 * <p>While that goal could be accomplished by taking a pair of TreeMaps and redirecting requests to
 * the appropriate TreeMap (e.g., containsKey would be directed to the TreeMap that maps values to
 * keys, containsValue would be directed to the TreeMap that maps keys to values), there are
 * problems with that implementation, particularly when trying to keep the two TreeMaps synchronized
 * with each other. And if the data contained in the TreeMaps is large, the cost of redundant
 * storage becomes significant. (See also the new {@link
 * lets.core.collections.bidimap.DualTreeBidiMap DualTreeBidiMap} and {@link
 * lets.core.collections.bidimap.DualHashBidiMap DualHashBidiMap} implementations.)
 *
 * <p>This solution keeps the data properly synchronized and minimizes the data storage. The
 * red-black algorithm is based on TreeMap's, but has been modified to simultaneously map a tree
 * node by key and by value. This doubles the cost of put operations (but so does using two
 * TreeMaps), and nearly doubles the cost of remove operations (there is a savings in that the
 * lookup of the node to be removed only has to be performed once). And since only one node contains
 * the key and value, storage is significantly less than that required by two TreeMaps.
 *
 * <p>There are some limitations placed on data kept in this Map. The biggest one is this:
 *
 * <p>When performing a put operation, neither the key nor the value may already exist in the Map.
 * In the java.util Map implementations (HashMap, TreeMap), you can perform a put with an already
 * mapped key, and neither cares about duplicate values at all ... but this implementation's put
 * method with throw an IllegalArgumentException if either the key or the value is already in the
 * Map.
 *
 * <p>Obviously, that same restriction (and consequence of failing to heed that restriction) applies
 * to the putAll method.
 *
 * <p>The Map.Entry instances returned by the appropriate methods will not allow setValue() and will
 * throw an UnsupportedOperationException on attempts to call that method.
 *
 * <p>New methods are added to take advantage of the fact that values are kept sorted independently
 * of their keys:
 *
 * <p>Object getKeyForValue(Object value) is the opposite of get; it takes a value and returns its
 * key, if any.
 *
 * <p>Object removeValue(Object value) finds and removes the specified value and returns the now
 * un-used key.
 *
 * <p>Set entrySetByValue() returns the Map.Entry's in a Set whose iterator will iterate over the
 * Map.Entry's in ascending order by their corresponding values.
 *
 * <p>Set keySetByValue() returns the keys in a Set whose iterator will iterate over the keys in
 * ascending order by their corresponding values.
 *
 * <p>Collection valuesByValue() returns the values in a Collection whose iterator will iterate over
 * the values in ascending order.
 *
 * @deprecated Replaced by TreeBidiMap in bidimap subpackage. Due to be removed in v4.0.
 * @see BidiMap
 * @see lets.core.collections.bidimap.DualTreeBidiMap
 * @see lets.core.collections.bidimap.DualHashBidiMap
...
public final class DoubleOrderedMap extends AbstractMap {
   ...
   * @throws ClassCastException if the keys in the map are not Comparable, or are not mutually
   *     comparable; also if the values in the map are not Comparable, or are not mutually
   *     Comparable
   * @throws NullPointerException if any key or value in the map is null
   * @throws IllegalArgumentException if there are duplicate keys or duplicate values in the map
   */
  ...
  /**
   * Returns the key to which this map maps the specified value. Returns null if the map contains no
   * mapping for this value.
   *
   * @param value value whose associated key is to be returned.
   * @return the key to which this map maps the specified value, or null if the map contains no
   *     mapping for this value.
   ...
   * @return previous key associated with specified value, or null if there was no mapping for
   *     value.
   */
   ...
   * Map.Entry. The set is backed by the map, so changes to the map are reflected in the set, and
   * vice-versa. If the map is modified while an iteration over the set is in progress, the results
   * of the iteration are undefined. The set supports element removal, which removes the
   * corresponding mapping from the map, via the Iterator.remove, Set.remove, removeAll, retainAll
   * and clear operations. It does not support the add or addAll operations.
   *
   * <p>The difference between this method and entrySet is that entrySet's iterator() method returns
   * an iterator that iterates over the mappings in ascending order by key. This method's iterator
   * method iterates over the mappings in ascending order by value.
   *
   ...
   * over the set is in progress, the results of the iteration are undefined. The set supports
   ...
   * operations.
   *
   * <p>The difference between this method and keySet is that keySet's iterator() method returns an
   * iterator that iterates over the keys in ascending order by key. This method's iterator method
   ...
   * are undefined. The collection supports element removal, which removes the corresponding mapping
   * from the map, via the Iterator.remove, Collection.remove, removeAll, retainAll and clear
   * operations. It does not support the add or addAll operations.
   *
   * <p>The difference between this method and values is that values's iterator() method returns an
   * iterator that iterates over the values in ascending order by key. This method's iterator method
   ...
   * @return the key, if remove by value, or the value, if remove by key. null if the specified key
   *     or value could not be found
   */
   ...
   * @param o the key or value that we're looking for
   * @param index KEY or VALUE
   * @return the key (if the value was mapped) or the value (if the key was mapped); null if we
   *     couldn't find the specified object
   */
  ...
  /**
   * copy the color from one node to another, dealing with the fact that one or both nodes may, in
   * fact, be null
   *
  ...
  /**
   * get a node's grandparent. mind you, the node, its parent, or its grandparent may not exist. no
   * problem
   *
   * @param node the node (may be null) in question
   * @param index KEY or VALUE
  ...
  /**
   * is this node its parent's left child? mind you, the node, or its parent, may not exist. no
   * problem. if the node doesn't exist ... it's its non-existent parent's left child. If the node
   * does exist but has no parent ... no, we're not the non-existent parent's left child. Otherwise
  ...
  /**
   * is this node its parent's right child? mind you, the node, or its parent, may not exist. no
   * problem. if the node doesn't exist ... it's its non-existent parent's right child. If the node
   * does exist but has no parent ... no, we're not the non-existent parent's right child. Otherwise
   * (both the specified node AND its parent exist), check.
   *
   * @param node the node (may be null) in question
   * @param index KEY or VALUE
  ...
  /**
   * complicated red-black insert stuff. Based on Sun's TreeMap implementation, though it's barely
  ...
  /**
   * complicated red-black delete stuff. Based on Sun's TreeMap implementation, though it's barely
   * recognizable any more
   *
  ...
  /**
   * complicated red-black delete stuff. Based on Sun's TreeMap implementation, though it's barely
   * recognizable any more. This rebalances the tree (somewhat, as red-black trees are not perfectly
   * balanced -- perfect balancing takes longer)
   *
  ...
  /**
   * swap two nodes (except for their content), taking care of special cases where one is the
   * other's parent ... hey, it happens.
   *
   * @param x one node
  ...
  private void swapPosition(final Node x, final Node y, final int index) {
    ...
    boolean xWasLeftChild =
        (x.getParent(index) != null) && (x == x.getParent(index).getLeft(index));
    boolean yWasLeftChild =
        (y.getParent(index) != null) && (y == y.getParent(index).getLeft(index));
   ...
   * @param index KEY or VALUE (used to put the right word in the exception message)
  ...
  /**
   * increment the modification count -- used to check for concurrent modification of the map
   * through the map and through an Iterator from one of its Set or Collection views
   */
  private void modify() {
    modifications++;
  }
   ...
   * Integer.MAXVALUE elements, returns Integer.MAXVALUE.
   *
  ...
  /**
   * Returns the value to which this map maps the specified key. Returns null if the map contains no
   ...
   * @throws IllegalArgumentException if the key duplicates an existing key, or if the value
  ...
  public Object put(final Object key, final Object value)
    ...
    if (node == null) {
      ...
      while (true) {
        ...
        if (cmp == 0) {
          throw new IllegalArgumentException(
              "Cannot store a duplicate key (\"" + key + "\") in this Map");
        } else if (cmp < 0) {
   ...
   *
   * <p>The set supports element removal, which removes the corresponding mapping from the map, via
   * the Iterator.remove, Set.remove, removeAll, retainAll and clear operations. It does not support
   * the add or addAll operations. The setValue method is not supported on the Map Entry.
   *
   * @return a set view of the mappings contained in this map.
   */
  public Set entrySet() {
      ...
      setOfEntries[KEY] =
          new AbstractSet() {
            ...
            public boolean contains(Object o) {
              ...
              Node node = lookup((Comparable) entry.getKey(), KEY);
  ...
  private abstract class DoubleOrderedMapIterator implements Iterator {
    ...
    /**
     * @return 'next', whatever that means for a given kind of DoubleOrderedMapIterator
     */
     ...
     * @throws ConcurrentModificationException if the DoubleOrderedMap is modified behind the
     *     iterator's back
     */
    ...
    /**
     * Removes from the underlying collection the last element returned by the iterator. This method
     * can be called only once per call to next. The behavior of an iterator is unspecified if the
     * underlying collection is modified while the iteration is in progress in any way other than by
     * calling this method.
     *
  ...
  private static final class Node implements Map.Entry, KeyValue {
     ...
     * is also a map entry and the two entries represent the same mapping.
     *
