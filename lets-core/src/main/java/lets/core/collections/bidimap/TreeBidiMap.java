...
import lets.core.collections.BidiMap;
import lets.core.collections.KeyValue;
import lets.core.collections.MapIterator;
...
import lets.core.collections.keyvalue.UnmodifiableMapEntry;

/**
 * Red-Black tree-based implementation of BidiMap where all objects added implement the <code>
 * Comparable</code> interface.
 *
 * <p>This class guarantees that the map will be in both ascending key order and ascending value
 * order, sorted according to the natural order for the key's and value's classes.
 *
 ...
 * problems with that implementation. If the data contained in the TreeMaps is large, the cost of
 * redundant storage becomes significant. The {@link DualTreeBidiMap} and {@link DualHashBidiMap}
 * implementations use this approach.
 *
 * <p>This solution keeps minimizes the data storage by holding data only once. The red-black
 * algorithm is based on java util TreeMap, but has been modified to simultaneously map a tree node
 * by key and by value. This doubles the cost of put operations (but so does using two TreeMaps),
 * and nearly doubles the cost of remove operations (there is a savings in that the lookup of the
 * node to be removed only has to be performed once). And since only one node contains the key and
 * value, storage is significantly less than that required by two TreeMaps.
 *
...
public class TreeBidiMap implements OrderedBidiMap {
   ...
   * @throws ClassCastException if the keys/values in the map are not Comparable or are not mutually
   *     comparable
   ...
   *
   * <p>The key must implement <code>Comparable</code>.
   *
   ...
   *
   * <p>The value must implement <code>Comparable</code>.
   *
  ...
  /**
   * Gets the value to which this map maps the specified key. Returns null if the map contains no
   ...
   * </pre>
   *
   * <p>Both key and value must implement <code>Comparable</code>.
   *
   ...
   *
   * <p>All keys and values must implement <code>Comparable</code>.
   *
   ...
   *
   * <p>The set is backed by the map, so changes to the map are reflected in the set, and
   * vice-versa. If the map is modified while an iteration over the set is in progress, the results
   * of the iteration are undefined.
   *
   * <p>The set supports element removal, which removes the corresponding mapping from the map. It
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns a set view of the values contained in this map in key order. The returned object can be
   * cast to a Set.
   *
   * <p>The set is backed by the map, so changes to the map are reflected in the set, and
   * vice-versa. If the map is modified while an iteration over the set is in progress, the results
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns a set view of the entries contained in this map in key order. For simple iteration
   * through the map, the MapIterator is quicker.
   *
   * <p>The set is backed by the map, so changes to the map are reflected in the set, and
   * vice-versa. If the map is modified while an iteration over the set is in progress, the results
   * of the iteration are undefined.
   *
   * <p>The set supports element removal, which removes the corresponding mapping from the map. It
   * does not support the add or addAll operations. The returned MapEntry objects do not support
   * setValue.
   *
   * @return a set view of the values contained in this map.
   */
   ...
   *
   * <p>For this map, this iterator is the fastest way to iterate over the entries.
   *
   * @return an iterator
   */
   ...
   *
   * <p>This iterator allows both forward and reverse iteration over the entries.
   *
   * @return an iterator
   */
