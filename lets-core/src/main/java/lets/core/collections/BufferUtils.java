...
import lets.core.collections.buffer.BlockingBuffer;
import lets.core.collections.buffer.BoundedBuffer;
import lets.core.collections.buffer.PredicatedBuffer;
import lets.core.collections.buffer.SynchronizedBuffer;
import lets.core.collections.buffer.TransformedBuffer;
import lets.core.collections.buffer.TypedBuffer;
import lets.core.collections.buffer.UnmodifiableBuffer;
...
public class BufferUtils {
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns a synchronized buffer backed by the given buffer. Much like the synchronized
   * collections returned by {@link java.util.Collections}, you must manually synchronize on the
   * returned buffer's iterator to avoid non-deterministic behavior:
   *
   * <pre>
  ...
  /**
   * Returns a synchronized buffer backed by the given buffer that will block on {@link
   * Buffer#get()} and {@link Buffer#remove()} operations. If the buffer is empty, then the {@link
   * Buffer#get()} and {@link Buffer#remove()} operations will block until new elements are added to
   * the buffer, rather than immediately throwing a <code>BufferUnderflowException</code>.
   *
   * @param buffer the buffer to synchronize, must not be null
  ...
  /**
   * Returns a synchronized buffer backed by the given buffer that will block on {@link
   * Buffer#get()} and {@link Buffer#remove()} operations until <code>timeout</code> expires. If the
   * buffer is empty, then the {@link Buffer#get()} and {@link Buffer#remove()} operations will
   * block until new elements are added to the buffer, rather than immediately throwing a <code>
   * BufferUnderflowException</code>.
   *
   * @param buffer the buffer to synchronize, must not be null
  ...
  /**
   * Returns a synchronized buffer backed by the given buffer that will block on {@link
   * Buffer#add(Object)} and {@link Buffer#addAll(java.util.Collection)} until enough object(s) are
   * removed from the buffer to allow the object(s) to be added and still maintain the maximum size.
   *
   ...
   *
   * <p>Only objects that pass the test in the given predicate can be added to the buffer. Trying to
   * add an invalid object results in an IllegalArgumentException. It is important not to use the
   * original buffer after invoking this method, as it is a backdoor for adding invalid objects.
   *
   ...
   *
   * <p>Only elements of the specified type can be added to the buffer.
   *
   * @param buffer the buffer to predicate, must not be null
   ...
   *
   * <p>Each object is passed through the transformer as it is added to the Buffer. It is important
   * not to use the original buffer after invoking this method, as it is a backdoor for adding
