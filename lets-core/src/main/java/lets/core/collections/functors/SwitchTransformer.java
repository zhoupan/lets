...
/**
 * Transformer implementation calls the transformer whose predicate returns true, like a switch
 * statement.
 *
...
public class SwitchTransformer implements Transformer, Serializable {
   ...
   * in the map with a null key. The ordering is that of the iterator() method on the entryset
  ...
  /**
   * Constructor that performs no validation. Use <code>getInstance</code> if you want that.
   *
   * @param predicates array of predicates, not cloned, no nulls
   * @param transformers matching array of transformers, not cloned, no nulls
   * @param defaultTransformer the transformer to use if no match, null means return null
   */
  public SwitchTransformer(
      Predicate[] predicates, Transformer[] transformers, Transformer defaultTransformer) {
    super();
    iPredicates = predicates;
    iTransformers = transformers;
    iDefault =
        (defaultTransformer == null ? ConstantTransformer.NULL_INSTANCE : defaultTransformer);
  }

  /**
   * Transforms the input to result by calling the transformer whose matching predicate returns
   * true.
   *
