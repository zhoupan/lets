 ...
 *
 * <p>Changes made to this set will actually be made on the decorated set. Add operations require
 * the use of a pluggable strategy. If no strategy is provided then add is unsupported.
 *
...
public class CompositeSet extends CompositeCollection implements Set {
   ...
   * @throws IllegalArgumentException if c does not implement java.util.Set or if a SetMutator is
   *     set, but fails to resolve a collision
   * @throws UnsupportedOperationException if there is no SetMutator set, or a CollectionMutator is
   *     set instead of a SetMutator
   * @see lets.core.collections.collection.CompositeCollection.CollectionMutator
   * @see SetMutator
   */
  ...
  /**
   * This can receive either a <code>CompositeCollection.CollectionMutator</code> or a <code>
   * CompositeSet.SetMutator</code>. If a <code>CompositeCollection.CollectionMutator</code> is used
   * than conflicts when adding composited sets will throw IllegalArgumentException
   *
   * <p>
   */
  ...
  /**
   * If a <code>CollectionMutator</code> is defined for this CompositeSet then this method will be
   * called anyway.
   *
   ...
   *
   * <p>Defining remove() on implementations of SetMutator is pointless as they are never called by
   * CompositeSet.
   */
  public static interface SetMutator extends CompositeCollection.CollectionMutator {
    /**
     * Called when a Set is added to the CompositeSet and there is a collision between existing and
     * added sets.
     *
     * <p>If <code>added</code> and <code>existing</code> still have any intersects after this
     * method returns an IllegalArgumentException will be thrown.
     *
