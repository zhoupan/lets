...
/**
 * Decorates another <code>SortedSet</code> to validate that all additions match a specified
 * predicate.
 *
 * <p>This set exists to provide validation for the decorated set. It is normally created to
 * decorate an empty set. If an object cannot be added to the set, an IllegalArgumentException is
 * thrown.
 *
 * <p>One usage would be to ensure that no null entries are added to the set.
 *
 * <pre>SortedSet set = PredicatedSortedSet.decorate(new TreeSet(), NotNullPredicate.INSTANCE);
 * </pre>
 *
...
public class PredicatedSortedSet extends PredicatedSet implements SortedSet {
   ...
   *
   * <p>If there are any elements already in the set being decorated, they are validated.
   *
