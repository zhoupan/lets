...
import lets.core.collections.keyvalue.TiedMapEntry;

/**
 * A <code>Map</code> implementation that holds a single item and is fixed size.
 *
 * <p>The single key/value pair is specified at creation. The map is fixed size so any action that
 ...
 *
 * <p>If trying to remove or clear the map, an UnsupportedOperationException is thrown. If trying to
 * put a new mapping into the map, an IllegalArgumentException is thrown. The put method will only
 * suceed if the key specified is the same as the singleton key.
 *
 * <p>The key and value can be obtained by:
 *
 * <ul>
...
public class SingletonMap implements OrderedMap, BoundedMap, KeyValue, Serializable, Cloneable {
   ...
   *
   * <p>An IllegalArgumentException is thrown if the key does not match as the map is fixed size.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Gets the entrySet view of the map. Changes made via <code>setValue</code> affect this map. To
   * simply iterate through the entries, use {@link #mapIterator()}.
   *
  ...
  /**
   * Gets an iterator over the map. Changes made to the iterator using <code>setValue</code> affect
   * this map. The <code>remove</code> method is unsupported.
   *
   ...
   *
   * <p>A ordered map iterator is an efficient way of iterating over maps in both directions.
   *
