...
import lets.core.collections.collection.CompositeCollection;
import lets.core.collections.set.CompositeSet;

/**
 * Decorates a map of other maps to provide a single unified view.
 *
 * <p>Changes made to this map will actually be made on the decorated map. Add and remove operations
 * require the use of a pluggable strategy. If no strategy is provided then add and remove are
 * unsupported.
 *
 * <p><strong>Note that CompositeMap is not synchronized and is not thread-safe.</strong> If you
 * wish to use this map from multiple threads concurrently, you must use appropriate
 * synchronization. The simplest approach is to wrap this map using {@link
 * java.util.Collections#synchronizedMap(Map)}. This class may throw exceptions when accessed by
...
public class CompositeMap implements Map {
  ...
  /**
   * Create a new CompositeMap which composites all of the Map instances in the argument. It copies
   * the argument array, it does not use it directly.
   *
   ...
   * @throws IllegalArgumentException if there is a key collision and there is no MapMutator set to
   *     handle it.
   */
   ...
   * returns <tt>true</tt> if and only if this map contains at a mapping for a key <tt>k</tt> such
   ...
   * @throws NullPointerException if the value is <tt>null</tt> and this map does not not permit
   ...
   * <code>Map.Entry</code>. The set is backed by the map, so changes to the map are reflected in
   ...
   *
   * <p>This implementation returns a <code>CompositeSet</code> which composites the entry sets from
   * all of the composited maps.
   *
   * @see CompositeSet
   ...
   *
   * <p>More formally, if this map contains a mapping from a key <tt>k</tt> to a value <tt>v</tt>
   * such that <tt>(key==null ? k==null : key.equals(k))</tt>, then this method returns <tt>v</tt>;
   * otherwise it returns <tt>null</tt>. (There can be at most one such mapping.)
   *
   ...
   *
   * <p>This implementation returns a <code>CompositeSet</code> which composites the key sets from
   * all of the composited maps.
   *
   ...
   * map previously contained a mapping for this key, the old value is replaced by the specified
   ...
   *     stored in this map.
   * @throws NullPointerException this map does not permit <tt>null</tt> keys or values, and the
  ...
  /**
   * Copies all of the mappings from the specified map to this map (optional operation). The effect
   * of this call is equivalent to that of calling {@link #put(Object,Object) put(k, v)} on this map
   * once for each mapping from key <tt>k</tt> to value <tt>v</tt> in the specified map. The
   * behavior of this operation is unspecified if the specified map is modified while the operation
   * is in progress.
   *
   * @param map Mappings to be stored in this map.
   * @throws UnsupportedOperationException if the <tt>putAll</tt> method is not supported by this
   *     map.
   ...
   *
   * <p>Returns the value to which the map previously associated the key, or <tt>null</tt> if the
   * map contained no mapping for this key. (A <tt>null</tt> return can also indicate that the map
   ...
   * @return previous value associated with specified key, or <tt>null</tt> if there was no mapping
   *     for key.
   * @throws ClassCastException if the key is of an inappropriate type for the composited map
   *     (optional).
   * @throws NullPointerException if the key is <tt>null</tt> and the composited map does not not
   *     permit <tt>null</tt> keys (optional).
   * @throws UnsupportedOperationException if the <tt>remove</tt> method is not supported by the
   *     composited map containing the key
   */
   ...
   * <tt>retainAll</tt> and <tt>clear</tt> operations. It does not support the add or
  ...
  /**
   * This interface allows definition for all of the indeterminate mutators in a CompositeMap, as
   * well as providing a hook for callbacks on key collisions.
   */
  public static interface MapMutator {
     ...
     * @param existing the Map already in the composite which contains the offending key
