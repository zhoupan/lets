...
import lets.core.collections.keyvalue.MultiKey;

/**
 * A <code>Map</code> implementation that uses multiple keys to map the value.
 *
 * <p>This class is the most efficient way to uses multiple keys to map to a value. The best way to
 * use this class is via the additional map-style methods. These provide <code>get</code>, <code>
 * containsKey</code>, <code>put</code> and <code>remove</code> for individual keys which operate
 * without extra object creation.
 *
 * <p>The additional methods are the main interface of this map. As such, you will not normally hold
 * this map in a variable of type <code>Map</code>.
 *
 * <p>The normal map methods take in and return a {@link MultiKey}. If you try to use <code>put()
 * </code> with any other object type a <code>ClassCastException</code> is thrown. If you try to use
 * <code>null</code> as the key in <code>put()</code> a <code>NullPointerException</code> is thrown.
 *
 * <p>This map is implemented as a decorator of a <code>AbstractHashedMap</code> which enables extra
 * behaviour to be added easily.
 *
 * <ul>
 ...
 *   <li><code>MultiKeyMap.decorate(new ReferenceMap())</code> creates a garbage collector sensitive
 *       map.
 * </ul>
 *
 * Note that <code>IdentityMap</code> and <code>ReferenceIdentityMap</code> are unsuitable for use
 * as the key comparison would work on the whole MultiKey, not the elements within.
 *
 * <p>As an example, consider a least recently used cache that uses a String airline code and a
 * Locale to lookup the airline's name:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p><strong>Note that MultiKeyMap is not synchronized and is not thread-safe.</strong> If you wish
 * to use this map from multiple threads concurrently, you must use appropriate synchronization.
...
public class MultiKeyMap implements IterableMap, Serializable {
  ...
  // -----------------------------------------------------------------------
  /**
   * Decorates the specified map to add the MultiKeyMap API and fast query. The map must not be null
   * and must be empty.
   *
  ...
  /**
   * Constructor that decorates the specified map and is called from {@link
   * #decorate(AbstractHashedMap)}. The map must not be null and should be empty or only contain
   * valid keys. This constructor performs no validation.
   *
   ...
   */
  public Object removeItem(Object key1, Object key2) {
    int hashCode = hash(key1, key2);
    int index = map.hashIndex(hashCode, map.data.length);
    AbstractHashedMap.HashEntry entry = map.data[index];
   ...
   * @return true if the key matches
   */
  protected boolean isEqualKey(
      AbstractHashedMap.HashEntry entry, Object key1, Object key2, Object key3) {
    MultiKey multi = (MultiKey) entry.getKey();
    return multi.size() == 3
        && (key1 == null ? multi.getKey(0) == null : key1.equals(multi.getKey(0)))
        && (key2 == null ? multi.getKey(1) == null : key2.equals(multi.getKey(1)))
   ...
   */
  protected boolean isEqualKey(
      AbstractHashedMap.HashEntry entry, Object key1, Object key2, Object key3, Object key4) {
    MultiKey multi = (MultiKey) entry.getKey();
    return multi.size() == 4
        && (key1 == null ? multi.getKey(0) == null : key1.equals(multi.getKey(0)))
        && (key2 == null ? multi.getKey(1) == null : key2.equals(multi.getKey(1)))
   ...
   *
   * <p>This method removes all the mappings where the <code>MultiKey</code> has one or more keys,
   * and the first matches that specified.
   *
   * @param key1 the first key
   ...
   *
   * <p>This method removes all the mappings where the <code>MultiKey</code> has two or more keys,
   * and the first two match those specified.
   *
   * @param key1 the first key
   * @param key2 the second key
   * @return true if any elements were removed
   ...
   *
   * <p>This method removes all the mappings where the <code>MultiKey</code> has three or more keys,
   * and the first three match those specified.
   *
   * @param key1 the first key
   * @param key2 the second key
   * @param key3 the third key
   ...
   *
   * <p>This method removes all the mappings where the <code>MultiKey</code> has four or more keys,
   * and the first four match those specified.
   *
   * @param key1 the first key
   * @param key2 the second key
   * @param key3 the third key
  ...
  /**
   * Copies all of the keys and values from the specified map to this map. Each key must be non-null
   * and a MultiKey object.
   *
