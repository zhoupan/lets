...
/**
 * A <code>Map</code> implementation that stores data in simple fields until the size is greater
 * than 3.
 *
 * <p>This map is designed for performance and can outstrip HashMap. It also has good garbage
 ...
 * </ul>
 *
 * The design uses two distinct modes of operation - flat and delegate. While the map is size 3 or
 * less, operations map straight onto fields using switch statements. Once size 4 is reached, the
 * map switches to delegate mode and only switches back when cleared. In delegate mode, all
 * operations are forwarded straight to a HashMap resulting in the 5% performance loss.
 *
 * <p>The performance gains on puts are due to not needing to create a Map Entry object. This is a
 * large saving not only in performance but in garbage collection.
 *
 * <p>Whilst in flat mode this map is also easy for the garbage collector to dispatch. This is
 * because it contains no complex objects or arrays which slow the progress.
 *
 * <p>Do not use <code>Flat3Map</code> if the size is likely to grow beyond 3.
 *
 * <p><strong>Note that Flat3Map is not synchronized and is not thread-safe.</strong> If you wish to
 ...
 * @version $Revision: 1713293 $ $Date: 2015-11-08 22:04:34 +0100 (Sun, 08 Nov 2015) $
...
public class Flat3Map implements IterableMap, Serializable, Cloneable {
   ...
   *
   * <p>This can be overridden by subclasses to provide a different map implementation. Not every
   * AbstractHashedMap is suitable, identity and reference based maps would be poor choices.
   *
