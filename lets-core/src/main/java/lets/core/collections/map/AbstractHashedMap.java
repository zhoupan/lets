...
import lets.core.collections.IterableMap;
...
public class AbstractHashedMap extends AbstractMap implements IterableMap {
   ...
   *
   * <p>This implementation iterates around the specified map and uses {@link #put(Object, Object)}.
   *
   ...
   *
   * <p>The reverse conversion can be changed, if required, by overriding the getKey() method in the
   * hash entry.
   *
   ...
   *
   * <p>This method exists for subclasses that may need to perform a multi-step process accessing
   ...
   *
   * <p>This implementation calls <code>setValue()</code> on the entry. Subclasses could override to
   ...
   *
   * <p>This implementation sets all the data fields on the entry. Subclasses could populate
  ...
  protected void reuseEntry(
      HashEntry entry, int hashIndex, int hashCode, Object key, Object value) {
   ...
   *
   * <p>This implementation calls <code>createEntry()</code>, <code>addEntry()</code> and <code>
   * checkCapacity()</code>. It also handles changes to <code>modCount</code> and <code>size</code>.
   ...
   *
   * <p>This implementation creates a new HashEntry instance. Subclasses can override this to return
   ...
   *
   * <p>This implementation adds the entry to the data storage table. Subclasses could override to
   * handle changes to the map.
   *
   ...
   *
   * <p>This implementation calls <code>removeEntry()</code> and <code>destroyEntry()</code>. It
   * also handles changes to <code>modCount</code> and <code>size</code>. Subclasses could override
   ...
   *
   * <p>This implementation removes the entry from the data storage table. The size is not updated.
   ...
   *
   * <p>This implementation prepares the HashEntry for garbage collection. Subclasses can override
   ...
   *
   * <p>This implementation uses the threshold to check if the map needs enlarging
   */
  ...
  // -----------------------------------------------------------------------
  /**
   * Gets the <code>next</code> field from a <code>HashEntry</code>. Used in subclasses that have no
  ...
  /**
   * Gets the <code>hashCode</code> field from a <code>HashEntry</code>. Used in subclasses that
  ...
  /**
   * Gets the <code>key</code> field from a <code>HashEntry</code>. Used in subclasses that have no
   * visibility of the field.
   *
   * @param entry the entry to query, must not be null
  ...
  /**
   * Gets the <code>value</code> field from a <code>HashEntry</code>. Used in subclasses that have
  ...
  // -----------------------------------------------------------------------
  /**
   * Writes the map data to the stream. This method must be overridden if a subclass must be setup
   * before <code>put()</code> is used.
   *
  ...
  /**
   * Reads the map data from the stream. This method must be overridden if a subclass must be setup
   * before <code>put()</code> is used.
   *
   * <p>Serialization is not one of the JDK's nicest topics. Normal serialization will initialise
   * the superclass before the subclass. Sometimes however, this isn't what you want, as in this
   ...
   *
   * <p>To implement <code>clone()</code>, a subclass must implement the <code>Cloneable</code>
