 ...
 * index in the list.
 *
 * <p>This list implementation utilises a tree structure internally to ensure that all insertions
 * and removals are O(log n). This provides much faster performance than both an <code>ArrayList
 * </code> and a <code>LinkedList</code> where elements are inserted and removed repeatedly from
 * anywhere in the list.
 *
 * <p>The following relative performance statistics are indicative of this class:
 *
 * <pre>
 ...
 * </pre>
 *
 * <code>ArrayList</code> is a good general purpose list implementation. It is faster than <code>
 * TreeList</code> for most operations except inserting and removing in the middle of the list.
 * <code>ArrayList</code> also uses less memory as <code>TreeList</code> uses one object per entry.
 *
 * <p><code>LinkedList</code> is rarely a good choice of implementation. <code>TreeList</code> is
 * almost always a good replacement for it, although it does use sligtly more memory.
 *
...
public class TreeList extends AbstractList {
   ...
   *
   * <p>This node contains the real work. TreeList is just there to implement {@link
   * java.util.List}. The nodes don't know the index of the object they are holding. They do know
   * however their position relative to their parent node. This allows to calculate the index of a
   * node while traversing the tree.
   *
   * <p>The Faedelung calculation stores a flag for both the left and right child to indicate if
   * they are a child (false) or a link as in linked list (true).
   */
  static class AVLNode {
     ...
     *
     * @param index is the index of the position relative to the position of the parent node.
     ...
     *
     * @param index is the index of the element to be removed relative to the position of the parent
  ...
  static class TreeListIterator implements ListIterator, OrderedIterator {
     ...
     *
     * @throws ConcurrentModificationException If the list's modification count isn't the value that
     *     was expected.
     */
