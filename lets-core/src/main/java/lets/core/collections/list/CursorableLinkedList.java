 ...
 *
 */
package lets.core.collections.list;
...
/**
 * A <code>List</code> implementation with a <code>ListIterator</code> that allows concurrent
 * modifications to the underlying list.
 *
 * <p>This implementation supports all of the optional {@link List} operations. It extends <code>
 * AbstractLinkedList</code> and thus provides the stack/queue/dequeue operations available in
 * {@link java.util.LinkedList}.
 *
 * <p>The main feature of this class is the ability to modify the list and the iterator at the same
 * time. Both the {@link #listIterator()} and {@link #cursor()} methods provides access to a <code>
 * Cursor</code> instance which extends <code>ListIterator</code>. The cursor allows changes to the
 * list concurrent with changes to the iterator. Note that the {@link #iterator()} method and
 * sublists do <b>not</b> provide this cursor behaviour.
 *
 * <p>The <code>Cursor</code> class is provided partly for backwards compatibility and partly
 * because it allows the cursor to be directly closed. Closing the cursor is optional because
 * references are held via a <code>WeakReference</code>. For most purposes, simply modify the
 * iterator and list at will, and then let the garbage collector to the rest.
 *
...
public class CursorableLinkedList extends AbstractLinkedList implements Serializable {
  ...
  /**
   * The equivalent of a default constructor called by any constructor and by <code>readObject
   * </code>.
   */
  protected void init() {
    super.init();
   ...
   *
   * <p>If the underlying list is modified while iterating using this iterator a
   * ConcurrentModificationException will occur. The cursor behaviour is available via {@link
   * #listIterator()}.
   *
   ...
   *
   * <p>The cursor enables iteration and list changes to occur in any order without invalidating the
   * iterator (from one thread). When elements are added to the list, an event is fired to all
   * active cursors enabling them to adjust to the change in the list.
   *
   * <p>When the "current" (i.e., last returned by {@link ListIterator#next} or {@link
   * ListIterator#previous}) element of the list is removed, the cursor automatically adjusts to the
   * change (invalidating the last returned value such that it cannot be removed).
   *
   ...
   *
   * <p>A <code>Cursor</code> is a <code>ListIterator</code> with an additional <code>close()</code>
   * method. Calling this method immediately discards the references to the cursor. If it is not
   * called, then the garbage collector will still remove the reference as it is held via a <code>
   * WeakReference</code>.
   *
   * <p>The cursor enables iteration and list changes to occur in any order without invalidating the
   * iterator (from one thread). When elements are added to the list, an event is fired to all
   ...
   * change (invalidating the last returned value such that it cannot be removed).
   *
   * <p>The {@link #listIterator()} method returns the same as this method, and can be cast to a
   * <code>Cursor</code> if the <code>close</code> method is required.
   *
   * @return a new cursor iterator
   */
  ...
  /**
   * Returns a {@link Cursor} for iterating through the elements of this list starting from a
   * specified index.
   *
   * <p>A <code>Cursor</code> is a <code>ListIterator</code> with an additional <code>close()</code>
   * method. Calling this method immediately discards the references to the cursor. If it is not
   ...
   * change (invalidating the last returned value such that it cannot be removed).
   *
   * <p>The {@link #listIterator(int)} method returns the same as this method, and can be cast to a
   * <code>Cursor</code> if the <code>close</code> method is required.
   *
   * @param fromIndex the index to start from
   * @return a new cursor iterator
  ...
  public static class Cursor extends AbstractLinkedList.LinkedListIterator {
     ...
     *
     * <p>There may have been subsequent alterations to the list since you obtained this item,
     * however you can still remove it. You can even remove it if the item is no longer in the main
     * list. However, you can't call this method on the same iterator more than once without calling
     * next() or previous().
     *
