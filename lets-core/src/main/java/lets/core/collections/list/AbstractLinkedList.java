 ...
 * override.
 *
 * <p>Overridable methods are provided to change the storage node and to change how nodes are added
 * to and removed. Hopefully, all you need for unusual subclasses is here.
 *
...
public abstract class AbstractLinkedList implements List {
   ...
   *
   * <p>If this constructor is used by a serializable subclass then the init() method must be
   * called.
   */
   ...
   * <code>node</code>.
   *
   * <p>This implementation uses {@link #createNode(Object)} and {@link
   ...
   * @param endMarkerAllowed whether or not the end marker can be returned if startIndex is set to
   *     the list's size
   * @throws IndexOutOfBoundsException if the index is less than 0; equal to the size of the list
   *     and endMakerAllowed is false; or greater than the size of the list
   */
   ...
   *
   * <p>The first serializable subclass must call this method from <code>writeObject</code>.
   */
   ...
   *
   * <p>The first serializable subclass must call this method from <code>readObject</code>.
   */
   ...
   *
   * <p>From Commons Collections 3.1, all access to the <code>value</code> property is via the
  ...
  protected static class LinkedListIterator implements ListIterator, OrderedIterator {
    ...
    /**
     * The node that will be returned by {@link #next()}. If this is equal to {@link
    ...
    /**
     * The last node that was returned by {@link #next()} or {@link #previous()}. Set to <code>null
     * </code> if {@link #next()} or {@link #previous()} haven't been called, or if the node has
     * been removed with {@link #remove()} or a new node added with {@link #add(Object)}. Should be
     * accessed through {@link #getLastNodeReturned()} to enforce this behaviour.
     */
     ...
     *     or if the node has been removed with {@link #remove()} or a new node added with {@link
     *     #add(Object)}.
     */
