...
/**
 * Decorates another <code>Buffer</code> to validate that additions match a specified predicate.
 *
 * <p>This buffer exists to provide validation for the decorated buffer. It is normally created to
 * decorate an empty buffer. If an object cannot be added to the buffer, an IllegalArgumentException
 * is thrown.
 *
 * <p>One usage would be to ensure that no null entries are added to the buffer.
 *
 * <pre>
