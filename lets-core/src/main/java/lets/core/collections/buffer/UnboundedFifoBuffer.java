...
/**
 * UnboundedFifoBuffer is a very efficient implementation of <code>Buffer</code> that can grow to
 * any size. According to performance testing, it exhibits a constant access time, but it also
 * outperforms ArrayList when used for the same purpose.
 *
 * <p>The removal order of an <code>UnboundedFifoBuffer</code> is based on the insertion order;
 * elements are removed in the same order in which they were added. The iteration order is the same
 * as the removal order.
 *
 * <p>The {@link #remove()} and {@link #get()} operations perform in constant time. The {@link
 * #add(Object)} operation performs in amortized constant time. All other operations perform in
 * linear time or worse.
 *
 * <p>Note that this implementation is not synchronized. The following can be used to provide
 * synchronized access to your <code>UnboundedFifoBuffer</code>:
 *
 * <pre>
...
public class UnboundedFifoBuffer extends AbstractCollection implements Buffer, Serializable {
  ...
  /**
   * Constructs an UnboundedFifoBuffer with the default number of elements. It is exactly the same
  ...
  /**
   * Constructs an UnboundedFifoBuffer with the specified number of elements. The integer must be a
   * positive integer.
   *
