...
import lets.core.collections.BoundedCollection;
import lets.core.collections.Buffer;
import lets.core.collections.BufferOverflowException;
 ...
 *
 * <p>Note: This class should only be used if you need to add bounded behaviour to another buffer.
 * If you just want a bounded buffer then you should use {@link BoundedFifoBuffer} or {@link
 * CircularFifoBuffer}.
 *
 * <p>The decoration methods allow you to specify a timeout value. This alters the behaviour of the
 * add methods when the buffer is full. Normally, when the buffer is full, the add method will throw
 * an exception. With a timeout, the add methods will wait for up to the timeout period to try and
 * add the elements.
 *
 * @author James Carman
...
public class BoundedBuffer extends SynchronizedBuffer implements BoundedCollection {
   ...
   *
   * <p>When the buffer is full, it will immediately throw a <code>BufferOverflowException</code> on
  ...
  // -----------------------------------------------------------------------
  /**
   * Constructor that wraps (not copies) another buffer, making it bounded waiting only up to a
