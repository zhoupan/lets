...
/**
 * CircularFifoBuffer is a first in first out buffer with a fixed size that replaces its oldest
 * element if full.
 *
 * <p>The removal order of a <code>CircularFifoBuffer</code> is based on the insertion order;
 ...
 * synchronized access to your <code>CircularFifoBuffer</code>:
 *
 * <pre>
...
public class CircularFifoBuffer extends BoundedFifoBuffer {
  ...
  /**
   * Constructor that creates a buffer from the specified collection. The collection size also sets
   * the buffer size
   *
