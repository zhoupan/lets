...
import lets.core.collections.BufferUnderflowException;

/**
 * Binary heap implementation of <code>Buffer</code> that provides for removal based on <code>
 * Comparator</code> ordering.
 *
 * <p>The removal order of a binary heap is based on either the natural sort order of its elements
 * or a specified {@link Comparator}. The {@link #remove()} method always returns the first element
 * as determined by the sort order. (The <code>ascendingOrder</code> flag in the constructors can be
 * used to reverse the sort order, in which case {@link #remove()} will always remove the last
 * element.) The removal order is <i>not</i> the same as the order of iteration; elements are
 * returned by the iterator in no particular order.
 *
 * <p>The {@link #add(Object)} and {@link #remove()} operations perform in logarithmic time. The
 * {@link #get()} operation performs in constant time. All other operations perform in linear time
 * or worse.
 *
 * <p>Note that this implementation is not synchronized. Use {@link
 * lets.core.collections.BufferUtils#synchronizedBuffer(Buffer)} or {@link
 * lets.core.collections.buffer.SynchronizedBuffer#decorate(Buffer)} to provide synchronized access
 * to a <code>PriorityBuffer</code>:
 *
 * <pre>
 * Buffer heap = SynchronizedBuffer.decorate(new PriorityBuffer());
 * </pre>
 *
 * <p>This class is Serializable from Commons Collections 3.2.
 *
...
public class PriorityBuffer extends AbstractCollection implements Buffer, Serializable {
  ...
  /**
   * If true, the first element as determined by the sort order will be returned. If false, the last
   * element as determined by the sort order will be returned.
   */
  ...
  // -----------------------------------------------------------------------
  /**
   * Constructs a new empty buffer that sorts in ascending order by the natural order of the objects
   * added.
   */
  ...
  /**
   * Constructs a new empty buffer specifying the sort order and using the natural order of the
   * objects added.
   *
   * @param ascendingOrder if <code>true</code> the heap is created as a minimum heap; otherwise,
   *     the heap is created as a maximum heap
   */
   ...
   *
   * @param ascendingOrder true to use the order imposed by the given comparator; false to reverse
   *     that order
   * @param comparator the comparator used to order the elements, null means use natural order
   */
  ...
  /**
   * Constructs a new empty buffer that sorts in ascending order by the natural order of the objects
   * added, specifying an initial capacity.
   *
  ...
  /**
   * Constructs a new empty buffer that sorts in ascending order using the specified comparator and
   * initial capacity.
   *
   * @param capacity the initial capacity for the buffer, greater than zero
   * @param comparator the comparator used to order the elements, null means use natural order
  ...
  /**
   * Constructs a new empty buffer that specifying initial capacity and sort order, using the
   ...
   *
   * <p>The element added will be sorted according to the comparator in use.
   *
   ...
   *
   * <p>Assumes it is a minimum heap.
   *
   ...
   * Percolates element down heap from the position given by the index.
   *
   * <p>Assumes it is a maximum heap.
   *
  ...
  /**
   * Returns a string representation of this heap. The returned string is similar to those produced
   * by standard JDK collections.
   *
