...
/**
 * <code>UnmodifiableBoundedCollection</code> decorates another <code>BoundedCollection</code> to
 * ensure it can't be altered.
 *
 * <p>If a BoundedCollection is first wrapped in some other collection decorator, such as
 * synchronized or predicated, the BoundedCollection methods are no longer accessible. The factory
 * on this class will attempt to retrieve the bounded nature by examining the package scope
 * variables.
 *
...
public final class UnmodifiableBoundedCollection extends AbstractSerializableCollectionDecorator
   ...
   * Factory method to create an unmodifiable bounded collection.
   *
   * <p>This method is capable of drilling down through up to 1000 other decorators to find a
   * suitable BoundedCollection.
   *
   * @param coll the <code>BoundedCollection</code> to decorate, must not be null
   * @return a new unmodifiable bounded collection
   * @throws IllegalArgumentException if bag is null
