 ...
 * This data structure has fast <i>O(1)</i> search time, deletion time, and insertion time.
 *
 * <p>Although this map is sequenced, it cannot implement {@link java.util.List} because of
 * incompatible interface definitions. The remove methods in List and Map have different return
 * values (see: {@link java.util.List#remove(Object)} and {@link java.util.Map#remove(Object)}).
 *
 * <p>This class is not thread safe. When a thread safe implementation is required, use {@link
 * java.util.Collections#synchronizedMap(Map)} as it is documented, or use explicit synchronization
 * controls.
 *
 * @deprecated Replaced by LinkedMap and ListOrderedMap in map subpackage. Due to be removed in
 *     v4.0.
 * @see lets.core.collections.map.LinkedMap
 * @see lets.core.collections.map.ListOrderedMap
...
public class SequencedHashMap implements Map, Cloneable, Externalizable {
  ...
  /**
   * Construct an empty sentinel used to hold the head (sentinel.next) and the tail (sentinel.prev)
   * of the list. The sentinel has a <code>null</code> key and value.
   */
  ...
  /**
   * Construct a new sequenced hash map and add all the elements in the specified map. The order in
  ...
  /**
   * Inserts a new internal entry to the tail of the linked list. This does not add the entry to the
   * underlying map.
   */
  ...
  /**
   * Return the entry for the "oldest" mapping. That is, return the Map.Entry for the key-value pair
   * that was first put into the map when compared to all the other pairings in the map. This
  ...
  /**
   * Return the key for the "oldest" mapping. That is, return the key for the mapping that was first
   * put into the map when compared to all the other objects in the map. This behavior is equivalent
   * to using <code>getFirst().getKey()</code>, but this method provides a slightly optimized
   * implementation.
   *
   ...
   * equivalent to using <code>getFirst().getValue()</code>, but this method provides a slightly
  ...
  /**
   * Return the entry for the "newest" mapping. That is, return the Map.Entry for the key-value pair
   * that was first put into the map when compared to all the other pairings in the map. The
   ...
   *  </pre>
   *
   * However, the implementation of this method ensures an O(1) lookup of the last key rather than
   * O(n).
   *
  ...
  /**
   * Return the key for the "newest" mapping. That is, return the key for the mapping that was last
   * put into the map when compared to all the other objects in the map. This behavior is equivalent
   * to using <code>getLast().getKey()</code>, but this method provides a slightly optimized
   * implementation.
   *
  ...
  /**
   * Fully remove an entry from the map, returning the old entry or null if there was no such entry
   ...
   * exist (as per {@link Map#putAll(Map)}). The order in which the entries are added is determined
   * by the iterator returned from {@link Map#entrySet()} for the specified map.
   *
   ...
   * only. If a specific format is required, use {@link #entrySet()}.{@link Set#iterator()
   * iterator()} and iterate over the entries in the map formatting them as appropriate.
   */
  public String toString() {
  ...
  private class OrderedIterator implements Iterator {
    /**
     * Holds the type that should be returned from the iterator. The value should be either {@link
     * #KEY}, {@link #VALUE}, or {@link #ENTRY}. To save a tiny bit of memory, this field is also
     * used as a marker for when remove has been called on the current object to prevent a second
     * remove on the same element. Essentially, if this value is negative (i.e. the bit specified by
     * {@link #REMOVED_MASK} is set), the current position has been removed. If positive, remove can
     * still be called.
     */
    private int returnType;

    /**
     * Holds the "current" position in the iterator. When pos.next is the sentinel, we've reached
     * the end of the list.
     */
    private Entry pos = sentinel;

    /**
     * Holds the expected modification count. If the actual modification count of the map differs
     * from this value, then a concurrent modification has occurred.
     */
    private transient long expectedModCount = modCount;

    /**
     * Construct an iterator over the sequenced elements in the order in which they were added. The
     * {@link #next()} method returns the type specified by <code>returnType</code> which must be
     * either {@link #KEY}, {@link #VALUE}, or {@link #ENTRY}.
     */
     ...
     *     {@link #next()} has never been called, or if {@link #remove()} was already called on the
     *     element.
     * @throws ConcurrentModificationException if a modification occurs in the underlying map.
     */
    public void remove() {
   ...
   * references. The keys and values themselves are not <code>clone()</code>'d. The cloned object
   ...
   *
   * @throws ArrayIndexOutOfBoundsException if the specified index is <code>&lt; 0</code> or <code>
   *     &gt;</code> the size of the map.
   */
   ...
   * @throws ArrayIndexOutOfBoundsException if the <code>index</code> is <code>&lt; 0</code> or
  ...
  /**
   * Returns a List view of the keys rather than a set view. The returned list is unmodifiable. This
   * is required because changes to the values of the list (using {@link
   * java.util.ListIterator#set(Object)}) will effectively remove the value from the list and
   * reinsert that value at the end of the list, which is an unexpected side effect of changing the
   * value of a list. This occurs because changing the key, changes when the mapping is added to the
   ...
   * @return The previous value corresponding the <code>key</code>, or <code>null</code> if none
   *     existed.
   * @throws ArrayIndexOutOfBoundsException if the <code>index</code> is <code>&lt; 0</code> or
   *     <code>&gt;</code> the size of the map.
   */
