 ...
 *
 * <p>The <code>PriorityQueue</code> interface has now been replaced for most uses by the <code>
 * Buffer</code> interface. This class and the interface are retained for backwards compatibility.
 * The intended replacement is {@link lets.core.collections.buffer.PriorityBuffer PriorityBuffer}.
 *
 * <p>The removal order of a binary heap is based on either the natural sort order of its elements
 * or a specified {@link Comparator}. The {@link #pop()} method always returns the first element as
 * determined by the sort order. (The <code>isMinHeap</code> flag in the constructors can be used to
 * reverse the sort order, in which case {@link #pop()} will always remove the last element.) The
 * removal order is <i>not</i> the same as the order of iteration; elements are returned by the
 * iterator in no particular order.
 *
 * <p>The {@link #insert(Object)} and {@link #pop()} operations perform in logarithmic time. The
 * {@link #peek()} operation performs in constant time. All other operations perform in linear time
 * or worse.
 *
 * <p>Note that this implementation is not synchronized. Use SynchronizedPriorityQueue to provide
...
public final class BinaryHeap extends AbstractCollection implements PriorityQueue, Buffer {
  ...
  /**
   * Constructs a new <code>BinaryHeap</code> that will use the given comparator to order its
   * elements.
   *
   ...
   *
   * @param capacity The initial capacity for the heap. This value must be greater than zero.
   ...
   *
   * @param isMinHeap if <code>true</code> the heap is created as a minimum heap; otherwise, the
   ...
   * Constructs a new <code>BinaryHeap</code>.
   *
   * @param isMinHeap true to use the order imposed by the given comparator; false to reverse that
   *     order
   * @param comparator the comparator used to order the elements, null means use natural order
   */
    ...
    return new Iterator() {
      ...
      public void remove() {
        ...
        if (m_size != 0 && lastReturnedIndex <= m_size) {
          ...
          if (lastReturnedIndex > 1) {
            compareToParent =
                compare(m_elements[lastReturnedIndex], m_elements[lastReturnedIndex / 2]);
          }
          if (m_isMinHeap) {
