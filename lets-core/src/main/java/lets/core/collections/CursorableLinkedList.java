...
/**
 * A doubly-linked list implementation of the {@link List} interface, supporting a {@link
 * ListIterator} that allows concurrent modifications to the underlying list.
 *
 * <p>Implements all of the optional {@link List} operations, the stack/queue/dequeue operations
 * available in {@link java.util.LinkedList} and supports a {@link ListIterator} that allows
 * concurrent modifications to the underlying list (see {@link #cursor}).
 *
 * <p><b>Note that this implementation is not synchronized.</b>
 *
 * @deprecated Use new version in list subpackage, which has been rewritten and now returns the
 *     cursor from the listIterator method. Will be removed in v4.0
...
public class CursorableLinkedList implements List, Serializable {
   ...
   * that they are returned by the specified {@link Collection}'s {@link Iterator}. The behavior of
   * this operation is unspecified if the specified collection is modified while the operation is in
   * progress. (Note that this will occur if the specified collection is this list, and it's
   * nonempty.)
   *
   * @param c collection whose elements are to be added to this list.
   * @return <tt>true</tt> if this list changed as a result of the call.
   * @throws ClassCastException if the class of an element in the specified collection prevents it
   *     from being added to this list.
   * @throws IllegalArgumentException if some aspect of an element in the specified collection
   ...
   * @return <tt>true</tt> if this list changed as a result of the call.
   * @throws ClassCastException if the class of one of elements of the specified collection prevents
  ...
  /**
   * Inserts the specified element at the beginning of this list. (Equivalent to {@link
   * #add(int,java.lang.Object) <tt>add(0,o)</tt>}).
   *
  ...
  /**
   * Inserts the specified element at the end of this list. (Equivalent to {@link
  ...
  public boolean contains(Object o) {
    for (Listable elt = _head.next(), past = null;
        null != elt && past != _head.prev();
        elt = (past = elt).next()) {
  ...
  /**
   * Returns a {@link ListIterator} for iterating through the elements of this list. Unlike {@link
   * #iterator}, a cursor is not bothered by concurrent modifications to the underlying list.
   *
   * <p>Specifically, when elements are added to the list before or after the cursor, the cursor
   * simply picks them up automatically. When the "current" (i.e., last returned by {@link
   * ListIterator#next} or {@link ListIterator#previous}) element of the list is removed, the cursor
   * automatically adjusts to the change (invalidating the last returned value--i.e., it cannot be
   * removed).
   *
   * <p>Note that the returned {@link ListIterator} does not support the {@link
   * ListIterator#nextIndex} and {@link ListIterator#previousIndex} methods (they throw {@link
   * UnsupportedOperationException} when invoked.
   *
   * <p>Historical Note: In previous versions of this class, the object returned from this method
   * was required to be explicitly closed. This is no longer necessary.
   *
   * @see #cursor(int)
   * @see #listIterator
  ...
  /**
   * Returns a {@link ListIterator} for iterating through the elements of this list, initialized
   * such that {@link ListIterator#next} will return the element at the specified index (if any) and
   * {@link ListIterator#previous} will return the element immediately preceding it (if any). Unlike
   * {@link #iterator}, a cursor is not bothered by concurrent modifications to the underlying list.
   *
   * @see #cursor
  ...
  /**
   * Compares the specified object with this list for equality. Returns <tt>true</tt> if and only if
   * the specified object is also a list, both lists have the same size, and all corresponding pairs
   * of elements in the two lists are <i>equal</i>. (Two elements <tt>e1</tt> and <tt>e2</tt> are
   * <i>equal</i> if <tt>(e1==null ? e2==null : e1.equals(e2))</tt>.) In other words, two lists are
   * defined to be equal if they contain the same elements in the same order. This definition
   * ensures that the equals method works properly across different implementations of the
  ...
  public boolean equals(Object o) {
    ...
    for (Listable elt = _head.next(), past = null;
        ...
        elt = (past = elt).next()) {
      if (!it.hasNext()
          || (null == elt.value() ? null != it.next() : !(elt.value().equals(it.next())))) {
        return false;
      }
    }
    return !it.hasNext();
   ...
   * <tt>list1.hashCode()==list2.hashCode()</tt> for any two lists, <tt>list1</tt> and
   * <tt>list2</tt>, as required by the general contract of <tt>Object.hashCode</tt>.
   *
   ...
   * list does not contain this element. More formally, returns the lowest index <tt>i</tt> such
   * that <tt>(o==null ? get(i)==null : o.equals(get(i)))</tt>, or -1 if there is no such index.
   *
   * @param o element to search for.
   * @return the index in this list of the first occurrence of the specified element, or -1 if this
   ...
   * list does not contain this element. More formally, returns the highest index <tt>i</tt> such
   * that <tt>(o==null ? get(i)==null : o.equals(get(i)))</tt>, or -1 if there is no such index.
   *
   * @param o element to search for.
   * @return the index in this list of the last occurrence of the specified element, or -1 if this
   *     list does not contain this element.
   */
  public int lastIndexOf(Object o) {
    ...
    // single time through the loop.
    if (null == o) {
      for (Listable elt = _head.prev(), past = null;
          null != elt && past != _head.next();
          elt = (past = elt).prev()) {
        if (null == elt.value()) {
          return ndx;
        }
  ...
  /**
   * Removes the first occurrence in this list of the specified element. If this list does not
   * contain the element, it is unchanged. More formally, removes the element with the lowest index
   * i such that <tt>(o==null ? get(i)==null : o.equals(get(i)))</tt> (if such an element exists).
   *
   ...
   * words, removes from this list all the elements that are not contained in the specified
   * collection.
   *
   ...
   * @exception ArrayStoreException if the runtime type of the specified array is not a supertype of
  ...
  public Object[] toArray(Object a[]) {
    ...
    if (a.length > _size) {
      a[_size] =
          null; // should we null out the rest of the array also? java.util.LinkedList doesn't
    }
    return a;
  }
  ...
  /**
   * Inserts a new <i>value</i> into my list, after the specified <i>before</i> element, and before
   * the specified <i>after</i> element
   *
   * @return the newly created {@link lets.core.collections.CursorableLinkedList.Listable}
   */
  ...
  /** Removes the given {@link lets.core.collections.CursorableLinkedList.Listable} from my list. */
  ...
  /**
   * Returns the {@link lets.core.collections.CursorableLinkedList.Listable} at the specified index.
   *
   * @throws IndexOutOfBoundsException if index is less than zero or greater than or equal to the
   *     size of this list.
   */
  ...
  /** Registers a {@link CursorableLinkedList.Cursor} to be notified of changes to this list. */
  ...
  /**
   * Removes a {@link CursorableLinkedList.Cursor} from the set of cursors to be notified of changes
   * to this list.
   */
   ...
   * A sentry node.
   *
   * <p><tt>_head.next()</tt> points to the first element in the list, <tt>_head.prev()</tt> to the
   * last. Note that it is possible for <tt>_head.next().prev()</tt> and
   * <tt>_head.prev().next()</tt> to be non-null, as when I am a sublist for some larger list. Use
   * <tt>== _head.next()</tt> and <tt>== _head.prev()</tt> to determine if a given {@link
   * lets.core.collections.CursorableLinkedList.Listable} is the first or last element in the list.
   */
  ...
  /** A list of the currently {@link CursorableLinkedList.Cursor}s currently open in this list. */
  ...
  public class Cursor extends ListIter implements ListIterator {
    ...
    /**
     * Mark this cursor as no longer being needed. Any resources associated with this cursor are
     * immediately released. In previous versions of this class, it was mandatory to close all
     * cursor objects to avoid memory leaks. It is <i>no longer</i> necessary to call this close
     * method; an instance of this class can now be treated exactly like a normal iterator.
     */
    public void close() {
      if (_valid) {
        _valid = false;
...
class CursorableSubList extends CursorableLinkedList implements List {
  ...
  protected Listable insertListable(Listable before, Listable after, Object value) {
    ...
    _size++;
    Listable elt =
        _list.insertListable(
            (null == before ? _pre : before), (null == after ? _post : after), value);
    if (null == _head.next()) {
      _head.setNext(elt);
      _head.setPrev(elt);
    }
  ...
  /**
   * Test to see if my underlying list has been modified by some other process. If it has, throws a
   * {@link ConcurrentModificationException}, otherwise quietly returns.
   *
