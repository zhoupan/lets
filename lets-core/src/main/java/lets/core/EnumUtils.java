...
public class EnumUtils {
   ...
   * @return {@code enumClass}
   * @throws NullPointerException if {@code enumClass} is {@code null}
   * @throws IllegalArgumentException if {@code enumClass} is not an enum class or has more than 64
   *     values
   * @since 3.0.1
   */
   ...
   *
   * <p>This generates a value that is usable by {@link EnumUtils#processBitVector}.
   *
   * <p>Do not use this method if you have more than 64 values in your Enum, as this would create a
   ...
   * @throws NullPointerException if {@code enumClass} or {@code values} is {@code null}
   * @throws IllegalArgumentException if {@code enumClass} is not an enum class or has more than 64
   *     values, or if any {@code values} {@code null}
   * @since 3.0.1
   * @see #generateBitVectors(Class, Iterable)
   */
  public static <E extends Enum<E>> long generateBitVector(
  ...
  /**
   * Creates a bit vector representation of the given subset of an Enum using as many {@code long}s
   * as needed.
   *
   * <p>This generates a value that is usable by {@link EnumUtils#processBitVectors}.
   *
   * <p>Use this method if you have more than 64 values in your Enum.
   *
   * @param enumClass the class of the enum we are working with, not {@code null}
   * @param values the values we want to convert, not {@code null}, neither containing {@code null}
   * @param <E> the type of the enumeration
   ...
   * @throws NullPointerException if {@code enumClass} or {@code values} is {@code null}
   * @throws IllegalArgumentException if {@code enumClass} is not an enum class, or if any {@code
   *     values} {@code null}
   * @since 3.2
   */
  @SafeVarargs
  public static <E extends Enum<E>> long[] generateBitVectors(
      final Class<E> enumClass, final E... values) {
    asEnum(enumClass);
    Validate.noNullElements(values);
   ...
   *
   * <p>This method differs from {@link Enum#valueOf} in that it does not throw an exception for an
   * invalid enum name.
   *
   * @param <E> the type of the enumeration
   ...
   *
   * <p>This method differs from {@link Enum#valueOf} in that it does not throw an exception for an
   * invalid enum name and performs case insensitive matching of the name.
   *
   * @param <E> the type of the enumeration
   * @param enumClass the class of the enum to query, not null
   * @param enumName the enum name, null returns null
   ...
   * @since 3.8
   */
  public static <E extends Enum<E>> E getEnumIgnoreCase(
   ...
   *
   * <p>This method is useful when you need a list of enums rather than an array.
   *
   * @param <E> the type of the enumeration
   * @param enumClass the class of the enum to query, not null
   ...
   *
   * <p>This method is useful when you need a map of enums by name.
   *
   * @param <E> the type of the enumeration
   * @param enumClass the class of the enum to query, not null
   ...
   *
   * <p>This method differs from {@link Enum#valueOf} in that checks if the name is a valid enum
   ...
   *
   * <p>This method differs from {@link Enum#valueOf} in that checks if the name is a valid enum
   * without needing to catch the exception and performs case insensitive matching of the name.
   *
   * @param <E> the type of the enumeration
   * @param enumClass the class of the enum to query, not null
   * @param enumName the enum name, null returns false
   ...
   * @since 3.8
   */
  public static <E extends Enum<E>> boolean isValidEnumIgnoreCase(
      final Class<E> enumClass, final String enumName) {
    return getEnumIgnoreCase(enumClass, enumName) != null;
  }

  /**
   * Convert a long value created by {@link EnumUtils#generateBitVector} into the set of enum values
   * that it represents.
   *
   * <p>If you store this value, beware any changes to the enum that would affect ordinal values.
   *
   * @param enumClass the class of the enum we are working with, not {@code null}
   ...
   * @since 3.0.1
   */
  public static <E extends Enum<E>> EnumSet<E> processBitVector(
      final Class<E> enumClass, final long value) {
   ...
   *
   * @param enumClass the class of the enum we are working with, not {@code null}
   * @param values the long[] bearing the representation of a set of enum values, least significant
   ...
   * @since 3.2
   */
  public static <E extends Enum<E>> EnumSet<E> processBitVectors(
      final Class<E> enumClass, final long... values) {
    final EnumSet<E> results = EnumSet.noneOf(asEnum(enumClass));
    final long[] lvalues = ArrayUtils.clone(Validate.notNull(values));
    ArrayUtils.reverse(lvalues);
    for (final E constant : enumClass.getEnumConstants()) {
      final int block = constant.ordinal() / Long.SIZE;
      if (block < lvalues.length
          && (lvalues[block] & 1L << (constant.ordinal() % Long.SIZE)) != 0) {
