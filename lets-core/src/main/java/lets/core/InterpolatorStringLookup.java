...
/**
 * Proxies other {@link StringLookup}s using a keys within ${} markers using the format
 * "${StringLookup:Key}".
 *
 * <p>Uses the {@link StringLookupFactory default lookups}.
 */
final class InterpolatorStringLookup extends AbstractStringLookup {
  ...
  /**
   * Constructs an instance using only lookups that work without initial properties and are
   * stateless.
   *
   * <p>Uses the {@link StringLookupFactory default lookups}.
   */
  ...
  InterpolatorStringLookup(
    ...
    this.stringLookupMap =
        stringLookupMap.entrySet().stream()
            .collect(Collectors.toMap(e -> StringLookupFactory.toKey(e.getKey()), Entry::getValue));
  ...
  /**
   * Resolves the specified variable. This implementation will try to extract a variable prefix from
   * the given variable name (the first colon (':') is used as prefix separator). It then passes the
   * name of the variable with the prefix stripped to the lookup object registered for this prefix.
   * If no prefix can be found or if the associated lookup object cannot resolve this variable, the
