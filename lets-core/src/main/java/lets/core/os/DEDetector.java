...
public class DEDetector {
  ...
  public static DesktopEnvironment detect() {
    ...
    if (os.getType() == OperatingSystem.WINDOWS) {
      ...
      if (wos.getVersion() == WindowsVersion.WIN7
          || wos.getVersion() == WindowsVersion.WIN8
          || wos.getVersion() == WindowsVersion.WIN81
          || wos.getVersion() == WindowsVersion.WINSERVER2012
          || wos.getVersion() == WindowsVersion.WINVISTA) {
      ...
      if (isSet("XDG_CURRENT_DESKTOP")) {
        ...
        try {
          version =
              Utils.readProcess(new String[] {"gnome-session-properties", "--version"})
                  .get(0)
                  .split(" ")[1];
        } catch (Exception ex) {
          ex.printStackTrace();
        ...
        if (version == null) {
          try {
            version =
                Utils.readProcess(new String[] {"gnome-session", "--version"}).get(0).split(" ")[1];
          } catch (Exception ex) {
            ex.printStackTrace();
          }
        }
        ...
        try {
          de.setVersion(
              Utils.readProcess(new String[] {"mate-session", "--version"}).get(0).split(" ")[0]);
        } catch (Exception e) {
          e.printStackTrace();
        }
        ...
        if (s.equals("budgie-desktop")) {
          ...
          try {
            de.setVersion(
                Utils.readProcess(new String[] {"cinnamon", "--version"}).get(0).split(" ")[0]);
          } catch (Exception e) {
            e.printStackTrace();
          }
        }
