...
abstract class AbstractStringMatcher implements StringMatcher {
  ...
  static final class AndStringMatcher extends AbstractStringMatcher {
     ...
     *
     * @param stringMatchers Matchers in order. Never null since the {@link StringMatcherFactory}
     *     uses the {@link NoneMatcher} instead.
     */
    ...
    @Override
    public int isMatch(
        final char[] buffer, final int start, final int bufferStart, final int bufferEnd) {
      int total = 0;
      int curStart = start;
  ...
  //                String.format("bufferStart(%,d) <= start(%,d) < bufferEnd(%,d)", bufferStart,
  // start, bufferEnd));
  //        }
  //    }
