...
/**
 * Controls {@code String} formatting for {@link ToStringBuilder}. The main public interface is
 * always via {@code ToStringBuilder}.
 *
 * <p>These classes are intended to be used as {@code Singletons}. There is no need to instantiate a
 * new style each time. A program will generally use one of the predefined constants on this class.
 * Alternatively, the {@link StandardToStringStyle} class can be used to set the individual
 ...
 * object type (from {@code boolean} to {@code long} to {@code Object} to {@code int[]}) has its own
...
public abstract class ToStringStyle implements Serializable {
  ...
  /**
   * The default toString style. Using the {@code Person} example from {@link ToStringBuilder}, the
  ...
  /**
   * The no field names toString style. Using the {@code Person} example from {@link
  ...
  /**
   * The short prefix toString style. Using the {@code Person} example from {@link ToStringBuilder},
   * the output would look like this:
   *
   * <pre>
  ...
  /**
   * The simple toString style. Using the {@code Person} example from {@link ToStringBuilder}, the
   * output would look like this:
   *
   * <pre>
   * John Doe,33,false
  ...
  /**
   * The no class name toString style. Using the {@code Person} example from {@link
   * ToStringBuilder}, the output would look like this:
   *
   * <pre>
  ...
  /**
   * The JSON toString style. Using the {@code Person} example from {@link ToStringBuilder}, the
   * output would look like this:
   *
   * <pre>
   ...
   * </pre>
   *
   * <strong>Note:</strong> Since field names are mandatory in JSON, this ToStringStyle will throw
   * an {@link UnsupportedOperationException} if no field name is passed in while appending.
   * Furthermore This ToStringStyle will only generate valid JSON if referenced objects also produce
   * JSON when calling {@code toString()} on them.
   *
   * @since 3.4
  ...
  /**
   * A registry of objects used by {@code reflectionToString} methods to detect cyclical object
   * references and avoid infinite loops.
   */
  private static final ThreadLocal<WeakHashMap<Object, Object>> REGISTRY = new ThreadLocal<>();

  /*
   * Note that objects of this class are generally shared between threads, so an instance variable would not be suitable
   * here.
   *
   * In normal use the registry should always be left empty, because the caller should call toString() which will clean
   * up.
   *
   * See LANG-792
   */
  ...
  /**
   * Returns the registry of objects being traversed by the {@code reflectionToString} methods in
   * the current thread.
   *
  ...
  /**
   * Returns {@code true} if the registry contains the given object. Used by the reflection methods
   ...
   *
   * <p>A {@code null} {@code superToString} is ignored.
   *
   ...
   * <p>NOTE: It assumes that the toString has been created from the same ToStringStyle.
   *
   * <p>A {@code null} {@code toString} is ignored.
   *
   * @param buffer the {@code StringBuffer} to populate
  ...
  /**
   * Append to the {@code toString} an {@code Object} value, printing the full {@code toString} of
   * the {@code Object} passed in.
   *
   * @param buffer the {@code StringBuffer} to populate
   * @param fieldName the field name
   * @param value the value to add to the {@code toString}
   * @param fullDetail {@code true} for detail, {@code false} for summary info, {@code null} for
   *     style decides
   */
  public void append(
   ...
   * Collections}, {@code Maps} and {@code Objects} to the appropriate method.
   *
   * <p>Either detail or summary views can be specified.
   *
   * <p>If a cycle is detected, an object will be appended with the {@code Object.toString()}
   * format.
   *
   * @param buffer the {@code StringBuffer} to populate
  ...
  protected void appendInternal(
      final StringBuffer buffer, final String fieldName, final Object value, final boolean detail) {
  ...
  /**
   * Append to the {@code toString} an {@code Object} value that has been detected to participate in
   * a cycle. This implementation will print the standard string value of the value.
   *
   * @param buffer the {@code StringBuffer} to populate
   * @param fieldName the field name, typically not used as already appended
   * @param value the value to add to the {@code toString}, not {@code null}
  ...
  /**
   * Append to the {@code toString} an {@code Object} value, printing the full detail of the {@code
   * Object}.
   *
   * @param buffer the {@code StringBuffer} to populate
   * @param fieldName the field name, typically not used as already appended
   ...
   * @param buffer the {@code StringBuffer} to populate
   * @param fieldName the field name, typically not used as already appended
   * @param coll the {@code Collection} to add to the {@code toString}, not {@code null}
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final Collection<?> coll) {
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final Map<?, ?> map) {
    buffer.append(map);
  }

  /**
   * Append to the {@code toString} an {@code Object} value, printing a summary of the {@code
   * Object}.
   *
   * @param buffer the {@code StringBuffer} to populate
   * @param fieldName the field name, typically not used as already appended
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final short value) {
    buffer.append(value);
  }
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final double value) {
    buffer.append(value);
  }
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final float value) {
    buffer.append(value);
  }
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final boolean value) {
    buffer.append(value);
  }
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final Object[] array) {
    buffer.append(arrayStart);
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final int i, final Object item) {
    if (i > 0) {
   ...
   * @since 2.0
   */
  protected void reflectionAppendArrayDetail(
      final StringBuffer buffer, final String fieldName, final Object array) {
    buffer.append(arrayStart);
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final long[] array) {
    buffer.append(arrayStart);
    for (int i = 0; i < array.length; i++) {
      if (i > 0) {
        buffer.append(arraySeparator);
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final int[] array) {
    buffer.append(arrayStart);
    for (int i = 0; i < array.length; i++) {
      if (i > 0) {
        buffer.append(arraySeparator);
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final short[] array) {
    buffer.append(arrayStart);
    for (int i = 0; i < array.length; i++) {
      if (i > 0) {
        buffer.append(arraySeparator);
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final byte[] array) {
    buffer.append(arrayStart);
    for (int i = 0; i < array.length; i++) {
      if (i > 0) {
        buffer.append(arraySeparator);
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final char[] array) {
    buffer.append(arrayStart);
    for (int i = 0; i < array.length; i++) {
      if (i > 0) {
        buffer.append(arraySeparator);
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final double[] array) {
    buffer.append(arrayStart);
    for (int i = 0; i < array.length; i++) {
      if (i > 0) {
        buffer.append(arraySeparator);
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final float[] array) {
    buffer.append(arrayStart);
    for (int i = 0; i < array.length; i++) {
      if (i > 0) {
        buffer.append(arraySeparator);
   ...
   */
  protected void appendDetail(
      final StringBuffer buffer, final String fieldName, final boolean[] array) {
    buffer.append(arrayStart);
    for (int i = 0; i < array.length; i++) {
      if (i > 0) {
        buffer.append(arraySeparator);
   ...
   *
   * <p>The default indicator is {@code '&lt;null&gt;'}.
   *
   * @param buffer the {@code StringBuffer} to populate
   * @param fieldName the field name, typically not used as already appended
   */
   ...
   *
   * <p>The size summary is used to summarize the contents of {@code Collections}, {@code Maps} and
   * arrays.
   *
   * <p>The output consists of a prefix, the passed in size and a suffix.
   *
   * <p>The default format is {@code '&lt;size=n&gt;'}.
   *
   * @param buffer the {@code StringBuffer} to populate
   * @param fieldName the field name, typically not used as already appended
   * @param size the size to append
   */
  protected void appendSummarySize(
      final StringBuffer buffer, final String fieldName, final int size) {
   ...
   * detail ({@code true}), but a subclass might ignore that and always return {@code false}. The
   * calling code may pass in {@code null} indicating that it doesn't care about the detail level.
   * In this case the default detail level is used.
   *
   ...
   *
   * <p>{@code null} is accepted, but will be converted to an empty String.
   *
   ...
   *
   * <p>This is an inner class rather than using {@code StandardToStringStyle} to ensure its
   * immutability.
   */
  ...
  /**
   * {@code ToStringStyle} that does not print out the classname, identity hashcode, content start
   * or field name.
   *
   * <p>This is an inner class rather than using {@code StandardToStringStyle} to ensure its
   * immutability.
  ...
  /**
   * {@code ToStringStyle} that does not print out the classname and identity hash code but prints
   * content start and field names.
   *
   * <p>This is an inner class rather than using {@code StandardToStringStyle} to ensure its
   * immutability.
   */
  ...
  private static final class JsonToStringStyle extends ToStringStyle {
    ...
    @Override
    protected void appendDetail(
        final StringBuffer buffer, final String fieldName, final char value) {
