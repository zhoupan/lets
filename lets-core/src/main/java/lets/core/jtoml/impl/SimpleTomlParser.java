 ...
 *
 */
package lets.core.jtoml.impl;

import java.util.*;
import java.util.regex.Matcher;
import java.util.regex.Pattern;
import lets.core.jtoml.TomlParser;
import lets.core.jtoml.Util;

/**
 * Builtin Toml parser.
 *
 * <p>Uses a first pass to make multi-line arrays one-liner, then iterate line by line, matching
 * against known regular expressions, to extract content and store it into a context map.
 *
...
public class SimpleTomlParser implements TomlParser {
  ...
  private static final String KEY_EQUALS =
      "(" + SPACES + "(\\w[a-zA-Z_0-9\\-]*)" + SPACES + "=" + SPACES + ")?";
  ...
  // Common patterns
  private static final Pattern ARRAY_LINE_PATTERN =
      Pattern.compile(KEY_EQUALS + ARRAY, Pattern.DOTALL);
  ...
  private static final Pattern ML_STRING_PATTERN =
      Pattern.compile("\"\"\"(.*?)\"\"\"", Pattern.DOTALL);
  private static final Pattern ML_LITERAL_STRING_PATTERN =
      Pattern.compile(ML_LITERAL_STRING, Pattern.DOTALL);
      ...
      new ArrayList<Handler>() {
        {
          ...
          // doubles
          add(
              new Handler(KEY_EQUALS + DOUBLE + SPACES + POSSIBLE_COMMENT) {
                Object cast(String v) {
          ...
          // longs
          add(
              new Handler(KEY_EQUALS + DIGITS + SPACES + POSSIBLE_COMMENT) {
                Object cast(String v) {
          ...
          // strings
          add(
              new Handler(KEY_EQUALS + STRING + SPACES) {
                Object cast(String v) {
          ...
          // literal strings
          add(
              new Handler(KEY_EQUALS + ML_LITERAL_STRING + SPACES) {
                Object cast(String v) {
                  return v.replaceAll(multiLineLiteralStringNewLine, "\n");
                }
              });
          add(
              new Handler(KEY_EQUALS + LITERAL_STRING + SPACES) {
                Object cast(String v) {
                  return v;
                }
              });
          // booleans
          add(
              new Handler(KEY_EQUALS + BOOLEAN + SPACES + POSSIBLE_COMMENT) {
                Object cast(String v) {
  ...
  /**
   * Find every arrays in the given String and make them one liner. prepareArrays('foo = [\n 1, 2,
   * 3,\n 4,\n 5, #this is ok\n ]') -> 'foo = [ 1, 2, 3, 4, 5 ]'
   *
  ...
  /**
   * Create the context if needed. createContextIfNeeded({}, "foo.bar.bazz") -> {"foo": {"bar":
   * {"bazz": {}}}}
   *
    ...
    for (String part : key.split("[.]")) {
      ...
      if (!(visitor.get(part) instanceof Map)) {
        throw new IllegalArgumentException( //
            "Overwriting a previous key is forbidden. Trying to overwrite key `"
                + key
                + "` having value `"
                + visitor.get(part)
                + "`");
  ...
  /**
   * Read the given line and returns an array of Object like the following: index 0: the key index
   * 1: the value
   *
  ...
  private Object[] readObject(String line) {
    ...
    if (arrayLineMatcher.reset(line).matches()) {
      ...
      if (values.size() > 0) {
        ...
        if (types.size() > 1) {
          throw new IllegalArgumentException(
              "Inconsistent types found while parsing array. "
                  + //
                  "Found all the following types in the same array declaration: "
                  + types);
        }
      }
