...
/**
 * This is the standard time based {@link Expiration}. It will invalidate objects based on about how
 * long ago they were allocated.
 *
...
public class TimeSpreadExpiration<T extends Poolable> implements Expiration<T> {
  ...
  /**
   * Construct a new Expiration that will invalidate objects that are older than the given lower
   * bound, before they get older than the upper bound, in the given time unit.
   *
   * <p>If the `lowerBound` is less than 1, the `upperBound` is less than the `lowerBound`, or the
   * `unit` is `null`, then an {@link java.lang.IllegalArgumentException} will be thrown.
   *
   * @param lowerBound Poolables younger than this, in the given unit, are not considered expired.
   *     This value must be at least 1.
   * @param upperBound Poolables older than this, in the given unit, are always considered expired.
   *     This value must be greater than the lowerBound.
  ...
  /**
   * Returns `true`, with uniformly increasing probability, if the {@link lets.core.pool.Poolable}
   * represented by the given {@link SlotInfo} is older than the lower bound, eventually returning
   * `true` with 100% certainty when the age of the Poolable is equal to or greater than the upper
   * bound.
   *
   * <p>The uniformity of the random expiration holds regardless of how often a Poolable is checked.
   * That is to say, checking a Poolable more times within an interval of time, does _not_ increase
   * its chances of being declared expired.
   */
  @Override
