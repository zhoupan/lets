...
/**
 * QueuePool is a fairly simple {@link LifecycledResizablePool} implementation that basically
 * consists of a queue of Poolable instances, and a Thread to allocate them.
 *
 * <p>This means that the object allocation always happens in a dedicated thread. This means that no
 * thread that calls any of the claim methods, will incur the overhead of allocating Poolables. This
 * should lead to reduced deviation in the times it takes claim method to complete, provided the
 * pool is not depleted.
 *
 * <p>The design is simple and straight forward, and exhibits a reasonable base-line performance in
 * all cases. If, however, the same threads are going to claim and release objects from the pool
 * over and over again — for instance in the case of a typical Java web application — then {@link
 * lets.core.pool.BlazePool} is likely going to yield better performance.
 *
...
public class QueuePool<T extends Poolable> implements LifecycledResizablePool<T>, ManagedPool {
  ...
  public QueuePool(Config<T> config) {
    ...
    synchronized (config) {
      ...
      allocator = new QAllocThread<T>(live, dead, config, poisonPill);
  ...
  private boolean isInvalid(QSlot<T> slot) {
    ...
    try {
      ...
      exception =
          new PoolException("Got exception when checking whether an object had expired", ex);
    }
    invalid |= expired;
    if (invalid) {
