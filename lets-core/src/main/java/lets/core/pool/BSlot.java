...
/**
 * This class is very sensitive to the memory layout, so be careful to measure the effect of even
 * the tiniest changes! False-sharing is a fickle and vengeful mistress.
 */
final class BSlot<T extends Poolable> extends BSlotColdFields<T> {
...
abstract class BSlotColdFields<T extends Poolable> extends Padding2 implements Slot, SlotInfo<T> {
  ...
  public BSlotColdFields(int state, BlockingQueue<BSlot<T>> live, AtomicInteger poisonedSlots) {
    super(state);
    this.live = live;
