...
final class BAllocThread<T extends Poolable> implements Runnable {
  /**
   * The amount of time, in nanoseconds, to wait for more work when the shutdown process has
   * deallocated all the dead and live slots it could get its hands on, but there are still
   * (claimed) slots left.
   */
