...
final class QueueFactory {
  ...
  static enum FactoryChoiceReason {
    DEFAULT_FIRST_CHOICE(
        "The "
            + QUEUE_IMPL_FIRST_CHOICE
            + " was available on the classpath,"
            + " and the "
            + QUEUE_IMPL
            + " system property did not specify any alternative"
            + " implementation to use."),
    SPECIFIED_CHOICE(
        "The "
            + QUEUE_IMPL
            + " system property specified the queue implementation"
            + " to use, specifically "
            + System.getProperty(QUEUE_IMPL)
            + "."),
    DEFAULT_SECOND_CHOICE(
        "The "
            + QUEUE_IMPL_FIRST_CHOICE
            + " was not available on the"
            + " classpath, and the "
            + QUEUE_IMPL
            + " system property did not specify any"
            + " alternative implementation to use, so "
            + QUEUE_IMPL_SECOND_CHOICE
            + " was used"
  ...
  static {
    ...
    try {
          ...
          new Factory() {
            ...
            public <T> BlockingQueue<T> createUnboundedBlockingQueue() {
              try {
                ...
                Exception exception =
                    new Exception("WARNING: Expected class to be constructable: " + constructor, e);
      ...
      if (theReason == FactoryChoiceReason.SPECIFIED_CHOICE) {
        Exception exception =
            new Exception(
                "WARNING: The queue implementation specified by the "
                    + QUEUE_IMPL
                    + " system"
                    + " property, specifically "
                    + queueType
                    + ", was determined to be unusable."
                    + " Falling back to the "
                    + QUEUE_IMPL_SECOND_CHOICE
                    + " implementation.",
                e);
    ...
    if (theFactory == null) {
      ...
      theReason =
          theReason == FactoryChoiceReason.DEFAULT_FIRST_CHOICE
              ? FactoryChoiceReason.DEFAULT_SECOND_CHOICE
              : FactoryChoiceReason.DEFAULT_FALLBACK;
    }
