 ...
 *
 * <p>Given two {@link Expiration}s, this class considers that a slot is expired if any of the
 * {@link Expiration} returns {@code true}. This makes it easy to have an {@link Expiration} that
 * expires both on time ({@link lets.core.pool.TimeExpiration}) and some other criteria.
 *
...
public class CompoundExpiration<T extends Poolable> implements Expiration<T> {
  ...
  public boolean hasExpired(SlotInfo<? extends T> info) throws Exception {
    return firstExpiration.hasExpired(info) || secondExpiration.hasExpired(info);
  }
}
