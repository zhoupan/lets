...
/**
 * This is a time based {@link Expiration}. It will invalidate objects based on how long ago they
 * were allocated.
 *
...
public class TimeExpiration<T extends Poolable> implements Expiration<T> {
  ...
  /**
   * Construct a new Expiration that will invalidate objects that are older than the provided span
   * of time in the given unit.
   *
   * <p>If the `maxPermittedAge` is less than one, or the `unit` is `null`, then an {@link
   * IllegalArgumentException} will be thrown.
   *
   * @param maxPermittedAge Poolables older than this, in the given unit, will be considered
   *     expired. This value must be at least 1.
  ...
  /**
   * Returns `true` if the {@link Poolable} represented by the given {@link SlotInfo} is older than
   * the maximum age permitted by this TimeExpiration.
   */
  @Override
