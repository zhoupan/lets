 ...
 *
 * <p>It is consistent with {@code equals(Object)} and {@code hashcode()} built with {@link
 * EqualsBuilder} and {@link HashCodeBuilder}.
 *
 * <p>Two Objects that compare equal using {@code equals(Object)} should normally also compare equal
 * using {@code compareTo(Object)}.
 *
 * <p>All relevant fields should be included in the calculation of the comparison. Derived fields
 * may be ignored. The same fields, in the same order, should be used in both {@code
 * compareTo(Object)} and {@code equals(Object)}.
 *
 ...
 * </pre>
 *
 * <p>Values are compared in the order they are appended to the builder. If any comparison returns a
 * non-zero result, then that value will be the result returned by {@code toComparison()} and all
 * subsequent comparisons are skipped.
 *
 * <p>Alternatively, there are {@link #reflectionCompare(Object, Object) reflectionCompare} methods
 * that use reflection to determine the fields to append. Because fields can be private, {@code
 * reflectionCompare} uses {@link java.lang.reflect.AccessibleObject#setAccessible(boolean)} to
 * bypass normal access control checks. This will fail under a security manager, unless the
 * appropriate permissions are set up correctly. It is also slower than appending explicitly.
 *
 * <p>A typical implementation of {@code compareTo(Object)} using {@code reflectionCompare} looks
 * like:
 *
 * <pre>
 * public int compareTo(Object o) {
 ...
 * </pre>
 *
 * <p>The reflective methods compare object fields in the order returned by {@link
 * Class#getDeclaredFields()}. The fields of the class are compared first, followed by those of its
 * parent classes (in order from the bottom to the top of the class hierarchy).
 *
...
public class CompareToBuilder implements Builder<Integer> {
   ...
   *
   * <p>Starts off assuming that the objects are equal. Multiple calls are then made to the various
   ...
   *
   * <p>Fields can be private, thus {@code AccessibleObject.setAccessible} is used to bypass normal
   * access control checks. This will fail under a security manager unless the appropriate
   * permissions are set.
   *
   * <ul>
   *   <li>Static fields will not be compared
   *   <li>Transient members will be not be compared, as they are likely derived fields
   *   <li>Superclass fields will be compared
   * </ul>
   *
   * <p>If both {@code lhs} and {@code rhs} are {@code null}, they are considered equal.
   *
   * @param lhs left-hand object
   * @param rhs right-hand object
   * @return a negative integer, zero, or a positive integer as {@code lhs} is less than, equal to,
   *     or greater than {@code rhs}
   ...
   * <ul>
   *   <li>Static fields will not be compared
   *   <li>If {@code compareTransients} is {@code true}, compares transient members. Otherwise
   *       ignores them, as they are likely derived fields.
   *   <li>Superclass fields will be compared
   * </ul>
   *
   * <p>If both {@code lhs} and {@code rhs} are {@code null}, they are considered equal.
   ...
   * @return a negative integer, zero, or a positive integer as {@code lhs} is less than, equal to,
   *     or greater than {@code rhs}
   * @throws NullPointerException if either {@code lhs} or {@code rhs} (but not both) is {@code
   *     null}
   * @throws ClassCastException if {@code rhs} is not assignment-compatible with {@code lhs}
   */
  public static int reflectionCompare(
      final Object lhs, final Object rhs, final boolean compareTransients) {
   ...
   */
  public static int reflectionCompare(
      final Object lhs, final Object rhs, final Collection<String> excludeFields) {
    return reflectionCompare(
        lhs, rhs, ReflectionToStringBuilder.toNoNullStringArray(excludeFields));
  }
   ...
   * <ul>
   *   <li>Static fields will not be compared
   *   <li>If the {@code compareTransients} is {@code true}, compares transient members. Otherwise
   *       ignores them, as they are likely derived fields.
   *   <li>Compares superclass fields up to and including {@code reflectUpToClass}. If {@code
   *       reflectUpToClass} is {@code null}, compares all superclass fields.
   * </ul>
   *
   * <p>If both {@code lhs} and {@code rhs} are {@code null}, they are considered equal.
   *
  ...
  /**
   * Appends to {@code builder} the comparison of {@code lhs} to {@code rhs} using the fields
   ...
   *
   * <ol>
   *   <li>Check if {@code lhs == rhs}
   *   <li>Check if either {@code lhs} or {@code rhs} is {@code null}, a {@code null} object is less
   *       than a non-{@code null} object
   *   <li>Check the object contents
   * </ol>
   *
   * <p>{@code lhs} must either be an array or implement {@link Comparable}.
   *
   * @param lhs left-hand object
   * @param rhs right-hand object
   * @return this - used to chain append calls
   ...
   * </ol>
   *
   * <p>If {@code lhs} is an array, array comparison methods will be used. Otherwise {@code
   * comparator} will be used to compare the objects. If {@code comparator} is {@code null}, {@code
   * lhs} must implement {@link Comparable} instead.
   *
   * @param lhs left-hand object
   * @param rhs right-hand object
   * @param comparator {@code Comparator} used to compare the objects, {@code null} means treat lhs
   *     as {@code Comparable}
   * @return this - used to chain append calls
   * @throws ClassCastException if {@code rhs} is not assignment-compatible with {@code lhs}
   * @since 2.0
   */
  public CompareToBuilder append(
      final Object lhs, final Object rhs, final Comparator<?> comparator) {
    if (comparison != 0) {
      return this;
    }
    if (lhs == rhs) {
   ...
   *
   * <ol>
   *   <li>Check if arrays are the same using {@code ==}
   *   <li>Check if for {@code null}, {@code null} is less than non-{@code null}
   ...
   * </ol>
   *
   * <p>This method will also will be called for the top level of multi-dimensional, ragged, and
   * multi-typed arrays.
   *
   ...
   * @param lhs left-hand array
   * @param rhs right-hand array
   * @param comparator {@code Comparator} to use to compare the array elements, {@code null} means
   *     to treat {@code lhs} elements as {@code Comparable}.
   * @return this - used to chain append calls
   * @throws ClassCastException if {@code rhs} is not assignment-compatible with {@code lhs}
   * @since 2.0
   */
