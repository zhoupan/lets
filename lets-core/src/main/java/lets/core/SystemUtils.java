...
public class SystemUtils {
  ...
  /** The Constant JAVA_SPECIFICATION_VERSION_AS_ENUM. */
  ...
  public static String getEnvironmentVariable(final String name, final String defaultValue) {
    try {
      CoreUtils.checkState(CoreUtils.isNotBlank(name), "name not allow blank.");
      String value = System.getenv(name);
      if (value == null) {
        value = CoreUtils.capitalize(name.toLowerCase());
      }
      if (value == null) {
  ...
  public static File getUserHome() {
    // for android
    // java.io.tmpdir=/data/user/0/<com.company.app>/cache,user.home=}
    // so return tmpdir.parentFile
    if (isAndroid()) {
      File tmpdir = SystemUtils.getJavaIoTmpDir();
      LoggerSupport.info("UserHome: {}", tmpdir.getParentFile());
      return tmpdir.getParentFile();
    }
  ...
  public static boolean isAndroid() {
    // http.agent=Dalvik/2.1.0 (Linux; U; Android 7.0; Android SDK built for x86 Build/NYC)
    String httpAgent = SystemUtils.getSystemProperty("http.agent");
    return StringUtils.containsIgnoreCase(httpAgent, "Android");
  }
   ...
   */
  public static DataResultSupport<List<String>> getEnvironmentVariables(
      String name, String separator, List<String> prefers) {
    DataResultSupport<List<String>> result = new DataResultSupport<List<String>>();
    List<String> valueList = new ArrayList<String>(prefers);
    result.setData(valueList);
    try {
      String values = getEnvironmentVariable(name, "");
      String[] valueArray = CoreUtils.split(values, separator);
      for (String value : valueArray) {
        if (CoreUtils.isNotBlank(value)) {
   ...
   * @return the paths
   */
  public static DataResultSupport<List<String>> getPaths() {
    return getPaths(Arrays.asList("."));
  }
   ...
   * @return the paths
   */
  public static DataResultSupport<List<String>> getPaths(List<String> prefers) {
    return getEnvironmentVariables("PATH", File.pathSeparator, prefers);
  }

  /**
   * Gets the path exts.
   *
   * @return the path exts
   */
  public static DataResultSupport<List<String>> getPathExts() {
    return getPathExts(Arrays.asList(""));
  }
   ...
   * @return the path exts
   */
  public static DataResultSupport<List<String>> getPathExts(List<String> prefers) {
    return getEnvironmentVariables("PATHEXT", File.pathSeparator, prefers);
  }
   ...
   */
  public static DataResultSupport<List<String>> which(String command) {
    return which(command, Arrays.asList("."), Arrays.asList(""));
  }
   ...
   *
   * @param command the command
   * @param pathPrefers the path prefers
   * @return the data result support
   */
  public static DataResultSupport<List<String>> which(String command, List<String> pathPrefers) {
    return which(command, pathPrefers, Arrays.asList(""));
  }
   ...
   * @param command the command
   * @param pathPrefers the path prefers
   * @param pathExtPrefers the path ext prefers
   * @return the data result support
   */
  public static DataResultSupport<List<String>> which(
      String command, List<String> pathPrefers, List<String> pathExtPrefers) {
    DataResultSupport<List<String>> result = new DataResultSupport<List<String>>();
    List<String> whichList = new ArrayList<String>();
    try {
      // 读取环境变量
      DataResultSupport<List<String>> paths = getPaths(pathPrefers);
      CoreUtils.checkState(paths.isPresent(), "paths are not present.");
      List<String> pathList = paths.getData();
      List<String> pathExtList = new ArrayList<String>();
      // 读取可执行文件扩展名: PATHEXT=.COM;.EXE;.BAT;.CMD;
      if (SystemUtils.IS_OS_WINDOWS) {
        DataResultSupport<List<String>> pathExts = getPathExts(pathExtPrefers);
        if (pathExts.isPresent()) {
          pathExtList.addAll(pathExts.getData());
        }
      }
      // 遍历路径环境变量,查找可执行的命令
      for (String path : pathList) {
        ...
        if (file.exists()) {
          if (!whichList.contains(file.getAbsolutePath())) {
            whichList.add(file.getAbsolutePath());
          }
        }
        // 带扩展名 PATHEXT=.COM;.EXE;.BAT;.CMD;
        for (String pathExt : pathExtList) {
          file = new File(path, String.format("%s%s", command, pathExt));
          if (file.exists()) {
            if (!whichList.contains(file.getAbsolutePath())) {
              whichList.add(file.getAbsolutePath());
            }
