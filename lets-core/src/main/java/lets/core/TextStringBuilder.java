...
/**
 * Builds a string from constituent parts providing a more flexible and powerful API than {@link
 * StringBuffer} and {@link StringBuilder}.
 *
 * <p>The main differences from StringBuffer/StringBuilder are:
 *
 * <ul>
 *   <li>Not synchronized
 *   <li>Not final
 *   <li>Subclasses have direct access to character array
 *   <li>Additional methods
 *       <ul>
 *         <li>appendWithSeparators - adds an array of values, with a separator
 *         <li>appendPadding - adds a length padding characters
 ...
 *         <li>delete - delete char or string
 *         <li>replace - search and replace for a char or string
 *         <li>leftString/rightString/midString - substring without exceptions
 *         <li>contains - whether the builder contains a char or string
 *         <li>size/clear/isEmpty - collections style API methods
 *       </ul>
 *   <li>Views
 *       <ul>
 ...
 * </ul>
 *
 * <p>The aim has been to provide an API that mimics very closely what StringBuffer provides, but
 * with additional methods. It should be noted that some edge cases, with invalid indices or null
 * input, have been altered - see individual methods. The biggest of these changes is that by
 * default, null will not output the text 'null'. This can be controlled by a property, {@link
 * #setNullText(String)}.
 *
 * <p>This class is called {@code TextStringBuilder} instead of {@code StringBuilder} to avoid
 * clashing with {@link StringBuilder}.
 *
 * @since 1.3
 */
public class TextStringBuilder implements CharSequence, Appendable, Serializable, Builder<String> {
   ...
   *
   * <p>This is set to the same size used in the JDK {@link java.util.ArrayList}:
   *
   * <blockquote>
   *
  ...
  /**
   * Creates a positive capacity at least as large the minimum required capacity. If the minimum
  ...
  /**
   * Constructs an instance from a reference to a character array. Changes to the input chars are
   * reflected in this instance until the internal buffer needs to be reallocated. Using a reference
   * to an array allows the instance to be initialized without copying the input array.
   *
  ...
  private TextStringBuilder(final char[] initialBuffer, final int length) {
    ...
    if (length < 0 || length > initialBuffer.length) {
      throw new IllegalArgumentException(
          "initialBuffer.length=" + initialBuffer.length + ", length=" + length);
    }
    this.size = length;
  }
   ...
   * @return this, to enable chaining
   * @throws StringIndexOutOfBoundsException if {@code startIndex} is not in the range {@code 0 <=
  ...
  /**
   * Appends the contents of a char buffer to this string builder. Appending null will call {@link
   * #appendNull()}.
   *
  ...
  public TextStringBuilder append(final CharBuffer buf, final int startIndex, final int length) {
    ...
    if (buf.hasArray()) {
      ...
      ensureCapacityInternal(len + length);
      System.arraycopy(
          buf.array(), buf.arrayOffset() + buf.position() + startIndex, buffer, len, length);
      size += length;
    } else {
  ...
  /**
   * Appends part of a CharSequence to this string builder. Appending null will call {@link
   * #appendNull()}.
   *
   * @param seq the CharSequence to append
   * @param startIndex the start index, inclusive, must be valid
   ...
   */
  @Override
  public TextStringBuilder append(
      final CharSequence seq, final int startIndex, final int endIndex) {
    if (seq == null) {
      return appendNull();
    }
    if (endIndex <= 0) {
  ...
  /**
   * Appends part of a string to this string builder. Appending null will call {@link
   * #appendNull()}.
   *
   * @param str the string to append
   * @param startIndex the start index, inclusive, must be valid
  ...
  /**
   * Appends part of a string buffer to this string builder. Appending null will call {@link
   * #appendNull()}.
   *
   * @param str the string to append
   * @param startIndex the start index, inclusive, must be valid
  ...
  /**
   * Appends part of a StringBuilder to this string builder. Appending null will call {@link
   * #appendNull()}.
   *
   * @param str the StringBuilder to append
   * @param startIndex the start index, inclusive, must be valid
  ...
  /**
   * Appends another string builder to this string builder. Appending null will call {@link
   * #appendNull()}.
   *
  ...
  /**
   * Appends part of a string builder to this string builder. Appending null will call {@link
   * #appendNull()}.
   *
   * @param str the string to append
   * @param startIndex the start index, inclusive, must be valid
   ...
   */
  public TextStringBuilder append(
      final TextStringBuilder str, final int startIndex, final int length) {
    if (str == null) {
      return appendNull();
    }
    if (startIndex < 0 || startIndex > str.length()) {
  ...
  /**
   * Appends each item in an iterable to the builder without any separators. Appending a null
   * iterable will have no effect. Each object is appended using {@link #append(Object)}.
   *
  ...
  /**
   * Appends each item in an iterator to the builder without any separators. Appending a null
   * iterator will have no effect. Each object is appended using {@link #append(Object)}.
   *
  ...
  /**
   * Appends each item in an array to the builder without any separators. Appending a null array
   * will have no effect. Each object is appended using {@link #append(Object)}.
   *
  ...
  /**
   * Appends an object to the builder padding on the left to a fixed width. The {@code
   * String.valueOf} of the {@code int} value is used. If the formatted value is larger than the
   * length, the left hand side is lost.
   *
   * @param value the value to append
   ...
   * @return this, to enable chaining
   */
  public TextStringBuilder appendFixedWidthPadLeft(
      final int value, final int width, final char padChar) {
    return appendFixedWidthPadLeft(String.valueOf(value), width, padChar);
  }

  /**
   * Appends an object to the builder padding on the left to a fixed width. The {@code toString} of
   * the object is used. If the object is larger than the length, the left hand side is lost. If the
   * object is null, the null text value is used.
   *
   ...
   */
  public TextStringBuilder appendFixedWidthPadLeft(
      final Object obj, final int width, final char padChar) {
    if (width > 0) {
  ...
  /**
   * Appends an object to the builder padding on the right to a fixed length. The {@code
   * String.valueOf} of the {@code int} value is used. If the object is larger than the length, the
   * right hand side is lost.
   *
   * @param value the value to append
   * @param width the fixed field width, zero or negative has no effect
   * @param padChar the pad character to use
   * @return this, to enable chaining
   */
  public TextStringBuilder appendFixedWidthPadRight(
      final int value, final int width, final char padChar) {
    return appendFixedWidthPadRight(String.valueOf(value), width, padChar);
  }

  /**
   * Appends an object to the builder padding on the right to a fixed length. The {@code toString}
   * of the object is used. If the object is larger than the length, the right hand side is lost. If
   * the object is null, null text value is used.
   *
   * @param obj the object to append, null uses null text
   * @param width the fixed field width, zero or negative has no effect
   * @param padChar the pad character to use
  ...
  /**
   * Appends a double value followed by a new line to the string builder using {@code
   * String.valueOf}.
   *
   * @param value the value to append
   * @return this, to enable chaining
  ...
  /**
   * Appends a float value followed by a new line to the string builder using {@code
   * String.valueOf}.
   *
   * @param value the value to append
   * @return this, to enable chaining
  ...
  /**
   * Appends an object followed by a new line to this string builder. Appending null will call
   * {@link #appendNull()}.
   *
   * @param obj the object to append
   * @return this, to enable chaining
  ...
  /**
   * Appends a string followed by a new line to this string builder. Appending null will call {@link
   * #appendNull()}.
   *
   * @param str the string to append
   * @return this, to enable chaining
  ...
  /**
   * Appends part of a string followed by a new line to this string builder. Appending null will
  ...
  /**
   * Appends part of a string buffer followed by a new line to this string builder. Appending null
   ...
   * @return this, to enable chaining
   */
  public TextStringBuilder appendln(
      final StringBuffer str, final int startIndex, final int length) {
    return append(str, startIndex, length).appendNewLine();
  }

  /**
   * Appends a string builder followed by a new line to this string builder. Appending null will
   * call {@link #appendNull()}.
   *
   * @param str the string builder to append
   * @return this, to enable chaining
  ...
  /**
   * Appends part of a string builder followed by a new line to this string builder. Appending null
   * will call {@link #appendNull()}.
   *
   * @param str the string builder to append
   * @param startIndex the start index, inclusive, must be valid
   ...
   */
  public TextStringBuilder appendln(
      final StringBuilder str, final int startIndex, final int length) {
    return append(str, startIndex, length).appendNewLine();
  }

  /**
   * Appends another string builder followed by a new line to this string builder. Appending null
   * will call {@link #appendNull()}.
   *
   * @param str the string builder to append
   * @return this, to enable chaining
   ...
   *
   * <p>The new line string can be altered using {@link #setNewLineText(String)}. This might be used
   * to force the output to always use UNIX line endings even when on Windows.
   *
   * @return this, to enable chaining
   */
  ...
  /**
   * Appends a separator if the builder is currently non-empty. The separator is appended using
   * {@link #append(char)}.
   *
   * <p>This method is useful for adding a separator each time around the loop except the first.
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>Note that for this simple example, you should use {@link #appendWithSeparators(Iterable,
   * String)}.
   *
  ...
  /**
   * Appends one of both separators to the builder If the builder is currently empty it will append
   * the defaultIfEmpty-separator Otherwise it will append the standard-separator
   *
   * <p>The separator is appended using {@link #append(char)}.
   *
  ...
  /**
   * Appends a separator to the builder if the loop index is greater than zero. The separator is
   * appended using {@link #append(char)}.
   *
   * <p>This method is useful for adding a separator each time around the loop except the first.
   *
   * <pre>
  ...
  /**
   * Appends a separator if the builder is currently non-empty. Appending a null separator will have
  ...
  /**
   * Appends a separator to the builder if the loop index is greater than zero. Appending a null
   * separator will have no effect. The separator is appended using {@link #append(String)}.
   *
   * <p>This method is useful for adding a separator each time around the loop except the first.
   *
   * <pre>
  ...
  /**
   * Appends one of both separators to the StrBuilder. If the builder is currently empty, it will
   * append the defaultIfEmpty-separator, otherwise it will append the standard-separator.
   *
   * <p>Appending a null separator will have no effect. The separator is appended using {@link
   * #append(String)}.
   *
   * <p>This method is for example useful for constructing queries
   *
   * <pre>
   ...
   *
   * <p>This method tries to avoid doing any extra copies of contents.
   *
  ...
  /**
   * Appends an iterable placing separators between each value, but not before the first or after
   * the last. Appending a null iterable will have no effect. Each object is appended using {@link
   * #append(Object)}.
   *
   * @param iterable the iterable to append
   * @param separator the separator to use, null means no separator
   * @return this, to enable chaining
   */
  public TextStringBuilder appendWithSeparators(
      final Iterable<?> iterable, final String separator) {
    if (iterable != null) {
  ...
  /**
   * Appends an iterator placing separators between each value, but not before the first or after
   * the last. Appending a null iterator will have no effect. Each object is appended using {@link
   * #append(Object)}.
   *
   * @param it the iterator to append
   * @param separator the separator to use, null means no separator
  ...
  /**
   * Appends an array placing separators between each value, but not before the first or after the
   * last. Appending a null array will have no effect. Each object is appended using {@link
   * #append(Object)}.
   *
   * @param array the array to append
   * @param separator the separator to use, null means no separator
   ...
   *
   * <p>This method allows the contents of the builder to be read using any standard method that
   * expects a Reader.
   *
   * <p>To use, simply create a {@code StrBuilder}, populate it with data, call {@code asReader},
   * and then read away.
   *
   * <p>The internal character array is shared between the builder and the reader. This allows you
   * to append to the builder after creating the reader, and the changes will be picked up. Note
   * however, that no synchronization occurs, so you must perform all operations with the builder
   * and the reader in one thread.
   *
   * <p>The returned reader supports marking, and ignores the flush method.
   *
   ...
   *
   * <p>This method allows the contents of this builder to be tokenized. The tokenizer will be setup
   * by default to tokenize on space, tab, newline and form feed (as per StringTokenizer). These
   * values can be changed on the tokenizer class, before retrieving the tokens.
   *
   * <p>The returned tokenizer is linked to this builder. You may intermix calls to the builder and
   * tokenizer within certain limits, however there is no synchronization. Once the tokenizer has
   * been used once, it must be {@link StringTokenizer#reset() reset} to pickup the latest changes
   ...
   * </pre>
   *
   * <p>In addition to simply intermixing appends and tokenization, you can also call the set
   * methods on the tokenizer to alter how it tokenizes. Just remember to call reset when you want
   * to pickup builder changes.
   *
   * <p>Calling {@link StringTokenizer#reset(String)} or {@link StringTokenizer#reset(char[])} with
   * a non-null value will break the link with the builder.
   *
   ...
   *
   * <p>This method allows you to populate the contents of the builder using any standard method
   * that takes a Writer.
   *
   * <p>To use, simply create a {@code StrBuilder}, call {@code asWriter}, and populate away. The
   * data is available at any time using the methods of the {@code StrBuilder}.
   *
   * <p>The internal character array is shared between the builder and the writer. This allows you
   * to intermix calls that append to the builder and write using the writer and the changes will be
   * occur correctly. Note however, that no synchronization occurs, so you must perform all
   * operations with the builder and the writer in one thread.
   *
   * <p>The returned writer ignores the close and flush methods.
   *
   ...
   *
   * <p>This method does not reduce the size of the internal character buffer. To do that, call
   * {@code clear()} followed by {@link #minimizeCapacity()}.
   *
   * <p>This method is the same as {@link #setLength(int)} called with zero and is provided to match
   ...
   *
   * <p>Matchers can be used to perform advanced searching behavior. For example you could write a
   * matcher to search for the character 'a' followed by a number.
   *
   ...
   *
   * @param startIndex the start index, inclusive, must be valid
   * @param endIndex the end index, exclusive, must be valid except that if too large it is treated
   *     as end of string
   * @return this, to enable chaining
   * @throws IndexOutOfBoundsException if the index is invalid
   */
   ...
   *
   * <p>Matchers can be used to perform advanced deletion behavior. For example you could write a
   * matcher to delete all occurrences where the character 'a' is followed by a number.
   *
   ...
   *
   * <p>Matchers can be used to perform advanced deletion behavior. For example you could write a
   * matcher to delete where the character 'a' is followed by a number.
   *
   * @param matcher the matcher to use to find the deletion, null causes no action
   * @return this, to enable chaining
   */
  ...
  /**
   * Drains (copies, then deletes) this character sequence into the specified array. This is
   * equivalent to copying the characters from this sequence into the target and then deleting those
   * character from this sequence.
   *
   ...
   * @return How many characters where copied (then deleted). If this builder is empty, return
   *     {@code 0}.
   * @since 1.9
   */
  public int drainChars(
      final int startIndex, final int endIndex, final char[] target, final int targetIndex) {
   ...
   *
   * <p>Note that this method handles null input quietly, unlike String.
   *
   ...
   *
   * <p>Note: This method can be used to minimise memory reallocations during repeated addition of
   * values by pre-allocating the character buffer. The method ignores a negative {@code capacity}
   * argument.
   *
  ...
  /**
   * Ensures that the buffer is at least the size specified. The {@code capacity} argument is
   * treated as an unsigned integer.
   *
   * <p>This method will raise an {@link OutOfMemoryError} if the capacity is too large for an
   * array, or cannot be allocated.
   *
   * @param capacity the capacity to ensure
   * @throws OutOfMemoryError if the capacity cannot be allocated
   */
  ...
  /**
   * Tests the contents of this builder against another to see if they contain the same character
   * content.
   *
   ...
   *
   * <p>Note that a null input string will return -1, whereas the JDK throws an exception.
   *
  ...
  /**
   * Searches the string builder to find the first reference to the specified string starting
   ...
   *
   * <p>Matchers can be used to perform advanced searching behavior. For example you could write a
   * matcher to find the character 'a' followed by a number.
   *
   * @param matcher the matcher to use, null returns -1
  ...
  /**
   * Searches the string builder using the matcher to find the first match searching from the given
   * index.
   *
   * <p>Matchers can be used to perform advanced searching behavior. For example you could write a
   * matcher to find the character 'a' followed by a number.
  ...
  /**
   * Inserts the character array into this builder. Inserting null will use the stored null text
   * value.
   *
   * @param index the index to add at, must be valid
  ...
  /**
   * Inserts part of the character array into this builder. Inserting null will use the stored null
   * text value.
   *
   * @param index the index to add at, must be valid
   * @param chars the char array to insert
   ...
   * @throws IndexOutOfBoundsException if any index is invalid
   */
  public TextStringBuilder insert(
  ...
  /**
   * Inserts the string representation of an object into this builder. Inserting null will use the
   * stored null text value.
   *
   * @param index the index to add at, must be valid
   ...
   *
   * <p>This method is the same as checking {@link #length()} and is provided to match the API of
   * Collections.
   *
   ...
   *
   * <p>This method is the same as checking {@link #length()}.
   *
  ...
  /**
   * Searches the string builder to find the last reference to the specified string starting
   * searching from the given index.
   *
   * <p>Note that a null input string will return -1, whereas the JDK throws an exception.
   *
  ...
  /**
   * Searches the string builder using the matcher to find the last match searching from the given
   * index.
   *
   * <p>Matchers can be used to perform advanced searching behavior. For example you could write a
   * matcher to find the character 'a' followed by a number.
   ...
   *
   * <p>This method extracts the left {@code length} characters from the builder. If this many
   * characters are not available, the whole builder is returned. Thus the returned string may be
   ...
   *
   * <p>This method extracts {@code length} characters from the builder at the specified index. If
   * the index is negative it is treated as zero. If the index is greater than the builder size, it
   * is treated as the builder size. If the length is negative, the empty string is returned. If
   * insufficient characters are available in the builder, as much as possible is returned. Thus the
   * returned string may be shorter than the length requested.
   *
  ...
  /**
   * If possible, reads chars from the provided {@link CharBuffer} directly into underlying
  ...
  /**
   * If possible, reads all chars from the provided {@link Readable} directly into underlying
   * character buffer without making extra copies.
   *
  ...
  /**
   * If possible, reads all chars from the provided {@link Reader} directly into underlying
   * character buffer without making extra copies.
   *
  ...
  /**
   * If possible, reads {@code count} chars from the provided {@link Reader} directly into
   * underlying character buffer without making extra copies.
   *
   * @param reader Reader to read.
   * @param count The maximum characters to read, a value &lt;= 0 returns 0.
   * @return The number of characters read. If less than {@code count}, then we've reached the
   *     end-of-stream, or -1 if we reached the end of stream.
   * @throws IOException if an I/O error occurs.
   * @see #appendTo(Appendable)
   * @since 1.9
   */
  ...
  /**
   * Replaces a portion of the string builder with another string. The length of the inserted string
   * does not have to match the removed length.
   *
   * @param startIndex the start index, inclusive, must be valid
   * @param endIndex the end index, exclusive, must be valid except that if too large it is treated
   *     as end of string
   ...
   *
   * <p>Matchers can be used to perform advanced behavior. For example you could write a matcher to
   * delete all occurrences where the character 'a' is followed by a number.
   *
   * @param matcher the matcher to use to find the deletion, null causes no action
   ...
   */
  public TextStringBuilder replace(
   ...
   *
   * <p>Matchers can be used to perform advanced replace behavior. For example you could write a
   * matcher to replace all occurrences where the character 'a' is followed by a number.
   *
   * @param matcher the matcher to use to find the deletion, null causes no action
   * @param replaceStr the replace string, null is equivalent to an empty string
   * @return this, to enable chaining
   ...
   *
   * <p>Matchers can be used to perform advanced replace behavior. For example you could write a
   * matcher to replace where the character 'a' is followed by a number.
   *
   * @param matcher the matcher to use to find the deletion, null causes no action
   * @param replaceStr the replace string, null is equivalent to an empty string
   * @return this, to enable chaining
   ...
   * @throws IndexOutOfBoundsException if any index is invalid
   */
  private TextStringBuilder replaceImpl(
      final StringMatcher matcher,
      final String replaceStr,
      final int from,
      int to,
   ...
   *
   * <p>This method extracts the right {@code length} characters from the builder. If this many
   * characters are not available, the whole builder is returned. Thus the returned string may be
   * shorter than the length requested.
   *
   * @param length the number of characters to extract, negative returns empty string
  ...
  /**
   * Updates the length of the builder by either dropping the last characters or adding filler of
   * Unicode zero.
   *
   ...
   * Gets the length of the string builder.
   *
   * <p>This method is the same as {@link #length()} and is provided to match the API of
   * Collections.
   *
   * @return The length
   */
   ...
   * Extracts a portion of this string builder as a string.
   *
   * <p>Note: This method treats an endIndex greater than the length of the builder as equal to the
   * length of the builder, and continues without error, unlike StringBuffer or String.
   *
   * @param startIndex the start index, inclusive, must be valid
   * @param endIndex the end index, exclusive, must be valid except that if too large it is treated
   *     as end of string
   ...
   * @throws IndexOutOfBoundsException if startIndex is invalid, or if endIndex is invalid (but
  ...
  public char[] toCharArray(final int startIndex, int endIndex) {
    ...
    return len == 0
        ...
        : Arrays.copyOfRange(buffer, startIndex, endIndex);
  }

  /**
   * Gets a String version of the string builder, creating a new instance each time the method is
   * called.
   *
   * <p>Note that unlike StringBuffer, the string version returned is independent of the string
   * builder.
   *
  ...
  /**
   * Gets a StringBuffer version of the string builder, creating a new instance each time the method
   * is called.
   *
  ...
  /**
   * Gets a StringBuilder version of the string builder, creating a new instance each time the
   * method is called.
   *
  ...
  /**
   * Trims the builder by removing characters less than or equal to a space from the beginning and
   * end.
   *
   * @return this, to enable chaining
   */
   ...
   * @throws IndexOutOfBoundsException Thrown when the index is not the range {@code 0 <= index <=
   *     size}.
   */
