 ...
 *
 * <p>Looks up the value for a given key in the format "Charset:URL".
 *
 * <p>For example: "UTF-8:https://www.apache.org".
 *
 * @since 1.5
 */
  ...
  public String lookup(final String key) {
    ...
    if (keyLen < 2) {
      throw IllegalArgumentExceptions.format(
          "Bad URL key format [%s]; expected format is DocumentPath:Key.", key);
    }
    ...
    try {
      ...
      throw IllegalArgumentExceptions.format(
          e, "Error looking up URL [%s] with Charset [%s].", urlStr, charsetName);
    }
  }
}
