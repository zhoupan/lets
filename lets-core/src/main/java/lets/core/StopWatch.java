 ...
 *
 * <p>To start the watch, call {@link #start()} or {@link StopWatch#createStarted()}. At this point
 * you can:
 *
 * <ul>
 *   <li>{@link #split()} the watch to get the time whilst the watch continues in the background.
 *       {@link #unsplit()} will remove the effect of the split. At this point, these three options
 *       are available again.
 *   <li>{@link #suspend()} the watch to pause it. {@link #resume()} allows the watch to continue.
 *       Any time between the suspend and resume will not be counted in the total. At this point,
 *       these three options are available again.
 *   <li>{@link #stop()} the watch to complete the timing session.
 * </ul>
 *
 * <p>It is intended that the output methods {@link #toString()} and {@link #getTime()} should only
 * be called after stop, split or suspend, however a suitable result will be returned at other
 * points.
 *
 * <p>NOTE: As from v2.1, the methods protect against inappropriate calls. Thus you cannot now call
 * stop before start, resume before suspend or unsplit before split.
 *
...
public class StopWatch {
  ...
  private enum State {
    ...
    /**
     * Returns whether the StopWatch is stopped. The stopwatch which's not yet started and
   ...
   *
   * <p>This is either the time between the start and the moment this method is called, or the
   * amount of time between start and stop. The resulting time will be expressed in the desired
   * TimeUnit with any remainder rounded down. For example, if the specified unit is {@code
   * TimeUnit.HOURS} and the stopwatch time is 59 minutes, then the result returned will be {@code
   * 0}.
   *
  ...
  /**
   * Returns whether StopWatch is stopped. The stopwatch which's not yet started and explicitly
   * stopped stopwatch is considered as stopped.
   *
   * @return boolean If the StopWatch is stopped.
   * @since 3.2
   */
   ...
   *
   * <p>The format used is ISO 8601-like, [<i>message</i>
