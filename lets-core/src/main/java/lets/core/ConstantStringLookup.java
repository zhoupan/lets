 ...
 * apackage.AClass.AFIELD}. The {@code lookup(String)} method will split the passed in string at the
 * last dot, separating the fully qualified class name and the name of the constant (i.e. <b>static
 * final</b>) member field. Then the class is loaded and the field's value is obtained using
 * reflection.
 *
 * <p>Once retrieved values are cached for fast access. This class is thread-safe. It can be used as
 * a standard (i.e. global) lookup object and serve multiple clients concurrently.
 *
...
class ConstantStringLookup extends AbstractStringLookup {
  ...
  /**
   * Loads the class with the specified name. If an application has special needs regarding the
   * class loaders to be used, it can hook in here. This implementation delegates to the {@code
   * getClass()} method of Commons Lang's <code>
  ...
  /**
   * Tries to resolve the specified variable. The passed in variable name is interpreted as the name
   * of a <b>static final</b> member field of a class. If the value has already been obtained, it
   * can be retrieved from an internal cache. Otherwise this method will invoke the {@code
  ...
  /**
   * Determines the value of the specified constant member field of a class. This implementation
   * will call {@code fetchClass()} to obtain the {@link Class} object for the target class. Then it
   * will use reflection to obtain the field's value. For this to work the field must be accessible.
   *
