 ...
 * EventListenerList to provide type-safe API (preferably conforming to the JavaBeans spec) and
 * methods which dispatch event notification methods to appropriate Event Listeners on the list.
 *
 * <p>The main benefits that this class provides are that it is relatively cheap in the case of no
 * listeners, and it provides serialization for event-listener lists in a single place, as well as a
 * degree of MT safety (when used correctly).
 *
 * <p>Usage example: Say one is defining a class that sends out FooEvents, and one wants to allow
 * users of the class to register FooListeners and receive notification when FooEvents occur. The
 * following should be added to the class definition:
 *
 * <pre>
 ...
 * </pre>
 *
 * foo should be changed to the appropriate name, and fireFooXxx to the appropriate method name. One
 * fire method should exist for each notification method in the FooListener interface.
 *
 * <p>
 */
...
public class EventListenerList implements Serializable {
   ...
   *
   * <p>WARNING!!! Absolutely NO modification of the data contained in this array should be made --
   * if any such manipulation is necessary, it should be done on a copy of the array returned rather
   ...
   * @since 1.3
   * @exception ClassCastException if the supplied class is not assignable to EventListener
   */
   ...
   */
  @SuppressWarnings("unchecked")
  public synchronized <T extends EventListener> void remove(Class<T> t) {
    EventListener[] listeners = this.getListeners(t);
    if (listeners != null) {
      for (EventListener listener : listeners) {
        this.remove(t, (T) listener);
      }
    }
  }
  ...
  public synchronized <T extends EventListener> void remove(Class<T> t, T l) {
    ...
    for (int i = listenerList.length - 2; i >= 0; i -= 2) {
      if ((listenerList[i] == t) && (ObjectUtils.equals(listenerList[i + 1], l))) {
        index = i;
        break;
      }
    }
