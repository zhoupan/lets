 ...
 *
 * <p>Looks up the value for a given key in the format "Document:XPath".
 *
  ...
  public String lookup(final String key) {
    ...
    if (keyLen != 2) {
      throw IllegalArgumentExceptions.format(
          "Bad XML key format [%s]; expected format is DocumentPath:XPath.", key);
    }
    ...
    try (InputStream inputStream = Files.newInputStream(getPath(documentPath))) {
      ...
      throw IllegalArgumentExceptions.format(
          e, "Error looking up XML document [%s] and XPath [%s].", documentPath, xpath);
    }
  }
}
