 ...
 *
 * <p>This class takes a piece of text and substitutes all the variables within it. The default
 * definition of a variable is {@code ${variableName}}. The prefix and suffix can be changed via
 * constructors and set methods.
 *
 * <p>Variable values are typically resolved from a map, but could also be resolved from system
 * properties, or by supplying a custom variable resolver.
 *
 * <h2>Using System Properties</h2>
 *
 * <p>The simplest example is to use this class to replace Java System properties. For example:
 *
 * <pre>
 * StringSubstitutor
 ...
 *
 * <p>Typical usage of this class follows the following pattern:
 *
 * <ul>
 *   <li>Create and initialize a StringSubstitutor with the map that contains the values for the
 *       variables you want to make available.
 *   <li>Optionally set attributes like variable prefix, variable suffix, default value delimiter,
 *       and so on.
 *   <li>Call the {@code replace()} method with in the source text for interpolation.
 *   <li>The returned text contains all variable references (as long as their values are known) as
 *       resolved.
 * </ul>
 *
 * <p>For example:
 ...
 *
 * <p>You can set a default value for unresolved variables. The default value for a variable can be
 * appended to the variable name after the variable default value delimiter. The default value of
 * the variable default value delimiter is ":-", as in bash and other *nix shells.
 *
 * <p>You can set the variable value delimiter with {@link
 * #setValueDelimiterMatcher(StringMatcher)}, {@link #setValueDelimiter(char)} or {@link
 ...
 * </pre>
 *
 * <p>{@code StringSubstitutor} supports throwing exceptions for unresolved variables, you enable
 * this by setting calling {@link #setEnableUndefinedVariableException(boolean)} with {@code true}.
 *
 * <h2>Reusing Instances</h2>
 *
 * <p>Static shortcut methods cover the most common use cases. If multiple replace operations are to
 * be performed, creating and reusing an instance of this class will be more efficient.
 *
 * <h2>Using Interpolation</h2>
 *
 * <p>The default interpolator lets you use string lookups like:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>For documentation and a full list of available lookups, see {@link StringLookupFactory}.
 *
 * <p><strong>NOTE:</strong> The list of lookups available by default in {@link
 * #createInterpolator()} changed in version {@code 1.10.0}. See the {@link StringLookupFactory}
 * documentation for details and an explanation on how to reproduce the previous functionality.
 *
 * <h2>Using Recursive Variable Replacement</h2>
 *
 * <p>Variable replacement can work recursively by calling {@link
 * #setEnableSubstitutionInVariables(boolean)} with {@code true}. If a variable value contains a
 * variable then that variable will also be replaced. Cyclic replacements are detected and will
 * throw an exception.
 *
 * <p>You can get the replace result to contain a variable prefix. For example:
 *
 * <pre>
 * "The variable ${${name}} must be used."
 * </pre>
 *
 * <p>If the value of the "name" variable is "x", then only the variable "name" is replaced
 * resulting in:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>To achieve this effect there are two possibilities: Either set a different prefix and suffix
 * for variables which do not conflict with the result text you want to produce. The other
 * possibility is to use the escape character, by default '$'. If this character is placed before a
 * variable reference, this reference is ignored and won't be replaced. For example:
 *
 * <pre>
 * "The variable $${${name}} must be used."
 * </pre>
 *
 * <p>In some complex scenarios you might even want to perform substitution in the names of
 ...
 * </pre>
 *
 * <p>{@code StringSubstitutor} supports this recursive substitution in variable names, but it has
 * to be enabled explicitly by calling {@link #setEnableSubstitutionInVariables(boolean)} with
 * {@code true}.
 *
 * <h2>Thread Safety</h2>
 *
...
public class StringSubstitutor {
   ...
   *
   * <p>This StringSubstitutor lets you perform substitutions like:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The table below lists the lookups available by default in the returned instance. These may
   * be modified through the use of the {@value
   * org.apache.commons.text.lookup.StringLookupFactory#DEFAULT_STRING_LOOKUPS_PROPERTY} system
   * property, as described in the {@link StringLookupFactory} documentation.
   *
   * <p><strong>NOTE:</strong> The list of lookups available by default changed in version {@code
   * 1.10.0}. Configuration via system property (as mentioned above) may be necessary to reproduce
  ...
  /**
   * Replaces all the occurrences of variables in the given source object with their matching values
   * from the map. This method allows to specify a custom variable prefix and suffix
   *
   * @param <V> the type of the values in the map
   * @param source the source text containing the variables to substitute, null returns null
   * @param valueMap the map with the values, may be null
   ...
   *     enableUndefinedVariableException is true
   */
  public static <V> String replace(
      final Object source,
  ...
  public static String replace(final Object source, final Properties valueProperties) {
    ...
    return StringSubstitutor.replace(
        source,
        valueProperties.stringPropertyNames().stream()
            .collect(Collectors.toMap(Function.identity(), valueProperties::getProperty)));
  }
  ...
  /**
   * Constructs a new instance with defaults for variable prefix and suffix and the escaping
   * character.
   */
  ...
  /**
   * Constructs a new initialized instance. Uses defaults for variable prefix and suffix and the
   * escaping character.
   *
   * @param <V> the type of the values in the map
  ...
  public <V> StringSubstitutor(final Map<String, V> valueMap) {
    this(
        StringLookupFactory.INSTANCE.mapStringLookup(valueMap),
        DEFAULT_PREFIX,
        DEFAULT_SUFFIX,
        DEFAULT_ESCAPE);
  }
   ...
   */
  public <V> StringSubstitutor(
      final Map<String, V> valueMap, final String prefix, final String suffix, final char escape) {
   ...
   */
  public StringSubstitutor(
      final StringLookup variableResolver,
      final String prefix,
      final String suffix,
      final char escape) {
  ...
  public StringSubstitutor(
      final StringLookup variableResolver,
      final StringMatcher prefixMatcher,
  ...
  public StringSubstitutor(
      ...
      final StringMatcher suffixMatcher,
      final char escape,
      final StringMatcher valueDelimiterMatcher) {
    this.setVariableResolver(variableResolver);
   ...
   *
   * <p>The variable default value delimiter is the character or characters that delimit the
   ...
   *
   * <p>If it returns null, then the variable default value resolution is disabled.
   *
   ...
   *
   * <p>The variable prefix is the character or characters that identify the start of a variable.
   * This prefix is expressed in terms of a matcher allowing advanced prefix matches.
   *
   ...
   *
   * <p>The variable suffix is the character or characters that identify the end of a variable. This
   * suffix is expressed in terms of a matcher allowing advanced suffix matches.
   *
  ...
  /**
   * Returns a flag whether substitution is disabled in variable values.If set to <b>true</b>, the
   * values of variables can contain other variables will not be processed and substituted original
   * variable is evaluated, e.g.
   *
   * <pre>
   * Map&lt;String, String&gt; valuesMap = new HashMap&lt;&gt;();
   ...
   * the given source array as a template. The array is not altered by this method.
   *
   * <p>Only the specified portion of the array will be processed. The rest of the array is not
   ...
   * @return The result of the replace operation
   * @throws IllegalArgumentException if variable is not found when its allowed to throw exception
   * @throws StringIndexOutOfBoundsException if {@code offset} is not in the range {@code 0 <=
   *     offset <= chars.length}
  ...
  /**
   * Replaces all the occurrences of variables with their matching values from the resolver using
   * the given source as a template. The source is not altered by this method.
   *
  ...
  public String replace(final CharSequence source, final int offset, final int length) {
    ...
    final TextStringBuilder buf =
        new TextStringBuilder(length).append(source.toString(), offset, length);
    substitute(buf, 0, length);
    return buf.toString();
  }
  ...
  /**
   * Replaces all the occurrences of variables in the given source object with their matching values
   * from the resolver. The input source object is converted to a string using {@code toString} and
   * is not altered.
   *
  ...
  /**
   * Replaces all the occurrences of variables with their matching values from the resolver using
   * the given source buffer as a template. The buffer is not altered by this method.
   *
   * @param source the buffer to use as a template, not changed, null returns null
   * @return The result of the replace operation
   * @throws IllegalArgumentException if variable is not found when its allowed to throw exception
  ...
  /**
   * Replaces all the occurrences of variables within the given source buffer with their matching
   * values from the resolver. The buffer is updated with the result.
   *
   ...
   * implementation uses the {@link #getStringLookup()} with the variable's name as the key.
   *
   ...
   */
  public StringSubstitutor setDisableSubstitutionInValues(
      final boolean disableSubstitutionInValues) {
    this.disableSubstitutionInValues = disableSubstitutionInValues;
    return this;
  }

  /**
   * Sets a flag whether substitution is done in variable names. If set to <b>true</b>, the names of
   * variables can contain other variables which are processed first before the original variable is
   * evaluated, e.g. {@code ${jre-${java.version}}}. The default value is <b>false</b>.
   *
   * @param enableSubstitutionInVariables the new value of the flag
   * @return this, to enable chaining
   */
  public StringSubstitutor setEnableSubstitutionInVariables(
   ...
   * @return this, to enable chaining
   */
  public StringSubstitutor setEnableUndefinedVariableException(
      final boolean failOnUndefinedVariable) {
    this.enableUndefinedVariableException = failOnUndefinedVariable;
    return this;
  }

  /**
   * Sets the escape character. If this character is placed before a variable reference in the
   * source text, this variable will be ignored.
   *
  ...
  /**
   * Sets a flag controlling whether escapes are preserved during substitution. If set to
   * <b>true</b>, the escape character is retained during substitution (e.g. {@code
   * $${this-is-escaped}} remains {@code $${this-is-escaped}}). If set to <b>false</b>, the escape
   * character is removed during substitution (e.g. {@code $${this-is-escaped}} becomes {@code
   * ${this-is-escaped}}). The default value is <b>false</b>
   *
   ...
   *
   * <p>The variable default value delimiter is the character or characters that delimit the
   * variable name and the variable default value. This method allows a single character variable
   ...
   *
   * <p>The variable default value delimiter is the character or characters that delimit the
   * variable name and the variable default value. This method allows a string variable default
   * value delimiter to be easily set.
   *
   * <p>If the {@code valueDelimiter} is null or empty string, then the variable default value
   ...
   * allowing advanced variable default value delimiter matches.
   *
   * <p>If the {@code valueDelimiterMatcher} is null, then the variable default value resolution
   * becomes disabled.
   *
   ...
   *
   * <p>The variable prefix is the character or characters that identify the start of a variable.
   * This method allows a single character prefix to be easily set.
   *
   ...
   *
   * <p>The variable prefix is the character or characters that identify the start of a variable.
   * This method allows a string prefix to be easily set.
   *
   * @param prefix the prefix for variables, not null
   * @return this, to enable chaining
   ...
   *
   * <p>The variable suffix is the character or characters that identify the end of a variable. This
   * method allows a single character suffix to be easily set.
   *
   ...
   *
   * <p>The variable suffix is the character or characters that identify the end of a variable. This
   * method allows a string suffix to be easily set.
   *
   * @param suffix the suffix for variables, not null
   * @return this, to enable chaining
   ...
   *
   * <p>Most users of this class do not need to call this method. This method will be called
   * automatically by another (public) method.
   *
   * <p>Writers of subclasses can override this method if they need access to the substitution
   * process at the start or end.
   *
  ...
  protected boolean substitute(
      final TextStringBuilder builder, final int offset, final int length) {
   ...
   *     <pre>isEnableUndefinedVariableException()==true</pre>
   *
   * @since 1.9
   */
        ...
        while (pos < bufEnd) {
          ...
          if (endMatchLen == 0) {
            ...
            if (nestedVarCount == 0) {
              ...
              // get var name
              String varNameExpr =
                  builder.midString(startPos + startMatchLen, pos - startPos - startMatchLen);
              if (substitutionInVariablesEnabled) {
              ...
              if (valueDelimMatcher != null) {
                ...
                for (int i = 0; i < varNameExprChars.length; i++) {
                  ...
                  if (valueDelimMatcher.isMatch(varNameExprChars, i, 0, varNameExprChars.length)
                      != 0) {
                    valueDelimiterMatchLen =
                        valueDelimMatcher.isMatch(varNameExprChars, i, 0, varNameExprChars.length);
              ...
              if (varValue != null) {
                ...
                throw new IllegalArgumentException(
                    String.format(
                        "Cannot resolve variable '%s' (enableSubstitutionInVariables=%s).",
