...
public class ReversedLinesFileReader implements Closeable {
  ...
  /**
   * Creates a ReversedLinesFileReader with default block size of 4KB and the platform's default
   * encoding.
   *
   ...
   *
   * @param file the file to be read
   * @param blockSize size of the internal buffer (for ideal performance this should match with the
  ...
  public ReversedLinesFileReader(final File file, final int blockSize, final Charset encoding)
    ...
    if (maxBytesPerChar == 1f) {
      ...
      // UTF-8 works fine out of the box, for multibyte sequences a second UTF-8 byte can never be a
      // newline byte
      ...
      byteDecrement = 1;
        ...
        charset
            == Charset.forName("x-windows-950")) { // Windows code page 950 (Traditional Chinese)
      byteDecrement = 1;
  ...
  private class FilePart {
    ...
    private FilePart(final long no, final int length, final byte[] leftOverOfLastFilePart)
      ...
      final int dataLength =
          length + (leftOverOfLastFilePart != null ? leftOverOfLastFilePart.length : 0);
