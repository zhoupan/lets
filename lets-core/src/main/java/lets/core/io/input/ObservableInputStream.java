...
/**
 * The {@link ObservableInputStream} allows, that an InputStream may be consumed by other receivers,
 * apart from the thread, which is reading it. The other consumers are implemented as instances of
 * {@link Observer}. A typical application may be the generation of a {@link
 * java.security.MessageDigest} on the fly. {@code Note}: The {@link ObservableInputStream} is
 * <em>not</em> thread safe, as instances of InputStream usually aren't. If you must access the
 * stream from multiple threads, then synchronization, locking, or a similar means must be used.
 *
...
public class ObservableInputStream extends ProxyInputStream {
  ...
  public abstract static class Observer {
    ...
    /**
     * Called to indicate, that {@link InputStream#read()} has been invoked on the {@link
     * ObservableInputStream}, and will return a value.
     *
     * @param pByte The value, which is being returned. This will never be -1 (EOF), because, in
     *     that case, {@link #finished()} will be invoked instead.
     * @throws IOException if an i/o-error occurs
     */
    void data(final int pByte) throws IOException {}

    /**
     * Called to indicate, that {@link InputStream#read(byte[])}, or {@link InputStream#read(byte[],
     * int, int)} have been called, and are about to invoke data.
     *
     * @param pBuffer The byte array, which has been passed to the read call, and where data has
     *     been stored.
    ...
    /**
     * Called to indicate, that EOF has been seen on the underlying stream. This method may be
     * called multiple times, if the reader keeps invoking either of the read methods, and they will
     * consequently keep returning EOF.
     *
     * @throws IOException if an i/o-error occurs
     */
  ...
  /**
   * Notifies the observers by invoking {@link Observer#data(byte[],int,int)} with the given
   * arguments.
   *
   ...
   */
  protected void noteDataBytes(final byte[] pBuffer, final int pOffset, final int pLength)
      throws IOException {
   ...
   * @throws IOException Some observer has thrown an exception, which is being passed down. This may
   *     be the same exception, which has been passed as an argument.
   */
   ...
   *
   * @throws IOException The underlying {@link InputStream}, or either of the observers has thrown
   *     an exception.
   */
