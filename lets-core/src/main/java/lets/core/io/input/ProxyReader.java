 ...
 * stream and doesn't change which methods are being called.
 *
 * <p>It is an alternative base class to FilterReader to increase reusability, because FilterReader
...
public abstract class ProxyReader extends FilterReader {
  ...
  /**
   * Invoked by the read methods before the call is proxied. The number of chars that the caller
   * wanted to read (1 for the {@link #read()} method, buffer length for {@link #read(char[])},
   ...
   *
   * <p>Note this method is <em>not</em> called from {@link #skip(long)} or {@link #reset()}. You
   * need to explicitly override those methods if you want to add pre-processing steps also to them.
   *
   * @since 2.0
   ...
   * chars returned to the caller (or -1 if the end of stream was reached) is given as an argument.
   *
