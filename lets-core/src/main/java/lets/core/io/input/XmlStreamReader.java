...
import lets.core.io.ByteOrderMark;

/**
 * Character stream that handles all the necessary Voodoo to figure out the charset encoding of the
 * XML document within the stream.
 *
 * <p>IMPORTANT: This class is not related in any way to the org.xml.sax.XMLReader. This one IS a
 * character stream.
 *
 * <p>All this has to be done without consuming characters from the stream, if not the XML parser
 * will not recognized the document as a valid XML. This is not 100% true, but it's close enough
 * (UTF-8 BOM is not handled by all parsers right now, XmlStreamReader handles it and things work in
 * all parsers).
 *
 * <p>The XmlStreamReader class handles the charset encoding of XML documents in Files, raw streams
 * and HTTP streams by offering a wide set of constructors.
 *
 * <p>By default the charset encoding detection is lenient, the constructor with the lenient flag
 * can be used for a script (following HTTP MIME and XML specifications). All this is nicely
 * explained by Mark Pilgrim in his blog, <a
 * href="http://diveintomark.org/archives/2004/02/13/xml-media-types">Determining the character
 * encoding of a feed</a>.
 *
 * <p>Originally developed for <a href="http://rome.dev.java.net">ROME</a> under Apache License 2.0.
 *
 * @see lets.core.io.output.XmlStreamWriter
 * @since 2.0
 */
public class XmlStreamReader extends Reader {
  ...
  private static final ByteOrderMark[] XML_GUESS_BYTES =
      new ByteOrderMark[] {
        ...
        new ByteOrderMark(
            UTF_32BE, 0x00, 0x00, 0x00, 0x3C, 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x78, 0x00,
            0x00, 0x00, 0x6D),
        new ByteOrderMark(
            UTF_32LE, 0x3C, 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x78, 0x00, 0x00, 0x00, 0x6D,
            0x00, 0x00, 0x00),
   ...
   *
   * <p>If it is NULL the content-type based rules are used.
   *
   ...
   *
   * <p>It looks for the UTF-8 BOM first, if none sniffs the XML prolog charset, if this is also
   * missing defaults to UTF-8.
   *
   * <p>It does a lenient charset encoding detection, check the constructor with the lenient
   ...
   *
   * <p>If the content type was 'text/html' it replaces it with 'text/xml' and tries the detection
   * again.
   *
   * <p>Else if the XML prolog had a charset encoding that encoding is used.
   *
   * <p>Else if the content type had a charset encoding that encoding is used.
   *
   * <p>Else 'UTF-8' is used.
   *
   * <p>If lenient detection is indicated an XmlStreamReaderException is never thrown.
   *
   * @param is InputStream to create a Reader from.
   * @param lenient indicates if the charset encoding detection should be relaxed.
   * @throws IOException thrown if there is a problem reading the stream.
   * @throws XmlStreamReaderException thrown if the charset encoding could not be determined
  ...
  public XmlStreamReader(final InputStream is, final boolean lenient, final String defaultEncoding)
    ...
    final BOMInputStream bom =
        new BOMInputStream(new BufferedInputStream(is, BUFFER_SIZE), false, BOMS);
   ...
   *
   * <p>If the URL is not of type HTTP and there is not 'content-type' header in the fetched data it
   * uses the same logic used for Files.
   *
   * <p>If the URL is a HTTP Url or there is a 'content-type' header in the fetched data it uses the
   ...
   *
   * <p>If the URLConnection is not of type HttpURLConnection and there is not 'content-type' header
   * in the fetched data it uses the same logic used for files.
   *
   * <p>If the URLConnection is a HTTP Url or there is a 'content-type' header in the fetched data
   * it uses the same logic used for an InputStream with content-type.
   *
   * <p>It does a lenient charset encoding detection, check the constructor with the lenient
   * parameter for details.
   *
   ...
   *
   * <p>First it checks if the stream has BOM. If there is not BOM checks the content-type encoding.
  ...
  /**
   * Creates a Reader using an InputStream and the associated content-type header. This constructor
   * is lenient regarding the encoding detection.
   *
   * <p>First it checks if the stream has BOM. If there is not BOM checks the content-type encoding.
   * If there is not content-type encoding checks the XML prolog encoding. If there is not XML
   * prolog encoding uses the default encoding mandated by the content-type MIME type.
   ...
   * @throws IOException thrown if there is a problem reading the stream.
   */
  private String doHttpStream(
      final BOMInputStream bom,
      final BOMInputStream pis,
      final String httpContentType,
      final boolean lenient)
      throws IOException {
    final String bomEnc = bom.getBOMCharsetName();
   ...
   * @throws IOException thrown if there is a problem reading the stream.
   */
  String calculateRawEncoding(final String bomEnc, final String xmlGuessEnc, final String xmlEnc)
      throws IOException {
  ...
  String calculateHttpEncoding(
    ...
    if (!appXml && !textXml) {
      final String msg =
          MessageFormat.format(HTTP_EX_3, cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc);
    ...
    if (cTEnc.equals(UTF_16)) {
      ...
      final String msg =
          MessageFormat.format(HTTP_EX_2, cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc);
      throw new XmlStreamReaderException(msg, cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc);
    }
  ...
  public static final Pattern ENCODING_PATTERN =
      Pattern.compile(
          "<\\?xml.*encoding[\\s]*=[\\s]*((?:\".[^\"]*\")|(?:'.[^']*'))", Pattern.MULTILINE);
    ...
    if (guessedEnc != null) {
      ...
      if (firstGT == -1) {
        if (c == -1) {
          ...
          throw new IOException(
              "XML prolog or ROOT element not found on first " + offset + " bytes");
        }
      }
      ...
      if (bytesRead > 0) {
        ...
        final BufferedReader bReader =
            new BufferedReader(new StringReader(xmlProlog.substring(0, firstGT + 1)));
