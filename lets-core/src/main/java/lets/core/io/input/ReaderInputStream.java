 ...
 *
 * @see lets.core.io.output.WriterOutputStream
 * @since 2.0
 */
public class ReaderInputStream extends InputStream {
  ...
  public ReaderInputStream(
      final Reader reader, final CharsetEncoder encoder, final int bufferSize) {
    this.reader = reader;
  ...
  /**
   * Construct a new {@link ReaderInputStream} that uses the default character encoding with a
   * default input buffer size of 1024 characters.
   *
   * @param reader the target {@link Reader}
  ...
  public int read(final byte[] b, int off, int len) throws IOException {
    ...
    if (len < 0 || off < 0 || (off + len) > b.length) {
      throw new IndexOutOfBoundsException(
          "Array Size=" + b.length + ", offset=" + off + ", length=" + len);
    }
    int read = 0;
    if (len == 0) {
