...
/**
 * InputStream proxy that transparently writes a copy of all bytes read from the proxied stream to a
 * given OutputStream. Using {@link #skip(long)} or {@link #mark(int)}/{@link #reset()} on the
 * stream will result on some bytes from the input stream being skipped or duplicated in the output
 * stream.
 *
 * <p>The proxied input stream is closed when the {@link #close()} method is called on this proxy.
 * It is configurable whether the associated output stream will also closed.
 *
 * @since 1.4
...
public class TeeInputStream extends ProxyInputStream {
   ...
   * to the given {@link OutputStream}. The given output stream will not be closed when this stream
   * gets closed.
   *
  ...
  /**
   * Creates a TeeInputStream that proxies the given {@link InputStream} and copies all read bytes
   * to the given {@link OutputStream}. The given output stream will be closed when this stream gets
   * closed if the closeBranch parameter is {@code true}.
   *
   * @param input input stream to be proxied
   * @param branch output stream that will receive a copy of all bytes read
  ...
  /**
   * Closes the proxied input stream and, if so configured, the associated output stream. An
  ...
  /**
   * Reads a single byte from the proxied input stream and writes it to the associated output
   * stream.
   *
  ...
  /**
   * Reads bytes from the proxied input stream and writes the read bytes to the associated output
   * stream.
   *
