 ...
 *
 * <p>This class originated in FileUpload processing. In this use case, you do not know in advance
 * the size of the file being uploaded. If the file is small you want to store it in memory (for
 * speed), but if the file is large you want to store it to file (to avoid memory issues).
 */
public class DeferredFileOutputStream extends ThresholdingOutputStream {
  ...
  public DeferredFileOutputStream(
      final int threshold, final int initialBufferSize, final File outputFile) {
  ...
  /**
   * Constructs an instance of this class which will trigger an event at the specified threshold,
   * and save data to a temporary file beyond that point. The initial buffer size will default to 32
   * bytes which is ByteArrayOutputStream's default buffer size.
   *
   * @param threshold The number of bytes at which to trigger an event.
   ...
   * null.
   *
   * <p>If the constructor specifying the file is used then it returns that same output file, even
   * when threshold has not been reached.
   *
   * <p>If constructor specifying a temporary file prefix/suffix is used then the temporary file
   * created once the threshold is reached is returned If the threshold was not reached then {@code
   * null} is returned.
   *
