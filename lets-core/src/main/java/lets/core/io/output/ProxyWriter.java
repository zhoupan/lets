 ...
 * stream and doesn't change which methods are being called. It is an alternative base class to
 * FilterWriter to increase reusability, because FilterWriter changes the methods being called, such
 * as write(char[]) to write(char[], int, int) and write(String) to write(String, int, int).
 */
public class ProxyWriter extends FilterWriter {
  ...
  /**
   * Invoked by the write methods before the call is proxied. The number of chars to be written (1
   * for the {@link #write(int)} method, buffer length for {@link #write(char[])}, etc.) is given as
   * an argument.
   *
   * <p>Subclasses can override this method to add common pre-processing functionality without
   * having to override all the write methods. The default implementation does nothing.
   *
   * @since 2.0
  ...
  /**
   * Invoked by the write methods after the proxied call has returned successfully. The number of
   * chars written (1 for the {@link #write(int)} method, buffer length for {@link #write(char[])},
   ...
   *
   * <p>This method provides a point to implement custom exception handling. The default behaviour
   * is to re-throw the exception.
   *
