 ...
 *
 * <p>This class provides a simple alternative to <code>FileWriter</code> that will use a lock file
 * to prevent duplicate writes.
 *
 * <p><b>N.B.</b> the lock file is deleted when {@link #close()} is called - or if the main file
 * cannot be opened initially. In the (unlikely) event that the lockfile cannot be deleted, this is
 * not reported, and subsequent requests using the same lockfile will fail.
 *
 * <p>By default, the file will be overwritten, but this may be changed to append. The lock
 * directory may be specified, but defaults to the system property <code>java.io.tmpdir</code>. The
 * encoding may also be specified, and defaults to the platform default.
 */
public class LockableFileWriter extends Writer {
   ...
   * @throws NullPointerException if the file is null
   * @throws IOException in case of an I/O error
   * @throws java.nio.charset.UnsupportedCharsetException thrown instead of {@link
   *     java.io.UnsupportedEncodingException} in version 2.2 if the encoding is not supported.
   */
  ...
  private Writer initWriter(final File file, final Charset encoding, final boolean append)
    ...
    try {
      return new OutputStreamWriter(
          new FileOutputStream(file.getAbsolutePath(), append), Charsets.toCharset(encoding));
