 ...
 *
 * <p>This class defines six components within a filename (example C:\dev\project\file.txt):
 *
 * <ul>
 *   <li>the prefix - C:\
 *   <li>the path - dev\project\
 *   <li>the full path - C:\dev\project\
 *   <li>the name - file.txt
 *   <li>the base name - file
 ...
 * directory. As a result, we have chosen to say it corresponds to a file.
 *
 * <p>This class only supports Unix and Windows style names. Prefixes are matched as follows:
 *
 * <pre>
 * Windows:
 * a\b\c.txt           --&gt; ""          --&gt; relative
 ...
 * running on.
 *
 * <p>Origin of code: Excalibur, Alexandria, Tomcat, Commons-Utils.
 *
 * @since 1.1
 */
public class FilenameUtils {
   ...
   * either Unix or Windows format. The output will contain separators in the format of the system.
   *
   * <p>A trailing slash will be retained. A double slash will be merged to a single slash (but UNC
   * names are handled). A single dot path segment will be removed. A double dot will cause that
   * path segment and the one before to be removed. If the double dot has no parent path segment to
   * work with, {@code null} is returned.
   *
   ...
   * </pre>
   *
   * <p>(Note the file separator returned will be correct for Windows/Unix)
   *
   ...
   * </pre>
   *
   * <p>The output will be the same on both Unix and Windows including the separator character.
   *
   * @param filename the filename to normalize, null returns null
  ...
  /**
   * Normalizes a path, removing double and single dot path steps, and removing any final directory
   * separator.
   *
   * <p>This method normalizes a path to a standard format. The input may contain separators in
   * either Unix or Windows format. The output will contain separators in the format of the system.
  ...
  private static String doNormalize(
      final String filename, final char separator, final boolean keepSeparator) {
    if (filename == null) {
      return null;
    }
   ...
   * built into the concatenated path. If this might be a problem, use {@link #getFullPath(String)}
   ...
   * directory).
   *
   * <p>The files names are expected to be normalized. Edge cases:
   *
   * <ul>
   ...
   * both Unix and Windows prefixes are matched regardless.
   *
   * <p>Note that a leading // (or \\) is used to indicate a UNC name on Windows. These must be
   * followed by a server name, so double-slashes are not collapsed to a single slash at the start
   * of the filename.
   *
   ...
   *
   * <p>This method will handle a file in either Unix or Windows format. The method is entirely text
   ...
   * method that retains the prefix.
   *
   * @param filename the filename to query, null returns null
   ...
   * <p>The output will be the same irrespective of the machine that the code is running on.
   *
   * <p>This method drops the prefix from the result. See {@link #getFullPathNoEndSeparator(String)}
  ...
  /**
   * Set the extension from a filename.
   *
   * @param filename
   * @param extension
   * @return
  ...
  public static String setExtension(final String filename, final String extension) {
    String removed = removeExtension(filename);
    if (StringUtils.isNotEmpty(extension)) {
      StringBuilder sb = new StringBuilder();
      sb.append(removed);
      if (!StringUtils.startsWith(extension, EXTENSION_SEPARATOR_STR)) {
        sb.append(EXTENSION_SEPARATOR_STR);
      }
      sb.append(extension);
      return sb.toString();
    } else {
  ...
  /**
   * Checks whether two filenames are equal after both have been normalized and using the case rules
   * of the system.
   *
   * <p>Both filenames are first passed to {@link #normalize(String)}. The check is then performed
   * case-sensitive on Unix and case-insensitive on Windows.
  ...
  public static boolean equals(
      String filename1, String filename2, final boolean normalized, IOCase caseSensitivity) {
   ...
   * case-sensitive.
   *
   * <p>The wildcard matcher uses the characters '?' and '*' to represent a single or multiple (zero
   ...
   * </pre>
   *
   * <p>N.B. the sequence "*?" does not work properly at present in match strings.
   *
   ...
   *
   * <p>The wildcard matcher uses the characters '?' and '*' to represent a single or multiple (zero
   * or more) wildcard characters. N.B. the sequence "*?" does not work properly at present in match
   * strings.
   *
   * @param filename the filename to match on
   * @param wildcardMatcher the wildcard string to match against
  ...
  public static boolean wildcardMatch(
      final String filename, final String wildcardMatcher, IOCase caseSensitivity) {
