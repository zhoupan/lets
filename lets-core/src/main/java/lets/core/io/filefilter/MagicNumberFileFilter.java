 ...
 *
 */
package lets.core.io.filefilter;
...
/**
 * File filter for matching files containing a "magic number". A magic number is a unique series of
 * bytes common to all files of a specific file format. For instance, all Java class files begin
 * with the bytes <code>0xCAFEBABE</code>.
 *
 * <pre>
 * File dir = new File(".");
 ...
 * </pre>
 *
 * <p>Sometimes, such as in the case of TAR files, the magic number will be offset by a certain
 * number of bytes in the file. In the case of TAR archive files, this offset is 257 bytes.
 *
 * <pre>
 * File dir = new File(".");
...
public class MagicNumberFileFilter extends AbstractFileFilter implements Serializable {
   ...
   * files. This constructor assumes a starting offset of <code>0</code>.
   *
   * <p>It is important to note that <em>the array is not cloned</em> and that any changes to the
  ...
  /**
   * Constructs a new MagicNumberFileFilter and associates it with the magic number to test for in
   * files. This constructor assumes a starting offset of <code>0</code>. Example usage:
   *
   * <pre>{@code
   ...
   * @throws IllegalArgumentException if <code>magicNumber</code> is {@code null} or the empty
   *     String, or <code>offset</code> is a negative number.
   */
  public MagicNumberFileFilter(final String magicNumber, final long offset) {
    ...
    this.magicNumbers =
        magicNumber.getBytes(Charset.defaultCharset()); // explicitly uses the platform default
    // charset
  ...
  /**
   * Accepts the provided file if the file contains the file filter's magic number at the specified
   * offset.
   *
   * <p>If any {@link IOException}s occur while reading the file, the file will be rejected.
   *
   * @param file the file to accept or reject.
   * @return {@code true} if the file contains the filter's magic number at the specified offset,
  ...
  public String toString() {
    ...
    builder.append("(");
    builder.append(
        new String(magicNumbers, Charset.defaultCharset())); // TODO perhaps use hex if value is not
    // printable
    builder.append(",");
