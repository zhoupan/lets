...
import lets.core.io.IOCase;
import lets.core.io.comparator.NameFileComparator;

/**
 * FileAlterationObserver represents the state of files below a root directory, checking the
 * filesystem and notifying listeners of create, change or delete events.
 *
 * <p>To use this implementation:
 *
 * <ul>
 *   <li>Create {@link FileAlterationListener} implementation(s) that process the file/directory
 *       create, change and delete events
 *   <li>Register the listener(s) with a {@link FileAlterationObserver} for the appropriate
 *       directory.
 *   <li>Either register the observer(s) with a {@link FileAlterationMonitor} or run manually.
 * </ul>
 *
 * <h2>Basic Usage</h2>
 *
 ...
 * </pre>
 *
 * To manually observe a directory, initialize the observer and invoked the {@link
 ...
 * </pre>
 *
 * Alternatively, register the observer(s) with a {@link FileAlterationMonitor}, which creates a new
 * thread, invoking the observer at the specified interval:
 *
 * <pre>
 *      long interval = ...
 ...
 * <h2>File Filters</h2>
 *
 * This implementation can monitor portions of the file system by using {@link FileFilter}s to
 * observe only the files and/or directories that are of interest. This makes it more efficient and
 * reduces the noise from <i>unwanted</i> file system events.
 *
 * <p><a href="http://commons.apache.org/io/">Commons IO</a> has a good range of useful, ready made
 * <a href="../filefilter/package-summary.html">File Filter</a> implementations for this purpose.
 *
 * <p>For example, to only observe 1) visible directories and 2) files with a ".java" suffix in a
 * root directory called "src" you could set up a {@link FileAlterationObserver} in the following
 * way:
 *
 * <pre>
 *      // Create a FileFilter
 ...
 * <h2>FileEntry</h2>
 *
 * {@link FileEntry} represents the state of a file or directory, capturing {@link File} attributes
 * at a point in time. Custom implementations of {@link FileEntry} can be used to capture additional
 * properties that the basic implementation does not support. The {@link FileEntry#refresh(File)}
 * method is used to determine if a file or directory has changed since the last check and stores
 * the current state of the {@link File}'s properties.
 *
...
public class FileAlterationObserver implements Serializable {
  ...
  protected FileAlterationObserver(
      final FileEntry rootEntry, final FileFilter fileFilter, final IOCase caseSensitivity) {
    if (rootEntry == null) {
