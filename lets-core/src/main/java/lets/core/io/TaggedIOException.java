...
/**
 * An {@link IOException} decorator that adds a serializable tag to the wrapped exception. Both the
 * tag and the original exception can be used to determine further processing when this exception is
 * caught.
 *
 * @since 2.0
 */
@SuppressWarnings(
    "deprecation") // needs to extend deprecated IOExceptionWithCause to preserve binary
// compatibility
public class TaggedIOException extends IOExceptionWithCause {
   ...
   *
   * <p>This check can only succeed if the throwable is a {@link TaggedIOException} and the tag is
   * {@link Serializable}, but the argument types are intentionally more generic to make it easier
   * to use this method without type casts.
   *
   * <p>A typical use for this method is in a <code>catch</code> block to determine how a caught
  ...
  /**
   * Throws the original {@link IOException} if the given throwable is a {@link TaggedIOException}
   * decorator the given tag. Does nothing if the given throwable is of a different type or if it is
   * tagged with some other tag.
   *
   * <p>This method is typically used in a <code>catch</code> block to selectively rethrow tagged
   * exceptions.
   *
   * <pre>
   * Serializable tag = ...;
