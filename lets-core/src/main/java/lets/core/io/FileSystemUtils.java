 ...
 *
 * <p>This class provides static utility methods for general file system functions not provided via
 * the JDK {@link java.io.File File} class.
 *
 * <p>The current functions provided are:
 *
 * <ul>
 ...
 *
 * @since 1.1
 * @deprecated As of 2.6 deprecated without replacement. Use equivalent methods in {@link
 *     java.nio.file.FileStore} instead, e.g. <code>
 *     Files.getFileStore(Paths.get("/home")).getUsableSpace()</code> or iterate over <code>
 *     FileSystems.getDefault().getFileStores()</code>
 */
@Deprecated
public class FileSystemUtils {
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns the free space on a drive or volume by invoking the command line. This method does not
   * normalize the result, and typically returns bytes on Windows, 512 byte units on OS X and
   * kilobytes on Unix. As this is not very useful, this method is deprecated in favour of {@link
   * #freeSpaceKb(String)} which returns a result in kilobytes.
   *
   * <p>Note that some OS's are NOT currently supported, including OS/390, OpenVMS.
   *
   * <pre>
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns the free space on a drive or volume in kibibytes (1024 bytes) by invoking the command
   * line.
   *
   * <pre>
   ...
   *
   * <p>In order to work, you must be running Windows, or have a implementation of Unix df that
   ...
   * the output from this class. If your operating system isn't supported, please raise a JIRA call
   * detailing the exact result from df -k and as much other detail as possible, thanks.
   *
   * @param path the path to get free space for, not null, not empty on Unix
  ...
  /**
   * Returns the free space for the working directory in kibibytes (1024 bytes) by invoking the
   * command line.
   *
   * <p>Identical to:
   *
  ...
  long freeSpaceOS(final String path, final int os, final boolean kb, final long timeout)
    ...
    switch (os) {
      case WINDOWS:
        return kb
            ? freeSpaceWindows(path, timeout) / FileUtils.ONE_KB
  ...
  long freeSpaceWindows(final String path, final long timeout) throws IOException {
    ...
    throw new IOException(
        "Command line 'dir /-c' did not return any info " + "for path '" + normPath + "'");
  }
  ...
  long parseDir(final String line, final String path) throws IOException {
    ...
    if (j < 0) {
      throw new IOException(
          "Command line 'dir /-c' did not return valid info " + "for path '" + path + "'");
    }
    ...
    if (tok.countTokens() < 4) {
      ...
      if (tok.countTokens() == 1 && lines.size() >= 3) {
        ...
        throw new IOException(
            "Command line '"
                + DF
                + "' did not return data as expected "
                + "for path '"
                + path
                + "'- check path is valid");
      }
   ...
   * @throws IOException if an error occurs
   */
  List<String> performCommand(final String[] cmdAttribs, final int max, final long timeout)
      throws IOException {
