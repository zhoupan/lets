...
public class GrammarDependencies {
  ...
  public GrammarDependencies(
      File sourceDirectory, File libDirectory, List<String> arguments, File status, Logger log) {
    this.log = log;
   ...
   * @return the package. Returns {@code null} to indicate that no package should be used.
   */
  private String getPackage(List<String> arguments) {
    int index = (arguments != null) ? arguments.indexOf("-package") : -1;

    return (index > -1)
        ? (arguments.get(index + 1).replace('.', File.separatorChar) + File.separatorChar)
        : null;
  }
  ...
  public void save() throws IOException {
    if (!grammars.isEmpty()) {
      ...
      ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(statusFile));
      ...
      if (!usages.isEmpty()) {
        grammars.put(
            ...
            new AbstractMap.SimpleImmutableEntry<byte[], Collection<String>>(
                Antlr4Tool.checksum(grammarFile), usages));
  ...
  public boolean isDependencyChanged(File grammarFile) throws IOException {
    ...
    for (Map.Entry<File, Map.Entry<byte[], Collection<String>>> e : grammars.entrySet()) {
      ...
      if (usages.contains(grammarPath)) {
        if (!depGrammarFile.exists()
            || !Arrays.equals(Antlr4Tool.checksum(depGrammarFile), checksum)) {
  ...
  private String getRelativePath(File grammarFile) {
    ...
    if (grammarFile.getPath().startsWith(libDirectory.getPath())) return grammarFile.getName();

    // if a package is given, we have to use it
    if (packageName != null) return packageName + grammarFile.getName();

    // otherwise resolve the path relative to the source directory
    String path = Antlr4Tool.findSourceSubdir(sourceDirectory, grammarFile);
      ...
      for (int i = 0, count = options.getChildCount(); i < count; i++) {
        ...
        if (option.getType() == ANTLRParser.ASSIGN) {
          ...
          if ("tokenVocab".equals(key)) {
            ...
            String grammarPath = Antlr4Tool.findSourceSubdir(sourceDirectory, grammarFile);
            ...
            if (packageName != null) grammarPath = packageName;
