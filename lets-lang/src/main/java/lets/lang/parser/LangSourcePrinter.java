...
public class LangSourcePrinter {
  ...
  public LangSourcePrinter() {
    this(new DefaultPrinterConfiguration());
  }

  public LangSourcePrinter(final PrinterConfiguration configuration) {
    this(
        configuration.get(new DefaultConfigurationOption(ConfigOption.INDENTATION)).get().asValue(),
        configuration
  ...
  public LangSourcePrinter(Indentation indentation, String eol) {
   ...
   */
  public LangSourcePrinter indent() {
  ...
  /**
   * Add to the current indentation until it is reaches "column" and push it on the indentation
   * stack. Does not actually output anything.
   */
  public LangSourcePrinter indentWithAlignTo(int column) {
    ...
    switch (indentation.getType()) {
      ...
      case TABS:
        ...
        if ((newIndent.length() >= currentIndentType.getWidth())
          ...
          newIndent.replace(
              i, i + currentIndentType.getWidth(), currentIndentType.getCar().toString());
        }
        break;
      default:
        throw new AssertionError("Unhandled indent type");
  ...
  public LangSourcePrinter unindent() {
    if (indents.isEmpty()) {
  ...
  /**
   * Append the source string passed as argument to the buffer. If this is being appended at the
   * beginning of a line, performs indentation first.
   *
   * <p>The source line to be printed should not contain newline/carriage-return characters; use
   * {@link #println(String)} to automatically append a newline at the end of the source string. If
   * the source line passed as argument contains newline/carriage-return characters would
   ...
   * @see LangSourcePrinter#println(String)
   */
  public LangSourcePrinter print(final String arg) {
    if (!indented) {
  ...
  /**
   * Append the source string passed as argument to the buffer, then append a newline. If this is
   * being appended at the beginning of a line, performs indentation first.
   *
   * <p>The source line to be printed should not contain newline/carriage-return characters. If the
   * source line passed as argument contains newline/carriage-return characters would impredictably
   * affect a correct computation of the current {@link #getCursor()} position.
   *
   * @param arg source line to be printed (should not contain newline/carriage-return characters)
   * @return this instance, for nesting calls to method as fluent interface
   */
  public LangSourcePrinter println(final String arg) {
    print(arg);
    println();
    return this;
  }
   ...
   * @return this instance, for nesting calls to method as fluent interface
   */
  public LangSourcePrinter println() {
   ...
   *
   * <p>Please notice in order to guarantee a correct computation of the cursor position, this
   * printer expect the contracts of the methods {@link #print(String)} and {@link #println(String)}
   * has been respected through all method calls, meaning the source string passed as argument to
   * those method calls did not contain newline/carriage-return characters.
   *
  ...
  /**
   * Set the top-most indent to the column the cursor is currently in, can be undone with {@link
   * #reindentToPreviousLevel()}. Does not actually output anything.
   */
  ...
  /**
   * Set the top-most indent to the column the cursor was before the last {@link
   * #reindentWithAlignToCursor()} call. Does not actually output anything.
   */
  ...
  /**
   * Adds an indent to the top of the stack that is a copy of the current top indent. With this you
   * announce "I'm going to indent the next line(s)" but not how far yet. Once you do know, you can
   * pop this indent ("unindent") and indent to the right column. (Does not actually output
   * anything.)
   */
  ...
  public void withIndent(Consumer<LangSourcePrinter> printer) {
    this.indent();
    printer.accept(this);
    this.unindent();
  }

  public void withBody(Consumer<LangSourcePrinter> printer, String bodyStart, String bodyEnd) {
    this.println(bodyStart);
    this.withIndent(printer);
    this.println(bodyEnd);
  }
}
