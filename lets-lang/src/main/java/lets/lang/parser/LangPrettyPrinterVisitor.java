...
/** Outputs the AST as formatted Lang source code. */
public class LangPrettyPrinterVisitor implements VoidVisitor<Void> {

  private static Pattern RTRIM = Pattern.compile("\\s+$");

  public final PrinterConfiguration configuration;

  public final LangSourcePrinter printer;

  public final LangFeatures features;

  public LangPrettyPrinterVisitor(PrinterConfiguration configuration) {
    this(configuration, new LangSourcePrinter(configuration), new LangFeatures());
  }

  public LangPrettyPrinterVisitor(
      PrinterConfiguration configuration, LangSourcePrinter printer, LangFeatures features) {
  ...
  public void printArguments(final NodeList<Expression> args, final Void arg) {
    ...
    if (!isNullOrEmpty(args)) {
      boolean columnAlignParameters =
          (args.size() > 1) && getOption(ConfigOption.COLUMN_ALIGN_PARAMETERS).isPresent();
  ...
  public void printPrePostFixOptionalList(
      final NodeList<? extends Visitable> args,
      final Void arg,
      String prefix,
      String separator,
      String postfix) {
  ...
  public void printPrePostFixRequiredList(
      final NodeList<? extends Visitable> args,
      final Void arg,
      String prefix,
      String separator,
  ...
  public void visit(final ClassOrInterfaceDeclaration n, final Void arg) {
    ...
    if (!n.getExtendedTypes().isEmpty()) {
      ...
      for (final Iterator<ClassOrInterfaceType> i = n.getExtendedTypes().iterator();
          i.hasNext(); ) {
    ...
    if (!n.getImplementedTypes().isEmpty()) {
      ...
      for (final Iterator<ClassOrInterfaceType> i = n.getImplementedTypes().iterator();
          i.hasNext(); ) {
        final ClassOrInterfaceType c = i.next();
        c.accept(this, arg);
        if (i.hasNext()) {
    ...
    if (!n.getPermittedTypes().isEmpty()) {
      ...
      for (final Iterator<ClassOrInterfaceType> i = n.getPermittedTypes().iterator();
          i.hasNext(); ) {
        final ClassOrInterfaceType c = i.next();
        c.accept(this, arg);
        if (i.hasNext()) {
    ...
    if (getOption(ConfigOption.PRINT_COMMENTS).isPresent()
      ...
      final String commentContent =
          normalizeEolInTextBlock(
              n.getContent(), getOption(ConfigOption.END_OF_LINE_CHARACTER).get().asString());
      ...
      boolean prependSpace =
          strippedLines.stream().anyMatch(line -> !line.isEmpty() && !line.startsWith(" "));
  ...
  public void visit(final MethodCallExpr n, final Void arg) {
    ...
    if (columnAlignFirstMethodChain.get()) {
      ...
      while (s.filter(NodeWithTraversableScope.class::isInstance).isPresent()) {
        Optional<Expression> parentScope =
            s.map(NodeWithTraversableScope.class::cast)
                .flatMap(NodeWithTraversableScope::traverseScope);
    ...
    printer.unindent();
    if (columnAlignFirstMethodChain.get()
        && methodCallWithScopeInScope.get()
        && lastMethodInCallChain.get()) {
  ...
  public void visit(final BlockComment n, final Void arg) {
    ...
    for (int i = 0; i < (lines.length - 1); i++) {
      ...
      // Avoids introducing indentation in blockcomments. ie: do not use println() as it would
      // trigger indentation at the next print call.
